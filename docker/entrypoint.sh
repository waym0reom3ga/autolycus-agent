...
# by /etc/cont-init.d/01-lycus-setup (installed by the Dockerfile). This
...
# on the pre-s6 contract "entrypoint.sh sets up state then execs lycus"
...
echo "[lycus] WARNING: docker/entrypoint.sh is a deprecated shim under " \
...
exec /opt/lycus/docker/stage2-hook.sh "$@"
