#!/bin/sh
# shellcheck shell=sh
# /opt/lycus/bin/lycus — `docker exec` privilege-drop shim.
#
# Background
# ----------
# The s6 image runs the supervised gateway/main process as the unprivileged
# `lycus` user (UID 10000). When an operator runs `docker exec <c> lycus ...`
# the default UID is root (0), and any file the command writes under
# $AUTOLYCUS_HOME — auth.json, .env, config.yaml — ends up root-owned and
# unreadable to the supervised gateway. The most common manifestation: the
# user runs `docker exec <c> lycus login`, this writes
# /opt/data/auth.json as root:root mode 0600, and from then on the gateway
# returns "Provider authentication failed: Lycus is not logged into Nous
# Portal" on every incoming message — even though `docker exec <c> lycus
...
# Fix
# ---
# This shim sits at /opt/lycus/bin/lycus and is placed earliest on PATH.
# When invoked as root, it drops to the lycus user (via s6-setuidgid)
# before exec'ing the real venv binary, so anything that writes under
# $AUTOLYCUS_HOME is uid-aligned with the supervised processes. When invoked
# as any non-root UID — including the supervised processes themselves,
# `docker exec --user lycus`, kanban subagents, etc. — it short-circuits
...
# (/opt/lycus/.venv/bin/lycus), so the second hop cannot re-enter this
...
# state via the lycus CLI. Default is to drop.

set -e

REAL=/opt/lycus/.venv/bin/lycus

# Defensive: if the venv binary is missing (corrupted image, partial
# install), fail loudly rather than silently masking it.
if [ ! -x "$REAL" ]; then
    echo "lycus-shim: $REAL not found or not executable" >&2
    exit 127
fi

# Already non-root? Just exec the real binary. This is the hot path for
# supervised processes (uid 10000) and for `docker exec --user lycus`.
if [ "$(id -u)" != "0" ]; then
    exec "$REAL" "$@"
fi
...
# Root, no opt-out. Drop to the lycus user.
#
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "lycus-shim: $S6_SUID not found; refusing to silently run as root." >&2
    echo "lycus-shim: re-run with --user lycus or set HERMES_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi

# Reset HOME to the lycus user's home before dropping privileges. Without
