"""Windows UTF-8 bootstrap for Lycus entry points.
...
should be imported at the very top of every Lycus entry point
(``lycus``, ``lycus-agent``, ``lycus-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import lycus_bootstrap``
# (or ``from lycus_bootstrap import apply_windows_utf8_bootstrap``) at
