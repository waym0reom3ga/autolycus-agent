...
# Python-driven tool session (e.g. Lycus terminal tool). A pre-set PYTHONPATH
...
#   code at /usr/local/lib/lycus-agent, command at /usr/local/bin/lycus,
#   data still at /root/.autolycus (AUTOLYCUS_HOME).  Matches Claude Code / Codex CLI
...
while [ $# -gt 0 ]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$AUTOLYCUS_HOME/.no-bundled-skills so future"
            echo "                   'lycus update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Lycus.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.autolycus/lycus-agent"
            echo "                   default (root, Linux): /usr/local/lib/lycus-agent"
            echo "  --lycus-home PATH  Data directory (default: ~/.autolycus, or \$AUTOLYCUS_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Lycus installs the code under"
            echo "  /usr/local/lib/lycus-agent and links the command into"
            echo "  /usr/local/bin/lycus (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$AUTOLYCUS_HOME"
            echo "  (default /root/.autolycus).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$AUTOLYCUS_HOME/lycus-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs lycus setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
# intentional, but they leave the checkout dirty — which forces `lycus update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Lycus-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Lycus Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install lycus command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `lycus` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $AUTOLYCUS_HOME/lycus-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $AUTOLYCUS_HOME/lycus-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/lycus-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $AUTOLYCUS_HOME/lycus-agent — then preserve it)
#
...
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in AUTOLYCUS_HOME.
    if is_termux; then
        INSTALL_DIR="$AUTOLYCUS_HOME/lycus-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$AUTOLYCUS_HOME/lycus-agent/.git" ]; then
            INSTALL_DIR="$AUTOLYCUS_HOME/lycus-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/lycus-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/lycus-agent"
        ...
        # /usr/local/bin/lycus wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/lycus"
        log_info "  Data:    $AUTOLYCUS_HOME (unchanged)"
...
# Point a Lycus-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $AUTOLYCUS_HOME/node/bin — which is NOT on
...
# Lycus's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Lycus-managed npm, so calling it on
...
configure_managed_node_npm_prefix() {
    ...
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$AUTOLYCUS_HOME/node/etc/npmrc"
}

get_lycus_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/lycus" ]; then
        echo "$link_dir/lycus"
    else
        echo "lycus"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://lycus-agent.nousresearch.com/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Lycus owns its own uv at $AUTOLYCUS_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (lycus_cli/managed_uv.py) looks in the same
    # place, so install.sh and `lycus update` stay in sync.
    local _managed_uv="$AUTOLYCUS_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $AUTOLYCUS_HOME/bin ..."
    mkdir -p "$AUTOLYCUS_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/lycus-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/lycus-uv-installer.$$.sh")"
    ...
    # directly into $AUTOLYCUS_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$AUTOLYCUS_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# string clears that floor; anything below it is replaced with the Lycus-
...
check_node() {
    ...
    # Repair pre-existing Lycus-managed installs where `npm install -g` lands
    ...
    # Prefer a Lycus-managed Node from a previous run over a too-old system one.
    if [ -x "$AUTOLYCUS_HOME/node/bin/node" ] && node_satisfies_build "$("$AUTOLYCUS_HOME/node/bin/node" --version)"; then
        export PATH="$AUTOLYCUS_HOME/node/bin:$PATH"
        log_success "Node.js $("$AUTOLYCUS_HOME/node/bin/node" --version) found (Lycus-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing Lycus-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    log_info "Extracting to ~/.autolycus/node/..."
    ...
    # Place into ~/.autolycus/node/ and symlink binaries into the same bin dir
    # the lycus command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$AUTOLYCUS_HOME/node"
    mkdir -p "$AUTOLYCUS_HOME"
    mv "$extracted_dir" "$AUTOLYCUS_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$AUTOLYCUS_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$AUTOLYCUS_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$AUTOLYCUS_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$AUTOLYCUS_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.autolycus/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Lycus install may complete, but web search and dependency downloads can fail."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `lycus update` path
                # (#4735).
                ...
                local stash_name
                stash_name="lycus-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Lycus behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up lycus and lycus commands..."

    if [ "$USE_VENV" = true ]; then
        HERMES_BIN="$INSTALL_DIR/venv/bin/lycus"
        LYCUS_BIN="$INSTALL_DIR/venv/bin/lycus"
    else
        HERMES_BIN="$(which lycus 2>/dev/null || echo "")"
        LYCUS_BIN="$(which lycus 2>/dev/null || echo "")"
        if [ -z "$HERMES_BIN" ]; then
            log_warn "lycus not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$HERMES_BIN" ]; then
        log_warn "lycus entry point not found at $HERMES_BIN"
    ...
    # Create a user-facing shim for the lycus command.
...
EOF
    ...
    # Create a user-facing shim for the lycus command (alias for lycus).
    cat > "$command_link_dir/lycus" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
...
EOF
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_success "lycus command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v lycus' \
                >/dev/null 2>&1; then
        ...
        log_info "lycus not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Lycus Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE '(PATH=|setenv PATH).*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Lycus Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Lycus Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so lycus works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.autolycus directory structure (config at top level, code in subdir)
    mkdir -p "$AUTOLYCUS_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.autolycus/.env (top level, easy to find)
    if [ ! -f "$AUTOLYCUS_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$AUTOLYCUS_HOME/.env"
            log_success "Created ~/.autolycus/.env from template"
        else
            touch "$AUTOLYCUS_HOME/.env"
            log_success "Created ~/.autolycus/.env"
        fi
    else
        log_info "~/.autolycus/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$AUTOLYCUS_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.autolycus/config.yaml (top level, easy to find)
    if [ ! -f "$AUTOLYCUS_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$AUTOLYCUS_HOME/config.yaml"
            log_success "Created ~/.autolycus/config.yaml from template"
        fi
    else
        log_info "~/.autolycus/config.yaml already exists, keeping it"
    fi
...
Edit this to customize how Lycus communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.autolycus/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.autolycus/"

    # Seed bundled skills into ~/.autolycus/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `lycus update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'lycus update'." \
            > "$AUTOLYCUS_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $AUTOLYCUS_HOME/.no-bundled-skills"
        log_info "  Future 'lycus update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.autolycus/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.autolycus/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$AUTOLYCUS_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$AUTOLYCUS_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.autolycus/skills/"
            fi
        fi
    fi
}
...
configure_browser_env_from_system_browser() {
    local env_file="$AUTOLYCUS_HOME/.env"
    ...
    {
        echo ""
        echo "# Lycus Agent browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; Lycus will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (lycus --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `lycus update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'lycus setup' after install."
        return 0
    fi
    ...
    # Run lycus setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m lycus_cli.main setup < /dev/tty
    else
        python -m lycus_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$AUTOLYCUS_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Lycus to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$AUTOLYCUS_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'lycus whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                HERMES_CMD="$(get_lycus_command_path)"
                $HERMES_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'lycus whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'lycus gateway install' later."
        return 0
    fi
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $HERMES_CMD gateway install 2>/dev/null; then
                ...
                if $HERMES_CMD gateway start 2>/dev/null; then
                    ...
                    log_warn "Service installed but failed to start. Try: lycus gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: lycus gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $HERMES_CMD gateway > "$AUTOLYCUS_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.autolycus/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: lycus gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: lycus gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $AUTOLYCUS_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $AUTOLYCUS_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $AUTOLYCUS_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}lycus${NC}              Start chatting"
    echo -e "   ${GREEN}lycus setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}lycus config${NC}       View/edit configuration"
    echo -e "   ${GREEN}lycus config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}lycus gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}lycus update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'lycus' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'lycus' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'lycus' command:${NC}"
        echo ""
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$AUTOLYCUS_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$AUTOLYCUS_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$AUTOLYCUS_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$AUTOLYCUS_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$AUTOLYCUS_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Lycus for pip install"
    ...
    HERMES_CMD="$(command -v lycus 2>/dev/null || echo "")"
    if [ -n "$HERMES_CMD" ]; then
        log_info "Running lycus setup..."
        "$HERMES_CMD" setup
    else
        log_warn "lycus command not found on PATH"
        log_info "Try: python -m lycus_cli.main setup"
    fi
}
...
# extract a tree MISSING the electron binary, so the `electron`->`Lycus` rename
...
# _purge_electron_build_cache() used by `lycus desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the Lycus-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `lycus desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Lycus" ]; then
            app="$desktop_dir/release/linux-unpacked/Lycus"
        elif [ -x "$desktop_dir/release/linux-unpacked/lycus" ]; then
            app="$desktop_dir/release/linux-unpacked/lycus"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Lycus.app" \
            "$desktop_dir/release/mac/Lycus.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # trips Gatekeeper's tamper check ("Lycus is damaged and can't be opened").
    ...
    # checkout stays clean for the next `lycus update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Lycus-managed Node
            # provisioned during prerequisites/node-deps (at $AUTOLYCUS_HOME/node/bin)
        ...
        complete)
            ...
            resolve_install_layout
            print_success
            echo "git" > "$AUTOLYCUS_HOME/.install_method"
            ;;
        *)
