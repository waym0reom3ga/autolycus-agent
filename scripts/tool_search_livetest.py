#!/usr/bin/env python3
"""Live test harness for Lycus Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any lycus imports.
ORIGINAL_HOME = os.environ.get("AUTOLYCUS_HOME")
ORIGINAL_AUTH = Path.home() / ".autolycus" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.autolycus/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.autolycus/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="lycus_ts_live_"))
    lycus_home = home_dir / ".autolycus"
    lycus_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, lycus_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".autolycus" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, lycus_home / ".env")
        ...
        load_lycus_dotenv(lycus_home=str(Path.home() / ".autolycus"))
    ...
    (lycus_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return lycus_home
...
def reset_module_state():
    """Drop cached modules so the new AUTOLYCUS_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "lycus_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["AUTOLYCUS_HOME"] = str(home)
...
def main():
    ...
    # Restore original AUTOLYCUS_HOME
    if ORIGINAL_HOME is not None:
        os.environ["AUTOLYCUS_HOME"] = ORIGINAL_HOME
    else:
