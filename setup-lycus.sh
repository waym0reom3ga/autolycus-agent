...
# 5. Symlinks the 'lycus' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Lycus Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/lycus-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink lycus into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up lycus command..."

LYCUS_BIN="$SCRIPT_DIR/venv/bin/lycus"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$LYCUS_BIN" "$COMMAND_LINK_DIR/lycus"
echo -e "${GREEN}✓${NC} Symlinked lycus → $COMMAND_LINK_DISPLAY_DIR/lycus"
...
# ============================================================================
# Seed bundled skills into ~/.autolycus/skills/
# ============================================================================

HERMES_SKILLS_DIR="${AUTOLYCUS_HOME:-$HOME/.autolycus}/skills"
mkdir -p "$HERMES_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.autolycus/skills/ ..."
...
if is_termux; then
    echo "  lycus gateway       # Run gateway in foreground"
else
    echo "  lycus gateway install # Install gateway service (messaging + cron)"
fi
echo "  lycus cron list     # View scheduled jobs"
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m lycus_cli.main setup
fi
