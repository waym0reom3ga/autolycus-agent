...
// ~/.autolycus. To exercise the real persistence path we clear that gate, point
// AUTOLYCUS_HOME at a temp dir, and re-import the module fresh (path + enabled
// flag are captured at module load).
const loadFresh = async (home: string) => {
  vi.resetModules()
  vi.stubEnv('VITEST', '')
  vi.stubEnv('AUTOLYCUS_HOME', home)
