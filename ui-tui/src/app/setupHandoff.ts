import type { RunExternalProcess } from '@lycus/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`lycus ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching lycus: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`lycus ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
