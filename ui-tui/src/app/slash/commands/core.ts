import { forceRedraw, type MouseTrackingMode } from '@lycus/ink'
...
export const coreCommands: SlashCommand[] = [
  ...
  {
    help: 'update Lycus Agent to the latest version (exits TUI)',
    name: 'update',
    run: (_arg, ctx) => {
      ctx.transcript.sys('exiting TUI to run update...')
      // Exit code 42 signals the Python wrapper to exec `lycus update`.
...

  {
    ...
    run: (arg, ctx) => {
      ...
      const lines = items.map((m, i) => {
        const tag = m.role === 'user' ? `You #${i + 1}` : `Lycus #${i + 1}`
