 ...
 * lycus-ink sets ``key.meta`` for plain Alt/Option on every platform
 ...
 * (``lycus_cli/voice.py::normalize_voice_record_key_for_prompt_toolkit``)
 ...
 * modifier — but lycus-ink reports Alt as ``key.meta`` on many
...
export const parseVoiceRecordKey = (raw: unknown): ParsedVoiceRecordKey => {
  ...
  // On macOS lycus-ink reports Alt as ``key.meta``, which ``isActionMod``
...
export const isVoiceToggleKey = (
  ...
  switch (configured.mod) {
    case 'alt':
      ...
      // alt binding.
      //
      // Bare Escape on lycus-ink can arrive as ``key.meta=true`` on some
    ...
    case 'ctrl':
      ...
      // ``key.meta`` is lycus-ink's Alt signal and accepting it would
