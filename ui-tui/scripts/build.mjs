...
await build({
  ...
  // Skip the prebuilt @lycus/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@lycus/ink': resolve(root, 'packages/lycus-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The lycus_cli launcher
