...
Lets the LLM write a Python script that calls Lycus tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `lycus_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `lycus_tools.py` with file-based RPC stubs
...
# available on every platform Lycus itself runs on.
...
# are non-secret runtime-location flags (the same set lycus_cli treats as the
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Lycus-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Lycus-profile soft guard."""',
...
def generate_lycus_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the lycus_tools.py stub module.
...
"""Auto-generated Lycus tools RPC stubs."""
...
"""Auto-generated Lycus tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("HERMES_RPC_DIR") or os.path.join(tempfile.gettempdir(), "lycus_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated lycus_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/lycus_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_lycus_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/lycus_tools.py", tools_src)
...
def execute_code(
    ...
    # --- Set up temp directory with lycus_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="lycus_sandbox_")
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"lycus_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated lycus_tools module.
        ...
        tools_src = generate_lycus_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "lycus_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        # Ensure the lycus-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from lycus_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _lycus_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _lycus_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # HERMES_TIMEZONE is an internal Lycus setting and must not leak
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.autolycus/.env')).
...
def _load_config() -> dict:
    ...
    ``lycus --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (lycus-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``lycus tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and lycus-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.autolycus/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Lycus tools programmatically. "
        ...
        f"Available via `from lycus_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into lycus_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from lycus_tools import {import_str}` "
