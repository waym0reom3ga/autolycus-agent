...
-----------------------------------------------------------------------------
    ...
    ~/.autolycus/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/lycus/<hash16>        — per-project branch tip
...
CHECKPOINT_BASE = get_lycus_home() / "checkpoints"

# Single shared store directory under CHECKPOINT_BASE.
_STORE_DIRNAME = "store"
_REFS_PREFIX = "refs/lycus"
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Lycus convention — don't recursively snapshot siblings)
    ".worktrees/",
...
def _git_env(
    ...
    The shared store is internal Lycus infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``lycus checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `lycus checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "lycus@local"], store, cfg_wd)
    _run_git(["config", "user.name", "Lycus Checkpoint"], store, cfg_wd)
...
# ---------------------------------------------------------------------------
# Public helpers for `lycus checkpoints` CLI
# ---------------------------------------------------------------------------
