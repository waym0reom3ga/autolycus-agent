#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Lycus Skills Hub.
...
Used by lycus_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            lycus_meta = metadata.get("lycus", {})
            if isinstance(lycus_meta, dict):
                tags = lycus_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Technetia Inc) but not activated
    by default — they don't appear in the system prompt and aren't copied to
    ~/.autolycus/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                lycus_meta = meta_block.get("lycus", {})
                if isinstance(lycus_meta, dict):
                    tags = lycus_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Lycus centralized index source
# ---------------------------------------------------------------------------

HERMES_INDEX_URL = "https://lycus-agent.nousresearch.com/docs/api/skills-index.json"
HERMES_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "lycus-index.json"
HERMES_INDEX_TTL = 6 * 3600  # 6 hours


def _load_lycus_index() -> Optional[dict]:
    ...
    try:
        ...
        if resp.status_code != 200:
            logger.debug("Lycus index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Lycus index fetch failed: %s", e)
        return _load_stale_index_cache()
...
class LycusIndexSource(SkillSource):
    """Skill source backed by the centralized Lycus Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "lycus-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "lycus-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        LycusIndexSource(auth=auth), # Centralized index (search + resolved install paths)
