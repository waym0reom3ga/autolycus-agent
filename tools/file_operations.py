...
class ShellFileOperations(FileOperations):
    ...
    def _atomic_write(self, path: str, content: str) -> "ExecuteResult":
        ...
        ``.lycus-tmp`` file next to the user's data, and the original file
        ...
        tmpl = self._escape_shell_arg(".lycus-tmp.XXXXXX")
        ...
        script = (
            ...
            '|| mktemp "$d/.lycus-tmp.$$.XXXXXX" 2>/dev/null '
            '|| { tmp="$d/.lycus-tmp.$$"; : > "$tmp" && echo "$tmp"; })"; '
    ...
    def read_file(self, path: str, offset: int = 1, limit: int = 500) -> ReadResult:
        ...
        if self._is_image(path):
            return ReadResult(
                ...
                file_size=file_size,
                hint=(
                    "Image file detected. The image can be viewed directly by the model."
                ),
            )
    ...
    def write_file(self, path: str, content: str) -> WriteResult:
        ...
        # clean the temp up on any failure so we never leak a ``.lycus-tmp``
