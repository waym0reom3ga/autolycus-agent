...
def iter_sync_files(container_base: str = "/root/.autolycus") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.autolycus to *container_base* because the remote
    ...
    for entry in get_credential_file_mounts():
        remote = entry["container_path"].replace(
            "/root/.autolycus", container_base, 1
        )
...
class FileSyncManager:
    ...
    def sync_back(self, lycus_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.autolycus/`` directory as a tar archive,
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="lycus-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.autolycus/skills/a.md`` →
        ``~/.autolycus/skills/a.md``, a new remote file at
        ``/root/.autolycus/skills/b.md`` maps to ``~/.autolycus/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
