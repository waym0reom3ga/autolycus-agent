...
it is automatically downloaded from GitHub releases to $AUTOLYCUS_HOME/bin/tirith.
...
def _get_lycus_home() -> str:
    """Return the Lycus home directory, respecting AUTOLYCUS_HOME env var."""
    return str(get_lycus_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_lycus_home(), ".tirith-install-failed")
...
def _lycus_bin_dir() -> str:
    """Return $AUTOLYCUS_HOME/bin, creating it if needed."""
    d = os.path.join(_get_lycus_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $AUTOLYCUS_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_lycus_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $AUTOLYCUS_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $AUTOLYCUS_HOME/bin/tirith
    ...
    lycus_bin = os.path.join(_lycus_bin_dir(), "tirith")
    if os.path.isfile(lycus_bin) and os.access(lycus_bin, os.X_OK):
        _resolved_path = lycus_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return lycus_bin
