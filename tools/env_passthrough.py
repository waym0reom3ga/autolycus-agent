...
def _is_lycus_provider_credential(name: str) -> bool:
    """True if ``name`` is a Lycus-managed provider credential (API key,
    ...
    Non-Lycus API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Lycus-managed provider credentials (from
    ...
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Lycus-managed
    ...
    Non-Lycus third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_lycus_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Lycus provider "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Lycus-managed provider credentials must not be passed
                ...
                if _is_lycus_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register Lycus "
