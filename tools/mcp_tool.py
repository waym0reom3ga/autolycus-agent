...
transport, discovers their tools, and registers them into the lycus-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.autolycus/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.autolycus/logs/mcp-stderr.log), tagged with the
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            lycus_home = os.path.expanduser(
                os.getenv(
                    "AUTOLYCUS_HOME", os.path.join(os.path.expanduser("~"), ".autolycus")
                )
            )
            candidates = [
                os.path.join(lycus_home, "node", "bin", resolved_command),
                ...
                # image (which the Lycus Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Lycus MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Lycus gateways know how to render.
    ...
    async def run(self, config: dict):
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Lycus is fully
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `lycus mcp login {server_name}` (or delete the tokens "
            f"file under ~/.autolycus/mcp-tokens/ and restart). Do NOT retry "
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local AUTOLYCUS_HOME override into ``coro``.
    ...
    try:
        ...
        home_override = get_lycus_home_override()
    except Exception:
        return coro
    ...
    async def _scoped():
        token = set_lycus_home_override(home_override)
        try:
            return await coro
        finally:
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local AUTOLYCUS_HOME override onto the MCP loop.
    ...
    # vanish here: OAuth token stores and any other get_lycus_home()
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Lycus config file.
    ...
    ``os.environ`` (which includes ``~/.autolycus/.env`` loaded at startup).
    """
    try:
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Lycus' MEDIA: tag convention and out to messaging
            ...
            # Lycus' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Lycus's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Lycus registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``lycus tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Lycus registry.
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    survivors, avoiding shared-resource collisions when multiple lycus
