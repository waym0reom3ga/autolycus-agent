...
Backend is selected during ``lycus tools`` setup (web.backend in config.yaml).
When available, Lycus can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Lycus config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Lycus' config/.env layer (``lycus config set``, ``lycus tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.autolycus/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``lycus tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `lycus tools` repaint.
        try:
