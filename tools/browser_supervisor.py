...
One ``CDPSupervisor`` runs per Lycus ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "lycus-dialog-bridge.invalid"
...
(() => {
  if (window.__lycusDialogBridgeInstalled) return;
  window.__lycusDialogBridgeInstalled = true;
  const ENDPOINT = "http://lycus-dialog-bridge.invalid/";
