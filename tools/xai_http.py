...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``lycus tools`` repaint, tool-registration scans,
    ...
    2. ``~/.autolycus/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_lycus_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.autolycus/.env`` first, then ``os.environ``.

    Wraps :func:`lycus_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from lycus_cli.config import get_env_value as _lycus_get_env_value

        value = _lycus_get_env_value(name)
...
def lycus_xai_user_agent() -> str:
    """Return a stable Lycus-specific User-Agent for xAI HTTP calls."""
    try:
        from lycus_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Lycus-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Lycus-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``lycus_cli.config.get_env_value`` so keys
    stored in ``~/.autolycus/.env`` (the standard Lycus location) are honored —
