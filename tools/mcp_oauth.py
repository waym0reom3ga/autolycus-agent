...
This module provides the glue:
    - ``LycusTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Lycus Agent"
"""
...
from lycus_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__lycus_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses AUTOLYCUS_HOME so each profile gets its own OAuth tokens.
    Layout: ``AUTOLYCUS_HOME/mcp-tokens/``
    """
    try:
        from lycus_constants import get_lycus_home
        base = Path(get_lycus_home())
    except ImportError:
        base = Path(os.environ.get("AUTOLYCUS_HOME", str(Path.home() / ".autolycus")))
...
# ---------------------------------------------------------------------------
# LycusTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class LycusTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        AUTOLYCUS_HOME/mcp-tokens/<server_name>.json         -- tokens
        AUTOLYCUS_HOME/mcp-tokens/<server_name>.client.json   -- client info
        AUTOLYCUS_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Lycus records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Lycus.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://lycus-agent.nousresearch.com/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `lycus mcp login <server>` later to "
...
def remove_oauth_tokens(server_name: str) -> None:
    ...
    storage = LycusTokenStorage(server_name)
    storage.remove()
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Lycus Agent")
    scope = cfg.get("scope")
...
def build_oauth_auth(
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`lycus mcp login {server_name}` interactively first to complete "
