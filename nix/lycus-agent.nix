# nix/lycus-agent.nix — Overridable Lycus Agent package
#
# callPackage auto-wires nixpkgs args; flake inputs are passed explicitly.
# Users override via:
#   pkgs.lycus-agent.override { extraPythonPackages = [...]; }
#   pkgs.lycus-agent.override { extraDependencyGroups = [ "hindsight" ]; }
{
  lib,
  stdenv,
  makeWrapper,
...
stdenv.mkDerivation (finalAttrs: {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/lycus-agent $out/bin
    cp -r ${bundledSkills} $out/share/lycus-agent/skills
    cp -r ${bundledPlugins} $out/share/lycus-agent/plugins
    cp -r ${bundledLocales} $out/share/lycus-agent/locales
    cp -r ${hermesWeb} $out/share/lycus-agent/web_dist
    ...
    ${lib.concatMapStringsSep "\n"
      (name: ''
        makeWrapper ${hermesVenv}/bin/${name} $out/bin/${name} \
          ...
          --set HERMES_BUNDLED_SKILLS $out/share/lycus-agent/skills \
          --set HERMES_BUNDLED_PLUGINS $out/share/lycus-agent/plugins \
          --set HERMES_BUNDLED_LOCALES $out/share/lycus-agent/locales \
          --set HERMES_WEB_DIST $out/share/lycus-agent/web_dist \
  ...
  passthru = {
    ...
    devShellHook = ''
      STAMP=".nix-stamps/lycus-agent"
      STAMP_VALUE="${pyprojectHash}:${uvLockHash}"
      if [ ! -f "$STAMP" ] || [ "$(cat "$STAMP")" != "$STAMP_VALUE" ]; then
        echo "lycus-agent: installing Python dependencies..."
