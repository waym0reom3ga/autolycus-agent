...
Launched by ``lycus meet node run``.

Token handling
--------------
On first boot we mint 32 hex chars of entropy and persist them at
``$AUTOLYCUS_HOME/workspace/meetings/node_token.json``. Subsequent boots
...
via ``lycus meet node approve <name> <url> <token>``.
...
def _default_token_path() -> Path:
    return Path(get_lycus_home()) / "workspace" / "meetings" / "node_token.json"
...
class NodeServer:
    ...
    def __init__(
        ...
        display_name: str = "lycus-meet-node",
    ) -> None:
        self.host = host
        self.port = port
