...
``$AUTOLYCUS_HOME/workspace/meetings/<meeting-id>/``. The main lycus process
...
# and exposes ``window.__lycusMeetDrain()`` to pull new entries. This
# mirrors the OpenUtter caption scraping approach.
_CAPTION_OBSERVER_JS = r"""
(() => {
  if (window.__lycusMeetInstalled) return;
  window.__lycusMeetInstalled = true;
  window.__lycusMeetQueue = [];
  ...
  window.__lycusMeetDrain = () => {
    const out = window.__lycusMeetQueue.slice();
    window.__lycusMeetQueue = [];
    return out;
  };
})();
...
def _start_realtime_speaker(
    *,
    ...
    if platform_tag == "linux":
        ...
        sink = (bridge_info or {}).get("write_target") or "lycus_meet_sink"
        try:
            proc = _sp.Popen(
                [
                    "paplay",
...
def run_bot() -> int:  # noqa: C901 — orchestration, explicit branches
    ...
    guest_name = os.environ.get("HERMES_MEET_GUEST_NAME", "Lycus Agent")
    ...
    try:
        with sync_playwright() as pw:
            ...
            while not stop_flag["stop"]:
                ...
                try:
                    queued = page.evaluate("window.__lycusMeetDrain && window.__lycusMeetDrain()")
...
def _detect_admission(page) -> bool:
    ...
    (() => {
      ...
      if (window.__lycusMeetInstalled) {
