...
lycus meet install                 # pip deps + Chromium (transcribe only)
lycus meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
lycus meet auth                    # optional; skips guest-lobby wait
lycus meet setup                   # preflight checks
```

`lycus meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or HERMES_MEET_REALTIME_KEY in ~/.autolycus/.env
```
...
pip install playwright websockets && python -m playwright install chromium
lycus plugins enable google_meet
lycus meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
lycus meet node approve my-mac ws://<mac-ip>:18789 <token>
lycus meet node ping my-mac                   # confirm reachable
```

Run `lycus meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A Lycus agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `lycus meet auth` avoids this.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `lycus_meet_src`). |
...
Local:
```
$AUTOLYCUS_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$AUTOLYCUS_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `lycus meet node approve`.
