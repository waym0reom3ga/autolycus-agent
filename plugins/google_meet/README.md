...
Let the lycus agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where lycus runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `lycus meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `lycus meet` CLI |
...
| `cli.py` | `lycus meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$AUTOLYCUS_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `lycus meet node {run,list,approve,remove,status,ping}` |
...
lycus meet install                                      # pip + Chromium
lycus meet setup                                        # preflight
lycus meet auth                                         # optional
lycus meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
lycus meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.autolycus/.env
lycus meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
lycus meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
lycus meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.autolycus/.env
lycus meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, lycus will **not** switch your system audio input automatically — the
...
lycus plugins enable google_meet
lycus meet node run --display-name my-mac --host 0.0.0.0 --port 18789
