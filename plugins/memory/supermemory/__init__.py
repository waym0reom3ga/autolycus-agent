...
def _load_supermemory_config(lycus_home: str) -> dict:
    config = _default_config()
    config_path = Path(lycus_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, lycus_home: str) -> None:
    config_path = Path(lycus_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str, search_mode: str = "hybrid"):
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "lycus"},
        )
    ...
    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        ...
        merged = {"sm_source": "lycus", **(metadata or {})}
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `lycus memory setup`.
        # All other options are documented for $AUTOLYCUS_HOME/supermemory.json
    ...
    def save_config(self, values, lycus_home):
        ...
        _save_supermemory_config(sanitized, lycus_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from lycus_constants import get_lycus_home
        self._lycus_home = kwargs.get("lycus_home") or str(get_lycus_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._lycus_home)
