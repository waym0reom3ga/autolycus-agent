...
Config file: `$AUTOLYCUS_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `lycus` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `lycus-{identity}` → `lycus-coder`). |
...
All Supermemory API calls send `x-sm-source: lycus`, and document writes stamp
`metadata.sm_source: lycus`. This is a **functional routing key, not telemetry**:
it groups Lycus-written memories into a dedicated "Lycus" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Lycus profile:

```json
{
  "container_tag": "lycus-{identity}"
}
```

For a profile named `coder`, this resolves to `lycus-coder`. The default profile resolves to `lycus-default`. Without `{identity}`, all profiles share the same container.
