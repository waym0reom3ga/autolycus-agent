...
Handles: lycus honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "lycus" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Lycus profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'lycus honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'lycus honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `lycus update` -- no output, no exceptions.
    """
    try:
        from lycus_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Lycus profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $AUTOLYCUS_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_lycus_home() / "honcho.json"
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        lycus_host.get("pinUserPeer"),
        lycus_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in lycus_host:
        aliases_src = lycus_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in lycus_host:
        prefix_src = lycus_host.get("runtimePeerPrefix")
...
def _scrub_identity_mapping(lycus_host: dict) -> None:
...
def _gateway_platforms() -> list[str] | None:
    ...
    guarded (matching the idiom lycus_cli already uses for gateway refs).
    """
    try:
...
def _apply_runtime_prefix(
    lycus_host: dict, current_prefix: str, prefix_from_root: bool, label: str
) -> None:
    ...
    if new_prefix and not (prefix_from_root and new_prefix == current_prefix):
        lycus_host["runtimePeerPrefix"] = new_prefix


def _echo_identity_mapping(lycus_host: dict) -> None:
    """Show the resulting keys so the operator can verify what was written."""
    aliases = lycus_host.get("userPeerAliases")
    prefix = lycus_host.get("runtimePeerPrefix")
    print("  resolved →")
    print(f"    pinUserPeer       = {bool(lycus_host.get('pinUserPeer'))}")
...
def _configure_raw_identity_mapping(
    ...
    _scrub_identity_mapping(lycus_host)
    lycus_host["pinUserPeer"] = pin
    if pin:
        return
    aliases = (
    ...
    while True:
        ...
        if "=" in entry:
            ...
            if rid and peer:
                aliases[rid] = peer
    if aliases:
        lycus_host["userPeerAliases"] = aliases
    _apply_runtime_prefix(
        lycus_host, current_prefix, prefix_from_root,
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Lycus persistent cross-session memory.")
    ...
    if is_local:
        ...
        current_host_key = lycus_host.get("apiKey", "")
        masked = (
        ...
        if new_local_key:
            lycus_host["apiKey"] = new_local_key
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'lycus honcho setup' again once you have a key.\n")
            return

    # --- 3. Identity ---
    current_peer = lycus_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        lycus_host["peerName"] = new_peer

    current_ai = lycus_host.get("aiPeer") or cfg.get("aiPeer", "lycus")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        lycus_host["aiPeer"] = new_ai

    current_workspace = lycus_host.get("workspace") or cfg.get("workspace", "lycus")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        lycus_host["workspace"] = new_workspace

    # --- 3b. Gateway identity mapping ---
    # These keys only affect the Lycus GATEWAY (Telegram/Discord/Slack/...),
    ...
    (
        ...
        aliases_from_root,
        prefix_from_root,
    ) = _resolve_effective_identity_mapping(cfg, lycus_host)
    ...
    if gw_platforms is None:
        ...
        run_mapping = _prompt(
            "Running the Lycus gateway (Telegram/Discord/etc.)? (y/N)",
            default="n",
    ...
    if run_mapping:
        peer_target = lycus_host.get("peerName") or current_peer or "user"
        ...
        if shape == "single":
            ...
            _scrub_identity_mapping(lycus_host)
            lycus_host["pinUserPeer"] = False
            if prior_aliases:
                lycus_host["userPeerAliases"] = prior_aliases
            _apply_runtime_prefix(
                lycus_host, current_prefix, prefix_from_root,
            ...
            if merged:
                lycus_host["userPeerAliases"] = merged
            _apply_runtime_prefix(
                lycus_host, current_prefix, prefix_from_root,
            ...
            _configure_raw_identity_mapping(
                lycus_host, current_pin, current_aliases, current_prefix,
    ...
    # --- 4. Observation mode ---
    current_obs = lycus_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        lycus_host["observationMode"] = new_obs
    else:
        lycus_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(lycus_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        lycus_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        lycus_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = lycus_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        lycus_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = lycus_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        lycus_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                lycus_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(lycus_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            lycus_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        lycus_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        lycus_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        lycus_host["dialecticReasoningLevel"] = new_reasoning
    else:
        lycus_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = lycus_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        lycus_host["sessionStrategy"] = new_strat

    lycus_host["enabled"] = True
    lycus_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        lycus_config = load_config()
        lycus_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(lycus_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: lycus config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    lycus honcho status     -- show full config")
    print("    lycus honcho mode       -- change recall/observation mode")
    print("    lycus honcho tokens     -- tune context and dialectic budgets")
    print("    lycus honcho peer       -- update peer names")
    print("    lycus honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Lycus profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: lycus honcho setup\n")
        return
    ...
    if not cfg:
        ...
        try:
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run 'lycus honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run 'lycus honcho setup' to configure.\n")
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: lycus honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        lycus = hosts.get(_host_key(), {})
        user = lycus.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = lycus.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = lycus.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = lycus.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Lycus' identity in Honcho. Seed with 'lycus honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: lycus honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: lycus honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = lycus.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = lycus.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = lycus.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Lycus asks Honcho's AI peer a question")
        ...
        print("\n  Set with: lycus honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'lycus honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'lycus honcho identity <file>' to seed one.")
        print()
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Lycus + Honcho."""
    from pathlib import Path
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Lycus persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  lycus honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'lycus honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'lycus honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  lycus honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'lycus'")
        print("  with Honcho configured and no prior session history.")
        print("  (Lycus calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    lycus honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'lycus honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Lycus, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'lycus honcho setup' first, then seed manually:")
            for f in agent_files:
    ...
    print("    Lycus:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Lycus:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Lycus:   Honcho observes every message and updates representations")
    ...
    print("    Lycus:   per-session by default — each run gets its own session")
    print("              Map a custom name:  lycus honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. lycus honcho setup              — configure API key (required)")
        print("  2. lycus honcho migrate            — re-run this walkthrough")
    else:
        print("  1. lycus honcho status             — verify Honcho connection")
        print("  2. lycus                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. lycus honcho identity --show    — verify AI peer representation")
        print("  4. lycus honcho tokens             — tune context and dialectic budgets")
        print("  5. lycus honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'lycus memory setup'...\n")
        from lycus_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``lycus honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``lycus honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to lycus memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Lycus Honcho",
    )
