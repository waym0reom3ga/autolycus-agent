...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/lycus>
...
```bash
lycus memory setup honcho   # configure Honcho directly (works on a fresh install)
lycus memory setup          # generic picker, choose Honcho from the list
```

Or manually:
```bash
lycus config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.autolycus/.env
```

> `lycus honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `lycus memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$AUTOLYCUS_HOME/honcho.json` | Profile-local (isolated Lycus instances) |
| 2 | `~/.autolycus/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Lycus profile: `lycus` (default) or `lycus_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `lycus honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Lycus session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`lycus-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`lycus`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-lycus-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening lycus in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Lycus session ID (timestamp + hex). Every `lycus` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Lycus profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`lycus`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Lycus profile: `lycus` (default) or `lycus_<profile>` (e.g. `lycus -p coder` -> host key `lycus_coder`). Older `lycus.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
|---------|-------------|
| `lycus memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `lycus honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `lycus memory setup`) |
| `lycus honcho status` | Show resolved config for active profile |
| `lycus honcho enable` / `disable` | Toggle Honcho for active profile |
| `lycus honcho mode <mode>` | Change recall or observation mode |
| `lycus honcho peer --user <name>` | Update user peer name |
| `lycus honcho peer --ai <name>` | Update AI peer name |
| `lycus honcho tokens --context <N>` | Set context token budget |
| `lycus honcho tokens --dialectic <N>` | Set dialectic max chars |
| `lycus honcho map <name>` | Map current directory to a session name |
| `lycus honcho sync` | Create host blocks for all Lycus profiles |
