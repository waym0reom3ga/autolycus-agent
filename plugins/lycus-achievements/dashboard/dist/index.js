(function () {
  ...
  // Bundled into lycus-agent. Upstream repo remains the staging ground for new
  ...
  async function buildShareImage(achievement) {
    ...
    ctx.fillText("HERMES AGENT  ·  lycus-agent.nousresearch.com", 70, H - 40);
  ...
  function ShareDialog({ achievement, onClose }) {
    ...
    function tweetText() {
      ...
      const tmpl = tx(t, "share.tweet_text", "Just unlocked {tier_part}\"{name}\" in Lycus Agent ☤", {
        tier_part: tierPart,
        name: achievement.name,
      });
      return tmpl + "\n\n@NousResearch · https://lycus-agent.nousresearch.com";
    }
