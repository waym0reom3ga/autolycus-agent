# Lycus Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: lycus-achievements plugin
...
### `GET /api/plugins/lycus-achievements/achievements`
...
### `POST /api/plugins/lycus-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/lycus-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/lycus-achievements/overview`
...
- Persist plugin data under:
  - `~/.autolycus/plugins/lycus-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.autolycus/plugins/lycus-achievements/scan_checkpoint.json`
...
- `~/.autolycus/plugins/lycus-achievements/`
