...
``lycus_cli.kanban_db`` or a direct SQL query. Writes use the same code
...
the in-browser PTY bridge in ``lycus_cli/web_server.py``.

This means ``lycus dashboard --host 0.0.0.0`` is safe to run on a LAN:
...
from lycus_cli import kanban_db
...
def _ws_upgrade_authorized(ws: "WebSocket") -> bool:
    ...
    WS auth gate (``lycus_cli.web_server._ws_auth_ok``).
    ...
    """
    try:
        from lycus_cli import web_server as _ws
    except Exception:
...
def _compute_task_diagnostics(
    ...
    Uses ``lycus_cli.kanban_diagnostics`` — see that module for the
    rule definitions.
    """
    from lycus_cli import kanban_diagnostics as kd
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from lycus_cli.kanban_diagnostics import SEVERITY_ORDER
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from lycus_cli.kanban import _check_dispatcher_presence
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``lycus kanban
...
# spawn failures, stuck-blocked). See lycus_cli.kanban_diagnostics for
# the rule engine.
# ---------------------------------------------------------------------------
...
def list_diagnostics(
    ...
    AND for ``lycus kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``lycus kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``lycus kanban specify <task_id>``.
    ...
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        ...
        from lycus_cli import kanban_specify  # noqa: WPS433 (intentional)
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``lycus kanban reassign <task_id> <profile> [--reclaim]``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.autolycus/config.yaml.
...
def _active_profile_name() -> str:
    """Return the current Lycus profile name for notify-sub ownership."""
    try:
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of ``~/.autolycus/profiles/*`` on disk and every
...
def update_profile_description(profile_name: str, payload: DescribeBody):
    ...
    try:
        ...
        if canon == "default":
            from lycus_constants import get_lycus_home  # type: ignore
            from pathlib import Path as _Path
            profile_dir = _Path(get_lycus_home())
        else:
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    Maps 1:1 to ``lycus profile describe <name> --auto``. Non-OK
    ...
    """
    try:
        from lycus_cli import profile_describer  # noqa: WPS433 (intentional)
...
def decompose_task_endpoint(
    ...
    1:1 to ``lycus kanban decompose <task_id>``.
    ...
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        from lycus_cli import kanban_decompose  # noqa: WPS433 (intentional)
...
def set_orchestration_settings(payload: OrchestrationSettingsBody):
    """Update the kanban orchestration knobs in ~/.autolycus/config.yaml.
