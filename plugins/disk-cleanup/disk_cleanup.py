"""disk_cleanup — ephemeral file cleanup for Lycus Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under AUTOLYCUS_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly AUTOLYCUS_HOME and /tmp/lycus-*
Never touches: ~/.autolycus/logs/ or any system directory.
"""
...
try:
    ...
    def get_lycus_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("AUTOLYCUS_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".autolycus").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$AUTOLYCUS_HOME/logs/``."""
    return get_lycus_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$AUTOLYCUS_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under AUTOLYCUS_HOME or ``/tmp/lycus-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    lycus_home = get_lycus_home()
    try:
        path.resolve().relative_to(lycus_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/lycus-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("lycus-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "lycus-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $AUTOLYCUS_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    """
    # Lazily build the set once per process so AUTOLYCUS_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        lycus_home = get_lycus_home()
        for parent in ("cron", "cronjobs"):
            base = lycus_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside AUTOLYCUS_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under AUTOLYCUS_HOME, but never recurse into known
    # durable state trees.  Some installs place the Lycus checkout, venv,
    # and desktop build under AUTOLYCUS_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    lycus_home = get_lycus_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in lycus_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    lycus_home = get_lycus_home()
    try:
        rel = path.resolve().relative_to(lycus_home)
        ...
        # Path isn't under AUTOLYCUS_HOME (e.g. /tmp/lycus-*) — fall through.
        pass
