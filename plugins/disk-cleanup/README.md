...
Auto-tracks and cleans up ephemeral files created during Lycus Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$AUTOLYCUS_HOME` and `/tmp/lycus-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `AUTOLYCUS_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `AUTOLYCUS_HOME` or `/tmp/lycus-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$AUTOLYCUS_HOME/disk-cleanup/` is itself excluded
- `$AUTOLYCUS_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
