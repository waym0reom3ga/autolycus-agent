...
def _xai_user_agent() -> str:
    try:
        ...
        return lycus_xai_user_agent()
    except Exception:
        return "lycus-agent/video_gen"
...
class XAIVideoGenProvider(VideoGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        # hook (``lycus_cli/tools_config.py``) so the picker doesn't blindly
    ...
    async def _generate_async(
        ...
        *,
        ...
        if not api_key:
            return error_response(
                error=(
                    "No xAI credentials found. Sign in via `lycus auth add xai-oauth` "
