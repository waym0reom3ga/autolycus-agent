"""langfuse — Lycus plugin for Langfuse observability.

Traces Lycus conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Lycus plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``lycus plugins enable
observability/langfuse`` or ``lycus tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``lycus tools`` or ~/.autolycus/.env):
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Lycus plugin system —
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Lycus branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Lycus also has a turn-scoped
    ...
    # messages is a list only for legacy Lycus branches that fired
    # pre_llm_call with API messages directly. Current Lycus fires
...
def register(ctx) -> None:
    ...
    # Lycus versions.  pre_api_request / post_api_request fire per API
