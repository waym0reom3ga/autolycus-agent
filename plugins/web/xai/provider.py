...
``{title, url, description, position}`` rows every other Lycus web
provider produces.
...
prefers Lycus-managed xAI Grok OAuth (via ``lycus auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.autolycus/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``lycus tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `lycus auth` to sign in with "
