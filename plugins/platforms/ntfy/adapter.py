"""ntfy platform adapter (Lycus plugin).
...
``plugins/platforms/ntfy/``. The Lycus plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "lycus-in"              # subscribe topic (incoming)
          publish_topic: "lycus-out"     # optional — defaults to topic
...
_ECHO_TAG = "lycus-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``lycus cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Lycus dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `lycus gateway status` without
