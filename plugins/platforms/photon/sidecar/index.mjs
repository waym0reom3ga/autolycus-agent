// Lycus Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Lycus): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Lycus -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Lycus-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `lycus photon telemetry`)
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-lycus-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Lycus supplied them so a known-good
