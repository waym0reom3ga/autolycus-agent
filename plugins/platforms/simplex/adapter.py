"""SimpleX Chat platform adapter (Lycus plugin).
...
This adapter ships as a Lycus platform plugin under
``plugins/platforms/simplex/``. The Lycus plugin loader scans the
...
discoverable and ``lycus setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Lycus — no
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``lycus setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.autolycus/.env`` via
    ``lycus_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "lycus_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.autolycus/.env"
        )
        return
