"""
IRC Platform Adapter for Lycus Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Lycus agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "lycus-bot")
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Lycus Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Lycus Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: lycus_, lycus_1, lycus_2...
...
def check_requirements() -> bool:
    ...
    # lycus setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `lycus gateway setup` flow for the IRC platform.

    Lazy-imports ``lycus_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect Lycus to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #lycus — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.autolycus/.env")
    print_info("Restart the gateway for changes to take effect: lycus gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "lycus-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "lycus-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Lycus Agent (cron)")
