...
load_lycus_dotenv(
    lycus_home=_lycus_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.autolycus/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_lycus_home, "logs", "tui_gateway_crash.log")
...
class _SlashWorker:
    """Persistent LycusCLI subprocess for slash commands."""
...
def _close_sessions_for_transport(
    ...
    for sid, session in owned:
        if session.get("close_on_disconnect"):
            ...
            # actually fire; a standalone `lycus --tui` keeps real _stdio.
...
def _transport_is_dead(transport) -> bool:
    ...
    # for a standalone `lycus --tui`, so it must NOT count as dead here (doing
...
# ``profile`` on those calls; we open that profile's db and bind its AUTOLYCUS_HOME
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    if home.resolve() == Path(_lycus_home).resolve():
        return None
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `lycus` shows the prompt before constructing AIAgent; the TUI used
    ...
    def _build() -> None:
        ...
        try:
            ...
            # AUTOLYCUS_HOME so config/skills/model resolve to it, and hand the
            # agent that profile's db so turns persist to the right state.db.
            session_db = None
            if profile_home:
                home_token = set_lycus_home_override(profile_home)
                try:
                    from lycus_state import SessionDB
            ...
            if home_token is not None:
                reset_lycus_home_override(home_token)
...
def _load_cfg() -> dict:
    ...
    try:
        ...
        # launch profile's _lycus_home. Cache is keyed on the resolved path, so
        # profiles don't clobber each other.
        override = get_lycus_home_override()
        home = override if isinstance(override, str) and override else _lycus_home
...
def resolve_skin() -> dict:
    try:
        from lycus_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    try:
        from lycus_cli.models import detect_static_provider_for_model
...
def _load_enabled_toolsets() -> list[str] | None:
    ...
    # Coding posture (base Lycus): with no explicit pin, collapse to the
    # coding toolset (+ enabled MCP servers) when sitting in a code workspace.
    # The desktop app and `lycus --tui` both land here. See
    ...
    if explicit and validate_toolset is not None:
        ...
        try:
            ...
            from lycus_cli.tools_config import _parse_enabled_flag
...
def _apply_model_switch(
    ...
    *,
    ...
    from lycus_cli.model_switch import parse_model_flags, switch_model
    ...
    custom_provs = None
    try:
        from lycus_cli.config import get_compatible_custom_providers, load_config
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    try:
        ...
        pass
    try:
        from lycus_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from lycus_cli.banner import get_update_result
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from lycus_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from lycus_cli.config import load_config as _load_full_cfg
...
def _load_fallback_model():
    ...
    stays in parity with ``LycusCLI.__init__`` and ``gateway/run.py``:
...
def _make_agent(
    ...
    # block. Dashboard /api/ws uses lycus_cli.mcp_startup; TUI stdio keeps
    ...
    if isinstance(model_override, dict) and model_override.get("model"):
        ...
        if (
            ...
            from lycus_cli.runtime_provider import find_custom_provider_identity
...
def _(rid, params: dict) -> dict:
    ...
    # and each turn re-bind AUTOLYCUS_HOME. None/own profile → launch (unchanged).
...
def _(rid, params: dict) -> dict:
    ...
    _enable_gateway_prompts()
    home_token = (
        set_lycus_home_override(str(profile_home)) if profile_home is not None else None
    )
    try:
        db.reopen_session(target)
        history = db.get_messages_as_conversation(target)
    ...
    with _session_resume_lock:
        ...
        try:
            init_home_token = (
                set_lycus_home_override(str(profile_home))
                if profile_home is not None
                else None
            )
            try:
                ...
                if init_home_token is not None:
                    reset_lycus_home_override(init_home_token)
            if sid in _sessions:
                ...
                # Remember the profile home so each turn re-binds AUTOLYCUS_HOME (the
...
def _(rid, params: dict) -> dict:
    ...
    # ``lycus --tui`` session legitimately rides the real stdio transport and
    # must stay visible.
...
def _(rid, params: dict) -> dict:
    ...
    transfer is performed by the separate ``lycus gateway`` process, whose
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the Lycus profile home
    # (~/.autolycus/sessions/saved/) rather than the project/workspace CWD, and
...
#
# Layout:  $AUTOLYCUS_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _spawn_trees_root():
    ...
    root = get_lycus_home() / "spawn-trees"
...
def _run_prompt_submit(rid, sid: str, session: dict, text: Any) -> None:
    ...
    def run():
        approval_token = None
        session_tokens = []
        home_token = None  # per-turn AUTOLYCUS_HOME override for a resumed remote profile
        ...
        try:
            ...
            if _profile_home_str:
                home_token = set_lycus_home_override(_profile_home_str)
            ...
            if images:
                try:
                    ...
                    from lycus_cli.config import load_config as _tui_load_config
            ...
            if (
                ...
                try:
                    from lycus_cli.voice import speak_text
                    ...
                    logger.warning("voice TTS skipped: lycus_cli.voice unavailable")
                except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from lycus_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
        return _err(rid, 5027, f"clipboard unavailable: {e}")

    session["image_counter"] = session.get("image_counter", 0) + 1
    img_dir = _lycus_home / "images"
...
def _desktop_attachment_dir(session: dict) -> Path:
    root = Path(_session_cwd(session)).resolve() / ".autolycus" / "desktop-attachments"
    root.mkdir(parents=True, exist_ok=True)
    return root
    ...
    *,
    ...
    Three cases:
      ...
      2. The path resolves to a gateway-visible file OUTSIDE the workspace — copy
         it into ``.autolycus/desktop-attachments/`` so the ``@file:`` ref resolves.
      3. The path doesn't exist on the gateway (the common remote case: it's a
         ...
         write them into ``.autolycus/desktop-attachments/``.
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not Lycus Desktop itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Lycus/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not Lycus Desktop. If it serves Lycus/Desktop UI or another unrelated app, stop that process and report failure.",
    ...
    if key == "model":
        try:
            ...
            if session:
                ...
                from lycus_cli.model_switch import parse_model_flags
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from lycus_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_lycus_home), "display": display_lycus_home()})
    if key == "project":
...
def _(rid, params: dict) -> dict:
    try:
        from lycus_cli.main import _has_any_provider_configured
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from lycus_cli.runtime_provider import resolve_runtime_provider
        from lycus_cli.auth import has_usable_secret
        from lycus_cli.main import _has_any_provider_configured
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Lycus provider is configured.",
                },
            )
...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.autolycus/.env`` into the gateway process via
    ``lycus_cli.config.reload_env``, matching classic CLI's ``/reload``
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `lycus` is interactive — use `/lycus chat -q …` or run `lycus` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`lycus setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`lycus gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`lycus sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`lycus config edit` needs $EDITOR in a real terminal"
    return None


@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m lycus_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "lycus_cli.main", *argv],
            capture_output=True,
            text=True,
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from lycus_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from lycus_cli.config import is_managed, save_env_value
        from lycus_cli.inventory import build_models_payload, load_picker_context
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `lycus model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.autolycus/.env
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from lycus_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from lycus_cli.config import remove_env_value
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        if not enabled:
            ...
            try:
                from lycus_cli.voice import stop_continuous
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from lycus_cli.voice import start_continuous
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from lycus_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from lycus_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from lycus_cli.browser_connect import try_launch_chrome_debug
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_lycus_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from lycus_cli.skills_hub import do_install
        ...
        if action == "browse":
            from lycus_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from lycus_cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``lycus plugins`` / the dashboard, so the three surfaces
