...
- ``lycus_cli.providers``        -- canonical provider identity + overlays
- ``lycus_cli.model_normalize``  -- per-provider name formatting
...
_HERMES_MODEL_WARNING = (
    "Nous Research Lycus 3 & 4 models are NOT agentic and are not designed "
    "for use with Autolycus. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Lycus 3 / Lycus 4 chat families.
# The previous substring check (`"lycus" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``lycus-brain:qwen3-14b-ctx16k`` that just
# happen to carry "lycus" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Lycus-3-Llama-3.1-70B, lycus-4-405b, openrouter/lycus3:70b
# Negative examples it must NOT match:
#   lycus-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
...
def is_nous_lycus_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Lycus 3/4 chat model.
...
def _check_lycus_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Lycus 3/4 chat model."""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``lycus config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from lycus_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'lycus model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'lycus doctor' — config issues detected:"
        ...
        from lycus_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from lycus_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from lycus_cli.providers import resolve_user_provider as _ruser
    ...
    # Mirrors the same logic in lycus_cli.runtime_provider.resolve_runtime_provider;
    ...
    lycus_warn = _check_lycus_model_warning(new_model)
    if lycus_warn:
        warnings.append(lycus_warn)
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from lycus_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from lycus_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from lycus_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by lycus provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://lycus-agent.nousresearch.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Lycus release. Falls back to the in-repo
    # _PROVIDER_MODELS["nous"] snapshot when the manifest is unreachable.
    curated["nous"] = get_curated_nous_model_ids()
    # Ollama Cloud uses dynamic discovery (no static curated list)
    if "ollama-cloud" not in curated:
        from lycus_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from lycus_cli.models import fetch_lmstudio_models
    ...
    # --- 1. Check Lycus-mapped providers ---
    from lycus_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from lycus_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for lycus_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(lycus_id)
        if (
            _alias_target
            and _alias_target != lycus_id
        ...
        pconfig = PROVIDER_REGISTRY.get(lycus_id)
        ...
        if not has_creds:
            try:
                from lycus_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and store.get("credential_pool", {}).get(lycus_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `lycus model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(lycus_id)
        if not model_ids:
            model_ids = curated.get(lycus_id, [])
            if lycus_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(lycus_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check Lycus-only providers (nous, openai-codex, copilot, opencode-go) ---
    from lycus_cli.providers import HERMES_OVERLAYS
    from lycus_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Lycus provider ID.
    # HERMES_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Lycus IDs ("copilot").
    _mdev_to_lycus = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in HERMES_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Lycus slug — e.g. "github-copilot" → "copilot"
        lycus_slug = _mdev_to_lycus.get(pid, pid)
        if lycus_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(lycus_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, lycus_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or lycus_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(lycus_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", lycus_slug, exc)
        ...
        # configured.
        if not has_creds and lycus_slug == "anthropic":
            try:
                ...
                if (lycus_creds and lycus_creds.get("accessToken")) or \
        ...
        if lycus_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(lycus_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(lycus_slug)
                model_ids = _ids if _ids else (curated.get(lycus_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(lycus_slug, []) or curated.get(pid, [])
        elif lycus_slug == "nous":
            # Nous serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `lycus model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from lycus_cli.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(lycus_slug, []) or curated.get(pid, [])
                if lycus_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(lycus_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": lycus_slug,
            "name": get_label(lycus_slug),
            "is_current": lycus_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "lycus",
        ...
        seen_slugs.add(lycus_slug.lower())
        _record_builtin_endpoint(lycus_slug)
    ...
    # with `lycus model`).
    try:
        from lycus_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Lycus's canonical write key (matches
            ...
            # Lycus writes ``models:`` as a dict keyed by model id
            # (see lycus_cli/main.py::_save_custom_provider); older
            ...
            if api_url and api_key and discover:
                try:
                    from lycus_cli.models import fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Lycus's own writer uses; a hyphen variant is accepted
            ...
            # active model. Lycus's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, lycus_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`lycus_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from lycus_cli.models import fetch_openrouter_models
