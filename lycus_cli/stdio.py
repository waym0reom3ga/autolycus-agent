...
on Japanese locales, etc.).  Lycus's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``lycus_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Lycus is running
...
def configure_windows_stdio() -> bool:
    ...
    # Augment PATH with the Lycus-managed Git install directories so
    ...
    # already-running shells don't see the change — which means lycus
    ...
    # install without Lycus-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Lycus's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Lycus's
    ``lycus config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Lycus (User env var in Windows Settings, or
    export in a PowerShell profile) and Lycus picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Lycus-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\lycus\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``lycus`` from right after install) retain their old PATH.

    Any subprocess Lycus spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Lycus-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "lycus", "git", "cmd"),
        os.path.join(local_appdata, "lycus", "git", "bin"),
        os.path.join(local_appdata, "lycus", "git", "usr", "bin"),
        # Lycus venv Scripts directory — host of the lycus.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes lycus, but harmless to include.
        os.path.join(local_appdata, "lycus", "lycus-agent", "venv", "Scripts"),
