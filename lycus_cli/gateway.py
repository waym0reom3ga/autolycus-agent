"""
Gateway subcommand for lycus CLI.

Handles: lycus gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_lycus_home is imported lazily at call sites to avoid ImportError
# when lycus_constants is cached from a pre-update version during `lycus update`.
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "lycus CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``lycus gateway status`` from falsely counting the ``lycus`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``lycus gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    exclude_pids = exclude_pids | _get_ancestor_pids()
    pids: list[int] = []
    patterns = [
        "lycus_cli.main gateway",
        "lycus_cli.main --profile",
        "lycus_cli.main -p",
        "lycus_cli/main.py gateway",
        "lycus_cli/main.py --profile",
        "lycus_cli/main.py -p",
        "lycus gateway",
        ...
        # ``lycus.exe --profile`` / ``lycus.exe -p`` would also match
        # ``lycus.exe --profile foo dashboard`` and other CLI subcommands,
        # producing false-positive gateway PIDs (Copilot review).
        "lycus.exe gateway",
        "lycus-gateway.exe",
        "gateway/run.py",
    ]
    current_home = str(get_lycus_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"lycus_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. AUTOLYCUS_HOME
        ...
        # AUTOLYCUS_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "lycus_home=" in command_lc
            and f"lycus_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``lycus update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Lycus profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Lycus profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid
        from lycus_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "lycus_cli.main"]
    if profile != "default":
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``lycus update`` users
    # with no running gateway until they re-invoke ``lycus gateway``
    ...
    from lycus_cli._subprocess_compat import (
...
def _sync_lycus_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``AUTOLYCUS_HOME``.

    Under ``sudo``, ``AUTOLYCUS_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_lycus_home` falls back to ``/root/.autolycus`` — the wrong
    profile. The unit file pins ``AUTOLYCUS_HOME`` for the actual gateway
    ...
    unit_home = env.get("AUTOLYCUS_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("AUTOLYCUS_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["AUTOLYCUS_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}lycus gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}lycus gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    from lycus_constants import is_container
    ...
    if is_linux() and is_container():
        ...
        try:
            from lycus_cli.service_manager import detect_service_manager, get_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `lycus gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``lycus gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from lycus_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (AUTOLYCUS_HOME-scoped).
...
from lycus_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Lycus Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``lycus gateway run`` must remain interruptible from
...
_SERVICE_BASE = "lycus-gateway"
SERVICE_DESCRIPTION = "Lycus Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current AUTOLYCUS_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (AUTOLYCUS_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from lycus_constants import get_default_lycus_root

    home = get_lycus_home().resolve()
    default = get_default_lycus_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary AUTOLYCUS_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(lycus_home: str | None = None, default_root: str | Path | None = None) -> str:
    """Return ``--profile <name>`` only when AUTOLYCUS_HOME is a named profile.

    For ``~/.autolycus/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        lycus_home: Optional explicit AUTOLYCUS_HOME path. Defaults to the current
            ``get_lycus_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
        default_root: Optional Lycus root to compare against. Used when
            generating a system service for another user from a sudo/root
            process, where ``Path.home()`` and ``get_default_lycus_root()``
            refer to root but the target profile lives under the service user.
    """
    import re
    from lycus_constants import get_default_lycus_root

    home = Path(lycus_home or str(get_lycus_home())).resolve()
    default = Path(default_root).resolve() if default_root else get_default_lycus_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
    try:
...
def _profile_arg_for_target_user(lycus_home: str, target_home_dir: str) -> str:
    """Return the profile arg for a system service running as another user."""
    target_root = Path(target_home_dir) / ".autolycus"
    try:
        Path(lycus_home).resolve().relative_to(target_root.resolve())
        return _profile_arg(lycus_home, default_root=target_root)
    except ValueError:
        return _profile_arg(lycus_home)


def get_service_name() -> str:
    """Derive a systemd service name scoped to this AUTOLYCUS_HOME.

    Default ``~/.autolycus`` returns ``lycus-gateway`` (backward compatible).
    Profile ``~/.autolycus/profiles/coder`` returns ``lycus-gateway-coder``.
    Any other AUTOLYCUS_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Lycus installs that predate the
# lycus-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (lycus-gateway-*.service) and unrelated third-party
# "lycus" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("lycus.service",)
...
def _find_legacy_lycus_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Lycus gateway units.

    Detects unit files installed by older Lycus versions that used a
    different service name (e.g. ``lycus.service`` before the rename to
    ``lycus-gateway.service``). When both a legacy unit and the current
    ``lycus-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``lycus-gateway-coder.service`` and unrelated third-party
      ``lycus-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``lycus.service`` running an unrelated
...
def has_legacy_lycus_units() -> bool:
    """Return True when any legacy Lycus gateway unit files exist."""
    return bool(_find_legacy_lycus_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Lycus gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_lycus_units()
    if not legacy:
        return
    print_warning("Legacy Lycus gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current lycus-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    lycus gateway migrate-legacy")


def remove_legacy_lycus_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Lycus gateway unit files.

    Iterates over whatever ``_find_legacy_lycus_units()`` returns — which is
    ...
    legacy = _find_legacy_lycus_units()
    if not legacy:
        print("No legacy Lycus gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Lycus gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: lycus gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo lycus gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    lycus gateway uninstall")
    print_info("    sudo lycus gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning(
                "  System service install requires sudo, so Lycus can't create it from this user session."
            )
            if run_as_user:
                print_info(
                    f"  After setup, run: sudo lycus gateway install --system --run-as-user {run_as_user}"
                )
            else:
                print_info(
                    "  After setup, run: sudo lycus gateway install --system --run-as-user <your-user>"
                )
            print_info("  Then start it with: sudo lycus gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Lycus often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.autolycus`` → ``ai.autolycus.gateway.plist`` (backward compatible).
    Profile ``~/.autolycus/profiles/coder`` → ``ai.autolycus.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.autolycus.gateway-{suffix}" if suffix else "ai.autolycus.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.autolycus/lycus-agent  -> /home/alice/.autolycus/lycus-agent
      /opt/lycus                 -> /opt/lycus  (kept as-is)
...
def _lycus_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current AUTOLYCUS_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_lycus_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.autolycus                    → /home/alice/.autolycus
      /root/.autolycus/profiles/coder     → /home/alice/.autolycus/profiles/coder
      /opt/custom-lycus               → /opt/custom-lycus  (kept as-is)
    """
    current_lycus = get_lycus_home().resolve()
    current_default = (Path.home() / ".autolycus").resolve()
    target_default = Path(target_home_dir) / ".autolycus"

    # Default ~/.autolycus → remap to target user's default
    if current_lycus == current_default:
        return str(target_default)

    # Profile or subdir of ~/.autolycus → preserve the relative structure
    try:
        relative = current_lycus.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.autolycus) — keep as-is
        return str(current_lycus)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    lycus_node = lycus_home / "node" / "bin"
    if _is_dir(lycus_node):
        candidates.append(str(lycus_node))
    lycus_nm = lycus_home / "node_modules" / ".bin"
    if _is_dir(lycus_nm):
        candidates.append(str(lycus_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m lycus_cli.main``, so module
    ...
    dir, or a clone that ``lycus update`` later relocates/removes — the path
    ...
    ``AUTOLYCUS_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if AUTOLYCUS_HOME
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        lycus_home = _lycus_home_for_target_user(home_dir)
        profile_arg = _profile_arg_for_target_user(lycus_home, home_dir)
        # Remap all paths that may resolve under the calling user's home
        # (e.g. /root/) to the target user's home so the service can
        # actually access them.
        python_path = _remap_path_for_user(python_path, home_dir)
        # Anchor cwd to the target user's AUTOLYCUS_HOME (stable, always exists)
        # rather than a remapped source-checkout path that can rot. See
        # _stable_service_working_dir() for the full rationale.
        working_dir = str(lycus_home) if lycus_home else _remap_path_for_user(working_dir, home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m lycus_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
Environment="AUTOLYCUS_HOME={lycus_home}"
Restart=always
RestartSec=5
...
"""
    ...
    lycus_home = str(get_lycus_home().resolve())
    profile_arg = _profile_arg(lycus_home)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir AUTOLYCUS_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved AUTOLYCUS_HOME
    ...
    exported a throwaway ``AUTOLYCUS_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``lycus gateway restart``
    with ``AUTOLYCUS_HOME=/tmp/lycus-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="AUTOLYCUS_HOME=..."`` lines and launchd
    ``<key>AUTOLYCUS_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'AUTOLYCUS_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>AUTOLYCUS_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp AUTOLYCUS_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: AUTOLYCUS_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported AUTOLYCUS_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only AUTOLYCUS_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp AUTOLYCUS_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../lycus_test`` path into
    # ``Environment="AUTOLYCUS_HOME=..."``. Writing that to the developer's
    ...
    if not system and (
        ...
        or '/lycus_test"' in new_unit
        or "/lycus_test/" in new_unit
    ):
        return False

    # Structural variant of the same belt: refuse to bake ANY temp-dir
    # AUTOLYCUS_HOME into the unit (manual E2E homes like /tmp/lycus-e2e-NNN
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Lycus install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo lycus gateway uninstall --system")
    print_info("         lycus gateway install")
    print_info("         lycus gateway start")
...
def systemd_install(
    ...
    # Offer to remove legacy units (lycus.service from pre-rename installs)
    # before installing the new lycus-gateway.service. If both remain, they
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}lycus gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}lycus gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}lycus gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    _sync_lycus_home_from_systemd_unit(system=system)
    try:
    ...
    try:
        ...
        print(
            ...
            "check `lycus gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `lycus gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}lycus gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}lycus gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}lycus gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}lycus gateway start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.autolycus.gateway-{suffix}" if suffix else "ai.autolycus.gateway"


# Cached launchd domain result — probing is cheap but should only run once per
# process invocation (each ``lycus gateway start/stop/status`` call).
...
# (the documented `nohup lycus gateway run` workaround). See #23387.
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m lycus_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active AUTOLYCUS_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "lycus_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup lycus gateway run --replace`
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    """
    from lycus_constants import display_lycus_home as _dhh
    ...
    if _spawn_detached_gateway():
        ...
        print("  Stop it with: lycus gateway stop")
        return True
    print_error("Failed to start the gateway as a background process.")
    print(
        f"  Try manually: nohup lycus gateway run --replace "
...
<dict>
    ...
    <dict>
        ...
        <key>AUTOLYCUS_HOME</key>
        <string>{lycus_home}</string>
    </dict>
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Lycus install"
    )
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  lycus gateway status             # Check status")
    from lycus_constants import display_lycus_home as _dhh
...
def launchd_stop():
    ...
    # `lycus gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Lycus install")
    else:
        print("⚠ Service definition is stale relative to the current Lycus install")
        print("  Run: lycus gateway start")
    ...
    if deep:
        log_file = get_lycus_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``lycus gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``lycus gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other AUTOLYCUS_HOME roots.
    """
    ...
    print("  Use 'lycus gateway restart' to replace it,")
    print("  or 'lycus gateway stop' first.")
    print("  Or use 'lycus gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print_error(
        "Refusing to run the Lycus gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'lycus' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/lycus/docker/entrypoint.sh before the Lycus command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$AUTOLYCUS_HOME and break later non-root dashboard/gateway runs."
    )
    print(
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `lycus gateway run` still
    ...
    # /restart) rather than through `lycus gateway restart` which already
    ...
    # `lycus gateway start/restart` — leaving the gateway vulnerable to
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("HERMES_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from lycus_constants import get_lycus_home as _ghh
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Lycus will log in directly",
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Full Matrix user ID, e.g. @lycus:matrix.example.org",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. lycus) and copy the bot token",
        ...
        "vars": [
            ...
            },
            {
                ...
                "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)",
                "password": False,
                "help": "Channel ID where Lycus delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Lycus agent",
        ...
        "vars": [
            {
                ...
                "password": False,
                "help": "The email address Lycus will use (e.g., lycus@gmail.com).",
            },
            {
                "name": "EMAIL_PASSWORD",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Lycus connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: lycus pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Lycus will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``lycus setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run lycus under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.autolycus/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_lycus_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_standard_platform(platform: dict):
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from lycus_cli.telegram_managed_bot import (
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'lycus pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: lycus pairing approve <platform> <code>"
                    )
                else:
                    print_info(
                        "  Skipped — configure later with 'lycus gateway setup'"
                    )
            continue
...
def _setup_whatsapp():
    ...
    from lycus_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from lycus_cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_wecom():
    ...
    if allowed:
        ...
        if access_idx == 0:
            ...
            print_info("  Approve with: lycus pairing approve <platform> <code>")
        elif access_idx == 2:
            save_env_value("WECOM_DM_POLICY", "disabled")
            print_warning("  Direct messages disabled.")
        else:
            print_info("  Skipped — configure later with 'lycus gateway setup'")
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        from lycus_cli import gateway_windows
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Lycus will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. Lycus will store the returned account_id/token in ~/.autolycus/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `lycus gateway setup`.")
        return
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_lycus_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `lycus pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `lycus pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "LycusAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``lycus_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from lycus_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.autolycus/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.autolycus/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: lycus gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: lycus gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: lycus gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo lycus gateway install --system"
                        )
                    print_info("  Or run in foreground:  lycus gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: lycus gateway run")
                print_info(
                    "  For persistence:   tmux new -s lycus 'lycus gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: lycus gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup lycus gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'lycus gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`lycus_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from lycus_cli.service_manager import (
    ...
    if profile is None:
        ...
        # AUTOLYCUS_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``lycus gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``lycus gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    # `docker logs` and ${AUTOLYCUS_HOME}/logs/gateways/<profile>/current,
    ...
    #
    # Prefer `sleep infinity` (matches the static main-lycus service's
    # pattern in docker/s6-rc.d/main-lycus/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: lycus gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: lycus gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s lycus 'lycus gateway run'"
                )
                print()
            ...
            print()
            print(
                "  lycus gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s lycus 'lycus gateway run'         # persistent via tmux"
            )
            print(
                "  nohup lycus gateway run > ~/.autolycus/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            from lycus_cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
                print("Per-profile gateways are auto-registered when you create a profile.")
                print()
                print("  lycus profile create <name>     # creates the s6 service slot")
                print("  lycus -p <name> gateway start   # bring it up via s6")
                print("  lycus status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: lycus gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: lycus gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: lycus gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  lycus profile delete <name>     # tears down the s6 service slot")
                print("  lycus -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `lycus -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: lycus gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_HERMES_GATEWAY") == "1":
            print_error(
                ...
                "Use `lycus gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_HERMES_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `lycus gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: lycus gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  lycus gateway install")
                else:
                    print("To install as a service:")
                    print("  lycus gateway install")
                    print("  sudo lycus gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  lycus gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup lycus gateway run > ~/.autolycus/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s lycus 'lycus gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  lycus gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  lycus gateway install  # Install as user service")
                    print(
                        "  sudo lycus gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy Lycus gateway unit files from
        # pre-rename installs (e.g. lycus.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_lycus_units(interactive=not yes, dry_run=dry_run)
