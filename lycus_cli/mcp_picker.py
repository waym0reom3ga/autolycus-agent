"""MCP picker — interactive `lycus mcp picker` (also the default `lycus mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``lycus mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `lycus plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from lycus_cli.cli_output import prompt_yes_no
from lycus_cli.curses_ui import curses_single_select
from lycus_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Lycus session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from lycus_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `lycus mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: lycus mcp install <name>    Picker: lycus mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Lycus is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Lycus — run `lycus update` "
...
def show_catalog() -> None:
    """`lycus mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`lycus mcp picker` (and default `lycus mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`lycus mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `lycus mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
