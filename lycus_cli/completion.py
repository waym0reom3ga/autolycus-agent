"""Shell completion script generation for lycus CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_lycus_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Lycus Agent bash completion
# Add to ~/.bashrc:
#   eval "$(lycus completion bash)"

_lycus_profiles() {{
    local profiles_dir="$HOME/.autolycus/profiles"
...
_lycus_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_lycus_profiles)" -- "$cur"))
        return
    fi
...
complete -F _lycus_completion lycus
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef lycus
# Lycus Agent zsh completion
# Add to ~/.zshrc:
...
_lycus_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.autolycus/profiles" ]]; then
        profiles+=($HOME/.autolycus/profiles/*(N/:t))
    fi
...
_lycus() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_lycus_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'lycus command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Lycus Agent fish completion",
        "# Add to your config:",
        "#   lycus completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __lycus_profiles",
        "    echo default",
        "    if test -d $HOME/.autolycus/profiles",
        "        for d in $HOME/.autolycus/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c lycus -f -s p -l profile"
        " -d 'Profile name' -xa '(__lycus_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__lycus_profiles)' -d 'Profile name'"
                )
