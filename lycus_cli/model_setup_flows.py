"""Per-provider model-selection wizard flows for ``lycus setup`` / ``lycus model``.

Extracted from ``lycus_cli/main.py`` as part of the god-file decomposition
campaign (``~/.autolycus/plans/god-file-decomposition.md``, Phase 2 — splitting
...
in main.py re-imports them (``from lycus_cli.model_setup_flows import *``-style
explicit import) so existing call sites — and test monkeypatches that target
``lycus_cli.main._model_flow_*`` — keep resolving against main.py's namespace.
...
imported lazily inside the flows (``from lycus_cli.main import ...`` resolves at
call time, when main.py is fully loaded) so this module never imports
``lycus_cli.main`` at import time -> no import cycle.
"""
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from lycus_cli.main import _prompt_api_key
    from lycus_constants import OPENROUTER_BASE_URL
    ...
    # in-flow (K/R/C) instead of having to edit ~/.autolycus/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `lycus setup` from scratch. OpenRouter
    ...
    from lycus_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from lycus_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from lycus_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    _codex_token = None
    # Prefer credential pool (where `lycus auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from lycus_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    if status.get("logged_in"):
        ...
        if choice == "reauth":
            ...
            print()
            try:
                # Forward CLI flags from ``lycus model --manual-paste``
                # / ``--no-browser`` / ``--timeout`` into the loopback
                # login. Without this, browser-only remotes (#26923)
                # can't reach the manual-paste path via ``lycus model``.
                mock_args = argparse.Namespace(
    ...
    # live only in the pool (e.g. after ``lycus auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from lycus_cli.main import _DEFAULT_QWEN_PORTAL_MODELS
    from lycus_cli.auth import (
...
def _model_flow_google_gemini_cli(_config, current_model=""):
    ...
    Flow:
      ...
      5. Save to ~/.autolycus/config.yaml.
    """
    from lycus_cli.auth import (
...
def _model_flow_custom(config):
    ...
    """
    from lycus_cli.main import _auto_provider_name, _prompt_custom_api_mode_selection, _save_custom_provider
    from lycus_cli.auth import _save_model_choice, deactivate_provider
    from lycus_cli.config import get_env_value, load_config, save_config
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `lycus model` to set a model.")
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from lycus_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from lycus_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from lycus_cli import azure_detect
    ...
    print("Anthropic-style API endpoints.  Lycus will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Lycus will install it now (the preflight below "
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from lycus_cli.main import _custom_provider_api_key_config_value, _custom_provider_base_url_config_value, _save_custom_provider
    from lycus_cli.auth import _save_model_choice, deactivate_provider
    from lycus_cli.config import load_config, save_config
    from lycus_cli.models import fetch_api_models
...
def _model_flow_copilot(config, current_model=""):
    ...
    from lycus_cli.main import _current_reasoning_effort, _prompt_reasoning_effort_selection, _set_reasoning_effort
    from lycus_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from lycus_cli.config import save_env_value, load_config, save_config
    from lycus_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from lycus_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from lycus_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Lycus turns to `copilot --acp`.")
    print("  Lycus currently starts its own ACP subprocess for each request.")
    print("  Lycus uses your selected model as a hint for the Copilot ACP session.")
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from lycus_cli.main import _infer_stepfun_region, _prompt_api_key, _prompt_provider_choice, _stepfun_base_url_for_region
    from lycus_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from lycus_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so lycus knows where to find it
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Lycus typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Lycus, enable billing on your "
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from lycus_cli.main import _run_anthropic_oauth_flow
    from lycus_cli.auth import (
        _prompt_model_selection,
        _save_model_choice,
        deactivate_provider,
    ...
    if selected:
        ...
        # providers if the user switches without running 'lycus model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
