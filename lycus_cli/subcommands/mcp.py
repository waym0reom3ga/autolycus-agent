"""``lycus mcp`` subcommand parser.

Extracted from ``lycus_cli/main.py:main()`` (god-file Phase 2 follow-up).
...
from lycus_cli.subcommands._shared import add_accept_hooks_flag
...
def build_mcp_parser(subparsers, *, cmd_mcp: Callable) -> None:
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Lycus as an MCP server",
        description=(
            "Manage MCP server connections and run Lycus as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'lycus mcp add' to connect to a new server, or\n"
            "'lycus mcp serve' to expose Lycus conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Lycus as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `lycus mcp add ...` to fall through to
    # interactive chat.
    ...
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `lycus mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
    ...
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `lycus mcp install n8n`)",
    )
