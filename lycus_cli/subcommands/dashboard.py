"""``lycus dashboard`` subcommand parser.
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Lycus Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    # for `lycus dashboard` cmdlines and SIGTERM them directly — the same
    # path `lycus update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running lycus dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running lycus dashboard processes and exit",
    )
    # Backward-compat shim: older Lycus desktop app shells (<= 0.15.x) spawn the
    # backend as `lycus dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Lycus couldn't start" with no actionable cause. Accept and silently
    ...
    # `lycus dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.autolycus/.env.
    # Nested subparser so bare `lycus dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "HERMES_DASHBOARD_OAUTH_CLIENT_ID into ~/.autolycus/.env, and prints "
            "how to engage the login gate. Requires being logged in (lycus setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://lycus.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
