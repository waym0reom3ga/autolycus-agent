"""``lycus gateway`` and ``lycus proxy`` subcommand parsers.
...
def build_gateway_parser(subparsers, *, cmd_gateway: Callable, cmd_proxy: Callable) -> None:
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy lycus.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Lycus gateway unit files "
            "(e.g. lycus.service) left over from older installs. Profile "
            "units (lycus-gateway-<profile>.service) and unrelated "
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `lycus proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
