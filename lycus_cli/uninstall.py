"""
Lycus Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.autolycus/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Lycus PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing lycus-agent or lycus PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Lycus Agent" comment and following line
                if '# Lycus Agent' in line or '# lycus-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('lycus' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing lycus
                if 'lycus' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the lycus wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "lycus",
        Path("/usr/local/bin/lycus"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains lycus_cli reference)
                content = wrapper.read_text()
                if 'lycus_cli' in content or 'lycus-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(lycus_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``lycus`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Lycus home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (lycus_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``lycus gateway run`` processes
    ...
    try:
        from lycus_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from lycus_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``AUTOLYCUS_HOME`` and ``HERMES_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\lycus\git\cmd``, ``%LOCALAPPDATA%\lycus\git\bin``,
#      ``%LOCALAPPDATA%\lycus\git\usr\bin``, ``%LOCALAPPDATA%\lycus\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\lycus\git\`` and Node to
#      ``%LOCALAPPDATA%\lycus\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``lycus dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\lycus\gateway-service\`` and sometimes
...
def _lycus_path_markers(lycus_home: Path) -> list[str]:
    """Path-entry substrings that identify Lycus-owned User-PATH entries."""
    root = str(lycus_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare lycus-agent install dir.
    markers = [root + "\\lycus-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if AUTOLYCUS_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "lycus".  We don't want to catch
    # unrelated entries like "clycus-foo" or "ephermeral", so we look for
    # backslash-lycus as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(lycus_home: Path) -> list[str]:
    """Strip Lycus-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _lycus_path_markers(lycus_home)
            kept: list[str] = []
...
def remove_lycus_env_vars_windows() -> list[str]:
    """Delete AUTOLYCUS_HOME and HERMES_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("AUTOLYCUS_HOME", "HERMES_GIT_BASH_PATH"):
                try:
...
def _is_default_lycus_home(lycus_home: Path) -> bool:
    """Return True when ``lycus_home`` points at the default (non-profile) root."""
    try:
        from lycus_constants import get_default_lycus_root
        return lycus_home.resolve() == get_default_lycus_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its AUTOLYCUS_HOME directory.

    We shell out to ``lycus -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current AUTOLYCUS_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m lycus_cli.main` so we don't depend on a `lycus`
    #    wrapper that may be half-removed mid-uninstall.
    lycus_invocation = [_sys.executable, "-m", "lycus_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                lycus_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's AUTOLYCUS_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``lycus uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$AUTOLYCUS_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from lycus_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(lycus_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         🔱 Lycus Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Lycus Chat GUI installation was found.")
        print(f"  Checked: {lycus_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Lycus agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(lycus_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Lycus agent at {lycus_home / 'lycus-agent'}")
        print(f"  • Your config, sessions, and secrets under {lycus_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Lycus agent is still installed. Run 'lycus' to use the CLI,")
    print("or 'lycus uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.autolycus/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.autolycus/ for future reinstall
    """
    project_root = get_project_root()
    lycus_home = get_lycus_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie AUTOLYCUS_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_lycus_home(lycus_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.autolycus data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            🦊 Lycus Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {lycus_home / 'config.yaml'}")
    print(f"  Secrets: {lycus_home / '.env'}")
    print(f"  Data:    {lycus_home / 'cron/'}, {lycus_home / 'sessions/'}, {lycus_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their AUTOLYCUS_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Lycus data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Lycus code but keep your configuration and data.")
...
def _perform_uninstall(
    *,
    ...
    ``lycus`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$AUTOLYCUS_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in lycus_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\lycus\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(lycus_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Lycus-owned PATH entries in User environment")

        log_info("Removing AUTOLYCUS_HOME / HERMES_GIT_BASH_PATH User env vars...")
        removed_env = remove_lycus_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Lycus-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing lycus command...")
    ...
    #     (only when they still point into this Lycus home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Lycus-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(lycus_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Lycus-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(lycus_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE AUTOLYCUS_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from lycus_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(lycus_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.autolycus/, just remove the lycus-agent subdir
            if lycus_home in project_root.parents or project_root.parent == lycus_home:
    ...
    #     under AUTOLYCUS_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(lycus_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(lycus_home)
    ...
    # 5. Optionally remove ~/.autolycus/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile AUTOLYCUS_HOME dirs live under
        ...
        try:
            if lycus_home.exists():
                shutil.rmtree(lycus_home)
                log_success(f"Removed {lycus_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {lycus_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {lycus_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://lycus-agent.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://lycus-agent.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Lycus Agent! 🦊")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m lycus_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import lycus_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``lycus_constants`` + ``lycus_cli.colors``
    (and lazily ``lycus_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m lycus_cli.uninstall")
    parser.add_argument(
        "--mode",
