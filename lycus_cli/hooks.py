"""lycus hooks — inspect and manage shell-script hooks.

Usage::

    lycus hooks list
    lycus hooks test <event> [--for-tool X] [--payload-file F]
    lycus hooks revoke <command>
    lycus hooks doctor

Consent records live under ``~/.autolycus/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.autolycus/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``lycus hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: lycus hooks {list|test|revoke|doctor}")
        print("Run 'lycus hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.autolycus/config.yaml.")
        print("See `lycus hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `lycus hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `lycus hooks test` and `lycus hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Lycus wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `lycus hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `lycus hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `lycus hooks doctor`.")
