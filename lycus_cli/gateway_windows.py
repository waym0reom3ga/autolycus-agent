...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m lycus_cli.main gateway run --replace`` invocation) and
...
_TASK_NAME_DEFAULT = "Lycus_Gateway"
_TASK_DESCRIPTION = "Autolycus Gateway - Messaging Platform Integration"
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Lycus profile."""
    from lycus_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "lycus_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def get_task_name() -> str:
    ...
    Default profile: ``Lycus_Gateway``
    Named profile X: ``Lycus_Gateway_<X>``
    """
    _assert_windows()
    # Local import to avoid circular module initialization during lycus_cli boot.
    from lycus_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\lycus\\gateway-service\\<task_name>.cmd``
    (or ``<AUTOLYCUS_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Lycus installs stay self-contained).
    """
    _assert_windows()
    from lycus_cli.config import get_lycus_home

    script_dir = Path(get_lycus_home()) / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``AUTOLYCUS_HOME`` whenever it
    ...
    source checkout only if ``AUTOLYCUS_HOME`` cannot be resolved yet.
    """
    from lycus_cli.config import get_lycus_home
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports AUTOLYCUS_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``pythonw -m lycus_cli.main [--profile X] gateway run``
    ...
    lines.append(f'set "AUTOLYCUS_HOME={lycus_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "HERMES_GATEWAY_DETACHED=1"')
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports lycus_constants-based logic during startup.
    venv_dir = str(Path(python_path).resolve().parent.parent)
    lines.append(f'set "VIRTUAL_ENV={venv_dir}"')

    pythonw_path = _derive_venv_pythonw(python_path)
    prog_args = [pythonw_path, "-m", "lycus_cli.main"]
    if profile_arg:
...
def _write_task_script() -> Path:
    ...
    lycus_home = str(Path(get_lycus_home()).resolve())
    profile_arg = _profile_arg(lycus_home)

    content = _build_gateway_cmd_script(python_path, working_dir, lycus_home, profile_arg)
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Lycus builds and failed
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "lycus_cli.main"]
    if profile_arg:
        argv.extend(profile_arg.split())
    argv.extend(["gateway", "run"])

    env_overlay = {
        "AUTOLYCUS_HOME": lycus_home,
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m lycus_cli.main gateway run``
    ...
    log_dir = Path(get_lycus_home()) / "logs"
...
def _prompt_install_choices(
    ...
    from lycus_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `lycus -p <profile> gateway install` must be safe.
    ...
    from lycus_cli.gateway import find_gateway_pids, _profile_arg
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"lycus {profile_arg} gateway start" if profile_arg else "lycus gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: lycus gateway start")
        return
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated Lycus gateway install prompt.")
                if start_now:
                    print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
                else:
                    print("  Approve the Windows UAC prompt, then run: lycus gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: lycus gateway start")
        _print_next_steps()
        return
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from lycus_cli.config import get_lycus_home
        print(f"    type {Path(get_lycus_home()).resolve()}\\logs\\gateway.log")
        print(f"    type {Path(get_lycus_home()).resolve()}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from lycus_cli.config import get_lycus_home

    lycus_home = Path(get_lycus_home()).resolve()
    print()
    print("Next steps:")
    print("  lycus gateway status                      # Check status")
    print(f"  type {lycus_home}\\logs\\gateway.log       # View logs")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated Lycus gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: lycus gateway status")
                    return
...
def _print_deep_probes() -> None:
    ...
    home = Path(get_lycus_home()).resolve()
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: lycus gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: lycus gateway start")
            return
...
def stop() -> None:
    ...
    """
    _assert_windows()
    from lycus_cli.gateway import kill_gateway_processes, _get_restart_drain_timeout
    from gateway.status import get_running_pid
