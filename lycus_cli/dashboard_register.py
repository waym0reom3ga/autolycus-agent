"""``lycus dashboard register`` — register a self-hosted dashboard OAuth client.
...
resulting ``agent:{id}`` OAuth client ID, and paste it into ``~/.autolycus/.env``
as ``HERMES_DASHBOARD_OAUTH_CLIENT_ID``.

This command:
  1. Resolves a fresh Nous Portal access token from the existing login
     (``~/.autolycus/auth.json``), refreshing it if needed. Fails fast with a
     "run `lycus setup`" hint when the user isn't logged in.
  ...
  3. Writes ``HERMES_DASHBOARD_OAUTH_CLIENT_ID`` and (if absent)
     ``HERMES_DASHBOARD_PORTAL_URL`` into ``~/.autolycus/.env`` idempotently.
...
def _resolve_portal_base_url(override: Optional[str] = None) -> str:
    ...
    try:
        from lycus_cli.auth import DEFAULT_NOUS_PORTAL_URL, get_provider_auth_state
...
def _register_self_hosted_client(
    *,
    ...
    makes re-running ``lycus dashboard register`` idempotent. The portal
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                ...
                "Try `lycus auth login nous` to re-authenticate."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def _print_post_register_hint(
    *,
    ...
    from lycus_cli.config import get_env_path
    ...
    print(
        ...
        "  `lycus dashboard` (localhost) leaves the gate off and serves locally\n"
    ...
    if custom_redirect_uri:
        ...
        print("    lycus dashboard --host 0.0.0.0")
...
def cmd_dashboard_register(args) -> None:
    ...
    from lycus_cli.auth import AuthError, resolve_nous_access_token
    from lycus_cli.config import get_env_value, is_managed, save_env_value
    ...
    if is_managed():
        print(
            "✗ `lycus dashboard register` is not available in a managed/hosted "
            "install.\n"
    ...
    try:
        ...
        if getattr(exc, "relogin_required", False):
            ...
            print("  Run `lycus setup` (or `lycus auth login nous`) first, then retry.")
        else:
    ...
    # the portal, e.g. https://lycus.example.com/auth/callback. At serve time
