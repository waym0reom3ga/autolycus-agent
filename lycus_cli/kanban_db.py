...
``<root>`` is the **shared Lycus root** (the parent of any active
...
``lycus -p <profile>`` joins the same board as the dispatcher that
...
the profile's Lycus home but are otherwise isolated: a worker spawned
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``lycus kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.autolycus``. In Docker / custom
deployments where ``AUTOLYCUS_HOME`` points outside ``~/.autolycus`` (e.g.
``/opt/lycus``), ``<root>`` is ``AUTOLYCUS_HOME``. Legacy env-var
...
coding workloads.  See ``docs/lycus-kanban-v1-spec.pdf`` for the full
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "lycus_kanban_current_board_override",
    default=None,
)
...
# enough that kebab-case names like ``atm10-server`` or ``lycus-agent``
...
def kanban_home() -> Path:
    """Return the shared Lycus root that anchors the kanban board.

    Resolution order:

    1. ``HERMES_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_lycus_root()``, which already returns ``<root>``
       when ``AUTOLYCUS_HOME`` is ``<root>/profiles/<name>``, and returns
       ``AUTOLYCUS_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``AUTOLYCUS_HOME`` would silently fork the board per profile,
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``lycus kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    2. ``<root>/kanban/current`` on disk (set by ``lycus kanban boards
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``lycus kanban boards switch <typo>`` returns an error
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``lycus kanban log`` unambiguous even when multiple
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Lycus treats as user-controlled-but-trusted on the
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See lycus_state._WAL_INCOMPAT_MARKERS for detection logic.
                from lycus_state import apply_wal_with_fallback
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``lycus kanban init`` and the
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from lycus_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``lycus --skills ...`` alongside the built-in
    ...
    # `lycus --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``lycus kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `lycus kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``lycus kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``lycus kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``lycus kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``lycus kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Lycus' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Lycus-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``lycus kanban show <id>`` and the dashboard.
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Lycus
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``lycus kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Lycus profile.
    ...
    if not rows:
        return False
    try:
        from lycus_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def dispatch_once(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from lycus_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Lycus profile.
        # `_default_spawn` invokes ``lycus -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Lycus
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `lycus kanban tail` — without
...
def _module_lycus_argv() -> list[str]:
    """Return the interpreter-bound Lycus CLI invocation."""
    # ``lycus_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``lycus`` package — there is no
    # ``lycus`` package to import.
    return [sys.executable, "-m", "lycus_cli.main"]


def _absolute_lycus_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Lycus shim."""
...
def _lycus_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Lycus executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_lycus_argv()
    return [_absolute_lycus_path(path)]


def _resolve_lycus_argv() -> list[str]:
    """Resolve the ``lycus`` invocation as argv parts for ``Popen``.
    ...
    2. ``shutil.which("lycus")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\lycus.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m lycus_cli.main`` — fallback for setups where
       Lycus is launched from a venv and the ``lycus`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_lycus_bin`` for the same reason. Kept
    local (not imported from gateway) because ``lycus_cli`` sits below
    ...
    if env_bin:
        if _looks_like_path(env_bin):
            return _lycus_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _lycus_path_argv(resolved_env_bin)
        return _module_lycus_argv()

    lycus_bin = _safe_which_no_cwd("lycus") if _IS_WINDOWS else shutil.which("lycus")
    if lycus_bin:
        return _lycus_path_argv(lycus_bin)
    return _module_lycus_argv()


def _kanban_worker_skill_available(lycus_home: Optional[str]) -> bool:
    """True if the bundled ``kanban-worker`` skill resolves for the home the
    spawned worker will run under.

    The dispatcher injects ``--skills kanban-worker`` into every worker. When
    the worker activates a profile (``lycus -p <name>``), its ``SKILLS_DIR``
    ...
    # An unset AUTOLYCUS_HOME means the worker falls back to the default root
    # home (``~/.autolycus``), which ships the bundled skill.
    base = _Path(lycus_home) if lycus_home else (_Path.home() / ".autolycus")
...
def _resolve_worker_cli_toolsets(lycus_home: Optional[str]) -> Optional[list[str]]:
    ...
    try:
        ...
        token = set_lycus_home_override(lycus_home)
        try:
            cfg = load_config()
        ...
        _log.debug(
            "kanban worker: could not resolve CLI toolsets for AUTOLYCUS_HOME=%r (%s)",
            lycus_home,
            exc,
        )
        return None
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``lycus -p <profile> chat -q ...`` subprocess.
    ...
    # Inject AUTOLYCUS_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `lycus -p <name>` the
    # _apply_profile_override() runs *before* lycus_constants is imported.
    # If AUTOLYCUS_HOME is absent from the child's env, get_lycus_home() falls
    # back to Path.home() / ".autolycus" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from lycus_cli.profiles import resolve_profile_env
    try:
        env["AUTOLYCUS_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via HERMES_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # AUTOLYCUS_HOME never had profiles created.
        pass
    if task.tenant:
    ...
    # that even when the worker activates a profile (`lycus -p <name>`
    # rewrites AUTOLYCUS_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_lycus_root()`
    ...
    # `lycus -p <assignee>` activates the profile, but the env var is
    ...
    cmd = [
        *_resolve_lycus_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped AUTOLYCUS_HOME above,
    ...
    if _kanban_worker_skill_available(env.get("AUTOLYCUS_HOME")):
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("AUTOLYCUS_HOME"))
    if worker_toolsets:
    ...
    # `lycus kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`lycus` executable not found on PATH. "
            "Install Autolycus or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``lycus kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    # the user to wire anything into MASK.md / MEMORY.md. Bounded to the
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled HERMES_PROFILE values like "lycus-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Lycus root exists

    Reads profile paths directly so this module has no import dependency on
    ``lycus_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from lycus_constants import get_default_lycus_root
        default_root = get_default_lycus_root()
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``lycus kanban assignees`` for the terminal.
