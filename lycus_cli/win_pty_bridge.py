"""Windows ConPTY bridge for the `lycus dashboard` chat tab.

Drop-in counterpart to ``lycus_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``lycus_cli.web_server`` consumes: ``spawn``, ``read``,
