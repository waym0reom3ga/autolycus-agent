"""
Backup and import commands for lycus CLI.

`lycus backup` creates a zip archive of the entire ~/.autolycus/ directory
(excluding the lycus-agent repo and transient files).

`lycus import` restores from a backup zip, overlaying onto the current
AUTOLYCUS_HOME root.
"""
...
from lycus_constants import get_default_lycus_root, get_lycus_home, display_lycus_home
...
# ``lycus-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/lycus-agent/``
# are not accidentally excluded.
_EXCLUDED_DIRS = {
    "lycus-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to lycus root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``lycus-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/lycus-agent/`` — must be preserved.
        if part == "lycus-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside AUTOLYCUS_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Lycus home directory."""
    lycus_root = get_default_lycus_root()

    if not lycus_root.is_dir():
        print(f"Error: Lycus home directory not found at {lycus_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"lycus-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"lycus-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_lycus_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(lycus_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(lycus_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``lycus-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "lycus-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(lycus_root)
    ...
    print(f"\nRestore with: lycus import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Lycus backup.
    ...
    # Look for telltale files that a lycus home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Lycus backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.autolycus/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a lycus dir name
        if prefix in {".autolycus", "lycus"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_lycus_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (lycus_root / "config.yaml").exists()
        has_env = (lycus_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Lycus configuration.")
        ...
        for member in members:
            ...
            target = lycus_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(lycus_root.resolve())
            except ValueError:
        ...
        profiles_dir = lycus_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # lycus_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: lycus profile list  (after installing lycus)")

        # Guidance
        print()
        if not (lycus_root / "lycus-agent").is_dir():
            print("Note: The lycus-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: lycus update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  lycus -p {pname} gateway install")

        print("Done. Your Lycus configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and lycus backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to AUTOLYCUS_HOME).
...
# — `lycus update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(lycus_home: Optional[Path] = None) -> Path:
    home = lycus_home or get_lycus_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    lycus_home: Optional[Path] = None,
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(lycus_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside AUTOLYCUS_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``lycus update``.
    ...
    Args:
        ...
        lycus_home: Override for the Lycus home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(lycus_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for lycus backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_lycus_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, lycus_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``lycus_root`` to ``out_path``.
...
def _pre_update_backup_dir(lycus_home: Optional[Path] = None) -> Path:
    home = lycus_home or get_lycus_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of AUTOLYCUS_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<AUTOLYCUS_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``lycus update``) should continue even if the backup fails.
    """
    lycus_root = lycus_home or get_default_lycus_root()
    if not lycus_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(lycus_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, lycus_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `lycus claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of AUTOLYCUS_HOME under ``backups/`` before a
    ``lycus claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``lycus import <archive>``.  Writes to
    ``<AUTOLYCUS_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `lycus import` and the
