"""Slash command definitions and autocomplete for the Lycus CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Lycus config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Lycus works on across turns until achieved", "Session",
    ...
    CommandDef("personality", "Set a predefined personality", "Configuration",
               args_hint="[name]"),
    CommandDef("lycus-greeting", "Switch Lycus greeting personality template", "Configuration",
               cli_only=True, args_hint="[template]"),
    ...
    CommandDef("busy", "Control what Enter does while Lycus is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.autolycus/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Lycus Agent to the latest version", "Info"),
    CommandDef("version", "Show Lycus Agent version", "Info", aliases=("v",)),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`lycus_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/lycus`` subcommand mapping, and
...
Telegram only displays a small BotCommand menu in practice.  The full Lycus
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``lycus skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``lycus skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``lycus skills list`` and
...
# unrelated command landed. These claim their slots right after /lycus,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /lycus <command>).
...
# native slot, the alias spelling stays reachable via /lycus reset).
...
# ``/lycus <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/lycus" decision, not a silent clamp.
#   - credits: the billing/top-up surface; reached via /lycus credits on Slack.
#   - debug: the log/report upload surface; reached via /lycus debug on Slack.
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/lycus <verb>`` subcommand.
    ...
    can still reach them via ``/lycus <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/lycus`` is always reserved as the first entry so the
    legacy ``/lycus <subcommand>`` form keeps working for anything that
    ...
    # Reserve /lycus as the catch-all top-level command.
    entries.append(("lycus", "Talk to Lycus or run a subcommand", "[subcommand] [args]"))
    seen.add("lycus")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_VIA_HERMES_ONLY:
            # Intentionally Slack-via-/lycus only (see _SLACK_VIA_HERMES_ONLY).
            return
    ...
    # everything except /lycus, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://lycus-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /lycus handler.

    Maps both canonical names and aliases so /lycus bg do stuff works
    the same as /lycus background do stuff.

    Plugin-registered slash commands are included so ``/lycus <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from lycus_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
