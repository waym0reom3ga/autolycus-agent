"""
Interactive setup wizard for Lycus Agent.
...
_DOCS_BASE = "https://lycus-agent.nousresearch.com/docs"
...
# display_lycus_home imported lazily at call sites (stale-module safety during lycus update)
...
from lycus_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from lycus_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("🦊 Lycus Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Lycus using environment variables or config commands:")
    print_info("  lycus config set model.provider custom")
    print_info("  lycus config set model.base_url http://localhost:8080/v1")
    print_info("  lycus config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'lycus setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from lycus_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'lycus setup')")


def _print_setup_summary(config: dict, lycus_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'lycus setup' to configure"))
    ...
    # Video generation — opt-in via `lycus tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from lycus_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'lycus setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'lycus setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'lycus setup terminal'"))
    ...
    # Spotify (OAuth via lycus auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'lycus setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {lycus_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('lycus setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('lycus setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('lycus setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('lycus setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('lycus setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('lycus config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('lycus config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('lycus config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('lycus', Colors.GREEN)}              Start chatting")
    print(f"   {color('lycus gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('lycus doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `lycus tools` and `lycus setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``lycus model``)
    ...
    provider added to ``lycus model`` is automatically available here.
    ...
    # Delegate to the shared lycus model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from lycus_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: lycus model")
    ...
    # on demand via `lycus auth add`, `lycus setup` vision, and
    # `lycus setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``lycus model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from lycus_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.autolycus/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Lycus auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via lycus setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'lycus setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Lycus runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `lycus setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `lycus setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    config["compression"]["threshold"] = 0.60
    ...
    print_info("  Compression threshold: 0.60")
    print_info("  Session reset: never (use /reset or compression)")
    print_info("  Run `lycus setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    current_threshold = cfg_get(config, "compression", "threshold", default=0.60)
    ...
    print_success(
        f"Context compression threshold set to {config['compression'].get('threshold', 0.60)}"
    )
...
def _setup_telegram_auto_result():
    ...
    try:
        from lycus_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_lycus_home(Path(get_lycus_home()))
    except Exception:
        pass
...
def _profile_name_from_lycus_home(lycus_home) -> str | None:
    """Return the active profile name when AUTOLYCUS_HOME is a profile dir."""
    if lycus_home.parent.name == "profiles":
        return lycus_home.name
    return None
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Lycus delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `lycus update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://lycus-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under AUTOLYCUS_HOME, and print
    ...
    """
    try:
        from lycus_cli.slack_cli import _build_full_manifest
        from lycus_constants import get_lycus_home

        manifest = _build_full_manifest(
            bot_name="Lycus",
            bot_description="Your Lycus agent on Slack",
        )
        target = Path(get_lycus_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `lycus slack manifest --write` anytime to refresh after "
            "Lycus adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Lycus delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Lycus to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from lycus_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://lycus-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://lycus-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  lycus config edit")
    print_info("   Open config in your editor:  lycus config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from lycus_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Lycus from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'lycus setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     lycus config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_lycus_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo lycus gateway install --system")
                print_info("  Or run in foreground:  lycus gateway")
        else:
            from lycus_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   lycus gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   lycus gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `lycus setup tools` and `lycus tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``lycus_cli.auth`` — lists every supported
    ...
    """
    try:
        from lycus_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from lycus_cli.config
    ...
    if section_key == "model":
        ...
        from lycus_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_lycus.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_lycus migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_lycus", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Lycus.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Lycus to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Lycus at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Lycus at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Lycus at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Lycus at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Lycus equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Lycus config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Lycus.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Lycus's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(lycus_home: Path) -> bool:
    ...
    print_info("Lycus can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: lycus claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=lycus_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: lycus claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Lycus configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Lycus config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Lycus (use lycus claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``lycus setup --portal`` and ``lycus portal``. This is the
    ...
    Lycus session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``lycus setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``lycus model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``lycus portal`` always offers a model picker,
    ...
    print(color("│     🔱 Lycus Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `lycus portal` == quick setup's Nous step.
    try:
        from lycus_cli.main import _model_flow_nous
        ...
        print_info("  You can retry later with `lycus portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `lycus portal`: %s", exc)
        print()
    ...
    print_info("  Run `lycus portal info` to inspect routing.")
    print_info("  Run `lycus` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      lycus setup           — full or quick (auto-detected)
      lycus setup model     — just model/provider
      lycus setup tts       — just text-to-speech
      lycus setup terminal  — just terminal backend
      lycus setup gateway   — just messaging platforms
      lycus setup tools     — just tool configuration
      lycus setup agent     — just agent settings
    """
    from lycus_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     🦊 Lycus Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             🦊 Lycus Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Lycus Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, lycus_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Lycus configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'lycus setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(lycus_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Lycus?",
            [
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, lycus_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {lycus_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'lycus config edit'")
    ...
    # Tune later with `lycus setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, lycus_home)


def _run_first_time_quick_setup(config: dict, lycus_home, is_existing: bool):
    ...
    the user can customize later via ``lycus setup <section>`` or switch
    providers with ``lycus model``.
    """
    from lycus_cli.config import load_config
    ...
    from lycus_cli.models import probe_api_models
    ...
    if detected_models:
        ...
        print(
            f"Warning: could not verify this endpoint. Lycus will still save it."
        )
    ...
    from lycus_cli.auth import _save_model_choice, deactivate_provider
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'lycus setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    lycus setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  lycus setup gateway")
    print()

    _print_setup_summary(config, lycus_home)


def _run_quick_setup(config: dict, lycus_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'lycus setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Lycus to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'lycus setup gateway'.")
