"""
Baked-in build metadata for Lycus Agent.

Source installs report their git revision live via ``git rev-parse`` (see
``lycus_cli/dump.py`` and ``lycus_cli/banner.py``).  That doesn't work inside
...
To make ``lycus dump`` and the startup banner identify the exact commit the
image was built from, the Docker build writes the build-time ``$HERMES_GIT_SHA``
arg into ``<project_root>/.autolycus_build_sha``.  This module is the single
...
_BUILD_SHA_FILE = Path(__file__).parent.parent / ".autolycus_build_sha"


def get_build_sha(short: int = 8) -> Optional[str]:
    """Return the baked-in build SHA, truncated to ``short`` chars, or None.

    Reads ``<project_root>/.autolycus_build_sha`` if present.  The file is
