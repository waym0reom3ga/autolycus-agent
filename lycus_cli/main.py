#!/usr/bin/env python3
"""
Lycus CLI - Main entry point.

Usage:
    lycus                     # Interactive chat (default)
    lycus chat                # Interactive chat
    lycus gateway             # Run gateway in foreground
    lycus gateway start       # Start gateway as service
    lycus gateway stop        # Stop gateway service
    lycus gateway status      # Show gateway status
    lycus gateway install     # Install gateway service
    lycus gateway uninstall   # Uninstall gateway service
    lycus setup               # Interactive setup wizard
    lycus logout              # Clear stored authentication
    lycus status              # Show status of all components
    lycus cron                # Manage cron jobs
    lycus cron list           # List cron jobs
    lycus cron status         # Check if cron scheduler is running
    lycus doctor              # Check configuration and dependencies
    lycus honcho setup                    # Configure Honcho AI memory integration
    lycus honcho status                   # Show Honcho config and connection status
    lycus honcho sessions                 # List directory → session name mappings
    lycus honcho map <name>               # Map current directory to a session name
    lycus honcho peer                     # Show peer names and dialectic settings
    lycus honcho peer --user NAME         # Set user peer name
    lycus honcho peer --ai NAME           # Set AI peer name
    lycus honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    lycus honcho mode                     # Show current memory mode
    ...
    lycus uninstall           Uninstall Lycus Agent
    lycus acp                 Run as an ACP server for editor integration
    lycus sessions browse     Interactive session picker with search

    lycus claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: lycus_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``lycus_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``lycus update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``lycus_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, lycus crashes on import and the user can't run
# ``lycus update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import lycus_bootstrap  # noqa: F401
...
def _set_process_title() -> None:
    """Set the process title to 'lycus' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``lycus tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``lycus.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("lycus")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"lycus", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"lycus")
        # Windows: the .exe name is already ``lycus.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* lycus_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("AUTOLYCUS_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".autolycus", "config.yaml")
...
def _suppress_mouse_residue_early() -> None:
    ...
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`lycus --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from lycus_cli import __release_date__, __version__

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    print(f"Lycus Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``lycus --version`` before config/logging imports on Termux."""
...
from lycus_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from lycus_cli.subcommands.cron import build_cron_parser
from lycus_cli.subcommands.gateway import build_gateway_parser
from lycus_cli.subcommands.profile import build_profile_parser
from lycus_cli.subcommands.model import build_model_parser
from lycus_cli.subcommands.setup import build_setup_parser
from lycus_cli.subcommands.postinstall import build_postinstall_parser
from lycus_cli.subcommands.whatsapp import build_whatsapp_parser
from lycus_cli.subcommands.slack import build_slack_parser
from lycus_cli.subcommands.login import build_login_parser
from lycus_cli.subcommands.logout import build_logout_parser
from lycus_cli.subcommands.auth import build_auth_parser
from lycus_cli.subcommands.status import build_status_parser
from lycus_cli.subcommands.webhook import build_webhook_parser
from lycus_cli.subcommands.hooks import build_hooks_parser
from lycus_cli.subcommands.doctor import build_doctor_parser
from lycus_cli.subcommands.security import build_security_parser
from lycus_cli.subcommands.dump import build_dump_parser
from lycus_cli.subcommands.debug import build_debug_parser
from lycus_cli.subcommands.backup import build_backup_parser
from lycus_cli.subcommands.import_cmd import build_import_cmd_parser
from lycus_cli.subcommands.config import build_config_parser
from lycus_cli.subcommands.version import build_version_parser
from lycus_cli.subcommands.update import build_update_parser
from lycus_cli.subcommands.uninstall import build_uninstall_parser
from lycus_cli.subcommands.dashboard import build_dashboard_parser
from lycus_cli.subcommands.gui import build_gui_parser
from lycus_cli.subcommands.logs import build_logs_parser
from lycus_cli.subcommands.prompt_size import build_prompt_size_parser
from lycus_cli.subcommands.memory import build_memory_parser
from lycus_cli.subcommands.acp import build_acp_parser
from lycus_cli.subcommands.tools import build_tools_parser
from lycus_cli.subcommands.insights import build_insights_parser
from lycus_cli.subcommands.skills import build_skills_parser
from lycus_cli.subcommands.pairing import build_pairing_parser
from lycus_cli.subcommands.plugins import build_plugins_parser
from lycus_cli.subcommands.mcp import build_mcp_parser
from lycus_cli.subcommands.claw import build_claw_parser


def _require_tty(command_name: str) -> None:
    """Exit with a clear error if stdin is not a terminal.

    Interactive TUI commands (lycus tools, lycus setup, lycus model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'lycus {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any lycus module import.
#
# Many modules cache AUTOLYCUS_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("AUTOLYCUS_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.autolycus/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set AUTOLYCUS_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `lycus mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Lycus' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo lycus -p <name>` against the invoking user's home.
        ...
        candidate = home / ".autolycus" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`lycus chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors lycus_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If AUTOLYCUS_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.autolycus/profiles/coder or
    # /opt/data/profiles/coder).  If AUTOLYCUS_HOME points to the lycus root
    # instead (e.g. systemd hardcodes AUTOLYCUS_HOME=/root/.autolycus), we must
    # still read active_profile — the user may have switched profiles via
    # `lycus profile use` and the gateway should honour that choice.
    # See issue #22502.
    lycus_home_env = os.environ.get("AUTOLYCUS_HOME", "")
    if profile_name is None and lycus_home_env:
        if Path(lycus_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the lycus root.
    #
    ...
    # reserved ``gateway-default`` slot runs bare ``lycus gateway run`` to mean
    # "the root AUTOLYCUS_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get("HERMES_S6_SUPERVISED_CHILD"):
        try:
            from lycus_constants import get_default_lycus_root

            active_path = get_default_lycus_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set AUTOLYCUS_HOME
    if profile_name is not None:
        try:
            from lycus_cli.profiles import resolve_profile_env

            lycus_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            lycus_home = _resolve_sudo_user_profile_env(profile_name)
            if not lycus_home:
            ...
            # A bug in profiles.py must NEVER prevent lycus from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["AUTOLYCUS_HOME"] = lycus_home
...
# Load .env from ~/.autolycus/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from lycus_cli.config import get_lycus_home
from lycus_cli.env_loader import load_lycus_dotenv

load_lycus_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → HERMES_REDACT_SECRETS env
# var BEFORE lycus_logging imports agent.redact (which snapshots the flag at
...
# Initialize centralized file logging early — all `lycus` subcommands
...
try:
    from lycus_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from lycus_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if lycus_constants not importable yet
...
from lycus_cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to lycus_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (lycus_cli.main._model_flow_*) keep resolving unchanged.
from lycus_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_lycus_home() / "skills" / ".termux_bundled_sync_stamp"
...
def _has_any_provider_configured() -> bool:
    ...
    from lycus_cli.config import get_env_path, get_lycus_home, load_config
    from lycus_cli.auth import get_auth_status

    # Determine whether Lycus itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_lycus_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    auth_file = get_lycus_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Lycus has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Lycus to use their tokens.
    if _has_lycus_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, lycus_bin
        cli_args: the original CLI arguments (everything after 'lycus')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    lycus_bin = container_info["lycus_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo lycus {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo lycus {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  lycus --tui --resume {target}")
    if title:
        print(f'  lycus --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @lycus/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@lycus" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/lycus-ink/package.json",
    "packages/lycus-ink/index.js",
    "packages/lycus-ink/text-input.js",
)
...
def _ensure_tui_node() -> None:
    ...
    lycus_home = os.environ.get("AUTOLYCUS_HOME") or str(Path.home() / ".autolycus")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "AUTOLYCUS_HOME": lycus_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(lycus_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(lycus_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if lycus_cli_dir is None:
        lycus_cli_dir = Path(__file__).parent
    bundled = lycus_cli_dir / "tui_dist" / "entry.js"
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    def _node_bin(bin: str) -> str:
        ...
        if not path and bin == "node":
            try:
                from lycus_cli.dep_ensure import ensure_dependency
    ...
    if tui_dev:
        # Keep the local @lycus/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @lycus/ink resolves through
        # packages/lycus-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "lycus-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from lycus_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    env = os.environ.copy()
    try:
        from lycus_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="lycus-tui-active-session-", suffix=".json"
    )
    ...
    # LYCUS_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `lycus --tui` try to
    ...
    # still set LYCUS_TUI_RESUME themselves.
    env.pop("LYCUS_TUI_RESUME", None)
    if resume_session_id:
        env["LYCUS_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `lycus update` so
    ...
    if code == 42:
        from lycus_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    ...
    (``lycus kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``lycus kanban boards switch`` from another session can flip the file
    ...
    if os.environ.get("HERMES_KANBAN_BOARD"):
        return
    try:
        from lycus_cli.kanban_db import get_current_board
...
def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.autolycus/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Lycus — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Lycus still functions without them; the user just sees an
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'lycus sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from lycus_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from lycus_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'lycus doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Lycus isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  lycus setup")
        print()

        from lycus_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'lycus setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from lycus_cli.banner import prefetch_update_check
    ...
    # setup (config, rules files, plugins, MCP servers) or from Lycus itself.
    ...
    # lycus_cli/plugins.py and tools/mcp_tool.py).
    ...
    # user's ~/.autolycus/config.yaml and return built-in defaults. Set BEFORE
...
def cmd_gateway(args):
    ...
    from lycus_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from lycus_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from lycus_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Lycus?")
        print()
    ...
    # has no creds.json.  Every subsequent `lycus gateway` then paid a 30s
    ...
    session_dir = get_lycus_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: lycus gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `lycus gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  lycus gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '🦊 Lycus Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  lycus gateway")
        ...
        print()
        print("  Or install as a service: lycus gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'lycus whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``lycus whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``lycus_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from lycus_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from lycus_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from lycus_cli.config import stamp_install_method
    from lycus_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("🔱 Lycus post-install bootstrap")
    print()
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``lycus model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from lycus_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    from lycus_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from lycus_cli.providers import resolve_provider_full
    ...
    if effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'lycus model' for "
                "available providers, or run 'lycus doctor' to diagnose config "
                "issues."
            )
    ...
    from lycus_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from lycus_cli.config import read_raw_config
    ...
    # PROVIDER_GROUPS in lycus_cli/models.py). A multi-member group shows one
    ...
    # a leftover OPENAI_BASE_URL in ~/.autolycus/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.autolycus/.env if the active provider is not 'custom'.
    ...
    """
    from lycus_cli.config import get_env_value, save_env_value, load_config
...
#
# Lycus uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `lycus model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `lycus model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`lycus_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from lycus_cli.plugins import get_plugin_auxiliary_tasks
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Lycus only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``lycus model`` flow, then route aux tasks to them here.
    """
    from lycus_cli.config import load_config
    from lycus_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from lycus_cli.auth import _prompt_model_selection
    from lycus_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from lycus_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from lycus_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from lycus_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Lycus URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _remove_custom_provider(config):
    ...
    try:
        from lycus_cli.curses_ui import curses_radiolist
...
# downstream call sites read `lycus_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `lycus` invocation — including
# fast paths like `lycus --version` and slash-command dispatch that never
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from lycus_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``lycus setup`` / ``lycus model``.
    ...
    recover from a malformed paste without editing ``~/.autolycus/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from lycus_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    from lycus_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `lycus setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from lycus_constants import display_lycus_home as _dhh_fn

            print(
                f"    Lycus will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate Lycus CLI with a provider."""
    from lycus_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from lycus_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from lycus_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from lycus_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from lycus_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from lycus_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``lycus slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: lycus slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `lycus slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from lycus_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from lycus_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from lycus_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from lycus_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `lycus security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from lycus_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from lycus_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from lycus_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from lycus_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Lycus home directory to a zip file."""
    if getattr(args, "quick", False):
        from lycus_cli.backup import run_quick_backup
...
def cmd_import(args):
    """Restore a Lycus backup from a zip file."""
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    try:
        from lycus_cli.banner import check_for_updates
        from lycus_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Lycus Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from lycus_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from lycus_cli.uninstall import run_gui_uninstall
    ...
    from lycus_cli.uninstall import run_uninstall
...
# Critical files that every ``lycus`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``lycus update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "lycus_cli/main.py",
    "lycus_cli/config.py",
    "lycus_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``lycus`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="lycus-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``lycus update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``lycus_cli/web_dist/`` (per vite.config.ts
    outDir: "../lycus_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "lycus_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``lycus`` launcher present but ``lycus_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the lycus venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — lycus venv python3, no nix-shell overhead
    ...
    # a lycus venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``lycus update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``lycus web``).
    ...
    # build path usable on a stock `py -m lycus_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (lycus web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``lycus update`` can unconditionally call ``lycus desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``lycus desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $AUTOLYCUS_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $AUTOLYCUS_HOME."""
    from lycus_constants import get_lycus_home
    return get_lycus_home() / "desktop-build-stamp.json"
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``lycus update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Lycus.app/Contents/MacOS/Lycus"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Lycus.exe",
            release_dir / "win-ia32-unpacked" / "Lycus.exe",
            release_dir / "win-arm64-unpacked" / "Lycus.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "lycus",
            release_dir / "linux-unpacked" / "Lycus",
            release_dir / "linux-arm64-unpacked" / "lycus",
            release_dir / "linux-arm64-unpacked" / "Lycus",
        ]
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Lycus'`` desktop build failure: a corrupt zip in the
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Lycus.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Lycus.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Lycus.exe: Access is
    ...
    an unrelated "Lycus" process is never touched. Best-effort: never raises.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Lycus is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../Lycus.app/Contents/MacOS/Lycus  ->  app bundle = .../Lycus.app
    app = exe.parents[2]
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Lycus Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Lycus Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from lycus_logging import setup_logging as _setup_logging_gui
    ...
    if getattr(args, "lycus_root", None):
        env["HERMES_DESKTOP_HERMES_ROOT"] = str(Path(args.autolycus_root).expanduser().resolve())
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  lycus gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if not source_mode:
                ...
                # holds Lycus.exe locked on Windows, so the pack can't replace
            ...
            if build_result.returncode != 0 and not source_mode:
                ...
                # on the final `electron` -> `Lycus` rename: unpack-electron
                ...
                if purged:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Lycus.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Lycus.exe, close any")
                    print("  running Lycus desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> lycus desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "Lycus is
    ...
    if source_mode:
        print("→ Launching Lycus Desktop from source build...")
    ...
    print(f"→ Launching packaged Lycus Desktop: {packaged_executable}")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``lycus dashboard`` processes other than ourselves.

    ``lycus dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``lycus update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Lycus Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``lycus dashboard`` as
    ...
    patterns = [
        "lycus dashboard",
        "lycus_cli.main dashboard",
        "lycus_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "lycus.*dashboard"`) keeps us consistent
            # with `lycus_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `lycus update`.
    ...
    print("  Preview now:  lycus curator run --dry-run")
    print("  Pause it:     lycus curator pause")
    print(
        "  Docs:         https://lycus-agent.nousresearch.com/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``lycus update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``lycus update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: lycus curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``lycus dashboard`` processes.

    Called at the end of ``lycus update`` (default ``reason``) and also
    from ``lycus dashboard --stop`` (which overrides ``reason``).  The
    ...
    """
    # When the Lycus Desktop Electron app spawns this dashboard as a
    ...
    if killed:
        ...
        print("    lycus dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Lycus Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `lycus update "
            f"--branch {branch}`, or update against main with `lycus update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://github.com/NousResearch/lycus-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="lycus-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"lycus-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for lycus-agent itself
        ...
        # GitHub ZIPs extract to lycus-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"lycus-agent-{branch}")
    ...
    from lycus_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        from lycus_cli.model_catalog import seed_cache_from_checkout
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "lycus-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Lycus behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes lycus completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Lycus couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Lycus couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Lycus behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Lycus couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Lycus couldn't drop "
...
# =========================================================================
# Fork detection and upstream management for `lycus update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/lycus-agent.git",
    "git@github.com:NousResearch/lycus-agent.git",
    "https://github.com/NousResearch/lycus-agent",
    "git@github.com:NousResearch/lycus-agent",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/lycus-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_lycus_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_lycus_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Lycus repository.")
        print("  This means you may miss updates from NousResearch/lycus-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NousResearch/lycus-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NousResearch/lycus-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``lycus update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from lycus_constants import get_default_lycus_root

    default_home = get_default_lycus_root()
...
# Install-scoped breadcrumb dropped right before ``lycus update`` mutates the
...
# survives and the next ``lycus`` launch finishes the install instead of
# limping along on a half-built venv (e.g. pip wiped, a core dep like Pillow
# never landed).  Lives next to the venv (not under $AUTOLYCUS_HOME) because the
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``lycus update`` left half-done.
    ...
    protocol stream (``lycus acp`` speaks JSON-RPC on stdout) must never get
    ...
    try:
        ...
        print(
            "⚠ A previous `lycus update` was interrupted mid-install — "
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``lycus update`` is still progressing even if pip/uv itself is silent.
    """
...
def _lycus_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "lycus.exe",
        scripts_dir / "lycus-gateway.exe",
    ]


def _detect_concurrent_lycus_instances(
    ...
    The Lycus Desktop Electron app spawns ``lycus.EXE`` as a backend child,
    so during ``lycus update`` the user-invoked process and the desktop's
    ...
    shims (``lycus.exe`` / ``lycus-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``lycus update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``lycus.exe``) is a separate process from the Python
    ...
    for shim in _lycus_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated lycus.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``lycus update``
    ...
    #      lycus.exe sitting *under* a non-Lycus parent (e.g. a Lycus
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "lycus.exe"
    lines = ["✗ Another lycus.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Lycus Desktop, exit any open `lycus` REPLs, and")
    lines.append("  stop the gateway (`lycus gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `lycus update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_lycus_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``lycus.exe``.
    ...
    when ``lycus update`` runs, ``lycus.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``lycus.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next lycus invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Lycus Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Lycus Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _lycus_exe_shims(scripts_dir):
        ...
        print(
            "    Close Lycus Desktop, exit other `lycus` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `lycus update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_lycus_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``lycus.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``lycus.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``lycus`` drops off PATH. This wraps
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_lycus_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``lycus.exe.old.*`` left by prior updates.

    Called early on every lycus invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``lycus update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `lycus update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``lycus.exe`` / ``lycus-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_lycus_exe`` for the rationale.
    """
    ...
    # hours later inside ``lycus update``'s desktop-rebuild or skill-sync
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``lycus update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``lycus.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``lycus`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `lycus update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `lycus update --force` after closing other lycus processes."
        )
        return
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from lycus_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$AUTOLYCUS_HOME/bin/uv``, but on Termux the official
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `lycus update`, so we install root-only first
...
class _UpdateOutputStream:
    """Stream wrapper used during ``lycus update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.autolycus/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``lycus update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``lycus update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.autolycus/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``lycus update --gateway``) the update is already
    ...
    try:
        ...
        # lycus_cli.config.get_lycus_home to simulate setup failure.
        from lycus_cli.config import get_lycus_home as _get_lycus_home

        logs_dir = _get_lycus_home() / "logs"
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``lycus update --check``: fetch and report without installing.
    ...
    dropping the flag.
    """
    from lycus_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``lycus update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from lycus_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from lycus_cli.config import recommended_update_command
        from lycus_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``lycus update`` without requiring a reinstall.  The
    ...
    ``lycus`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/lycus.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v lycus'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
    ...
    # /usr/local/bin/lycus, code at /usr/local/lib/lycus-agent).
    fhs_link = Path("/usr/local/bin/lycus")
    ...
    path_comment = (
        "# Lycus Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of AUTOLYCUS_HOME before running the update.
    ...
    AUTOLYCUS_HOME directories.  The ``--backup`` flag on ``lycus update``
    ...
    # Render path using display_lycus_home so the user sees ~/.autolycus/...
    try:
        from lycus_constants import get_lycus_home, display_lycus_home

        home = get_lycus_home()
        try:
            display_path = f"{display_lycus_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  lycus import {out_path}")
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    lycus.exe concurrent-instance guard does not see them. They still import
    ...
    print("→ Stopping Windows gateway process(es) before updating Lycus...")
    try:
    ...
    if unmapped_pids:
        ...
        print("    Restart manually after update: lycus gateway run")
...
def _resume_windows_gateways_after_update(token: dict | None) -> None:
    ...
    try:
        from lycus_cli.gateway import launch_detached_profile_gateway_restart
    except Exception as exc:
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``lycus update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update Lycus Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Lycus Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `lycus update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Lycus via pip (for PyPI installs)."""
    from lycus_cli import __version__
    from lycus_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "lycus-agent"]
        ...
        cmd = [pipx, "upgrade", "lycus-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "lycus-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "lycus-agent"]
    ...
    print("✓ Update complete! Restart lycus to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # On Windows, abort early if another lycus.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_lycus_instances(scripts_dir)
            if concurrent:
    ...
    try:
        ...
        # though `git pull` can't touch $AUTOLYCUS_HOME, this is cheap
        ...
        # orphan merge-conflict markers in lycus_cli/config.py bricked
        # every user who ran ``lycus update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``lycus
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``lycus update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_lycus_home added to lycus_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # the marker survives and the next ``lycus`` launch finishes the
        ...
        # build.  ``lycus desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``lycus desktop`` shouldn't be forced into a full
        # Electron build by ``lycus update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        has_desktop_app = _desktop_packaged_executable(desktop_dir) is not None or _desktop_dist_exists(desktop_dir)
        if (desktop_dir / "package.json").exists() and shutil.which("npm") and has_desktop_app:
            print("→ Checking if desktop app needs rebuilding...")
            _desktop_build_cmd = [sys.executable, "-m", "lycus_cli.main", "desktop", "--build-only"]
            ...
            if build_result.returncode != 0:
                build_result = subprocess.run(_desktop_build_cmd, cwd=PROJECT_ROOT, check=False)
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `lycus desktop` to retry)")
        ...
        # current — so copy it straight over ~/.autolycus/cache/model_catalog.json
        ...
        # modules in this process.  Reload lycus_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_lycus_home() added since the last release.
        try:
            import importlib
            import lycus_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit AUTOLYCUS_HOME so
        # it is not affected by sync_skills()'s module-level AUTOLYCUS_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's AUTOLYCUS_HOME env var points at the default or a named profile.
        try:
            from lycus_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        try:
            from lycus_cli.profiles import backfill_profile_envs
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'lycus config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: lycus config migrate")
            else:
                print()
                print("Skipped. Run 'lycus config migrate' later to configure.")
        else:
        ...
        # check `lycus curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``lycus update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from lycus_cli.tools_config import install_cua_driver
        ...
        # When running as ``lycus update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_lycus_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... lycus-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... lycus-gateway*)
            ...
            # Discover all lycus-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. lycus-gateway.service or lycus-gateway-coder.service
                            ...
                            if _manage_cmd is None:
                                print(
                                    ...
                                    f"    To let `lycus update` restart it automatically, allow\n"
                        ...
                        print(
                            ...
                            f"Check the gateway with: lycus gateway status"
                        )
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: lycus gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: lycus -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Lycus gateway unit files are still installed.
        # When both lycus.service (from a pre-rename install) and the
        # current lycus-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `lycus update` surfaces the issue until the user migrates.
        try:
            from lycus_cli.gateway import (
                has_legacy_lycus_units,
                _find_legacy_lycus_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_lycus_units():
                print()
                print("⚠ Legacy Lycus gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_lycus_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (lycus.service) fight the current")
                print("  lycus-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    lycus gateway migrate-legacy")
                print()
        ...
        print("  lycus model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``lycus -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `lycus profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_lycus_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → lycus -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.autolycus)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  lycus profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     lycus -p {name} chat")
                else:
        ...
        from lycus_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from lycus_constants import get_lycus_home as _hh
        ...
        from lycus_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `lycus profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → lycus -p {name}  ({wrapper})")
        print()
        ...
        from lycus_cli.profiles import profile_exists
        ...
        from lycus_cli.profiles import rename_profile
        ...
        from lycus_cli.profiles import export_profile
        ...
        from lycus_cli.profiles import import_profile
        ...
        import tempfile
        from lycus_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="lycus_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  lycus -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      lycus -p {plan.manifest.name} chat")
        ...
        from lycus_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `lycus profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"lycus -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from lycus_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("lycus_requires"):
            print(f"Requires:     Lycus {data['lycus_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from lycus_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.autolycus_requires:
        print(f"  Requires: Lycus {mf.autolycus_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``lycus dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``lycus dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No lycus dashboard processes running.")
        return 0

    print(f"{len(pids)} lycus dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No lycus dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `lycus update`.
    ...
    # launches the dashboard (`worker dashboard` → AUTOLYCUS_HOME points into
    ...
    if (
        ...
        reexec_argv = [
            sys.executable, "-m", "lycus_cli.main",
            "-p", "default",
            "dashboard",
            "--port", str(args.port),
            "--host", args.host,
        ...
        # AUTOLYCUS_HOME.  We must resolve the root explicitly instead of just
        # dropping AUTOLYCUS_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV AUTOLYCUS_HOME=/opt/data`), so an unset
        # AUTOLYCUS_HOME falls back to $HOME/.autolycus = /opt/data/.autolycus — an
        ...
        # update-button guard also misfires).  get_default_lycus_root()
        # returns the root for both layouts: ~/.autolycus for a standard install
        ...
        try:
            from lycus_constants import get_default_lycus_root
            env["AUTOLYCUS_HOME"] = str(get_default_lycus_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop AUTOLYCUS_HOME) rather than block the reroute.
            env.pop("AUTOLYCUS_HOME", None)
    ...
    # the same logs directory as every other Lycus surface.
    try:
        from lycus_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if "HERMES_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            ...
            else PROJECT_ROOT / "lycus_cli" / "web_dist"
        )
    ...
    try:
        from lycus_cli.mcp_startup import start_background_mcp_discovery
    ...
    from lycus_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from lycus_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_completion(args, parser=None):
    ...
    from lycus_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from lycus_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Lycus log files."""
    from lycus_cli.logs import tail_log, list_logs
...
# so that in ``lycus -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``lycus_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``lycus -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``lycus`` or only flags → defaults to ``chat``.
        return False
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from lycus_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from lycus_cli.oneshot import run_oneshot
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `lycus --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_lycus_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_lycus_home()}/memories/\n")
    else:
        from lycus_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch Lycus Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from lycus_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from lycus_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from lycus_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from lycus_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from lycus_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from lycus_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from lycus_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from lycus_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from lycus_cli.claw import claw_command

    claw_command(args)


def main():
    """Main entry point for lycus CLI."""
    # Cosmetic: make the process show up as 'lycus' instead of 'python3.11'
    # in ps/top/htop.  Non-fatal — just a nicer UX.
    _set_process_title()

    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from lycus_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``lycus.exe.old.*`` quarantine files left by previous
    # ``lycus update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_lycus_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # Self-heal a venv left half-built by an interrupted ``lycus update``
    ...
    # ``lycus skills install update``) merely defers recovery one launch;
    # under-matching (missing ``lycus -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in lycus_cli/subcommands/model.py)
    # =========================================================================
    ...
    # =========================================================================
    from lycus_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://lycus-agent.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `lycus model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.autolycus/.env.  Currently supports "
            "Bitwarden Secrets Manager.  See: "
            "https://lycus-agent.nousresearch.com/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    from lycus_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from lycus_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in lycus_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in lycus_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in lycus_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in lycus_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `lycus whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in lycus_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from lycus_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in lycus_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in lycus_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in lycus_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in lycus_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in lycus_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in lycus_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from lycus_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from lycus_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    # hooks command  (parser built in lycus_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in lycus_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in lycus_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in lycus_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in lycus_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in lycus_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.autolycus/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo lycus uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from lycus_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in lycus_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in lycus_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # pairing command  (parser built in lycus_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in lycus_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from lycus_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in lycus_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``lycus --help``, ``lycus version``, ``lycus logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from lycus_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from lycus_cli.curator import register_cli as _register_curator_cli
    ...
    # =========================================================================
    # memory command  (parser built in lycus_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in lycus_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `lycus computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `lycus tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: lycus computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: lycus computer-use install")
            return
    ...
    # =========================================================================
    # mcp command  (parser built in lycus_cli/subcommands/mcp.py)
    # =========================================================================
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_lycus_home() / "sessions"
            ...
            # Launch lycus --resume <id> by replacing the current process
    ...
    # =========================================================================
    # insights command  (parser built in lycus_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in lycus_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in lycus_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in lycus_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in lycus_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in lycus_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in lycus_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in lycus_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Lycus-Setup.exe success screen tells users to
    # run `lycus desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in lycus_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in lycus_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in lycus_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``lycus -c Pokemon Agent Dev`` → ``lycus -c 'Pokemon Agent Dev'``
    ...
    from lycus_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'lycus -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (lycus hooks list, cron
