...
``cli.py``'s ``LycusCLI`` class. ``LycusCLI`` inherits ``CLICommandsMixin`` so
...
from lycus_constants import display_lycus_home, is_termux as _is_termux_environment
...
class CLICommandsMixin:
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: lycus --checkpoints")
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Lycus config/state.
        ...
        if subcmd in {"list", "ls"}:
            ...
            if not snaps:
                ...
                return
            print(f"  State snapshots ({display_lycus_home()}/state-snapshots/):\n")
    ...
    def _handle_paste_command(self):
        ...
        from lycus_cli.clipboard import has_clipboard_image
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run lycus chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in lycus_cli/colors.py
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `lycus gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                ...
                return
            _cprint("  Tip:   Use /history or `lycus sessions list` to find sessions.")
            return
        ...
        if target.isdigit():
            ...
            from lycus_cli.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `lycus sessions list` to see available sessions.")
            return
    ...
    def _handle_lycus_greeting_command(self, cmd: str):
        ...
        try:
            ...
            if not personality_file.exists():
                print("(._.) Lycus personality file not found at ~/.autolycus/lycus_personality.yaml")
                return
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to lycus_cli.curator so the CLI and the `lycus curator`
        ...
        try:
            from lycus_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to lycus_cli.skills_hub."""
        ...
        if args and args[0].lower() in {"pending", "approve", "apply", "reject",
            ...
            if out is not None:
                print(out)
                return
        from lycus_cli.skills_hub import handle_skills_slash
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``lycus bundles list`` but renders inside the running
        ...
        if not bundles:
            ...
            _cprint(
                f"  {_DIM}Create one with: lycus bundles create "
        ...
        _cprint(
            ...
            f"Manage with `lycus bundles`.{_RST}"
        )
        ...
        if sub.startswith("connect"):
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically a Lycus-managed isolated debug "
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"Lycus keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from lycus_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_lycus_home()}/skins/\n")
            return
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Lycus is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Lycus is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while Lycus is busy."
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from lycus_cli.models import _is_anthropic_fast_model
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update Lycus Agent to the latest version.

        In the classic CLI this exits the session and relaunches as
        ``lycus update`` so the user sees update output directly and gets
        ...
        if is_managed():
            print(f"  ✗ {format_managed_message('update Lycus Agent')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Lycus Agent"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="🔱  Update Lycus Agent",
            detail="This will exit the current session and run `lycus update`.",
            choices=choices,
        )
        if raw is None:
