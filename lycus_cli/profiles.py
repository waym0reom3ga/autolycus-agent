"""
Profile management for multiple isolated Lycus instances.

Each profile is a fully independent AUTOLYCUS_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.autolycus/profiles/<name>/`` by default.

The "default" profile is ``~/.autolycus`` itself — backward compatible,
...
Usage::
    ...
    lycus profile use coder             # set as sticky default
    lycus profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See lycus_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.autolycus``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   lycus-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
# Marker file written by `lycus profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `lycus
...
# `lycus skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Lycus infrastructure directories that only the default profile
         (``~/.autolycus``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_lycus_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.autolycus) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "lycus-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "lycus", "default", "test", "tmp", "root", "sudo",
})

# Lycus subcommands that cannot be used as profile names/aliases
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the lycus root, NOT to the current AUTOLYCUS_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where AUTOLYCUS_HOME points outside
    ``~/.autolycus``, profiles live under ``AUTOLYCUS_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_lycus_home() / "profiles"


def _get_default_lycus_home() -> Path:
    """Return the default (pre-profile) AUTOLYCUS_HOME path.

    In standard deployments this is ``~/.autolycus``.
    In Docker/custom deployments where AUTOLYCUS_HOME is outside ``~/.autolycus``
    (e.g. ``/opt/data``), returns AUTOLYCUS_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_lycus_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Lycus installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its AUTOLYCUS_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_lycus_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, lycus subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _HERMES_SUBCOMMANDS:
        return f"'{canon}' conflicts with a lycus subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            lycus_exe = shutil.which("lycus") or "lycus"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(lycus_exe)} -p {profile} "$@"\n')
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Lycus. If we
    ...
    try:
        ...
        # not be migrated. The next `lycus doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``lycus -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``lycus profile alias
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``lycus profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing Lycus config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_lycus_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``lycus update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.autolycus)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.autolycus/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Keys pages, `lycus -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Lycus profile.\n"
    ...
    # Write the opt-out marker so seed_profile_skills() and `lycus update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`lycus profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Lycus (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `lycus profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `lycus -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches AUTOLYCUS_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``lycus profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "AUTOLYCUS_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_lycus_home() / ".env"
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.autolycus).\n"
            "To remove everything, use: lycus uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``AUTOLYCUS_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from lycus_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set AUTOLYCUS_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("AUTOLYCUS_HOME")
    try:
        os.environ["AUTOLYCUS_HOME"] = str(profile_dir)
        from lycus_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["AUTOLYCUS_HOME"] = old_home
        elif "AUTOLYCUS_HOME" in os.environ:
            del os.environ["AUTOLYCUS_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.autolycus/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: lycus profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from AUTOLYCUS_HOME.

    Returns ``"default"`` if AUTOLYCUS_HOME is not set or points to ``~/.autolycus``.
    Returns the profile name if AUTOLYCUS_HOME points into ``~/.autolycus/profiles/<name>``.
    Returns ``"custom"`` if AUTOLYCUS_HOME is set to an unrecognized path.
    """
    from lycus_constants import get_lycus_home
    lycus_home = get_lycus_home()
    resolved = lycus_home.resolve()

    default_resolved = _get_default_lycus_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.autolycus itself — its parent is ~/ and its
        # directory name is ".autolycus", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: lycus profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.autolycus itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.autolycus). "
            "Specify a different name: lycus profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="lycus_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"lycus_{old_name}"
    legacy_old_host = f"lycus.{old_name}"
    new_host = f"lycus_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_lycus_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("lycus_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a AUTOLYCUS_HOME path string.

    Called early in the CLI entry point, before any lycus modules
    are imported, to set the AUTOLYCUS_HOME environment variable.
    """
    canon = normalize_profile_name(profile_name)
    validate_profile_name(canon)
    profile_dir = get_profile_dir(canon)
