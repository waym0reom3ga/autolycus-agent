...
Add, remove, or reorder entries here — both `lycus setup` and
`lycus` provider-selection will pick up the change automatically.
"""
...
from lycus_cli import __version__ as _HERMES_VERSION

# Identify ourselves so endpoints fronted by Cloudflare's Browser Integrity
# Check (error 1010) don't reject the default ``Python-urllib/*`` signature.
_HERMES_USER_AGENT = f"lycus-cli/{_HERMES_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `lycus model`
    flow without maintaining a separate static list.
    """
    from lycus_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $AUTOLYCUS_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $AUTOLYCUS_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Lycus refreshes the cache from
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Lycus that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Lycus that doesn't ship
...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_lycus_home() / "cache" / "nous_recommended_cache.json"
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    (``$AUTOLYCUS_HOME/cache/nous_recommended_cache.json``) as last-known-good.
...
# this list:  lycus model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `lycus model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `lycus model` TUI
...
#
# Some vendors expose several Lycus provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `lycus model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``lycus model``, the
...
# path. The *interactive* default (GUI onboarding / ``lycus model``) uses the
# richer free/paid-tier-aware resolver — see ``get_recommended_default_model``
# in lycus_cli/web_server.py and ``partition_nous_models_by_tier`` — which can
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``lycus auth add openai-codex`` without
    ``lycus model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``lycus model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    lycus-agent is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from lycus_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — lycus-agent
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from lycus_cli.model_catalog import get_curated_nous_models
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    ...
    """
    try:
        from lycus_cli.auth import resolve_nous_runtime_credentials
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``lycus model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from lycus_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from lycus_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Lycus' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``lycus_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Lycus should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``lycus auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.autolycus/.env``.
    ...
    """
    try:
        from lycus_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        ...
        from lycus_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Lycus release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Lycus release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        try:
            from lycus_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
        ...
        # added Portal models still surface without a Lycus release.
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``lycus model`` shows.
...
#
# Cache strategy:
#   - One JSON file at $AUTOLYCUS_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from lycus_constants import get_lycus_home
    return get_lycus_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$AUTOLYCUS_HOME/auth.json`` and external
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from lycus_cli.copilot_auth import copilot_request_headers
        return copilot_request_headers(is_agent_turn=True)
    except ImportError:
        return {
            "Editor-Version": COPILOT_EDITOR_VERSION,
            "User-Agent": "LycusAgent/1.0",
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Lycus' default Claude IDs elsewhere use
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from lycus_constants import get_lycus_home
    return get_lycus_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Lycus will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Lycus cannot verify the model name."
