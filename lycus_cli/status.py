"""
Status command for lycus CLI.

Shows the status of all Lycus Agent components.
"""
...
from lycus_cli.auth import AuthError, resolve_provider
from lycus_cli.colors import Colors, color
from lycus_cli.config import get_env_path, get_env_value, get_lycus_home, load_config
from lycus_cli.models import provider_label
from lycus_cli.nous_account import (
    format_nous_portal_entitlement_message,
    get_nous_portal_account_info,
)
from lycus_cli.nous_subscription import get_nous_subscription_features
from lycus_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``lycus config``'s
...
def show_status(args):
    """Show status of all Lycus Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 🦊 Lycus Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: lycus portal)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: lycus model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: lycus auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: lycus auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: lycus model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from lycus_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from lycus_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   lycus gateway")
    ...
    jobs_file = get_lycus_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_lycus_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'lycus doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'lycus setup' to configure", Colors.DIM))
    print()
