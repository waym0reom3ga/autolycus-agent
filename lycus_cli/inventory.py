...
def build_models_payload(
    ...
    *,
    ...
    - ``pricing``: enrich each row with formatted per-model pricing and,
      ...
      mirroring the ``lycus model`` CLI picker. Adds network calls
    ...
    try:
        from lycus_cli.providers import is_aggregator as _is_aggregator
    except Exception:
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (
            ...
            else f"run `lycus model` to configure ({auth_type})"
        )
