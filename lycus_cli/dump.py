"""
Dump command for lycus CLI.

Outputs a compact, plain-text summary of the user's Lycus setup
...
from lycus_cli.config import get_lycus_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.autolycus_build_sha`` by the Dockerfile's
    ``HERMES_GIT_SHA`` build-arg (see ``lycus_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``lycus dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from lycus_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(lycus_home: Path) -> int:
...
def _cron_summary(lycus_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = lycus_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"lycus_home:      {display_lycus_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # A credential added via `lycus auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `lycus auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["lycus-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(lycus_home)}")
    lines.append(f"  skills:             {_count_skills(lycus_home)}")
