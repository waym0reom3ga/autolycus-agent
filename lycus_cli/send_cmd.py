"""CLI subcommand: ``lycus send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"lycus send: {file_path} is not a text file. --file reads the "
                ...
                f'  lycus send --to telegram "MEDIA:{file_path}"\n'
                f'  lycus send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  lycus send --to telegram "[[as_document]] MEDIA:{file_path}"',
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
        except OSError as exc:
            print(f"lycus send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"lycus send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"lycus send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"lycus send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"lycus send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `lycus gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.autolycus/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_lycus_env() -> None:
    """Populate ``os.environ`` from ``~/.autolycus/.env`` AND bridge top-level
    ...
    startup that ``lycus send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.autolycus/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.autolycus/config.yaml`` into
       ...
       via ``lycus config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``lycus send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.autolycus/.env and ~/.autolycus/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "lycus send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  lycus send --to telegram \"hello\"\n"
            "  lycus send --to discord:#ops --file report.md\n"
            "  lycus send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "lycus send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `lycus send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Lycus "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.autolycus/.env + ~/.autolycus/config.yaml) — no LLM, "
        ...
        epilog=(
            ...
            "  lycus send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `lycus send --list telegram`.",
    )
