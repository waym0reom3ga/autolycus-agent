...
# xAI's public API is OpenAI-compatible for the endpoints Lycus commonly
# uses. The Responses endpoint is included because Lycus' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Lycus-managed OAuth credentials."""

    auth_hint = "lycus auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`lycus auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`lycus auth reset xai-oauth` or re-authenticate with "
                    "`lycus auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `lycus auth add xai-oauth --type oauth`."
            )
