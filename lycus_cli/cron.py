...
_GATEWAY_LIFECYCLE_PATTERNS = re.compile(
    r"(?i)"
    r"(lycus\s+gateway\s+(restart|stop|start))"
    r"|(launchctl\s+(kickstart|unload|load|stop|restart)\s+.*lycus)"
    r"|(systemctl\s+(restart|stop|start)\s+.*lycus)"
    r"|(p?kill\s+.*lycus.*gateway)"
)
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'lycus cron create ...' or the /cron command in chat.", Colors.DIM))
        return
    ...
    if not find_gateway_pids():
        ...
        print(color("     Start it with: lycus gateway install", Colors.DIM))
        print(color("                    sudo lycus gateway install --system  # Linux servers", Colors.DIM))
        print()
...
def cron_status():
    ...
    if pids:
        ...
        print("    lycus gateway install    # Install as a user service")
        print("    sudo lycus gateway install --system  # Linux servers: boot-time system service")
        print("    lycus gateway            # Or run in foreground")
...
def cron_create(args):
    ...
    if _contains_gateway_lifecycle_command(combined):
        print(color(
            ...
            "Use `lycus gateway restart` from a shell outside the gateway.",
            Colors.RED,
        ))
        return 1
...
def cron_command(args):
    ...
    print("Usage: lycus cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
