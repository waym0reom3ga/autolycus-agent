"""
Unified tool configuration for Lycus Agent.

`lycus tools` and `lycus setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.autolycus/config.yaml under
...
from lycus_cli.nous_subscription import (
    apply_nous_managed_defaults,
    get_nous_subscription_features,
)
from lycus_cli.nous_account import format_nous_portal_entitlement_message
...
from lycus_cli.cli_output import (  # noqa: E402 — late import block
...
# who want it opt in via `lycus tools` → Video Generation, which walks
...
# `lycus tools` → X (Twitter) Search setup walks users through credential
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from lycus_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `lycus tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``lycus tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from lycus_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``lycus tools`` checklist actually offers
    for ``platform``.
    ...
    so ``lycus tools`` never claims to add or remove a toolset the user was
...
from lycus_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Lycus routes X searches through xAI's built-in x_search "
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``lycus update`` and
      by ``lycus computer-use install --upgrade``.
    ...
    if _plat.system() != "Darwin":
        if upgrade:
            # Silent on non-macOS — `lycus update` calls this for every
    ...
    try:
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            if verbose:
                ...
                _print_info("      System Settings > Privacy & Security > Accessibility")
                _print_info("      System Settings > Privacy & Security > Screen Recording")
                _print_info("    Both must allow the terminal / Lycus process.")
            return True
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_lycus_home()}/lycus-agent && npm install --workspaces=false")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in lycus-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/nousresearch/lycus-agent:latest"
            )
            return
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.autolycus/config.yaml")
        ...
        # Run the full `lycus auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.autolycus/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from lycus_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: lycus auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `lycus auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: lycus auth spotify")
        except Exception as exc:
        ...
        try:
            from lycus_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: lycus plugins enable observability/langfuse")
        _print_info("    Restart Lycus for tracing to take effect.")
        _print_info("    Verify: lycus plugins list")
        ...
        try:
            ...
            _print_info("    Run later: lycus auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `lycus auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: lycus auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: lycus auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``lycus tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``lycus tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: lycus tools post-setup <key>")
        return 2
...
def _get_platform_tools(
    ...
    *,
    ...
    # "lycus-cli" (which include all _HERMES_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[lycus-cli, spotify]`` after enabling Spotify via ``lycus
        ...
        # (e.g. "lycus-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``lycus tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `lycus tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"lycus-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("lycus-")}
    ...
    # shipped as a bundled plugin but user must opt in via `lycus tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `lycus tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by lycus tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (lycus-cli, lycus-telegram, etc.)
    ...
    # Opening `lycus tools` is the user's opt-in to reconfigure tools, so treat
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `lycus tools` gets a silent no-op
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `lycus model`).
    if managed_feature:
        from lycus_cli.nous_subscription import (
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `lycus tools` and `lycus setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("🦊 Lycus Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://lycus-agent.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print()
    from lycus_constants import display_lycus_home
    print(color(f"  Tool configuration saved to {display_lycus_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'lycus' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, lycus mcp
