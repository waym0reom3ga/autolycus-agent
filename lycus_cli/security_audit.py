"""On-demand supply-chain audit for Lycus Agent installs.

Scans three surfaces a Lycus user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Lycus venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.autolycus/plugins``
...
def _discover_plugins(lycus_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.autolycus/plugins``.
...
def run_audit(
    *,
    ...
    home = lycus_home or Path(get_lycus_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, lycus_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(lycus_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `lycus security audit`."""
    home = Path(get_lycus_home())
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, lycus_home=home
    )
    if total == 0:
