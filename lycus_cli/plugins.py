...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with lycus-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.autolycus/plugins/<name>/``
3. **Project plugins** – ``./.autolycus/plugins/<name>/`` (opt-in via
   ``HERMES_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``lycus_agent.plugins``
   entry-point group.
...
from lycus_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE
...
# stderr in addition to ~/.autolycus/logs/agent.log. Aimed at plugin authors
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    ...
    is passed. Does not touch the root logger or other Lycus loggers.
    """
...
ENTRY_POINTS_GROUP = "lycus_agent.plugins"

_NS_PARENT = "lycus_plugins"
...
class PluginManifest:
    ...
    #              in ~/.autolycus/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``lycus plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``lycus honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``lycus <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`lycus_cli.dashboard_auth.DashboardAuthProvider`. Used by
    ...
    def register_slack_action_handler(
        ...
        Lycus' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``lycus model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: lycus_cli.main imports plugins indirectly
        from lycus_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.autolycus/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to lycus_cli/. Two layouts are
        ...
        # 2. User plugins (~/.autolycus/plugins/)
        user_dir = get_lycus_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.autolycus/plugins/)
        if _env_enabled("HERMES_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".autolycus" / "plugins"
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with lycus and must
            ...
            # all ~20 of them added several seconds to every `lycus`
            # invocation — including plain `lycus chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform Lycus ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `lycus plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `lycus plugins list`.
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``lycus_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``lycus_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="lycus-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``lycus tools`` TUI so plugin-provided toolsets appear
