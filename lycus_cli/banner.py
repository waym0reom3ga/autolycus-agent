...
Pure display functions with no LycusCLI state dependency.
"""
...
from lycus_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
[bold #00CED1]              v e r s i o n  0 . 1 . 6[/]"""
...
# (e.g. nix-built lycus — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/NousResearch/lycus-agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/nousresearch/lycus-agent"
...
def _fetch_pypi_latest(package: str = "lycus-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a Lycus update is available.
    ...
    cache_file = lycus_home / ".update_check"
    ...
    # and `lycus update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/lycus/update/check`
    ...
    if embedded_rev:
        ...
        # $AUTOLYCUS_HOME/lycus-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = lycus_home / "lycus-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Lycus git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$AUTOLYCUS_HOME/lycus-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``lycus_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from lycus_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/NousResearch/lycus-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Lycus checkout. Cached per-process. Release URL always points at the
    canonical NousResearch/lycus-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Lycus Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        from lycus_cli.codex_runtime_switch import get_current_runtime
        from lycus_cli.config import load_config as _load_cfg
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from lycus_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built lycus; we know an update
    ...
    # Pip-install warning — `pip install lycus-agent` is not the supported
