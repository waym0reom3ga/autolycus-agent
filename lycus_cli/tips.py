...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or lycus -c.",
    ...
    "Ctrl+Z suspends Lycus to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "lycus -c resumes your most recent CLI session. lycus -c \"project name\" resumes by title.",
    "lycus -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "lycus -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "lycus chat -t web,terminal enables only specific toolsets for a focused session.",
    "lycus chat -s github-pr-workflow preloads a skill at launch.",
    "lycus chat -q \"query\" runs a single non-interactive query and exits.",
    "lycus chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "lycus chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "lycus --yolo bypasses all dangerous command approval prompts for the entire session.",
    "lycus chat --source telegram tags the session for filtering in lycus sessions list.",
    "lycus -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "lycus doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "lycus dump outputs a compact setup summary — great for bug reports.",
    "lycus config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "lycus config edit opens config.yaml in your default editor.",
    "lycus config check scans for missing or stale configuration options.",
    "lycus sessions browse opens an interactive session picker with search.",
    "lycus sessions stats shows session counts by platform and database size.",
    "lycus sessions prune --older-than 30 cleans up old sessions.",
    "lycus skills search react --source skills-sh searches the skills.sh public directory.",
    "lycus skills check scans installed hub skills for upstream updates.",
    "lycus skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "lycus skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "lycus mcp add github --command npx adds MCP servers from the command line.",
    "lycus mcp serve runs Lycus itself as an MCP server for other agents.",
    "lycus auth add lets you add multiple API keys for credential pool rotation.",
    "lycus completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "lycus logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "lycus backup creates a zip backup of your entire Lycus home directory.",
    "lycus profile create coder creates an isolated profile that becomes its own command.",
    "lycus profile create work --clone copies your current config and keys to a new profile.",
    "lycus update syncs new bundled skills to ALL profiles automatically.",
    "lycus gateway install sets up Lycus as a system service (systemd/launchd).",
    "lycus memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    ...
    "Set compression.threshold: 0.60 to control when auto-compression fires (default: 60% of context).",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as lycus -w).",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Lycus tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'lycus profile create coder' creates the 'coder' command.",
    "lycus profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Lycus prints a resume command with session ID and stats.",
    "lycus sessions export backup.jsonl exports all sessions for backup or analysis.",
    "lycus -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "lycus skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .autolycus/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: lycus cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Lycus runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "lycus gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Lycus via a local macOS server.",
    ...
    "Context files (.autolycus.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "MASK.md is the agent's primary identity file — customize it to shape behavior.",
    "Lycus loads project context from .autolycus.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "lycus mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "lycus mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "lycus mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — lycus tools can toggle them per platform.",
    ...
    "lycus chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "lycus plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "lycus acp runs Lycus as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "lycus auth add nous or lycus auth add openai-codex sets up OAuth-based providers.",
    ...
    "lycus status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.autolycus/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "MASK.md completely replaces the agent's default identity — rewrite it to make Lycus your own.",
    "MASK.md is auto-seeded with a default personality on first run. Edit it to customize.",
    ...
    "Before a gateway session reset, Lycus auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.autolycus/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Profiles scope Lycus state via AUTOLYCUS_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "AUTOLYCUS_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in AUTOLYCUS_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.autolycus/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'lycus profile create ls' would be rejected.",
    "lycus profile create backup --clone-all copies everything (config, keys, MASK.md, memories, skills, sessions).",
    ...
    "lycus claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via lycus tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.autolycus/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.autolycus/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Lycus auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Lycus config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Lycus is working.',
    ...
    '/reload reloads ~/.autolycus/.env into the running session — pick up new API keys without restarting.',
    ...
    'Drop a ~/.autolycus/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'lycus curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "lycus curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'lycus curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'lycus auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'lycus dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'lycus dashboard embeds the full Lycus TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.autolycus/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.autolycus/dashboard-plugins/ — no npm build required.',
    ...
    'HERMES_IGNORE_RULES skips auto-injection of AGENTS.md, MASK.md, .cursorrules, memory, and preloaded skills.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.autolycus/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.autolycus/skills/ so drop-in skills appear without restarting the session.',
    ...
    'lycus -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'lycus chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'lycus chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'lycus chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "lycus chat --source tool tags programmatic chats so they don't clutter lycus sessions list.",
    'lycus dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'lycus sessions rename <ID> "new title" renames any past session; lycus sessions delete <ID> removes one.',
    'lycus import restores a session export or profile archive produced by sessions export or profile export.',
    'lycus fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'lycus pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'lycus setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'lycus status --deep runs the full health sweep across every component; plain lycus status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'LYCUS_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'HERMES_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.autolycus/plugins/ — trust-gated by design.',
    ...
    'LYCUS_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Lycus block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from lycus sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.autolycus/dashboard-plugins/.',
]
