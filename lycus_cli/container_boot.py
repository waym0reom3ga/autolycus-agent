...
``$AUTOLYCUS_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-lycus-setup
(the stage2 hook) has chowned the volume and seeded $AUTOLYCUS_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$AUTOLYCUS_HOME``,
    not under ``profiles/``). The dispatcher in ``lycus_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``lycus gateway start`` (no ``-p``) targets. Without it,
    bare ``lycus gateway start`` inside the container would land on
    ...
    ``$AUTOLYCUS_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        lycus_home: The container's AUTOLYCUS_HOME (typically /opt/data).
            Profiles live under ``<lycus_home>/profiles/<name>/``;
            the default profile lives at ``<lycus_home>`` itself.
    ...
    # ``lycus gateway start`` (no ``-p``) has somewhere to land;
    ...
    default_prior_state = legacy_default_state or _read_desired_state(lycus_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(lycus_home)
    ...
    profiles_root = lycus_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            ...
            # via ``lycus -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(lycus_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    """
    state_file = lycus_home / "gateway_state.json"
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off PID 1 argv, leaving the lycus args.

    The container PID 1 argv looks like
    ``/init /opt/lycus/docker/main-wrapper.sh <subcommand> [args...]`` and
    the wrapper re-execs ``lycus <subcommand>``. Peel ``init`` →
    ``main-wrapper.sh`` → ``lycus`` so callers can match on the bare
    ...
    if args and args[0].endswith("main-wrapper.sh"):
        args = args[1:]
    if args and Path(args[0]).name == "lycus":
        args = args[1:]
    return args
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``lycus dashboard ...``) never spawns or
    ...
    the gateway and dashboard containers share a bind-mounted AUTOLYCUS_HOME,
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `lycus -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit lycus
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the lycus user) won't hit EACCES. See
...
def _write_reconcile_log(
    lycus_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $AUTOLYCUS_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    # bind-mounted AUTOLYCUS_HOME, both race to flock() the same s6-log lock
    ...
    actions = reconcile_profile_gateways(
        lycus_home=lycus_home, scandir=scandir,
    )
    for a in actions:
        print(
