"""
Configuration management for Lycus Agent.

Config files are stored in ~/.autolycus/ for easy access:
- ~/.autolycus/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.autolycus/.env         - API keys and secrets

This module provides:
- lycus config          - Show current configuration
- lycus config edit     - Open config in editor
- lycus config set      - Set a specific value
- lycus config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``lycus config set`` (which rewrites
    ...
    ``config.yaml`` in place: lycus never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.autolycus/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``lycus logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  lycus config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Lycus makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Lycus itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``lycus update``, the TUI build.
...
#   on every plugin install / ``lycus update``.
...
# * ``AUTOLYCUS_HOME`` / ``HERMES_PROFILE`` / ``HERMES_CONFIG`` /
#   ``HERMES_ENV`` — Lycus runtime location flags. Writing these into
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Lycus runtime location — never via dashboard env writer.
    # NOT a HERMES_* blanket: integration credentials (HERMES_GEMINI_*,
    # HERMES_LANGFUSE_*, HERMES_SPOTIFY_*, ...) ARE allowed.
    "AUTOLYCUS_HOME", "HERMES_PROFILE", "HERMES_CONFIG", "HERMES_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Lycus runtime location "
            "(AUTOLYCUS_HOME, HERMES_PROFILE, ...) cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.autolycus/.env directly."
        )
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `lycus plugins enable
    # observability/langfuse` or `lycus tools → Langfuse`); credentials gate
...
def get_managed_system() -> Optional[str]:
    ...
    managed_marker = get_lycus_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Lycus is running in package-manager-managed mode.

    Two signals: the HERMES_MANAGED env var (set by the systemd service),
    or a .managed marker file in AUTOLYCUS_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade lycus-agent"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Lycus was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.autolycus/.install_method`` file (written by installers)
    ...
    container detection here:
      ...
      - the published ``nousresearch/lycus-agent`` image stamps ``docker``
    ...
    so ``lycus update`` bailed with "doesn't apply inside the Docker
    ...
    """
    stamp = get_lycus_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.autolycus/.install_method."""
    stamp = get_lycus_home() / ".install_method"
    try:
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Lycus lives in a ``uv tool`` layout.

    ``uv tool install lycus-agent`` places the install at
    ``.../uv/tools/lycus-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``lycus-agent`` happens to be uv-tool-installed on the machine while
    the *active* Lycus is a regular pip/venv install, causing
    ``lycus update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/lycus-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade lycus-agent"
    if method == "docker":
        return "docker pull nousresearch/lycus-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade lycus-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade lycus-agent"
        return "pip install --upgrade lycus-agent"
    return "lycus update"
...
# Long-form text for ``lycus update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# lycus_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Lycus, it doesn't update
...
✗ ``lycus update`` doesn't apply inside the Docker container.

Lycus Agent runs as a published image (nousresearch/lycus-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/lycus-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate lycus-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/lycus-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/lycus-agent/tags
  • Your config and session history live under ``$AUTOLYCUS_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``lycus update`` inside Docker.
...
def format_managed_message(action: str = "modify this Lycus installation") -> str:
    ...
    if managed_system == "NixOS":
        ...
        return (
            f"Cannot {action}: this Lycus installation is managed by NixOS "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Edit services.autolycus-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Lycus installation is managed by Homebrew "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade lycus-agent"
        )

    return (
        f"Cannot {action}: this Lycus installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Lycus."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from AUTOLYCUS_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, lycus_bin
    ...
    container_mode_file = get_lycus_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "lycus-agent")
    exec_user = info.get("exec_user", "lycus")
    lycus_bin = info.get("lycus_bin", "/data/current-package/bin/lycus")
...
# Re-export from lycus_constants — canonical definition lives there.
from lycus_constants import get_lycus_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_lycus_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_lycus_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_lycus_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the HERMES_UID / HERMES_GID env vars set by Docker deployments.

    Docker containers running Lycus commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level AUTOLYCUS_HOME once, but
    subdirectories created at runtime by ``ensure_lycus_home()`` (especially
...
def _chown_to_lycus_uid(path) -> None:
    ...
    directories created by :func:`ensure_lycus_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_lycus_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the lycus group can
    share state with the gateway service.

    The mode can be overridden via the AUTOLYCUS_HOME_MODE environment variable
    (e.g. AUTOLYCUS_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse AUTOLYCUS_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("AUTOLYCUS_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_lycus_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Lycus runs in a container with volume-mounted config files, forcing
...
def ensure_lycus_home():
    """Ensure ~/.autolycus directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_lycus_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Lycus-level retry loop that wraps the whole call.  Lower
        ...
        # app, ACP) in a code workspace, Lycus adds a coding operating brief
    ...
    "terminal": {
        ...
        #             AUTOLYCUS_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force AUTOLYCUS_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Lycus auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Lycus sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Lycus
        ...
        # bundled Lycus image, which drops to the `lycus` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Lycus sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Lycus should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``lycus chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.autolycus/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: lycus sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Lycus truncates the
    ...
    "compression": {
        "enabled": True,
        "threshold": 0.60,            # compress when context usage exceeds this ratio
    ...
    "auxiliary": {
        ...
        # Invoked by ``lycus kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``lycus kanban decompose`` and the kanban
        ...
        # ``lycus profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `lycus model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `lycus` (and `lycus chat`) launches by default:
        ...
        "interface": "cli",
        # When true, `lycus --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `lycus -c` muscle memory.  Default off so existing
        # users aren't surprised.  HERMES_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `lycus --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/lycus`. Per-platform overrides go under
    ...
    "dashboard": {
        ...
        # optional path prefix (e.g. ``https://example.com/lycus``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Lycus appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.autolycus/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.autolycus/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Lycus feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Lycus auto-pauses the goal and
    ...
    # always goes to ~/.autolycus/skills/.
    "skills": {
    ...
    # cron daemon.
    #
    # See `lycus curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # normally restored on every `lycus update`, so pruning them only
        ...
        # skipped), snapshot ~/.autolycus/skills/ into
        # ~/.autolycus/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `lycus curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for lycus-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "🔱 *Lycus Agent*" header.
    ...
    # stored approval from ~/.autolycus/shell-hooks-allowlist.json.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `lycus doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``lycus_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Lycus to lazy-install opt-in backend packages from PyPI
    ...
    # todos to ready, and fires `lycus -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `lycus` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `lycus kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     lycus-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Lycus tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.autolycus/logs/.
    ...
    # update model picker lists without shipping a lycus-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://lycus-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `lycus model` invocation; network failures
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.autolycus/.env,
        ...
        # behavior: files must live under the Lycus cache, under
        ...
        # uploaded as native gateway attachments. Files inside the Lycus
        # cache (~/.autolycus/cache/{documents,images,audio,video,screenshots})
    ...
    # Session storage — controls automatic cleanup of ~/.autolycus/state.db.
    ...
    "sessions": {
        ...
        # default of ``lycus sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.autolycus/sessions/session_{sid}.json`` on every turn
    ...
    # ``lycus update`` behaviour.
    "updates": {
        # Run a full ``lycus backup``-style zip of AUTOLYCUS_HOME before every
        # ``lycus update``.  Backups land in ``<AUTOLYCUS_HOME>/backups/`` and
        # can be restored with ``lycus import <path>``.  Off by default —
        # on large AUTOLYCUS_HOME directories the zip can add minutes to every
        ...
        "backup_keep": 5,
        # What `lycus update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<AUTOLYCUS_HOME>/lsp/bin/`` on first use.
    ...
    # `lycus tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.autolycus/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.autolycus/.env (or your shell) and never in config.yaml.
            ...
            # ~/.autolycus/bin/ on first use.  When False you must install
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'lycus model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Lycus auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @lycus:example.org)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #lycus)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: lycus-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'lycus-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Lycus API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Lycus API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Lycus API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.autolycus.config`` entries, then checks
    ...
    try:
        ...
        # should never break `lycus update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Lycus versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``lycus_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'lycus setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Lycus won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'lycus doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("AUTOLYCUS_HOME", "~/.autolycus")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$AUTOLYCUS_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_lycus_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `lycus plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `lycus curator status` has no
    ...
    #   3. Creates `~/.autolycus/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_lycus_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_lycus_home() / "logs" / "curator"
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from lycus_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: lycus config set <key> <value>")
    ...
    # metadata.autolycus.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _lycus_home / "config.yaml"`` local variable
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.autolycus/config.yaml.
    ...
    (which change ``AUTOLYCUS_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
#   openai-codex (OAuth — lycus auth) — OpenAI Codex
#   nous         (OAuth — lycus auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.autolycus/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.autolycus/.env.
    ...
    interactive menu render (`lycus tools`, `lycus setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `lycus tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Lycus env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.autolycus/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.autolycus/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.autolycus/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.autolycus/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Lycus — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Lycus vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.autolycus/.env or environment."""
...
def show_config():
    ...
    print(color("│              🦊 Lycus Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from lycus_cli.auth import get_anthropic_key
    ...
    try:
        ...
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                ...
                f"(run 'lycus doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    if enabled:
        print(f"  Threshold:    {compression.get('threshold', 0.60) * 100:.0f}%")
    ...
    print(color("  lycus config edit     # Edit config file", Colors.DIM))
    print(color("  lycus config set <key> <value>", Colors.DIM))
    print(color("  lycus setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: lycus config set <key> <value>")
            print()
            print("Examples:")
            print("  lycus config set model anthropic/claude-sonnet-4")
            print("  lycus config set terminal.backend docker")
            print("  lycus config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'lycus config migrate' to add them")
        ...
        print("  lycus config           Show current configuration")
        print("  lycus config edit      Open config in editor")
        print("  lycus config set <key> <value>   Set a config value")
        print("  lycus config check     Check for missing/outdated config")
        print("  lycus config migrate   Update config with new options")
        print("  lycus config path      Show config file path")
        print("  lycus config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``lycus config`` UI so users
