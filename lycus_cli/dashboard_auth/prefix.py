...
prefix (e.g. ``mission-control.tilos.com/lycus/*`` -> dashboard on
:9119), injecting ``X-Forwarded-Prefix: /lycus`` so the backend can
...
def _warn_if_malformed(source: str, raw: str) -> None:
    ...
    missing scheme (``lycus.example.com`` instead of
    ``https://lycus.example.com``) — the single most common cause of
    ...
    _log.warning(
        ...
        source,
        cleaned,
        cleaned.split("://")[-1] or "lycus.example.com",
    )
...
def normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/lycus"`` (no trailing slash) or ``""``
