...
Profile-aware location: ``$AUTOLYCUS_HOME/logs/dashboard-auth.log``.
...
from ``lycus_constants`` or other lycus_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$AUTOLYCUS_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``lycus_constants.get_lycus_home`` semantics: env var wins,
    else ``~/.autolycus``. A local copy avoids an import cycle with the
    middleware which lives below ``lycus_cli``.
    """
    home = os.environ.get("AUTOLYCUS_HOME") or str(Path.home() / ".autolycus")
