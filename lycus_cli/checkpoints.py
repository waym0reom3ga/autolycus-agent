"""`lycus checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.autolycus/checkpoints/``.  Actions:

    lycus checkpoints               # same as `status`
    lycus checkpoints status        # total size, project count, breakdown
    lycus checkpoints list          # per-project checkpoint counts + workdir
    lycus checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    lycus checkpoints clear [-f]    # nuke the entire base (asks first)
    lycus checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    lycus checkpoints
    lycus checkpoints prune --retention-days 3 --max-size-mb 200
    lycus checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: lycus checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``lycus checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `lycus checkpoints` → status
