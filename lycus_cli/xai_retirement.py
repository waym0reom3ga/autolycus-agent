...
Pure logic: walks a Lycus config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `lycus doctor` and a future `lycus migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Lycus config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Lycus config and return retirement issues.
