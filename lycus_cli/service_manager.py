...
the existing module-level functions in lycus_cli.gateway and
lycus_cli.gateway_windows directly. This protocol is a thin facade
...
``lycus gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``lycus gateway start/stop/restart``).
    """
    ...
    # `lycus gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged lycus
    ...
    detection always fails. Since every Lycus runtime call inside the
    container drops to lycus via ``s6-setuidgid``, that silent failure
...
# in ``lycus_cli.gateway`` (systemd/launchd) and ``lycus_cli.gateway_windows``
...
# active (set via the ``lycus -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in lycus_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from lycus_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from lycus_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from lycus_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from lycus_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in lycus_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from lycus_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from lycus_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from lycus_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from lycus_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``lycus_cli.gateway_windows`` (Scheduled Task /
...
#
# Per-profile gateways are registered dynamically when `lycus profile create`
# runs inside the container (Phase 4). Static services (main-lycus, dashboard)
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's AUTOLYCUS_HOME blindly;
    derive the shared profile root from the current AUTOLYCUS_HOME and map the
    ...
    lycus_home = Path(os.environ.get("AUTOLYCUS_HOME", "/opt/data"))
    if lycus_home.parent.name == "profiles":
        root = lycus_home.parent.parent
    else:
        root = lycus_home
...
# UID/GID of the in-image ``lycus`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the lycus user.
    ...
    (the ``lycus`` user — UID 10000 — running every Lycus runtime
    ...
    is dispatched as the lycus user.
    ...
    with lycus ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           lycus:lycus, 0755 (parent must already exist)
    ``svc_dir/event/``                     lycus:lycus, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 lycus:lycus, 0755
    ``svc_dir/supervise/event/``           lycus:lycus, 03730
    ``svc_dir/supervise/control``          lycus:lycus, 0660    (FIFO)
    ...
    slot's supervise/ was lycus-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        try:
            ...
            # Running as the lycus user already — directory is lycus-
    ...
    # when the parent slot's supervise/ is lycus-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``lycus -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`lycus profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-lycus, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources AUTOLYCUS_HOME (and any extra env) via with-contenv —
             so e.g. ``-e AUTOLYCUS_HOME=/data/lycus`` is honored at run
          ...
          4. Drops to the lycus user and exec's
             ``lycus -p <profile> gateway run`` (or just ``lycus
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``lycus gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root AUTOLYCUS_HOME profile" (the implicit profile that exists at
        the top of $AUTOLYCUS_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``lycus_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$AUTOLYCUS_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``AUTOLYCUS_HOME``,
        ...
        allocator in ``lycus_cli.profiles._allocate_gateway_port``).
        ...
        if profile == "default":
            gateway_cmd = "lycus gateway run"
        else:
            gateway_cmd = f"lycus -p {shlex.quote(profile)} gateway run"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid lycus {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${AUTOLYCUS_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the AUTOLYCUS_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e AUTOLYCUS_HOME=/data/lycus``
        gets its logs under /data/lycus/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``lycus logs`` reads and what
        ...
        return (
            ...
            f': "${{AUTOLYCUS_HOME:=/opt/data}}"\n'
            f'log_dir="$AUTOLYCUS_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            ...
            # later then runs its log service as lycus and crash-loops on
            ...
            f'chown lycus:lycus "$AUTOLYCUS_HOME/logs/gateways" 2>/dev/null || true\n'
            f'chown -R lycus:lycus "$log_dir" 2>/dev/null || true\n'
            f'rm -f "$log_dir/lock"\n'
            # Skip the drop when already non-root (CAP_SETGID).
            f'[ "$(id -u)" = 0 ] || exec s6-log 1 n10 s1000000 T "$log_dir"\n'
            f'exec s6-setuidgid lycus s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``lycus gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``lycus -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with lycus ownership
            ...
            # the lycus user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is lycus-owned (see ``_seed_supervise_skeleton``), and on
