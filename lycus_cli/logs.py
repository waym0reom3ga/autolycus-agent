"""``lycus logs`` — view and filter Lycus log files.

Supports tailing, following, session filtering, level filtering,
component filtering, and relative time ranges.  All log files live
under ``~/.autolycus/logs/``.

Usage examples::

    lycus logs                    # last 50 lines of agent.log
    lycus logs -f                 # follow agent.log in real time
    lycus logs errors             # last 50 lines of errors.log
    lycus logs gateway -n 100    # last 100 lines of gateway.log
    lycus logs gui -f            # follow gui.log (dashboard/pty/ws)
    lycus logs desktop -f        # follow desktop.log (Electron app boot/backend)
    lycus logs --level WARNING    # only WARNING+ lines
    lycus logs --session abc123   # filter by session ID substring
    lycus logs --component tools  # only tool-related lines
    lycus logs --since 1h         # lines from the last hour
    lycus logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_lycus_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Lycus runs — try 'lycus chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_lycus_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_lycus_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_lycus_home()}/logs/")
        return

    print(f"Log files in {display_lycus_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'lycus chat' to generate logs)")
