"""
MCP Server Management CLI — ``lycus mcp`` subcommand.

Implements ``lycus mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.autolycus/config.yaml under the ``mcp_servers`` key.
"""
...
from lycus_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_lycus_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from lycus_cli.cli_output import prompt as _shared_prompt
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.autolycus/.env`` into ``os.environ`` and recursively interpolate any
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``lycus mcp login`` to distinguish a genuine authentication
    ...
    try:
        ...
        return LycusTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # lycus_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  lycus mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  lycus mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  lycus mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_lycus_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: lycus mcp test " + name)
        return
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(f"Saved '{name}' to {display_lycus_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `lycus mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  lycus mcp add <name> --url <endpoint>')
        _info('  lycus mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `lycus mcp remove` + `lycus mcp add` to reconfigure auth.")
        return
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `lycus mcp login " + name + "`.")
            return
        if tools:
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'lycus mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``lycus mcp`` subcommands."""
    ...
    if action == "picker":
        from lycus_cli.mcp_picker import run_picker
        run_picker()
        return
    if action == "catalog":
        from lycus_cli.mcp_picker import show_catalog
        show_catalog()
        return
    if action == "install":
        from lycus_cli.mcp_picker import install_by_name
        import sys as _sys
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `lycus plugin`.
        from lycus_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("lycus mcp                                    Open the catalog picker (default)")
        _info("lycus mcp catalog                            List Nous-approved MCPs")
        _info("lycus mcp install <name>                     Install a catalog MCP")
        _info("lycus mcp serve                              Run as MCP server")
        _info("lycus mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("lycus mcp add <name> --command <cmd>         Add a stdio server")
        _info("lycus mcp add <name> --preset <preset>       Add from a known preset")
        _info("lycus mcp remove <name>                      Remove a server")
        _info("lycus mcp list                               List configured servers")
        _info("lycus mcp test <name>                        Test connection")
        _info("lycus mcp configure <name>                   Toggle tools")
        _info("lycus mcp login <name>                       Re-authenticate OAuth")
        print()
