"""
Lycus Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m lycus_cli.main web          # Start on http://127.0.0.1:9119
    python -m lycus_cli.main web --port 8080
"""
...
try:
    ...
    # running `lycus dashboard` needs fastapi+uvicorn; lazy install keeps
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``lycus gateway run`` — but the
    desktop app spawns a ``lycus dashboard`` backend, not a gateway, so a cron
    ...
    AUTOLYCUS_HOME — whichever process grabs the lock first wins the tick.
    """
    from cron.temporal_bridge import bridge_tick as cron_tick
...
async def _lifespan(app: "FastAPI"):
    ...
    # since the app has no gateway running the scheduler. Server `lycus
...
app = FastAPI(title="Autolycus Agent", version=__version__, lifespan=_lifespan)
...
_SESSION_HEADER_NAME = "X-Lycus-Session-Token"
...
#
# This list is defined in ``lycus_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from lycus_cli.dashboard_auth.public_paths import (
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Lycus-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
async def _dashboard_auth_gate(request: Request, call_next):
    from lycus_cli.dashboard_auth.middleware import gated_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Lycus (no terminal prompt), "
...
class ModelAssignment(BaseModel):
    ...
    # ``lycus model`` custom flow collects. Honored only on the main slot for
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Lycus provider slug — fine.
    ...
    1. Vendor-name → Lycus-provider mapping: when the provider string is
       not a known Lycus provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    """
    from lycus_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from lycus_cli.model_normalize import normalize_model_for_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and "/" in model_in:
        ...
        try:
            ...
            cur_provider = ""
        from lycus_cli.models import _AGGREGATOR_PROVIDERS
...
def _default_lycus_root_is_opt_data() -> bool:
    raw = os.environ.get("AUTOLYCUS_HOME", "").strip()
    if not raw:
        return False
    try:
        from lycus_constants import get_default_lycus_root

        root = get_default_lycus_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``lycus update``.
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's Lycus root is actually /opt/data
    # (the container/hosted layout) or when HERMES_DASHBOARD_FILES_ROOT is set.
    if _default_lycus_root_is_opt_data():
...
async def get_status():
    ...
    # and which providers are registered so ``lycus status`` and the
    # SPA's StatusPage can show "OAuth gate ON via Nous Research" or
    # "loopback only — no auth gate" with no extra round trips.
    auth_required = bool(getattr(app.state, "auth_required", False))
    auth_providers: list[str] = []
    try:
        from lycus_cli.dashboard_auth import list_providers as _list_providers
    ...
    status = {
        ...
        "can_update_lycus": not _dashboard_local_update_managed_externally(),
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the lycus home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_lycus_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def run_curator():
    ...
    try:
        proc = _spawn_lycus_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from lycus_cli.auth import get_nous_auth_status
    ...
    features = []
    try:
        from lycus_cli.nous_subscription import get_nous_subscription_features
...
async def run_prompt_size():
    try:
        proc = _spawn_lycus_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_lycus_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_lycus_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from lycus_cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.autolycus/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_lycus_home() / "logs"
...
def _spawn_lycus_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``lycus <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``lycus_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "lycus_cli.main", *subcommand]
...
def _spawn_gateway_restart() -> Tuple[subprocess.Popen, bool]:
    """Spawn ``lycus gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``lycus gateway restart`` children race each other on the
    ...
    return _spawn_lycus_action(["gateway", "restart"], "gateway-restart"), False
...
async def restart_gateway():
    """Kick off a ``lycus gateway restart`` in the background."""
    try:
...
@app.post("/api/lycus/update")
async def update_lycus():
    """Kick off ``lycus update`` in the background."""
    if _dashboard_local_update_managed_externally():
        message = (
            "Lycus updates are managed outside this dashboard in "
            "containerized environments. The built-in local updater is "
            "disabled here."
        )
        _record_completed_action("lycus-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "lycus-update",
    ...
    try:
        proc = _spawn_lycus_action(["update"], "lycus-update")
    except Exception as exc:
        _log.exception("Failed to spawn lycus update")
...
@app.get("/api/lycus/update/check")
async def check_lycus_update(force: bool = False):
    """Report whether a Lycus update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/lycus/update`` actually runs ``lycus update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed Lycus version string
    ...
    try:
        ...
        if force:
            try:
                (get_lycus_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.autolycus/config.yaml`` under ``tts.``.
    """
...
async def get_profiles_sessions(
    ...
    from lycus_state import SessionDB
    from lycus_cli import profiles as profiles_mod
...
async def search_sessions(q: str = "", limit: int = 20, profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            ...
            # logs, or another Lycus surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Lycus supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in lycus_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_model_options(profile: Optional[str] = None):
    ...
    """
    try:
        from lycus_cli.inventory import build_models_payload, load_picker_context
        ...
        # onboarding picker) sees the SAME full provider universe `lycus model`
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `lycus model` does so GUI onboarding lands on a
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.autolycus/config.yaml`` — applies to **new** sessions only.
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (lycus_cli/main.py
        ...
        if provider.strip().lower() == "nous":
            try:
                from lycus_cli.nous_subscription import apply_nous_managed_defaults
                from lycus_cli.tools_config import _get_platform_tools
        ...
        # endpoint, mirroring the ``lycus model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from lycus_cli.main import _auto_provider_name, _save_custom_provider
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Lycus from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Lycus to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Lycus from Slack via Socket Mode.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Lycus to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Lycus in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Lycus through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Lycus via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Lycus through an IMAP/SMTP mailbox.",
        "docs_url": "https://lycus-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Lycus to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Lycus inside Feishu / Lark.",
    ...
    "weixin": {
        ...
        "docs_url": "https://lycus-agent.nousresearch.com/docs/user-guide/messaging/weixin/",
        "env_vars": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN", "WEIXIN_BASE_URL"),
        "required_env": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN"),
    },
    "bluebubbles": {
        "name": "BlueBubbles (iMessage)",
        "description": "Use Lycus through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Lycus to a QQ Bot from the QQ Open Platform.",
    ...
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Lycus to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Lycus as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://lycus-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "webhook": {
        ...
        "docs_url": "https://lycus-agent.nousresearch.com/docs/user-guide/messaging/webhooks/",
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in lycus gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in lycus gateway setup",
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.autolycus-agent.nousresearch.com"
_TELEGRAM_ONBOARDING_USER_AGENT = f"LycusDashboard/{__version__}"
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Lycus Agent").strip() or "Lycus Agent"
...
def _restart_gateway_after_telegram_onboarding() -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Lycus appears
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # all resolve against the requested profile's AUTOLYCUS_HOME.
...
# the canonical ``lycus auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.autolycus/.anthropic_oauth.json`` — Lycus-managed PKCE flow (what
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_lycus_dotenv()``, so the same check covers them)
    ...
    try:
        from agent.anthropic_adapter import (
            read_lycus_oauth_credentials,
            _HERMES_OAUTH_FILE,
        )
    except ImportError:
        read_lycus_oauth_credentials = None  # type: ignore
        _HERMES_OAUTH_FILE = None  # type: ignore

    lycus_creds = None
    if read_lycus_oauth_credentials:
        try:
            lycus_creds = read_lycus_oauth_credentials()
        except Exception:
            lycus_creds = None
    if lycus_creds and lycus_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "lycus_pkce",
            "source_label": f"Lycus PKCE ({_HERMES_OAUTH_FILE})",
            "token_preview": _truncate_token(lycus_creds.get("accessToken")),
            "expires_at": lycus_creds.get("expiresAt"),
            "has_refresh_token": bool(lycus_creds.get("refreshToken")),
        }
    ...
    try:
        ...
        pass
    try:
        from lycus_cli.config import get_env_value
    except ImportError:
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Lycus even
    when they also have a separate Lycus-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "lycus auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "lycus auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "lycus auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "lycus auth add minimax-oauth",
    ...
    },
    {
        ...
        "flow": "loopback",
        "cli_command": "lycus auth add xai-oauth",
        "docs_url": "https://lycus-agent.nousresearch.com/docs/guides/xai-grok-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "lycus auth add anthropic",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from lycus_cli import auth as hauth
        if provider_id == "nous":
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Lycus, so the disconnect
    ...
    user sees exactly what executes, and Lycus then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Lycus — run the disconnect command to remove it."
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("lycus_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        # Anthropic clears only the Lycus-managed PKCE file and auth-store entry.
        ...
        if provider_id == "anthropic":
            ...
            try:
                from lycus_cli.auth import clear_provider_auth
        ...
        try:
            from lycus_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
#          → persists to ~/.autolycus/.anthropic_oauth.json AND credential pool
...
# Guarded so lycus web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Lycus file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``lycus auth add anthropic``.
    """
    from agent.anthropic_adapter import _HERMES_OAUTH_FILE
    payload = {
...
def _submit_anthropic_pkce(
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "lycus-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
# the tokens exactly like `lycus auth add xai-oauth`.
...
def _add_xai_oauth_pool_entry(
    ...
    """Mirror `lycus auth add xai-oauth`'s credential-pool insert.
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from lycus_cli.auth import persist_nous_credentials
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from lycus_cli.auth import _save_codex_tokens
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `lycus sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `lycus sessions prune`)."""
    if body.older_than_days < 1:
        raise HTTPException(status_code=400, detail="older_than_days must be >= 1")
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_lycus_home()
    db = _open_session_db_for_profile(body.profile)
    try:
...
async def get_logs(
    ...
    from lycus_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_lycus_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from lycus_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, AUTOLYCUS_HOME)."""
    from lycus_cli import profiles as profiles_mod
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process AUTOLYCUS_HOME at import time. The dashboard is a single
...
async def get_cron_delivery_targets():
    ...
    try:
        from cron.temporal_bridge import bridge_cron_delivery_targets as cron_delivery_targets
...
#
# Wraps the same config data layer the CLI uses (lycus_cli.mcp_config), so
# servers managed here show up under `lycus mcp list` and vice versa.  Secrets
...
async def list_mcp_servers(profile: Optional[str] = None):
    from lycus_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from lycus_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from lycus_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from lycus_cli.mcp_config import _get_mcp_servers, _probe_single_server
    ...
    def _probe_scoped():
        ...
        # AUTOLYCUS_HOME override (see mcp_tool._wrap_with_home_override), so
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # The -p subprocess rebinds AUTOLYCUS_HOME-derived paths in the child.
    if entry.install is not None:
        try:
            proc = _spawn_lycus_action(
...
#
# Wraps the same JSON store the CLI uses (lycus_cli.webhook); the webhook
...
async def list_webhooks():
    import lycus_cli.webhook as wh
...
# spawn the real `lycus gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway():
    try:
        proc = _spawn_lycus_action(["gateway", "start"], "gateway-start")
    except Exception as exc:
...
async def stop_gateway():
    try:
        proc = _spawn_lycus_action(["gateway", "stop"], "gateway-stop")
    except Exception as exc:
...
async def list_credential_pool():
    ...
    from lycus_cli.auth import read_credential_pool
...
# `lycus memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `lycus memory setup` to configure a new one.",
            )
...
async def run_doctor():
    try:
        proc = _spawn_lycus_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_lycus_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_lycus_action(args, "backup")
    except Exception as exc:
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `lycus import`. The spawned action runs with
...
async def run_import(body: ImportRequest):
    ...
    if body.force:
        args.append("--force")
    try:
        proc = _spawn_lycus_action(args, "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    form can offer them.
    """
    from lycus_cli.config import load_config as _load_config
    from agent import shell_hooks

    try:
        from lycus_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <lycus_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_lycus_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_lycus_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``lycus`` subprocess, and
...
async def preview_skill_hub(identifier: str = ""):
    ...
    def _run():
        from lycus_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`lycus -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the AUTOLYCUS_HOME
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_lycus_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "lycus setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local AUTOLYCUS_HOME override so the write lands in the target
    ...
    """
    from lycus_constants import set_lycus_home_override, reset_lycus_home_override

    token = set_lycus_home_override(str(profile_dir))
    try:
        provider, model = _normalize_main_model_assignment(provider, model)
        cfg = load_config()
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local AUTOLYCUS_HOME override (same mechanism as
    ...
    """
    from lycus_constants import set_lycus_home_override, reset_lycus_home_override
    from lycus_cli.mcp_security import validate_mcp_server_entry
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the AUTOLYCUS_HOME override. Returns the
    number of skills newly disabled.
    """
    from lycus_constants import set_lycus_home_override, reset_lycus_home_override
    from lycus_cli.skills_config import get_disabled_skills, save_disabled_skills
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's AUTOLYCUS_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``lycus profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from AUTOLYCUS_HOME).
    """
    from lycus_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``lycus profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the AUTOLYCUS_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``lycus profile describe
    <name> --auto``.
    ...
    _resolve_profile_dir(name)
    try:
        from lycus_cli import profile_describer
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_lycus_home()`` at call
       time — the context-local override from ``set_lycus_home_override``
    ...
    retargeted to the *current* ``get_lycus_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a AUTOLYCUS_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
async def get_skills(profile: Optional[str] = None):
    ...
    from lycus_cli.skills_config import get_disabled_skills
    with _profile_scope(profile):
        config = load_config()
...
async def get_toolsets(profile: Optional[str] = None):
    from lycus_cli.tools_config import (
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``lycus tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``lycus tools`` picker shows
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``lycus tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.autolycus/.env`` via ``save_env_value`` —
    the same store ``lycus tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``lycus tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``lycus -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same AUTOLYCUS_HOME the rest of the
...
#
# The endpoint spawns the same ``lycus --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from lycus_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from lycus_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from lycus_cli.dashboard_auth.audit import AuditEvent, audit_log
        from lycus_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Default: whatever ``lycus --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``HERMES_TUI_RESUME`` env var —
    matching what ``lycus_cli.main._launch_tui`` does for the CLI path.
    ...
    ``AUTOLYCUS_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_lycus_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``lycus -p <name>`` performs. The in-process
    ...
    """
    from lycus_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    if profile_dir is not None:
        env["AUTOLYCUS_HOME"] = str(profile_dir)
    ...
    # inherits the profile AUTOLYCUS_HOME set above.
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from lycus_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Lycus inside WSL2 to use the dashboard's /chat "
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`lycus_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from lycus_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/lycus/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /lycus`` on every request. We
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/lycus``)
    ...
    # under ``/lycus`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Lycus backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Lycus Teal",         "description": "Classic dark teal — the canonical Lycus look"},
    {"name": "default-large", "label": "Lycus Teal (Large)", "description": "Lycus Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.autolycus/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.autolycus/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_lycus_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.autolycus/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as lycus_cli.plugins):
    1. User plugins:    ~/.autolycus/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.autolycus/plugins/  (only if HERMES_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from lycus_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_lycus_home() / "plugins", "user"),
    ...
    # ``lycus_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("HERMES_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".autolycus" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from lycus_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_lycus_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"lycus auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from lycus_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from lycus_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from lycus_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from lycus_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.autolycus/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.autolycus/plugins/ if you trust it)",
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"lycus_dashboard_plugin_{plugin['name']}"
...
from lycus_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        from lycus_cli.dashboard_auth import list_providers
        if not list_providers():
            # Surface the *specific* reason any bundled provider declined
            # to register (e.g. missing HERMES_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Lycus Agent exposes a
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            print(f"  Lycus Web UI → http://{host}:{actual_port}")
