"""Profile distributions — shareable, packaged Lycus profiles via git.

A distribution is a Lycus profile published as a git repository (or
...
* ``lycus profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``lycus skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``lycus profile``, not a parallel tree):

    lycus profile install <source> [--name N] [--alias] [--force] [--yes]
...
* Distribution-owned paths (MASK.md, mcp.json, skills/, cron/,
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "lycus_state.db", "response_store.db",
    "response_store.db-shm", "response_store.db-wal",
    "gateway.pid", "gateway_state.json", "processes.json",
    "auth.lock", "active_profile", ".update_check",
    "errors.log", ".autolycus_history",
    # User data
    ...
    "backups", "cache",
    # Infrastructure
    "lycus-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            lycus_requires=str(data.get("lycus_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.autolycus_requires:
            out["lycus_requires"] = self.autolycus_requires
        if self.author:
...
def check_lycus_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Lycus {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``lycus profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Lycus profile distribution."
            )
...
def plan_install(
    ...
    from lycus_cli import __version__ as lycus_version
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Lycus distribution."
        )

    # Version check up-front so we fail fast
    check_lycus_requires(manifest.autolycus_requires, lycus_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.autolycus).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="lycus_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `lycus profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`lycus profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="lycus_dist_update_") as tmp:
        plan = plan_install(
