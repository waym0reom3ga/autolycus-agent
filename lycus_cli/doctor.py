"""
Doctor command for lycus CLI.

Diagnoses issues with Lycus Agent setup.
"""
...
from lycus_cli.config import get_project_root, get_lycus_home, get_env_path
from lycus_cli.env_loader import load_lycus_dotenv
from lycus_constants import display_lycus_home

PROJECT_ROOT = get_project_root()
AUTOLYCUS_HOME = get_lycus_home()
_DHH = display_lycus_home()  # user-facing display path (e.g. ~/.autolycus or ~/.autolycus/profiles/coder)

# Load environment variables from ~/.autolycus/.env so API key checks work
_env_path = get_env_path()
load_lycus_dotenv(lycus_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from lycus_cli.colors import Colors, color
from lycus_cli.models import _HERMES_USER_AGENT
from lycus_constants import OPENROUTER_MODELS_URL
...
from lycus_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.autolycus/.env contains provider auth/base URL settings."""
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized in {"google / gemini", "gemini"}:
        try:
            from lycus_cli.auth import get_gemini_oauth_auth_status
            return bool((get_gemini_oauth_auth_status() or {}).get("logged_in"))
        except Exception:
            return False
    if normalized == "minimax":
        try:
            from lycus_cli.auth import get_minimax_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches lycus_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``lycus --version`` reporting a stale version while
    ...
    """
    try:
        from lycus_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != lycus_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'lycus update', or set "
            "lycus_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-lycus and dashboard static services are up
    ...
    for static in ("main-lycus", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `lycus profile create <name>`")
        return
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from lycus_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `lycus`.
    os.environ.setdefault("HERMES_INTERACTIVE", "1")

    # Handle `lycus doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.autolycus/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 Lycus Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`lycus doctor --ack {hit.advisory.id}`."
                )
    ...
    # Detect drift between pyproject.toml and lycus_cli/__init__.py versions
    ...
    # Check ~/.autolycus/.env (primary location for user config)
    env_path = AUTOLYCUS_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'lycus setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'lycus setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'lycus setup' to create one")
                issues.append("Run 'lycus setup' to create .env")
    
    # Check ~/.autolycus/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = AUTOLYCUS_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from lycus_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from lycus_cli.providers import (
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'lycus config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from lycus_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.autolycus/.env or run 'lycus setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'lycus setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'lycus config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from lycus_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from lycus_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'lycus setup' to migrate config")
                else:
                    issues.append("Run 'lycus doctor --fix' or 'lycus setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'lycus doctor --fix'")
        except Exception:
            pass
        ...
        try:
            import yaml
            from lycus_cli.config import load_env, remove_env_value
            with open(config_path, encoding="utf-8") as f:
                raw_config = yaml.safe_load(f) or {}
            ...
            if drift:
                check_warn(
                    ...
                    "(stale ghost from an earlier `lycus setup` run)",
                )
                if should_fix:
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Lycus' own device-code flow — the Codex CLI is
    ...
    for subdir_name in expected_subdirs:
        subdir_path = lycus_home / subdir_name
    ...
    if soul_path.exists():
        ...
        if should_fix:
            ...
            soul_path.write_text(
                "# Lycus Agent Persona\n\n"
                "<!-- Edit this file to customize how Lycus communicates. -->\n\n"
    ...
    memories_dir = lycus_home / "memories"
    ...
    state_db_path = lycus_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            from lycus_state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'lycus doctor --fix' "
                        "(or 'lycus sessions repair') to recover hidden sessions"
                    )
            else:
    ...
    wal_path = lycus_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'lycus doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "lycus"
        ...
        _cmd_link = _cmd_link_dir / "lycus"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(lycus not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/lycus → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/lycus points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/lycus → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/lycus — run 'lycus doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/lycus exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/lycus not found",
                    "(lycus command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/lycus → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/lycus symlink — run 'lycus doctor --fix'")
    ...
    try:
        from lycus_constants import is_container as _is_container
    ...
    if _safe_which("node"):
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `lycus doctor` invocation.
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a Lycus one.
                        check_info(
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'lycus config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``lycus doctor``.
    ...
    # never the legitimate source for `lycus doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'lycus setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = AUTOLYCUS_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: lycus skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = AUTOLYCUS_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: lycus memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: lycus memory setup",
                    "No Honcho API key — run 'lycus memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run lycus memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: lycus memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: lycus memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from lycus_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "lycus -p" in content:
                            _m = _re.search(r"lycus -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'lycus doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
