"""
Lycus MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Lycus-specific extra)

Usage:
    lycus mcp serve
    lycus mcp serve --verbose
...
logger = logging.getLogger("lycus.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using AUTOLYCUS_HOME."""
    try:
        from lycus_constants import get_lycus_home
        return get_lycus_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("AUTOLYCUS_HOME", Path.home() / ".autolycus")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from lycus_constants import get_lycus_home
        directory_file = get_lycus_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("AUTOLYCUS_HOME", Path.home() / ".autolycus")
...
class EventBridge:
    ...
    This is the Lycus equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from lycus_constants import get_lycus_home
            db_file = get_lycus_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("AUTOLYCUS_HOME", Path.home() / ".autolycus")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Lycus MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "lycus",
        instructions=(
            "Lycus Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Lycus MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
