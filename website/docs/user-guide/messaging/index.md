...
description: "Chat with Lycus from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Lycus from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Lycus](/guides/use-voice-mode-with-lycus).
...
```mermaid
flowchart TB
    subgraph Gateway["Lycus Gateway"]
...
For group chats, hooks, and automation flows, Lycus supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Lycus keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Lycus does not hide failures just because the text resembles a silence token.
...
```bash
lycus gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
lycus gateway              # Run in foreground
lycus gateway setup        # Configure messaging platforms interactively
lycus gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo lycus gateway install --system   # Linux only: install a boot-time system service
lycus gateway start        # Start the default service
lycus gateway stop         # Stop the default service
lycus gateway status       # Check default service status
lycus gateway status --system         # Linux only: inspect the system service explicitly
```
...
Configure per-platform overrides in `~/.autolycus/gateway.json`:
...
lycus pairing approve telegram XKGH5N7P

# Other pairing commands:
lycus pairing list          # View pending + approved users
lycus pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Lycus appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.autolycus/config.yaml`:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.autolycus/config.yaml`:
...
```bash
lycus gateway install               # Install as user service
lycus gateway start                 # Start the service
lycus gateway stop                  # Stop the service
lycus gateway status                # Check status
journalctl --user -u lycus-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo lycus gateway install --system
sudo lycus gateway start --system
sudo lycus gateway status --system
journalctl -u lycus-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `lycus update`. When `lycus update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart lycus-gateway` command (it never blocks on an interactive password prompt).
...
```bash
lycus gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `lycus update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo lycus update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/lycus-gateway`:

```
lycus ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed lycus-gateway*, /usr/bin/systemctl --no-ask-password start lycus-gateway*, /usr/bin/systemctl --no-ask-password restart lycus-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Lycus will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Lycus installations on the same machine (with different `AUTOLYCUS_HOME` directories), each gets its own systemd service name. The default `~/.autolycus` uses `lycus-gateway`; other installations use `lycus-gateway-<hash>`. The `lycus gateway` commands automatically target the correct service for your current `AUTOLYCUS_HOME`.
:::

### macOS (launchd)

```bash
lycus gateway install               # Install as launchd agent
lycus gateway start                 # Start the service
lycus gateway stop                  # Stop the service
lycus gateway status                # Check status
tail -f ~/.autolycus/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.autolycus.gateway.plist`. It includes three environment variables:
...
- **AUTOLYCUS_HOME** — scopes the gateway to your Lycus installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `lycus gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `AUTOLYCUS_HOME` directory gets its own launchd label. The default `~/.autolycus` uses `ai.autolycus.gateway`; other installations use `ai.autolycus.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `lycus-cli` | Full access |
| Telegram | `lycus-telegram` | Full tools including terminal |
| Discord | `lycus-discord` | Full tools including terminal |
| WhatsApp | `lycus-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `lycus-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `lycus-slack` | Full tools including terminal |
| Google Chat | `lycus-google_chat` | Full tools including terminal |
| Signal | `lycus-signal` | Full tools including terminal |
| SMS | `lycus-sms` | Full tools including terminal |
| Email | `lycus-email` | Full tools including terminal |
| Home Assistant | `lycus-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `lycus-mattermost` | Full tools including terminal |
| Matrix | `lycus-matrix` | Full tools including terminal |
| DingTalk | `lycus-dingtalk` | Full tools including terminal |
| Feishu/Lark | `lycus-feishu` | Full tools including terminal |
| WeCom | `lycus-wecom` | Full tools including terminal |
| WeCom Callback | `lycus-wecom-callback` | Full tools including terminal |
| Weixin | `lycus-weixin` | Full tools including terminal |
| BlueBubbles | `lycus-bluebubbles` | Full tools including terminal |
| QQBot | `lycus-qqbot` | Full tools including terminal |
| Yuanbao | `lycus-yuanbao` | Full tools including terminal |
| Microsoft Teams | `lycus-teams` | Full tools including terminal |
| API Server | `lycus-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `lycus-webhook` | Full tools including terminal |
...
1. **Gateway log** (`~/.autolycus/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
