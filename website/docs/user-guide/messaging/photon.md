...
Connect Lycus to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Lycus — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Lycus runs it in a
...
lycus photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Lycus Agent` project on your account.
...
Runtime credentials are written to `~/.autolycus/.env`
...
(device token, dashboard project id) lives in `~/.autolycus/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Lycus
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Lycus replies with a pairing code. Approve it with:

```bash
lycus pairing approve photon <CODE>
```

Use `lycus pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.autolycus/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.autolycus/.env`):
...
By default Lycus responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Lycus` and
`@Lycus agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Lycus will reply.
...
assigned iMessage line Lycus uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run lycus photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `lycus photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `lycus photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `lycus photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Lycus sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Lycus wake words  | JSON list / comma / newline regex patterns for group mentions |
