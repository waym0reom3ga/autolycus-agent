...
| | |
|---|---|
| Source | Optional — install with `lycus skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Lycus Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Lycus terminal calls (desktop app flow only)
...
### Service Account (recommended for Lycus)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.autolycus/.env` (the skill will prompt for this on first load).
...
## Lycus Execution Pattern (desktop app flow)

Lycus terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/lycus-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/lycus-op.sock"
