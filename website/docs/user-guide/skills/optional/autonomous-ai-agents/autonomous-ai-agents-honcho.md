...
description: "Configure and use Honcho memory with Lycus -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Lycus -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `lycus skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Lycus

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Lycus profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/lycus#running-honcho-locally-with-lycus

### Verify

```bash
lycus honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Lycus profile's AI peer
...
Honcho models conversations as interactions between **peers**. Lycus creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Lycus instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Lycus run |
| `global` | Single session across all directories |

Manual override: `lycus honcho map my-project-name`
...
Each Lycus profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block lycus.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `lycus.coder` host block in `honcho.json`
...
```bash
lycus honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Lycus when Honcho memory is active.
...
Config file: `$AUTOLYCUS_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `lycus honcho setup`. Ensure `memory.provider: honcho` is in `~/.autolycus/config.yaml`.

### Memory not persisting across sessions
Check `lycus honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `lycus profile create <name> --clone`. For existing profiles: `lycus honcho sync`.
...
| Command | Description |
|---------|-------------|
| `lycus honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `lycus honcho status` | Show resolved config, connection test, peer info for active profile |
| `lycus honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `lycus honcho disable` | Disable Honcho for the active profile |
| `lycus honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `lycus honcho peers` | Show peer identities across all profiles |
| `lycus honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `lycus honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `lycus honcho sessions` | List known directory-to-session-name mappings |
| `lycus honcho map <name>` | Map current working directory to a Honcho session name |
| `lycus honcho identity` | Seed AI peer identity or show both peer representations |
| `lycus honcho sync` | Create host blocks for all Lycus profiles that don't have one yet |
| `lycus honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Lycus + Honcho |
| `lycus memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `lycus memory status` | Show active memory provider and config |
| `lycus memory off` | Disable external memory provider |
