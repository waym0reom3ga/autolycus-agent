---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Lycus Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Lycus Agent"
---
...
Migrate a user's OpenClaw customization footprint into Lycus Agent. Imports Lycus-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
...
| Related skills | [`lycus-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-lycus-agent) |
...
# OpenClaw -> Lycus Migration

Use this skill when a user wants to move their OpenClaw setup into Lycus Agent with minimal manual cleanup.
...
```bash
lycus claw migrate              # Full interactive migration
lycus claw migrate --dry-run    # Preview what would be migrated
lycus claw migrate --preset user-data   # Migrate without secrets
lycus claw migrate --overwrite  # Overwrite existing conflicts
lycus claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `lycus setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.
...
- copy OpenClaw skills into `~/.autolycus/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Lycus workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.autolycus/tts/`
- archive non-secret docs that do not have a direct Lycus destination
...
- `scripts/openclaw_to_lycus.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.autolycus/skills/migration/openclaw-migration/scripts/openclaw_to_lycus.py`

Do not guess a shorter path like `~/.autolycus/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.autolycus/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Lycus-compatible secrets, currently:
...
Lycus CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Lycus skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.autolycus/skills/migration/openclaw-migration/scripts/openclaw_to_lycus.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.autolycus/skills/migration/openclaw-migration/scripts/openclaw_to_lycus.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.autolycus/skills/migration/openclaw-migration/scripts/openclaw_to_lycus.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.autolycus/skills/migration/openclaw-migration/scripts/openclaw_to_lycus.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.autolycus/skills/migration/openclaw-migration/scripts/openclaw_to_lycus.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.autolycus/skills/migration/openclaw-migration/scripts/openclaw_to_lycus.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Lycus unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Lycus targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
- Lycus persona state imported
- Lycus memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.autolycus/skills/openclaw-imports/`
