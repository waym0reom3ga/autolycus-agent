---
title: "Telephony — Give Lycus phone capabilities without core tool changes"
sidebar_label: "Telephony"
description: "Give Lycus phone capabilities without core tool changes"
---
...
Give Lycus phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `lycus skills install official/productivity/telephony` |
...
This optional skill gives Lycus practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `~/.autolycus/.env`
...
It does **not** turn Lycus into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Lycus memory
...
### 1) "I want Lycus to own a real phone number"
Use **Twilio**.
...
- pairs well with Lycus `text_to_speech` plus a public file host or tunnel
...
### `~/.autolycus/telephony_state.json`
...
```bash
SCRIPT="$(find ~/.autolycus/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
lycus skills search telephony
lycus skills install official/productivity/telephony
```
...
Then save credentials into Lycus:
...
3. Buy it and save it into `~/.autolycus/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python3 "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Lycus calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Lycus's existing `text_to_speech` support.

Use this when:
- you want the call to use Lycus's configured TTS voice rather than Twilio `<Say>`
...
Recommended Lycus TTS -> Twilio Play workflow:

1. Generate the audio with Lycus `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Lycus to create the message audio with `text_to_speech`
...
Important note:
- Lycus TTS is great for prerecorded outbound messages
...
- Lycus STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Lycus memory.
...
- Do not store arbitrary third-party phone numbers in Lycus memory.
...
3. persist that number to `~/.autolycus/.env`
