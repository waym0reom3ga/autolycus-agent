...
Run multiple independent Lycus agents on the same machine — each with its own config, API keys, memory, sessions, skills, and gateway state.
...
```bash
lycus profile create coder       # creates profile + "coder" command alias
coder setup                       # configure API keys and model
coder chat                        # start chatting
```

That's it. `coder` is now its own Lycus profile with its own config, memory, and state.

## Creating a profile

:::tip
Quickest setup: run `lycus setup --portal` inside the new profile to wire up models + tools at once. See [Nous Portal](/integrations/nous-portal).
:::
...
```bash
lycus profile create researcher --description "Reads source code and external docs, writes findings."
```

You can also set or auto-generate the description later with `lycus profile describe` — see the [Kanban guide](./features/kanban#auto-vs-manual-orchestration) for the full routing model.
...
```bash
lycus profile create work-backup --clone-from coder --clone-all
```
...
The alias works with every lycus subcommand — it's just `lycus -p <name>` under the hood.
...
```bash
lycus -p coder chat
lycus --profile=coder doctor
lycus chat -p coder -q "hello"    # works in any position
```

### Sticky default (`lycus profile use`)

```bash
lycus profile use coder
lycus chat                   # now targets coder
lycus tools                  # configures coder's tools
lycus profile use default    # switch back
```

Sets a default so plain `lycus` commands target that profile. Like `kubectl config use-context`.
...
- **`lycus profile`**: Shows current profile name, path, model, gateway status
...
Using `cwd: "."` on the local backend means "the directory Lycus was launched from", not "the profile directory".
...
```bash
# Edit coder's tokens
nano ~/.autolycus/profiles/coder/.env

# Edit assistant's tokens
nano ~/.autolycus/profiles/assistant/.env
```
...
```bash
coder gateway install         # creates lycus-gateway-coder systemd/launchd service
assistant gateway install     # creates lycus-gateway-assistant service
```

Each profile gets its own service name. They run independently.

:::note Inside the official Docker image
Per-profile gateways are supervised by [s6-overlay](https://github.com/just-containers/s6-overlay) (PID 1 in the container), so `lycus profile create <name>` automatically registers an s6 service slot at `/run/service/gateway-<name>/`. `lycus -p <name> gateway start/stop/restart` dispatches to `s6-svc` instead of spawning a bare process — crashes are auto-restarted and `docker restart` preserves the previously-running set of gateways. See [Per-profile gateway supervision](/user-guide/docker#per-profile-gateway-supervision) for details.
:::
...
default for **future CLI/gateway runs** (same as `lycus profile use`) —
to edit a profile from the dashboard, use the switcher instead.

## Updating

`lycus update` pulls code once (shared) and syncs new bundled skills to **all** profiles automatically:
...
```bash
lycus profile list           # show all profiles with status
lycus profile show coder     # detailed info for one profile
lycus profile rename coder dev-bot   # rename (updates alias + service)
lycus profile export coder   # export to coder.tar.gz
lycus profile import coder.tar.gz   # import from archive
```

## Deleting a profile

```bash
lycus profile delete coder
```

This stops the gateway, removes the systemd/launchd service, removes the command alias, and deletes all profile data. You'll be asked to type the profile name to confirm.

Use `--yes` to skip confirmation: `lycus profile delete coder --yes`

:::note
You cannot delete the default profile (`~/.autolycus`). To remove everything, use `lycus uninstall`.
:::
...
```bash
# Bash
eval "$(lycus completion bash)"

# Zsh
eval "$(lycus completion zsh)"
```
...
Profiles use the `AUTOLYCUS_HOME` environment variable. When you run `coder chat`, the wrapper script sets `AUTOLYCUS_HOME=~/.autolycus/profiles/coder` before launching lycus. Since 119+ files in the codebase resolve paths via `get_lycus_home()`, Lycus state automatically scopes to the profile's directory — config, sessions, memory, skills, state database, gateway PID, logs, and cron jobs.

This is separate from terminal working directory. Tool execution starts from `terminal.cwd` (or the launch directory when `cwd: "."` on the local backend), not automatically from `AUTOLYCUS_HOME`.
...
isolated by `AUTOLYCUS_HOME`, not by changing `HOME`. Container backends still use
`{AUTOLYCUS_HOME}/home` for persistent tool state, and host users who need strict
per-profile tool config can opt in with `terminal.home_mode: profile`.

This means two things that are easy to mix up:

- `AUTOLYCUS_HOME` is the profile boundary. It controls Lycus config, `.env`,
  memory, sessions, skills, logs, cron jobs, gateway state, and other Lycus
  data.
- `HOME` is the operating-system/user home that external CLIs expect. On host
  installs, Lycus keeps it as the real user home by default so tools like
...
profile` in that profile's `config.yaml`. In that mode Lycus launches tool
subprocesses with `HOME={AUTOLYCUS_HOME}/home`; you then need to initialize or link
...
Lycus also exposes `HERMES_REAL_HOME` to subprocesses so scripts can still find
the actual account home when `home_mode: profile` is active.

The default profile is simply `~/.autolycus` itself. No migration needed — existing installs work identically.
