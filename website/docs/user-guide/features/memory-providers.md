...
```bash
lycus memory setup      # interactive picker + configuration
lycus memory status     # check what's active
lycus memory off        # disable external provider
```

You can also select the active memory provider via `lycus plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.autolycus/config.yaml`:
...
When a memory provider is active, Lycus automatically:
...
**Setup Wizard:**
```bash
lycus memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `lycus honcho setup` command still works (it now redirects to `lycus memory setup`), but is only registered after Honcho is selected as the active memory provider.

**Config:** `$AUTOLYCUS_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$AUTOLYCUS_HOME/honcho.json` > `~/.autolycus/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/NousResearch/lycus-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/lycus).
...
:::tip Migrating from `lycus honcho`
If you previously used `lycus honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Lycus profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Lycus profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Lycus profile. Host key `lycus` → default; `lycus.<profile>` for others. |
...
```bash
lycus profile create coder --clone
```

`--clone` creates a `lycus.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Lycus profile, creates host blocks for any profile without one, inherits settings from the default `lycus` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `lycus memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
**Setup:**
```bash
lycus memory setup    # select "mem0"
# Or manually:
lycus config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.autolycus/.env
```

**Config:** `$AUTOLYCUS_HOME/mem0.json`

| Key | Default | Description |
|-----|---------|-------------|
| `user_id` | `lycus-user` | User identifier |
| `agent_id` | `lycus` | Agent identifier |
...
**Setup:**
```bash
lycus memory setup    # select "hindsight"
# Or manually:
lycus config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.autolycus/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p lycus ui start`

**Config:** `$AUTOLYCUS_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `lycus` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Lycus Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/NousResearch/lycus-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
lycus memory setup    # select "holographic"
# Or manually:
lycus config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.autolycus-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$AUTOLYCUS_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
lycus memory setup    # select "retaindb"
# Or manually:
lycus config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.autolycus/.env
```
...
# Then configure Lycus
lycus memory setup    # select "byterover"
# Or manually:
lycus config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$AUTOLYCUS_HOME/byterover/` (profile-scoped)
...
**Setup:**
```bash
lycus memory setup    # select "supermemory"
# Or manually:
lycus config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.autolycus/.env
```

**Config:** `$AUTOLYCUS_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `lycus` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `lycus-{identity}` → `lycus-coder`) to isolate memories per Lycus profile
...
| **Requires** | `pip install lycus-memori` + `lycus-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install lycus-memori
lycus-memori install
lycus config set memory.provider memori
lycus memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `lycus-memori` | Tool-aware memory + structured recall |
