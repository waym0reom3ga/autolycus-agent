...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Lycus Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Lycus Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Lycus](/guides/use-voice-mode-with-lycus).

## Prerequisites

Before using voice features, make sure you have:

1. **Lycus Agent installed** — `pip install lycus-agent` (see [Installation](/getting-started/installation))
2. **An LLM provider configured** — run `lycus model` or set your preferred provider credentials in `~/.autolycus/.env`
3. **A working base setup** — run `lycus` to verify the agent responds to text before enabling voice

:::tip
The `~/.autolycus/` directory and default `config.yaml` are created automatically the first time you run `lycus`. You only need to create `~/.autolycus/.env` manually for API keys.
:::

:::tip Nous Portal covers both
A paid [Nous Portal](/user-guide/features/tool-gateway) subscription supplies the LLM (step 2) **and** OpenAI TTS via the Tool Gateway — no separate OpenAI key needed. On a fresh install, `lycus setup --portal` wires both up at once.
:::
...
pip install "lycus-agent[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
pip install "lycus-agent[messaging]"

# Premium TTS (ElevenLabs)
pip install "lycus-agent[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "lycus-agent[all]"
```
...
Voice mode is available in both the **classic CLI** (`lycus chat`) and the **TUI** (`lycus --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.autolycus/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
lycus                # Start the interactive CLI
```
...
1. Start the CLI with `lycus` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.autolycus/config.yaml` (default: `ctrl+b`).
:::
...
```bash
lycus gateway        # Start the gateway (connects to configured platforms)
lycus gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@lycusbyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.autolycus/.env`:
```bash
...
```bash
lycus gateway        # Start with existing configuration
```
...
If you are running Lycus inside Docker on a Linux desktop, the container also needs access to your host audio socket. See the [Docker audio bridge](/user-guide/docker#optional-linux-desktop-audio-bridge) notes for a PulseAudio/PipeWire-compatible setup.
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.autolycus/.env`
...
- Review gateway logs: `tail -f ~/.autolycus/logs/gateway.log`
