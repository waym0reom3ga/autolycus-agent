...
**Tool Search** is Lycus' opt-in progressive-disclosure layer for that
...
:::info Built-in Lycus tools never defer
The tools that make up Lycus' core capability set (`terminal`,
...
When the model invokes `tool_call`, Lycus **unwraps the bridge** and
...
- **No JS sandbox.** Lycus uses the simpler "structured tools" mode
