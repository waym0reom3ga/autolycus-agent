...
When using [OpenRouter](https://openrouter.ai) as your LLM provider, Lycus Agent supports **provider routing** — fine-grained control over which underlying AI providers handle your requests and how they're prioritized.
...
Add a `provider_routing` section to your `~/.autolycus/config.yaml`:
...
- **CLI mode** — configured in `~/.autolycus/config.yaml`, loaded at startup
