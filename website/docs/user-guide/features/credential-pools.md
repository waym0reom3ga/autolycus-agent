...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, Lycus automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
If you already have an API key set in `.env`, Lycus auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
lycus auth add openrouter --api-key sk-or-v1-your-second-key

# Add a second Anthropic key
lycus auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (requires Claude Max plan + extra usage credits)
lycus auth add anthropic --type oauth
...
Run `lycus auth` with no subcommand for an interactive wizard:
...
| Command | Description |
|---------|-------------|
| `lycus auth` | Interactive pool management wizard |
| `lycus auth list` | Show all pools and credentials |
| `lycus auth list <provider>` | Show a specific provider's pool |
| `lycus auth add <provider>` | Add a credential (prompts for type and key) |
| `lycus auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `lycus auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `lycus auth remove <provider> <index>` | Remove credential by 1-based index |
| `lycus auth reset <provider>` | Clear all cooldowns/exhaustion status |

## Rotation Strategies

Configure via `lycus auth` → "Set rotation strategy" or in `config.yaml`:
...
When you set up a custom endpoint via `lycus model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via lycus model:
lycus auth list
# Shows:
...
lycus auth add Together.ai --api-key sk-together-second-key
```
...
Lycus automatically discovers credentials from multiple sources and seeds the pool on startup:
...
| Lycus PKCE OAuth | `~/.autolycus/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `lycus auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `lycus auth add`) are never auto-pruned.

Borrowed runtime secrets (for example env vars, Bitwarden/Vault/keyring/systemd references, and custom config values) are reference-only at the `auth.json` boundary. Lycus can use the resolved value in memory for the current run, but it persists only metadata such as the source ref, label, status, request counters, and a non-reversible fingerprint. Manual entries and Lycus-owned OAuth/device-code state keep the durable tokens they need to refresh.
...
2. **`lycus_cli/auth_commands.py`** — CLI commands and interactive wizard
3. **`lycus_cli/runtime_provider.py`** — Pool-aware credential resolution
4. **`run_agent.py`** — Error recovery: 429/402/401 → pool rotation → fallback

## Storage

Pool state is stored in `~/.autolycus/auth.json` under the `credential_pool` key:
...
The OpenRouter entry above was borrowed from an external source, so the raw key is not stored in `auth.json`. The manual Anthropic entry was intentionally added to Lycus' credential store, so its token remains persistable.
