...
If the file is empty, whitespace-only, or cannot be read, Lycus falls back to a built-in default identity ("You are Lycus Agent, an intelligent AI assistant created by Technetia Inc..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
