...
Lycus runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Lycus
...
1. Lycus captures a baseline of current diagnostics for the file.
...
…). Lycus auto-detects the binary on PATH or in
`<AUTOLYCUS_HOME>/lsp/bin/`.
...
`node_modules` tree — Lycus installs both packages together when you
run `lycus lsp install typescript` or auto-install fires on first
use.

## CLI

```
lycus lsp status          # service state + per-server install status
lycus lsp list            # registry, optionally --installed-only
lycus lsp install <id>    # eagerly install one server
lycus lsp install-all     # try every server with a known recipe
lycus lsp restart         # tear down running clients
lycus lsp which <id>      # print resolved binary path
```

`lycus lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <AUTOLYCUS_HOME>/lsp/bin
...
When `install_strategy: auto`, Lycus installs binaries into
`<AUTOLYCUS_HOME>/lsp/bin/`. NPM packages land in
`<AUTOLYCUS_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Lycus-owned and is
...
Servers are kept alive for the life of the Lycus process. There's
...
**`lycus lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<AUTOLYCUS_HOME>/lsp/bin/`. Run
`lycus lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `lycus lsp status`**
...
When `lycus lsp status` shows a `Backend warnings` section, install
...
`~/.autolycus/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.autolycus/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `lycus lsp restart` to clear the set;
