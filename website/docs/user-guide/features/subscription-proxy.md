...
| Use case | "Use Lycus as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Lycus stores
the refresh token in `~/.autolycus/auth.json` — the same place all Lycus
...
Model:      Lycus-4-70B    # or Lycus-4.3-36B, Lycus-4-405B
```
...
interface in `lycus_cli/proxy/adapters/`.
...
If you see `not logged in`, run `lycus portal`. If you see
...
`lycus_cli/proxy/adapters/<provider>.py` and registering it in
