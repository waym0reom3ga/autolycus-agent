...
title: "Git Worktrees"
description: "Run multiple Lycus agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Lycus Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Lycus so each session has a clean, isolated working directory.

## Why Use Worktrees with Lycus?

Lycus treats the **current working directory** as the project root:

- CLI: the directory where you run `lycus` or `lycus chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.autolycus/config.yaml`
...
git worktree add ../repo-feature feature/lycus-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/lycus-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Lycus there:

```bash
cd ../repo-feature

# Start Lycus in the worktree
lycus
```
...
git worktree add ../repo-experiment-a feature/lycus-a
git worktree add ../repo-experiment-b feature/lycus-b
```
...
- Works on its own branch (`feature/lycus-a` vs `feature/lycus-b`).
...
- Lycus checkpoint data under `~/.autolycus/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Lycus experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/lycus-checkpoints-docs`, `feature/lycus-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Lycus from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `lycus -w` (Automatic Worktree Mode)

Lycus has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `lycus/lycus-<hash>`).
...
```bash
lycus -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `lycus -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Lycus session its own clean checkout.
