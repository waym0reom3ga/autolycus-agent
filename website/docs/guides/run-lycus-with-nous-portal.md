...
3. Sets `model.provider: nous` in `~/.autolycus/config.yaml`
...
OAuth needs a browser, but the loopback callback runs on the machine where Lycus is running. Two options:

```bash
# Option A: SSH port forwarding (preferred)
ssh -N -L 8642:127.0.0.1:8642 user@remote-host    # in a local terminal
lycus setup --portal                              # on the remote, open the printed URL in your local browser

# Option B: manual paste (for Cloud Shell, Codespaces, EC2 Instance Connect)
lycus auth add nous --type oauth --manual-paste
# Then re-run `lycus setup --portal` to wire the provider + gateway
```
...
```
Hey, search the web for "Lycus Agent release notes" and summarize the top 3 hits.
```

You should see Lycus call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.

## 5. Pick the model you actually want

`lycus setup --portal` lets you pick a model during setup, but the whole point of the subscription is access to the full catalog — switch any time with `/model` mid-session:
...
lycus config set model.default anthropic/claude-sonnet-4.6
```

### Don't pick Lycus-4 for agent work

Lycus-4-70B and Lycus-4-405B are available on the Portal at deep discounts, but they're **chat/reasoning models**, not tool-call-tuned. They will struggle with multi-step agent loops. Use them via [Nous Chat](https://chat.nousresearch.com) for conversation/research work, or through the [subscription proxy](/user-guide/features/subscription-proxy) from non-agent tools. For Lycus Agent itself, stick to the frontier agentic models above.

The Portal's own [info page](https://portal.nousresearch.com/info) carries this warning too — it's the official Nous guidance, not just a Lycus-side opinion.
...
These rows appear in `lycus tools` even before you've logged into Nous Portal — if you pick "Nous Subscription" without an active session, Lycus runs the Portal login inline (without changing your inference provider or your other tools).
...
Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and Lycus will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
```bash
lycus cron create "every day at 9am" \
...
If you use [Lycus profiles](/user-guide/profiles) (e.g. a separate config per project), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically.

For team setups where multiple humans share a machine, each human has their own Portal account → each home directory holds its own `~/.autolycus/auth.json` → no token sharing across users. This is the right boundary.

## Troubleshooting

### `lycus portal info` shows "not logged in" after `lycus setup --portal`
...
```bash
lycus config set model.provider nous
```
...
Re-verify with `lycus portal info`.
...
Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so Lycus doesn't replay it endlessly. Just log in again:
...
If a model is genuinely unavailable, [open an issue](https://github.com/NousResearch/lycus-agent/issues) — most gaps are routing config we can update.

### Billing not appearing on my Portal account

`lycus portal info` will tell you whether you're actually routing through the Portal or some other provider. Common causes:
...
- Multiple Lycus profiles where you're using the wrong one (check `lycus profile current`)

### Want to revoke and start clean

```bash
lycus auth remove nous       # wipes the local refresh token
...
- **[Subscription proxy](/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-Lycus tools
...
- **[Profiles](/user-guide/profiles)** — Share one Portal login across multiple Lycus configurations
