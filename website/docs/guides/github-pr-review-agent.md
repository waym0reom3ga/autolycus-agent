...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Lycus Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Lycus instantly when PRs are opened or updated.
:::
...
- **Lycus Agent installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  lycus gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.autolycus/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Lycus can access GitHub. Start a chat:
...
```
Run: gh pr list --repo NousResearch/lycus-agent --state open --limit 3
```
...
Still in the chat, ask Lycus to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo NousResearch/lycus-agent
```
...
A skill gives Lycus consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.autolycus/skills/code-review
```

Create `~/.autolycus/skills/code-review/SKILL.md`:
...
Verify it loaded — start `lycus` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Lycus your team's standards:
...
2. Teach Lycus your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
lycus gateway status    # Is the gateway running?
lycus cron list         # Is the job enabled?
```
...
- **[Build a Plugin](/guides/build-a-lycus-plugin)** — wrap the review logic into a shareable plugin
