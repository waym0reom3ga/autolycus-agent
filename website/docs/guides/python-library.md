---
sidebar_position: 5
title: "Using Lycus as a Python Library"
description: "Embed AIAgent in your own Python scripts, web apps, or automation pipelines — no CLI required"
---

# Using Lycus as a Python Library

Lycus isn't just a CLI tool. You can import `AIAgent` directly and use it programmatically in your own Python scripts, web applications, or automation pipelines. This guide shows you how.

---

## Installation

Install Lycus directly from the repository:

```bash
pip install git+https://github.com/NousResearch/lycus-agent.git
```

Or with [uv](https://docs.astral.sh/uv/):

```bash
uv pip install git+https://github.com/NousResearch/lycus-agent.git
```

You can also pin it in your `requirements.txt`:

```text
lycus-agent @ git+https://github.com/NousResearch/lycus-agent.git
```

:::tip
The same environment variables used by the CLI are required when using Lycus as a library. At minimum, set `OPENROUTER_API_KEY` (or `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` if using direct provider access).
:::
...
The simplest way to use Lycus is the `chat()` method — pass a message, get a string back:
...
:::warning
Always set `quiet_mode=True` when embedding Lycus in your own code. Without it, the agent prints CLI spinners, progress indicators, and other terminal output that will clutter your application's output.
:::
...
For running many prompts in parallel, Lycus includes `batch_runner.py`. It manages concurrent `AIAgent` instances with proper resource isolation:
...
async def on_message(message):
    if message.author == client.user:
        return
    if message.content.startswith("!lycus "):
...
|-----------|------|---------|-------------|
| `model` | `str` | `""` | Model in OpenRouter format (defaults to empty; resolved from your lycus config at runtime) |
