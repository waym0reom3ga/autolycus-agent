---
sidebar_position: 8
title: "Use Voice Mode with Lycus"
description: "A practical guide to setting up and using Lycus voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Lycus
...
- you want Lycus sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Lycus.
...
## Step 1: make sure normal Lycus works first
...
Lycus supports both local and cloud speech stacks.
...
### If you use `lycus setup`

If you choose NeuTTS in the setup wizard, Lycus checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Lycus transcribes and responds
...
- asking Lycus to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Lycus loop.

## Tuning CLI behavior

### Silence threshold

If Lycus starts/stops too aggressively, tune:
...
Lycus stays a normal chat bot, but can speak replies.
...
- you want Lycus to function like a portable research or ops assistant
...
Lycus joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Lycus detects speech boundaries
- transcripts are posted in the associated text channel
- Lycus responds in text and audio
...
1. get text Lycus working
2. install `lycus-agent[voice]`
