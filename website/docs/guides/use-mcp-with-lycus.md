---
sidebar_position: 6
title: "Use MCP with Lycus"
description: "A practical guide to connecting MCP servers to Lycus Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Lycus

This guide shows how to actually use MCP with Lycus Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Lycus tool
- you want Lycus to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Lycus to internal APIs, databases, or company systems without modifying Lycus core

Do not use MCP when:
- a built-in Lycus tool already solves the job well
...
- Lycus remains the agent
- MCP servers contribute tools
- Lycus discovers those tools at startup or reload time
...
If you installed Lycus with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Lycus banner/status should show MCP integration when configured
- ask Lycus what tools it has available
...
## WSL2: bridge Lycus in WSL to Windows Chrome

This is the practical setup when:

- Lycus runs inside WSL2
...
In this setup, Lycus does **not** connect to Chrome directly. Instead:

- Lycus runs in WSL
- Lycus starts a local stdio MCP server
...
```text
Lycus (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Lycus stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Lycus core browser transport
...
```bash
lycus mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
lycus mcp test chrome-devtools-win
```

Then start a fresh Lycus session or run:
...
Once loaded, Lycus can use the MCP-prefixed browser tools directly. For example:
...
If Lycus runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Lycus from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Lycus from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Lycus:
...
2. Lycus-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Lycus will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Lycus to reason over a bounded workspace.
...
Now Lycus can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Lycus core.
...
Because Lycus now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
