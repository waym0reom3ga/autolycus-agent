...
This tutorial walks you through setting up a Telegram bot powered by Lycus Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Lycus Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.autolycus/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Lycus itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Lycus Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_lycus_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Lycus Agent. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.autolycus/.env`:
...
```
[Gateway] Starting Lycus Gateway...
...
# View live logs
journalctl --user -u lycus-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `lycus gateway install` to update the plist.
:::
...
```bash
lycus gateway stop && lycus gateway start
```
...
lycus pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.autolycus/.env`:
...
Control how much detail the bot shows when using tools. In `~/.autolycus/config.yaml`:
...
```bash
# From the CLI
lycus cron list          # View all scheduled jobs
lycus cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u lycus-gateway` (Linux) or `~/.autolycus/logs/gateway.log` (macOS) |
| Cron job output | `~/.autolycus/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.autolycus/cron/jobs.json` |
| Pairing data | `~/.autolycus/pairing/` |
| Session history | `~/.autolycus/sessions/` |
