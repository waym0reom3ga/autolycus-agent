...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `lycus send`."
---

# Pipe Script Output to Messaging Platforms

`lycus send` is a small, scriptable CLI that pushes a message to any
messaging platform Lycus is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | lycus send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `lycus
...
lycus send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
lycus send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
lycus send --list

# Filter by platform
lycus send --list telegram
```
...
Any platform Lycus ships adapters for works as a target:
...
`lycus send` resolves the message body in this order:

1. **Positional argument** — `lycus send --to telegram "hi"`
2. **`--file PATH`** — `lycus send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | lycus send --to telegram`

When stdin is a TTY (no pipe), Lycus does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  lycus send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `lycus send` reuses your Lycus config, the same script works on
any host where Lycus is installed — no need to export bot tokens into
...
`lycus send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  lycus send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | lycus send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.autolycus/bin/lycus send \
...
```bash
./train.py --epochs 200 && \
  lycus send --to telegram "training done" || \
  lycus send --to telegram "training failed (exit $?)"
```
...
lycus send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(lycus send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `lycus send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `lycus send` calls
the platform's REST endpoint directly using credentials from
`~/.autolycus/.env` and `~/.autolycus/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `lycus gateway start` and retry.
...
# Machine-readable
lycus send --list --json
```

The listing is built from `~/.autolycus/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`lycus gateway
...
| Approach | Multi-platform | Reuses Lycus creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `lycus send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`lycus send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `lycus send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `lycus send` shares with cron delivery.
