...
of fields, and Lycus schedules it as a cron job — no cron syntax required.
...
- **CLI, TUI, and messengers** — type `/blueprint <name>` (e.g.
  `/blueprint morning-brief`) and Lycus asks you for what it needs, one
...
A blueprint is just a skill with a `metadata.lycus.blueprint` block in its
