...
description: "Reference for Lycus core, composite, platform, and dynamic toolsets"
---
...
- **Platform** — A complete tool configuration for a specific deployment context (e.g., `lycus-cli` is the default for interactive CLI sessions)
...
```bash
lycus chat --toolsets web,file,terminal
lycus chat --toolsets debugging        # composite — expands to file + terminal + web
lycus chat --toolsets all              # everything
```
...
```yaml
toolsets:
  - lycus-cli          # default for CLI
  # - lycus-telegram   # override for Telegram gateway
```

### Interactive management

```bash
lycus tools                            # curses UI to enable/disable per platform
```
...
| `code_execution` | `execute_code` | Run Python scripts that call Lycus tools programmatically. |
| `cronjob` | `cronjob` | Schedule and manage recurring tasks. |
| `debugging` | composite (`file` + `terminal` + `web`) | Debug bundle — file, process/terminal, web extract/search. |
| `delegation` | `delegate_task` | Spawn isolated subagent instances for parallel work. |
| `discord` | `discord` | Core Discord text/embed/DM actions (gateway-only). Active on the `lycus-discord` toolset. |
| `discord_admin` | `discord_admin` | Discord moderation (bans, role changes, channel management). Active on the `lycus-discord` toolset; requires the bot to hold the relevant Discord permissions. |
...
| `feishu_drive` | `feishu_drive_add_comment`, `feishu_drive_list_comments`, `feishu_drive_list_comment_replies`, `feishu_drive_reply_comment` | Feishu/Lark drive comment operations. Scoped to the comment agent; not exposed on `lycus-cli` or other messaging toolsets. |
...
| `x_search` | `x_search` | Search X (Twitter) posts and threads via xAI's built-in `x_search` Responses tool. Off by default; opt in via `lycus tools`. Schema only registered when xAI credentials (SuperGrok OAuth or `XAI_API_KEY`) are configured. |
| `yuanbao` | `yb_query_group_info`, `yb_query_group_members`, `yb_search_sticker`, `yb_send_dm`, `yb_send_sticker` | Yuanbao DM/group actions and sticker search. Registered only on `lycus-yuanbao`. |

## Platform Toolsets

Platform toolsets define the complete tool configuration for a deployment target. Most messaging platforms use the same set as `lycus-cli`:

| Toolset | Differences from `lycus-cli` |
|---------|-------------------------------|
| `lycus-cli` | Full toolset — the default for interactive CLI sessions. Includes file, terminal, web, browser, memory, skills, vision, image_gen, todo, tts, delegation, code_execution, cronjob, session_search, clarify, and `safe` (read-only) bundles plus the standard messaging tools. |
| `lycus-acp` | Drops `clarify`, `cronjob`, `image_generate`, `send_message`, `text_to_speech`, and all four Home Assistant tools. Focused on coding tasks in IDE context. |
| `lycus-api-server` | Drops `clarify`, `send_message`, and `text_to_speech`. Keeps everything else — suitable for programmatic access where user interaction isn't possible. |
| `lycus-cron` | Same as `lycus-cli`. |
| `lycus-telegram` | Same as `lycus-cli`. |
| `lycus-discord` | Adds `discord` and `discord_admin` on top of `lycus-cli`. |
| `lycus-slack` | Same as `lycus-cli`. |
| `lycus-whatsapp` | Same as `lycus-cli`. |
| `lycus-signal` | Same as `lycus-cli`. |
| `lycus-matrix` | Same as `lycus-cli`. |
| `lycus-mattermost` | Same as `lycus-cli`. |
| `lycus-email` | Same as `lycus-cli`. |
| `lycus-sms` | Same as `lycus-cli`. |
| `lycus-bluebubbles` | Same as `lycus-cli`. |
| `lycus-dingtalk` | Same as `lycus-cli`. |
| `lycus-feishu` | Adds the five `feishu_doc_*` / `feishu_drive_*` tools (only used by the document-comment handler, not the regular chat adapter). |
| `lycus-qqbot` | Same as `lycus-cli`. |
| `lycus-wecom` | Same as `lycus-cli`. |
| `lycus-wecom-callback` | Same as `lycus-cli`. |
| `lycus-weixin` | Same as `lycus-cli`. |
| `lycus-yuanbao` | Adds the five `yb_*` tools (DM/group/sticker) on top of `lycus-cli`. |
| `lycus-homeassistant` | Same as `lycus-cli` (the Home Assistant tools are already present by default and activate when `HASS_TOKEN` is set). |
| `lycus-webhook` | Same as `lycus-cli`. |
| `lycus-gateway` | Internal gateway orchestrator toolset — union of every `lycus-<platform>` toolset; used when the gateway needs to accept any message source. |
...
## Relationship to `lycus tools`

The `lycus tools` command provides a curses-based UI for toggling individual tools on or off per platform. This operates at the tool level (finer than toolsets) and persists to `config.yaml`. Disabled tools are filtered out even if their toolset is enabled.
