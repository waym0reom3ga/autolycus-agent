...
Lycus Agent uses a SQLite database (`~/.autolycus/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `lycus_state.py`


## Architecture Overview

```
~/.autolycus/state.db (SQLite, WAL mode)
...
Multiple lycus processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.autolycus/state.db
...
Default path: `~/.autolycus/state.db`

This is derived from `lycus_constants.get_lycus_home()` which resolves to
`~/.autolycus/` by default, or the value of `AUTOLYCUS_HOME` environment variable.
