...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to Lycus Agent — schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in Lycus tool** to the repository itself.
If you want a personal, project-local, or otherwise custom tool without
modifying Lycus core, use the plugin route instead:

- [Plugins](/user-guide/features/plugins)
- [Build a Lycus Plugin](/guides/build-a-lycus-plugin)
...
If your tool requires an API key, add it to `lycus_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `lycus_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `lycus chat -q "Use the weather tool for London"`
