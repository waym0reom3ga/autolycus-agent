...
description: "How Lycus stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `lycus_cli/cron.py` | CLI `lycus cron` subcommands |
...
Jobs are stored in `~/.autolycus/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `lycus cron` commands are run or during active CLI sessions.
...
```python
# ~/.autolycus/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.autolycus/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `lycus cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `lycus cron` CLI provides direct job management:

```bash
lycus cron list                    # Show all jobs
lycus cron create                  # Interactive job creation (alias: add)
lycus cron edit <job_id>           # Edit job configuration
lycus cron pause <job_id>          # Pause a running job
lycus cron resume <job_id>         # Resume a paused job
lycus cron run <job_id>            # Trigger immediate execution
lycus cron remove <job_id>         # Delete a job
```
