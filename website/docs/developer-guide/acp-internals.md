...
The ACP adapter wraps Lycus' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
lycus acp / lycus-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.autolycus/.env
  -> configure stderr logging
  -> construct LycusACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'lycus-agent[acp]==<version>' lycus-acp`, pointed at the `lycus-agent` PyPI release.
...
- `allow_once` -> Lycus `once`
- `allow_always` -> Lycus `always`
- reject options -> Lycus `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Lycus tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["lycus-acp"])
...
Instead it reuses Lycus' runtime resolver:

- `acp_adapter/auth.py`
- `lycus_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Lycus provider/credentials. It also always advertises a terminal setup auth method (`lycus-setup`, args `--setup`) so first-run registry clients can open Lycus' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.autolycus/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `lycus-acp` toolset definition
- `lycus_cli/main.py` — `lycus acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `lycus-acp` script
