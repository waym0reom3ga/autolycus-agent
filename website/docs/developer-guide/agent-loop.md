...
Lycus supports three API execution modes, resolved from provider selection, explicit args, and base URL heuristics:
...
After each turn:
- Messages are saved to the session store (SQLite via `lycus_state.py`)
- Memory changes are flushed to `MEMORY.md` / `USER.md`
- The session can be resumed later via `/resume` or `lycus chat --resume`
