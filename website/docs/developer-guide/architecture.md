...
sidebar_position: 1
title: "Architecture"
description: "Lycus Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Lycus Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ lycus_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # LycusCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── lycus_state.py           # SQLite session/state database with FTS5
├── lycus_constants.py       # AUTOLYCUS_HOME, profile-aware paths
...
│
├── lycus_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `lycus` subcommands (large file)
...
│   ├── skills_config.py      # lycus skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # lycus tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # lycus gateway start/stop
│
...
```text
User input → LycusCLI.process_input()
...
Three discovery sources: `~/.autolycus/plugins/` (user), `.autolycus/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `lycus plugins` or `config.yaml`.

→ [Plugin Guide](/guides/build-a-lycus-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Lycus as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`lycus -p <name>`) gets its own AUTOLYCUS_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
