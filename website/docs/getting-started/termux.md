...
description: "Run Lycus Agent directly on an Android phone with Termux"
---

# Lycus on Android with Termux

This is the tested path for running Lycus Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Lycus from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Lycus now ships a Termux-aware installer path:
...
- links `lycus` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `lycus` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/lycus" "$PREFIX/bin/lycus"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `lycus` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.autolycus/.env`.
...
### `lycus doctor` says ripgrep or Node is missing
