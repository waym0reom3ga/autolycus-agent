...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Lycus Agent — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Lycus setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Lycus Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Lycus Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Lycus for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Lycus working on my machine | `lycus setup` | Run a real chat and verify it responds |
| I already know my provider | `lycus model` | Save the config, then start chatting |
| I want a bot or always-on setup | `lycus gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `lycus model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `lycus model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Lycus cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Lycus Agent
...
The single most important setup step. Use `lycus model` to walk through the choice interactively:
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `lycus model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `lycus model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `lycus model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `lycus_cli/models.py` may change between releases) | `lycus model` → MiniMax (OAuth) |
...
| **Google Gemini (OAuth)** | Gemini via the `google-gemini-cli` OAuth flow — no key needed | `lycus model` → Google Gemini (OAuth) |
| **xAI** | Grok models via direct API | Set `XAI_API_KEY` |
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `lycus model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `lycus model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `lycus model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `lycus model` (requires `copilot` CLI + `copilot login`) |
...
Lycus Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `lycus model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Lycus separates secrets from normal config:

- **Secrets and tokens** → `~/.autolycus/.env`
- **Non-secret settings** → `~/.autolycus/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
lycus config set model anthropic/claude-opus-4.6
lycus config set terminal.backend docker
lycus config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
lycus            # classic CLI
lycus --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Lycus ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `lycus` vs `lycus --tui`.
:::
...
```bash
lycus --continue    # Resume the most recent session
lycus -c            # Short form
```
...
```bash
lycus gateway setup    # Interactive platform configuration
```
...
- `lycus tools` — tune tool access per platform
- `lycus skills` — browse and install reusable workflows
...
```bash
lycus config set terminal.backend docker    # Docker isolation
lycus config set terminal.backend ssh       # Remote server
```

### Voice mode

```bash
# From the Lycus install directory (the curl installer placed it at
# ~/.autolycus/lycus-agent on Linux/macOS or %LOCALAPPDATA%\lycus\lycus-agent on Windows):
...
Skills are on-demand instruction documents that teach Lycus how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Lycus ships with a catalog of bundled skills already installed in `~/.autolycus/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
lycus skills browse                      # list everything available
lycus skills search kubernetes           # find skills by keyword
lycus skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `lycus skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Lycus ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.autolycus/lycus-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Lycus opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `lycus model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `lycus gateway setup` and check `lycus gateway status` |
| `lycus --continue` can't find old session | Switched profiles or session never saved | Check `lycus sessions list` and confirm you're in the right profile |
...
| `lycus doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `lycus model`
3. `lycus setup`
4. `lycus sessions list`
5. `lycus --continue`
6. `lycus gateway status`
...
| `lycus tools` | Configure which tools are enabled per platform |
| `lycus setup` | Full setup wizard (configures everything at once) |
| `lycus doctor` | Diagnose issues |
| `lycus update` | Update to latest version |
| `lycus gateway` | Start the messaging gateway |
| `lycus --continue` | Resume last session |
