...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/lycus-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    lycus_meta = (fm.get("metadata") or {}).get("lycus") or {}
    tags = lycus_meta.get("tags") or []
    related = lycus_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `lycus skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Lycus Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Lycus ships with a large built-in skill library copied into `~/.autolycus/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Lycus also syncs bundled skills on `lycus update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.autolycus/skills/` tree, it is still shipped with Lycus; restore it with `lycus skills reset <name> --restore`.",
        "",
