...
sidebar_position: 10
title: "从 OpenClaw 迁移"
description: "将 OpenClaw / Clawdbot 配置迁移到 Lycus Agent 的完整指南——包括迁移内容、配置键映射及迁移后的检查事项。"
---

# 从 OpenClaw 迁移

`lycus claw migrate` 将你的 OpenClaw（或旧版 Clawdbot/Moldbot）配置导入 Lycus。本指南详细说明迁移内容、配置键映射以及迁移后的验证步骤。
...
| `--overwrite` | 冲突时覆盖已有 Lycus 文件（默认：计划存在冲突时拒绝执行）。 |
| `--migrate-secrets` | 包含 API 密钥。即使使用 `--preset full` 也需要显式指定——没有任何预设会静默导入密钥。 |
| `--no-backup` | 跳过迁移前对 `~/.autolycus/` 的 zip 快照备份（默认在执行前写入单个还原点归档，位于 `~/.autolycus/backups/pre-migration-*.zip`；可通过 `lycus import` 还原）。 |
...
| 每日记忆文件 | `workspace/memory/*.md` | `~/.autolycus/memories/MEMORY.md` | 所有每日文件合并至主记忆 |
...
| 来源 | OpenClaw 位置 | Lycus 目标 |
|--------|------------------|-------------------|
| 工作区 skills | `workspace/skills/` | `~/.autolycus/skills/openclaw-imports/` |
| 托管/共享 skills | `~/.openclaw/skills/` | `~/.autolycus/skills/openclaw-imports/` |
| 个人跨项目 skills | `~/.agents/skills/` | `~/.autolycus/skills/openclaw-imports/` |
| 项目级共享 skills | `workspace/.agents/skills/` | `~/.autolycus/skills/openclaw-imports/` |

Skill 冲突由 `--skill-conflict` 处理：`skip` 保留现有 Lycus skill，`overwrite` 替换，`rename` 创建带 `-imported` 后缀的副本。

### 模型与 Provider 配置

| 内容 | OpenClaw 配置路径 | Lycus 目标 | 备注 |
|------|---------------------|-------------------|-------|
...
| Provider API 密钥 | `models.providers.*.apiKey` | `~/.autolycus/.env` | 需要 `--migrate-secrets`。参见下方 [API 密钥解析](#api-key-resolution) |

### Agent 行为

| 内容 | OpenClaw 配置路径 | Lycus 配置路径 | 映射规则 |
|------|---------------------|-------------------|---------|
...
| TTS 资源文件 | `~/.autolycus/tts/`（文件复制） |

### 消息平台

| 平台 | OpenClaw 配置路径 | Lycus `.env` 变量 | 备注 |
|----------|---------------------|----------------------|-------|
...
以下内容保存至 `~/.autolycus/migration/openclaw/<timestamp>/archive/` 供人工审查：
...
| Cron 作业 | `archive/cron-config.json` | 通过 `lycus cron create` 重建 |
| 插件 | `archive/plugins-config.json` | 参见 [插件指南](/user-guide/features/hooks) |
| Hooks/webhooks | `archive/hooks-config.json` | 使用 `lycus webhook` 或 gateway hooks |
| 记忆后端 | `archive/memory-backend-config.json` | 通过 `lycus honcho` 配置 |
| Skills 注册表 | `archive/skills-registry-config.json` | 使用 `lycus skills config` |
| UI/身份 | `archive/ui-identity-config.json` | 使用 `/skin` 命令 |
| 日志 | `archive/logging-diagnostics-config.json` | 在 `config.yaml` 日志部分设置 |
| 多 Agent 列表 | `archive/agents-list.json` | 使用 Lycus profiles |
...
迁移会解析所有三种格式。对于环境变量模板和 `source: "env"` 的 SecretRef 对象，会从 `~/.openclaw/.env` 和 `openclaw.json` 的 env 子对象中查找值。`source: "file"` 或 `source: "exec"` 的 SecretRef 对象无法自动解析——迁移会对此发出警告，相关值需通过 `lycus config set` 手动添加至 Lycus。

## 迁移后

1. **检查迁移报告** — 完成后打印，包含已迁移、已跳过和冲突项的计数。

2. **审查归档文件** — `~/.autolycus/migration/openclaw/<timestamp>/archive/` 中的所有内容需要人工处理。

3. **开启新会话** — 导入的 skills 和记忆条目在新会话中生效，当前会话不受影响。

4. **验证 API 密钥** — 运行 `lycus status` 检查 provider 认证状态。

5. **测试消息平台** — 若迁移了平台 token，重启 gateway：`systemctl --user restart lycus-gateway`

6. **检查会话策略** — 验证 `lycus config get session_reset` 是否符合预期。

7. **重新配对 WhatsApp** — WhatsApp 使用二维码配对（Baileys），不支持 token 迁移。运行 `lycus whatsapp` 进行配对。

8. **清理归档** — 确认一切正常后，运行 `lycus claw cleanup` 将残留的 OpenClaw 目录重命名为 `.pre-migration/`（防止状态混淆）。
...
根据 OpenClaw 版本不同，密钥可能存储在多个位置：`openclaw.json` 中 `models.providers.*.apiKey` 内联、`~/.openclaw/.env`、`openclaw.json` 的 `"env"` 子对象，或 `agents/main/agent/auth-profiles.json`。迁移会检查所有四个位置。若密钥使用 `source: "file"` 或 `source: "exec"` 的 SecretRef，则无法自动解析——请通过 `lycus config set` 手动添加。

### 迁移后 skills 未出现

导入的 skills 位于 `~/.autolycus/skills/openclaw-imports/`。开启新会话后生效，或运行 `/skills` 验证是否已加载。

### TTS 语音未迁移

OpenClaw 在两处存储 TTS 设置：`messages.tts.providers.*` 和顶层 `talk` 配置。迁移会检查两处。若你的 voice ID 是通过 OpenClaw UI 设置的（存储路径不同），可能需要手动设置：`lycus config set tts.elevenlabs.voice_id YOUR_VOICE_ID`。