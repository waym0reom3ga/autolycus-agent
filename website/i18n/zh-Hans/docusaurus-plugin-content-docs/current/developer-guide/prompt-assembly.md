...
sidebar_position: 5
title: "Prompt 组装"
description: "Lycus 如何构建系统 prompt、保持缓存稳定性并注入临时层"
---
...
| 优先级 | 文件 | 搜索范围 | 说明 |
|--------|------|----------|------|
| 1 | `.autolycus.md`、`HERMES.md` | 从 CWD 向上至 git 根目录 | Lycus 原生项目配置 |
...
- **剥离 YAML frontmatter** — `.autolycus.md` 的 frontmatter 会被移除（保留供未来配置覆盖使用）
...
1. `.autolycus.md` / `HERMES.md`（向上遍历至 git 根目录）
...
大多数用户应将 `agent/prompt_builder.py` 视为实现代码，而非配置入口。推荐的自定义路径是修改 Lycus 已加载的 prompt 输入，而非直接编辑 Python 模板。
...
- 可选系统 prompt 配置 / API 覆盖 — 添加部署特定的指令文本，无需 fork Lycus。
