...
| | |
|---|---|
| 来源 | 可选 — 通过 `lycus skills install official/blockchain/hyperliquid` 安装 |
| 路径 | `optional-skills/blockchain/hyperliquid` |
| 版本 | `0.1.0` |
| 作者 | Hugo Sequier (Hugo-SEQUIER), Lycus Agent |
| 许可证 | MIT |
...
辅助脚本：`~/.autolycus/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
```bash
python3 ~/.autolycus/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py <command> [args]
```
...
对于 `state`、`spot-balances`、`fills`、`orders` 和 `review`，当 `~/.autolycus/.env` 中设置了 `HYPERLIQUID_USER_ADDRESS` 时，地址参数为可选。
...
```bash
python3 ~/.autolycus/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py dexs

python3 ~/.autolycus/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py \
