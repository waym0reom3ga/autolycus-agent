...
在同一台机器上运行多个独立的 Lycus agent——每个 agent 拥有各自的配置、API 密钥、记忆、会话、技能和 gateway 状态。
...
```bash
lycus profile create coder       # 创建 profile + "coder" 命令别名
coder setup                       # 配置 API 密钥和模型
coder chat                        # 开始对话
```

就这些。`coder` 现在是拥有独立配置、记忆和状态的 Lycus profile。
...
你也可以稍后通过 `lycus profile describe` 设置或自动生成描述——完整路由模型请参阅 [Kanban 指南](./features/kanban#auto-vs-manual-orchestration)。
...
复制**所有内容**——配置、API 密钥、人格、记忆、技能、cron 任务、插件。会排除每个 profile 自己的历史数据（会话历史、`state.db`、`backups/`、`state-snapshots/`、`checkpoints/`），这些数据属于源 profile 且可能达到数十 GB。若要包含历史的完整备份，请使用 `lycus profile export` 或 `lycus backup`。
...
别名支持所有 lycus 子命令——底层实际上是 `lycus -p <name>`。
...
lycus chat -p coder -q "hello"    # 可在任意位置使用
```

### 粘性默认值（`lycus profile use`）

```bash
lycus profile use coder
lycus chat                   # 现在指向 coder
lycus tools                  # 配置 coder 的工具
lycus profile use default    # 切换回默认
```

设置默认值后，普通 `lycus` 命令将指向该 profile。类似于 `kubectl config use-context`。
...
- **`lycus profile`**：显示当前 profile 名称、路径、模型、gateway 状态
...
在 local 后端使用 `cwd: "."` 表示"Lycus 启动时所在的目录"，而非"profile 目录"。
...
```bash
coder gateway install         # 创建 lycus-gateway-coder systemd/launchd 服务
assistant gateway install     # 创建 lycus-gateway-assistant 服务
```

每个 profile 拥有独立的服务名称，各自独立运行。

:::note 在官方 Docker 镜像中
各 profile 的 gateway 由 [s6-overlay](https://github.com/just-containers/s6-overlay)（容器中的 PID 1）监管，因此 `lycus profile create <name>` 会自动在 `/run/service/gateway-<name>/` 注册 s6 服务槽。`lycus -p <name> gateway start/stop/restart` 会调度到 `s6-svc` 而非直接启动裸进程——崩溃后自动重启，`docker restart` 会保留之前运行的 gateway 集合。详见 [各 profile gateway 监管](/user-guide/docker#per-profile-gateway-supervision)。
:::
...
`lycus update` 拉取一次代码（共享），并自动将新的内置技能同步到**所有** profile：
...
```bash
lycus profile list           # 显示所有 profile 及其状态
lycus profile show coder     # 显示某个 profile 的详细信息
lycus profile rename coder dev-bot   # 重命名（同步更新别名和服务）
lycus profile export coder   # 导出为 coder.tar.gz
lycus profile import coder.tar.gz   # 从归档文件导入
```
...
使用 `--yes` 跳过确认：`lycus profile delete coder --yes`

:::note
你无法删除默认 profile（`~/.autolycus`）。如需删除所有内容，请使用 `lycus uninstall`。
:::
...
profile 使用 `AUTOLYCUS_HOME` 环境变量。运行 `coder chat` 时，包装脚本在启动 lycus 前将 `AUTOLYCUS_HOME` 设置为 `~/.autolycus/profiles/coder`。由于代码库中 119+ 个文件通过 `get_lycus_home()` 解析路径，Lycus 状态会自动限定在 profile 目录范围内——包括配置、会话、记忆、技能、状态数据库、gateway PID、日志和 cron 任务。

这与终端工作目录是分开的。工具执行从 `terminal.cwd` 开始（或在 local 后端使用 `cwd: "."` 时从启动目录开始），而非自动从 `AUTOLYCUS_HOME` 开始。

默认 profile 就是 `~/.autolycus` 本身。无需迁移——现有安装的工作方式完全不变。
