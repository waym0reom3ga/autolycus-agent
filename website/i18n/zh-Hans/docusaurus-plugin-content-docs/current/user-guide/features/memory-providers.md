...
```bash
lycus memory setup      # 交互式选择器 + 配置
lycus memory status     # 查看当前激活状态
lycus memory off        # 禁用外部提供者
```

也可以通过 `lycus plugins` → Provider Plugins → Memory Provider 选择激活的记忆提供者。

或在 `~/.autolycus/config.yaml` 中手动设置：
...
**安装向导：**
```bash
lycus memory setup        # 选择 "honcho" — 运行 Honcho 专属的安装后配置
```

旧版 `lycus honcho setup` 命令仍然有效（现在会重定向到 `lycus memory setup`），但只有在 Honcho 被选为激活记忆提供者后才会注册。

**配置：** `$AUTOLYCUS_HOME/honcho.json`（profile 本地）或 `~/.honcho/config.json`（全局）。解析顺序：`$AUTOLYCUS_HOME/honcho.json` > `~/.autolycus/honcho.json` > `~/.honcho/config.json`。参见[配置参考](https://github.com/lycus-ai/lycus-agent/blob/main/plugins/memory/honcho/README.md)和 [Honcho 集成指南](https://docs.honcho.dev/v3/guides/integrations/lycus)。
...
:::tip 从 `lycus honcho` 迁移
如果你之前使用过 `lycus honcho setup`，你的配置和所有服务端数据均完好无损。只需通过安装向导重新启用，或手动设置 `memory.provider: honcho`，即可通过新系统重新激活。
:::

**多 peer 配置：**

Honcho 将对话建模为 peer 之间的消息交换——每个 Lycus profile 对应一个用户 peer 加一个 AI peer，共享同一个 workspace。workspace 是共享环境：用户 peer 在各 profile 间全局共享，每个 AI peer 拥有独立身份。每个 AI peer 从自身的观察中独立构建表示/card，因此 `coder` profile 保持代码导向，而 `writer` profile 针对同一用户保持编辑导向。

映射关系：

| 概念 | 含义 |
|---------|-----------|
| **Workspace** | 共享环境。同一 workspace 下的所有 Lycus profile 共享同一用户身份。 |
| **用户 peer**（`peerName`） | 人类用户。在 workspace 内跨 profile 共享。 |
| **AI peer**（`aiPeer`） | 每个 Lycus profile 一个。host key `lycus` → 默认；其他 profile 使用 `lycus.<profile>`。 |
...
`--clone` 在 `honcho.json` 中创建一个 `lycus.coder` host 块，包含 `aiPeer: "coder"`、共享的 `workspace`、继承的 `peerName`、`recallMode`、`writeFrequency`、`observation` 等。AI peer 会在 Honcho 中提前创建，确保在第一条消息之前就已存在。
...
扫描所有 Lycus profile，为没有 host 块的 profile 创建 host 块，从默认 `lycus` 块继承设置，并提前创建新的 AI peer。幂等操作——跳过已有 host 块的 profile。
...
参见[配置参考](https://github.com/lycus-ai/lycus-agent/blob/main/plugins/memory/honcho/README.md)和 [Honcho 集成指南](https://docs.honcho.dev/v3/guides/integrations/lycus)。
...
**安装：**
```bash
lycus memory setup    # 选择 "mem0"
# 或手动配置：
lycus config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.autolycus/.env
```

**配置：** `$AUTOLYCUS_HOME/mem0.json`

| 键 | 默认值 | 描述 |
|-----|---------|-------------|
| `user_id` | `lycus-user` | 用户标识符 |
| `agent_id` | `lycus` | Agent 标识符 |
...
**安装：**
```bash
lycus memory setup    # 选择 "hindsight"
# 或手动配置：
...
**本地模式 UI：** `hindsight-embed -p lycus ui start`

**配置：** `$AUTOLYCUS_HOME/hindsight/config.json`

| 键 | 默认值 | 描述 |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` 或 `local` |
| `bank_id` | `lycus` | 记忆库标识符 |
...
| `retain_context` | `conversation between Lycus Agent and the User` | 保留记忆的上下文标签 |
...
完整配置参考参见[插件 README](https://github.com/NousResearch/lycus-agent/blob/main/plugins/memory/hindsight/README.md)。
...
**安装：**
```bash
lycus memory setup    # 选择 "holographic"
# 或手动配置：
lycus config set memory.provider holographic
```

**配置：** `plugins.autolycus-memory-store` 下的 `config.yaml`

| 键 | 默认值 | 描述 |
|-----|---------|-------------|
| `db_path` | `$AUTOLYCUS_HOME/memory_store.db` | SQLite 数据库路径 |
...
**安装：**
```bash
lycus memory setup    # 选择 "retaindb"
# 或手动配置：
...
# 然后配置 Lycus
lycus memory setup    # 选择 "byterover"
# 或手动配置：
...
- 知识树存储于 `$AUTOLYCUS_HOME/byterover/`（profile 范围隔离）
...
**安装：**
```bash
lycus memory setup    # 选择 "supermemory"
# 或手动配置：
lycus config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.autolycus/.env
```

**配置：** `$AUTOLYCUS_HOME/supermemory.json`

| 键 | 默认值 | 描述 |
|-----|---------|-------------|
| `container_tag` | `lycus` | 用于搜索和写入的容器标签。支持 `{identity}` 模板用于 profile 范围隔离。 |
...
- **Profile 范围容器**——在 `container_tag` 中使用 `{identity}`（例如 `lycus-{identity}` → `lycus-coder`），按 Lycus profile 隔离记忆
