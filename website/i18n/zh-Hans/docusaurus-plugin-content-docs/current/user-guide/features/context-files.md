...
sidebar_position: 8
title: "上下文文件"
description: "项目上下文文件 — .autolycus.md、AGENTS.md、CLAUDE.md、全局 SOUL.md 以及 .cursorrules — 自动注入每次对话"
---

# 上下文文件

Lycus Agent 会自动发现并加载上下文文件，以塑造其行为方式。部分文件属于项目本地文件，从工作目录中发现。`SOUL.md` 现在对整个 Lycus 实例全局生效，仅从 `AUTOLYCUS_HOME` 加载。
...
| **SOUL.md** | 当前 Lycus 实例的全局个性与语气定制 | 仅 `AUTOLYCUS_HOME/SOUL.md` |
| **.cursorrules** | Cursor IDE 编码规范 | 仅 CWD |
| **.cursor/rules/*.mdc** | Cursor IDE 规则模块 | 仅 CWD |

:::info 优先级系统
每次会话仅加载**一种**项目上下文类型（先匹配先生效）：`.autolycus.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`。**SOUL.md** 始终作为 agent 身份独立加载（插槽 #1）。
:::
...
- `~/.autolycus/SOUL.md`
- 或 `$AUTOLYCUS_HOME/SOUL.md`（若使用自定义主目录运行 Lycus）

重要说明：

- 若 `SOUL.md` 尚不存在，Lycus 会自动生成一个默认文件
- Lycus 仅从 `AUTOLYCUS_HOME` 加载 `SOUL.md`
- Lycus 不会在工作目录中探测 `SOUL.md`
