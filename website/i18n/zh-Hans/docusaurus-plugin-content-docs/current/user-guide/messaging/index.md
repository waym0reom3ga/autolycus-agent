...
sidebar_position: 1
title: "消息网关"
description: "通过 Telegram、Discord、Slack、WhatsApp、Signal、SMS、Email、Home Assistant、Mattermost、Matrix、DingTalk、Yuanbao、Microsoft Teams、LINE、Webhooks 或任何兼容 OpenAI 的前端与 Lycus 对话 — 架构与配置概览"
---

# 消息网关

通过 Telegram、Discord、Slack、WhatsApp、Signal、SMS、Email、Home Assistant、Mattermost、Matrix、DingTalk、Feishu/Lark、WeCom、Weixin、BlueBubbles（iMessage）、QQ、Yuanbao、Microsoft Teams、LINE、ntfy 或浏览器与 Lycus 对话。网关是一个单一后台进程，连接所有已配置的平台，管理会话，运行 cron 任务，并传递语音消息。

完整的语音功能集——包括 CLI 麦克风模式、消息中的语音回复以及 Discord 语音频道对话——请参阅 [Voice Mode](/user-guide/features/voice-mode) 和 [Use Voice Mode with Lycus](/guides/use-voice-mode-with-lycus)。
...
```bash
lycus gateway setup        # 交互式配置所有消息平台
```
...
```bash
lycus gateway              # 在前台运行
lycus gateway setup        # 交互式配置消息平台
lycus gateway install      # 安装为用户服务（Linux）/ launchd 服务（macOS）
sudo lycus gateway install --system   # 仅 Linux：安装开机启动的系统服务
lycus gateway start        # 启动默认服务
lycus gateway stop         # 停止默认服务
lycus gateway status       # 检查默认服务状态
lycus gateway status --system         # 仅 Linux：显式检查系统服务
```
...
在 `~/.autolycus/gateway.json` 中配置各平台的覆盖设置：
...
# 其他配对命令：
lycus pairing list          # 查看待审核和已批准的用户
lycus pairing revoke telegram 123456789  # 撤销访问权限
```
...
第一次在任意平台向繁忙的 agent 发送消息时，Lycus 会在繁忙确认中附加一行提示，说明该配置项（`"💡 First-time tip — …"`）。该提示每次安装只触发一次——由 `onboarding.seen.busy_input_prompt` 下的标志锁定。删除该键可再次看到提示。

如果你觉得繁忙确认消息过多——尤其是使用语音输入或快速连续发送消息时——可设置 `display.busy_ack_enabled: false`。你的输入仍会正常排队/引导/中断，只是聊天回复被静默。

## 工具进度通知

在 `~/.autolycus/config.yaml` 中控制显示多少工具活动信息：
...
当运行后台会话的 agent 使用 `terminal(background=true)` 启动长时间运行的进程（服务器、构建等）时，网关可以向你的聊天推送状态更新。通过 `~/.autolycus/config.yaml` 中的 `display.background_process_notifications` 控制：
...
lycus gateway start                 # 启动服务
lycus gateway stop                  # 停止服务
lycus gateway status                # 检查状态
journalctl --user -u lycus-gateway -f  # 查看日志
...
除非你确实有此需要，否则避免同时安装用户和系统网关单元。Lycus 检测到两者同时存在时会发出警告，因为 start/stop/status 行为会变得不明确。

:::info 多个安装
如果你在同一台机器上运行多个 Lycus 安装（使用不同的 `AUTOLYCUS_HOME` 目录），每个安装都有自己的 systemd 服务名称。默认的 `~/.autolycus` 使用 `lycus-gateway`；其他安装使用 `lycus-gateway-<hash>`。`lycus gateway` 命令会自动针对当前 `AUTOLYCUS_HOME` 对应的正确服务。
:::

### macOS（launchd）

```bash
lycus gateway install               # 安装为 launchd agent
lycus gateway start                 # 启动服务
lycus gateway stop                  # 停止服务
lycus gateway status                # 检查状态
tail -f ~/.autolycus/logs/gateway.log   # 查看日志
```

生成的 plist 文件位于 `~/Library/LaunchAgents/ai.autolycus.gateway.plist`。它包含三个环境变量：
...
- **AUTOLYCUS_HOME** — 将网关限定到你的 Lycus 安装。

:::tip 安装后 PATH 变更
launchd plist 是静态的——如果你在配置网关后安装了新工具（例如通过 nvm 安装新版 Node.js，或通过 Homebrew 安装 ffmpeg），请重新运行 `lycus gateway install` 以捕获更新后的 PATH。网关会检测到过时的 plist 并自动重新加载。
:::

:::info 多个安装
与 Linux systemd 服务类似，每个 `AUTOLYCUS_HOME` 目录都有自己的 launchd 标签。默认的 `~/.autolycus` 使用 `ai.autolycus.gateway`；其他安装使用 `ai.autolycus.gateway-<suffix>`。
:::
...
|----------|---------|--------------|
| CLI | `lycus-cli` | 完全访问 |
| Telegram | `lycus-telegram` | 完整工具，包括终端 |
| Discord | `lycus-discord` | 完整工具，包括终端 |
| WhatsApp | `lycus-whatsapp` | 完整工具，包括终端 |
| Slack | `lycus-slack` | 完整工具，包括终端 |
| Google Chat | `lycus-google_chat` | 完整工具，包括终端 |
| Signal | `lycus-signal` | 完整工具，包括终端 |
| SMS | `lycus-sms` | 完整工具，包括终端 |
| Email | `lycus-email` | 完整工具，包括终端 |
| Home Assistant | `lycus-homeassistant` | 完整工具 + HA 设备控制（ha_list_entities、ha_get_state、ha_call_service、ha_list_services） |
| Mattermost | `lycus-mattermost` | 完整工具，包括终端 |
| Matrix | `lycus-matrix` | 完整工具，包括终端 |
| DingTalk | `lycus-dingtalk` | 完整工具，包括终端 |
| Feishu/Lark | `lycus-feishu` | 完整工具，包括终端 |
| WeCom | `lycus-wecom` | 完整工具，包括终端 |
| WeCom Callback | `lycus-wecom-callback` | 完整工具，包括终端 |
| Weixin | `lycus-weixin` | 完整工具，包括终端 |
| BlueBubbles | `lycus-bluebubbles` | 完整工具，包括终端 |
| QQBot | `lycus-qqbot` | 完整工具，包括终端 |
| Yuanbao | `lycus-yuanbao` | 完整工具，包括终端 |
| Microsoft Teams | `lycus-teams` | 完整工具，包括终端 |
| API Server | `lycus-api-server` | 完整工具（去除 `clarify`、`send_message`、`text_to_speech`——程序化访问没有交互用户） |
| Webhooks | `lycus-webhook` | 完整工具，包括终端 |
...
1. **网关日志**（`~/.autolycus/logs/gateway.log` 或 systemd / launchd 单元日志）。搜索平台名称以及 `circuit breaker`、`paused` 或 `disabled`。触发事件包含失败次数和最后一个错误。
