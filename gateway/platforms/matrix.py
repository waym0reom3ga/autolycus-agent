...
def _resolve_matrix_bang_command(name: str) -> str | None:
    """Resolve a ``!command`` token to a dispatchable Lycus command token.

    Matrix clients often reserve leading ``/`` for local client commands.
    Lycus accepts ``!command`` as a Matrix-friendly alias, but only for
...
def _normalize_matrix_bang_command(text: str) -> str:
    """Convert Matrix ``!command`` aliases to normal Lycus ``/command`` text."""
...
# Uses get_lycus_home() so each profile gets its own Matrix store.
from lycus_constants import get_lycus_dir as _get_lycus_dir

_STORE_DIR = _get_lycus_dir("platforms/matrix/store", "matrix/store")
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    # the adapter accepts "!command" as the alias that always reaches Lycus
        ...
        if self._encryption:
            ...
            if self._encryption:
                try:
                    ...
                    _acct_id = self._user_id or "lycus"
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "Lycus Agent" become "Agent".
        """
        if not body:
            return ""
