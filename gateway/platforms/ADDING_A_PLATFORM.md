...
There are two ways to add a platform to the Lycus gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.autolycus/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Lycus code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `lycus gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `lycus_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Lycus core.
...
And add it to the `lycus-gateway` composite:

```python
"lycus-gateway": {
    "includes": [..., "lycus-your-platform"]
}
```
...
## 12. Status Display (`lycus_cli/status.py`)
...
## 13. Gateway Setup Wizard (`lycus_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ lycus_cli/ toolsets.py \
  --include="*.py" -l | sort -u
