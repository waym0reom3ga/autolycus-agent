...
# Audio file extensions Lycus recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Lycus' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Lycus-created Telegram private-chat
...
from lycus_constants import get_default_lycus_root, get_lycus_dir, get_lycus_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.autolycus/.env manually."
)
...
# Default location: {AUTOLYCUS_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_lycus_dir("cache/images", "image_cache")
        ...
        for attempt in range(retries + 1):
            try:
                response = await client.get(
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; LycusAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                )
...
AUDIO_CACHE_DIR = get_lycus_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_lycus_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_lycus_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_lycus_dir("cache/screenshots", "browser_screenshots")
_AUTOLYCUS_HOME = get_lycus_home()
_HERMES_ROOT = get_default_lycus_root()
...
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _AUTOLYCUS_HOME / "image_cache",
    _AUTOLYCUS_HOME / "audio_cache",
    _AUTOLYCUS_HOME / "video_cache",
    _AUTOLYCUS_HOME / "document_cache",
    _AUTOLYCUS_HOME / "browser_screenshots",
    ...
    _AUTOLYCUS_HOME / "cache" / "images",
    _AUTOLYCUS_HOME / "cache" / "audio",
    _AUTOLYCUS_HOME / "cache" / "videos",
    _AUTOLYCUS_HOME / "cache" / "documents",
    _AUTOLYCUS_HOME / "cache" / "screenshots",
)
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Lycus profile and shared Lycus root both contain control
    # files and credentials. Only cache subdirectories under them are
    # explicitly allowlisted above.
    for lycus_root in (_AUTOLYCUS_HOME, _HERMES_ROOT):
        denied.append(lycus_root / ".env")
        denied.append(lycus_root / "auth.json")
        denied.append(lycus_root / "credentials")
        denied.append(lycus_root / "config.yaml")
    return denied
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Lycus
    secrets (``~/.autolycus/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    Lycus-managed cache, under an operator-allowlisted root
    ...
    # The denylist still blocks /etc, /proc, ~/.ssh, ~/.aws, ~/.autolycus/.env,
    # ~/.autolycus/auth.json, etc. — so the obvious prompt-injection sites
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?lycus\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+lycus[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # Lycus commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Lycus sends
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.autolycus/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from lycus_cli.commands import should_bypass_active_session
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _lycus_run_generation is set on
