...
The PID file lives at ``{AUTOLYCUS_HOME}/gateway.pid``.  AUTOLYCUS_HOME defaults to
``~/.autolycus`` but can be overridden via the environment variable.  This means
separate AUTOLYCUS_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting AUTOLYCUS_HOME."""
    home = get_lycus_home()
...
def _get_lock_dir() -> Path:
    ...
    return state_home / "lycus" / _LOCKS_DIRNAME
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Lycus gateway."""
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different AUTOLYCUS_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``lycus.service`` + ``lycus-
# gateway.service``).
#
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # ``lycus gateway stop`` on Windows would be misclassified as an
