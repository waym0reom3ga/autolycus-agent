"""ACP auth helpers — detect and advertise Lycus authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "lycus-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Lycus runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Lycus can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Lycus.
    ...
    not have Lycus provider credentials configured yet, so Lycus always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Lycus using the currently configured "
