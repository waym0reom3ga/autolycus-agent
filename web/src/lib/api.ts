...
// (e.g. https://mission-control.tilos.com/lycus/). The Python backend
...
const SESSION_HEADER = "X-Lycus-Session-Token";
...
export async function fetchJSON<T>(
  ...
  if (res.status === 401) {
    ...
    // (``lycus update``, ``lycus gateway restart``, etc.). A tab kept
    ...
    if (!window.__HERMES_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
      ...
      try {
        alreadyReloaded =
          sessionStorage.getItem("lycus.tokenReloadAttempted") === "1";
      } catch {
        /* SSR / privacy mode — fall through to throw */
      }
      if (!alreadyReloaded) {
        try {
          sessionStorage.setItem("lycus.tokenReloadAttempted", "1");
        } catch {
  ...
  if (res.ok) {
    ...
    try {
      sessionStorage.removeItem("lycus.tokenReloadAttempted");
    } catch {
      /* SSR / privacy mode — ignore */
    }
  }
...
async function getSessionToken(): Promise<string> {
  ...
  throw new Error("Session token not available — page must be served by the Lycus dashboard server");
}

/**
 * Fetch a single-use ticket for a WebSocket upgrade in gated mode.
 *
 * The dashboard's gated-mode WS auth (``lycus_cli.web_server._ws_auth_ok``)
 ...
 *  - loopback / ``--insecure``: attach the ``X-Lycus-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``lycus_session_at`` cookie rides along via ``credentials: 'include'``.
 *
...
export const api = {
  ...
  updateLycus: () =>
    fetchJSON<ActionResponse>("/api/lycus/update", { method: "POST" }),
  checkLycusUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/lycus/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface SkillHubSource {
  ...
  /** lycus-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``lycus update``. */
  can_update_lycus?: boolean;
  config_path: string;
