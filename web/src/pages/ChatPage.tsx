/**
 * ChatPage — embeds `lycus --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (lycus_cli/web_server.py)                          .
 *          ▼                                                              .
...
function buildWsUrl(
  ...
  // Profile-scoped chat: the PTY child gets AUTOLYCUS_HOME pointed at the
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__HERMES_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `lycus dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const term = new Terminal({
      ...
      // even when the inner Lycus TUI has enabled xterm mouse-events
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[lycus-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // forwarding them into Lycus. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="lycus-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
