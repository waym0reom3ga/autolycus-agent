...
import type { SessionInfo } from '@/types/lycus'
...
describe('workspaceGroupsFor', () => {
  it('groups by full cwd, not by basename — same-named folders are separate groups', () => {
    const groups = workspaceGroupsFor(
      [makeSession('/a/lycus-agent/apps/desktop'), makeSession('/a/lycus-agent-wt-rtl/apps/desktop')],
      'No workspace'
    )
  ...
  it('disambiguates colliding basenames by walking up the path', () => {
    expect(
      labels([makeSession('/a/lycus-agent/apps/desktop'), makeSession('/a/lycus-agent-wt-rtl/apps/desktop')])
    ).toEqual(['lycus-agent/apps/desktop', 'lycus-agent-wt-rtl/apps/desktop'])
  })

  it('leaves a unique basename as its short label', () => {
    expect(labels([makeSession('/a/lycus-agent/apps/desktop'), makeSession('/b/heval-py')])).toEqual([
      'desktop',
      'heval-py'
    ])
  })
...
const info = (over: Partial<LycusWorktreeInfo> & Pick<LycusWorktreeInfo, 'repoRoot' | 'worktreeRoot'>): LycusWorktreeInfo => ({
  branch: null,
...
describe('workspaceTreeFor', () => {
  it('heuristic nests `<repo>-wt-<branch>` under its sibling repo', () => {
    const tree = workspaceTreeFor(
      [makeSession('/www/lycus-agent'), makeSession('/www/lycus-agent-wt-rtl')],
      'No workspace'
    )

    expect(tree).toHaveLength(1)
    expect(tree[0].label).toBe('lycus-agent')
    expect(tree[0].groups.map(g => g.label).sort()).toEqual(['lycus-agent', 'rtl'])
  })
  ...
  it('git metadata is authoritative — worktrees group by repoRoot regardless of directory naming', () => {
    const resolver: WorktreeResolver = cwd => {
      if (cwd === '/www/lycus-agent') {
        return info({ repoRoot: '/www/lycus-agent', worktreeRoot: '/www/lycus-agent', isMainWorktree: true, branch: 'main' })
      }

      if (cwd === '/elsewhere/ha-rtl') {
        return info({ repoRoot: '/www/lycus-agent', worktreeRoot: '/elsewhere/ha-rtl', branch: 'rtl' })
      }
    ...
    const tree = workspaceTreeFor(
      [makeSession('/www/lycus-agent'), makeSession('/elsewhere/ha-rtl')],
      'No workspace',
      resolver
    )
    ...
    expect(tree[0].groups.map(g => g.label)).toEqual(['lycus-agent', 'rtl'])
  })
