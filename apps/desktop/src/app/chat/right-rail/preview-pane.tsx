        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.autolycusDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Lycus to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.autolycusDesktop?.watchPreviewFile ||
      !window.autolycusDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.autolycusDesktop.onPreviewFileChanged(payload => {
    ...
    void window.autolycusDesktop
      .watchPreviewFile(target.url)
      .then(watch => {
        if (!active) {
          void window.autolycusDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.autolycusDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:lycus-preview')
