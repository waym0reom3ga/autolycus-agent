...
import { disconnectOAuthProvider, listOAuthProviders } from '@/lycus'
import { useI18n } from '@/i18n'
import { Check, ChevronDown, ChevronRight, KeyRound, Loader2, Terminal, Trash2 } from '@/lib/icons'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import { $desktopOnboarding, startManualProviderOAuth } from '@/store/onboarding'
import type { EnvVarInfo, OAuthProvider } from '@/types/lycus'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.autolycusDesktop?.terminal)
...
function ConnectedProviderRow({
  ...
  // Lycus can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Lycus can't clear via the API, but ships a
...
export function ProvidersSettings({ onClose, onViewChange, view }: ProvidersSettingsProps) {
  ...
  // Lycus never deletes creds another tool owns behind a silent API call.
