...
export const EMPTY_SELECT_VALUE = '__lycus_empty__'
...
export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    prefix: 'NOUS_',
    name: 'Nous Portal',
    description: 'Hosted Lycus & Nous-trained models',
...
export const ENUM_OPTIONS: Record<string, string[]> = {
  ...
  // lycus_cli/config.py (local/groq/openai/mistral/elevenlabs).
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  timezone: 'Used when Lycus needs local time context. Blank uses the system timezone.',
  agent: {
    imageInputMode: 'Controls how image attachments are sent to the model.',
    maxTurns: 'Upper bound for tool-calling turns before Lycus stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters Lycus can read from one file request.',
  approvals: {
    mode: 'How Lycus handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When Lycus updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
