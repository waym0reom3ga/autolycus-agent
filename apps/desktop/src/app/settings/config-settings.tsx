...
import {
  getElevenLabsVoices,
  getLycusConfigDefaults,
  getLycusConfigRecord,
  getLycusConfigSchema,
  saveLycusConfig
} from '@/lycus'
import { useI18n } from '@/i18n'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import type { ConfigFieldSchema, LycusConfigRecord } from '@/types/lycus'
...
export function ConfigSettings({
  ...
  const { t } = useI18n()
  const c = t.settings.config
  const [config, setConfig] = useState<LycusConfigRecord | null>(null)
  const [_defaults, setDefaults] = useState<LycusConfigRecord | null>(null)
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([getLycusConfigRecord(), getLycusConfigDefaults(), getLycusConfigSchema()])
      .then(([c, d, s]) => {
        if (cancelled) {
          return
        }
  ...
  const updateConfig = (next: LycusConfigRecord) => {
