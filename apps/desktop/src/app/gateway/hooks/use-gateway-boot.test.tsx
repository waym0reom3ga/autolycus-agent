...
// bug that drives the REAL useGatewayBoot hook + REAL LycusGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshLycusConfig: async () => undefined,
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForLycus) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startLycus()'s remote branch awaits waitForLycus() for 45s before it
    ...
    // After ~45s waitForLycus gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Lycus backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
