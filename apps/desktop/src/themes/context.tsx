...
const SKIN_KEY = 'lycus-desktop-theme-v2'
const MODE_KEY = 'lycus-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'lycus-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'lycus-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'lycus-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.autolycusTheme = skinName
  root.dataset.autolycusMode = rendered
  ...
  window.autolycusDesktop?.setTitleBarTheme?.({
  ...
  try {
    window.localStorage.setItem('lycus-boot-background', c.background)
    window.localStorage.setItem('lycus-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.autolycusThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.autolycusDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
/** Sync the desktop skin with the active Lycus backend theme on connect. */
