...
declare global {
  interface Window {
    lycusDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<LycusConnection>
      ...
      profile: {
        ...
        // backend under the new AUTOLYCUS_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: LycusApiRequest) => Promise<T>
      notify: (payload: LycusNotification) => Promise<boolean>
      requestMicrophoneAccess: () => Promise<boolean>
      readFileDataUrl: (filePath: string) => Promise<string>
      readFileText: (filePath: string) => Promise<LycusReadFileTextResult>
      selectPaths: (options?: LycusSelectPathsOptions) => Promise<string[]>
      writeClipboard: (text: string) => Promise<boolean>
      saveImageFromUrl: (url: string) => Promise<boolean>
      saveImageBuffer: (data: ArrayBuffer | Uint8Array, ext: string) => Promise<string>
      saveClipboardImage: () => Promise<string>
      getPathForFile: (file: File) => string
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<LycusPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<LycusPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setTitleBarTheme?: (payload: LycusTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<LycusReadDirResult>
      ...
      worktrees?: (cwds: string[]) => Promise<Record<string, LycusWorktreeInfo | null>>
      terminal: {
        dispose: (id: string) => Promise<boolean>
        onData: (id: string, callback: (payload: string) => void) => () => void
        onExit: (id: string, callback: (payload: LycusTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<LycusTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: LycusWindowState) => void) => () => void
      onFocusSession?: (callback: (sessionId: string) => void) => () => void
      onNotificationAction?: (callback: (payload: { actionId: string; sessionId?: string }) => void) => () => void
      onPreviewFileChanged: (callback: (payload: LycusPreviewFileChanged) => void) => () => void
...
export interface LycusTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface LycusTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `lycus update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  lycusRoot?: string
}
...
export interface LycusConnection {
  baseUrl: string
...
export interface LycusTitleBarTheme {
  background: string
  foreground: string
}

export interface LycusWindowState {
  isFullscreen: boolean
  nativeOverlayWidth: number
  windowButtonPosition: { x: number; y: number } | null
}
...
export interface LycusApiRequest {
  path: string
  method?: string
  body?: unknown
  timeoutMs?: number
...
export interface LycusNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface LycusPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface LycusReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface LycusPreviewWatch {
  id: string
  path: string
}

export interface LycusWorktreeInfo {
...
export interface LycusReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface LycusReadDirResult {
  entries: LycusReadDirEntry[]
  error?: string
}

export interface LycusPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface LycusSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
