...
/** Why a known Lycus command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Lycus profile', surface: action('profile') },
  ...
  { name: '/version', description: 'Show Lycus Agent version', surface: exec() },
...
function isKnownLycusSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Lycus' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownLycusSlashCommand(normalized)
}
