...
import { getLycusConfigRecord, type LycusConfigRecord, saveLycusConfig } from '@/lycus'
...
export interface I18nConfigClient {
  getConfig: () => Promise<LycusConfigRecord>
  saveConfig: (config: LycusConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.autolycusDesktop?.api) {
      return Promise.resolve({})
    }

    return getLycusConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.autolycusDesktop?.api) {
...
export function getConfigDisplayLanguage(config: LycusConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: LycusConfigRecord, locale: Locale): LycusConfigRecord {
