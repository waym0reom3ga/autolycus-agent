...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Lycus doesn't know how to remove it. */
...
export type LycusConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `lycus model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
export interface ToolProvider {
   ...
   *  CLI `lycus tools` active-provider detection). */
  is_active: boolean
}
...
/** Shape of `GET /api/lycus/update/check` — the backend's own update state.
