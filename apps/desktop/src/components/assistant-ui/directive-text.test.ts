...
import { formatRefValue, lycusDirectiveFormatter } from './directive-text'
...
describe('lycusDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = lycusDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = lycusDirectiveFormatter.parse('@file:src/main.tsx the entry point')
