...
//! Electron desktop's resolveHermesHome() ALL use ~/.lycus on macOS — there
...
/// Returns the canonical Hermes home directory, respecting $LYCUS_HOME if set.
pub fn hermes_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("LYCUS_HOME") {
    ...
    // macOS + Linux + fallback: ~/.lycus (matches Python get_hermes_home(),
...
/// LYCUS_HOME so it survives repo checkout deletion (unlike anything under
/// lycus-agent/).
///
...
pub fn copy_self_to_hermes_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to LYCUS_HOME");
    Ok(())
}
...
/// Initializes tracing to bootstrap-installer.log under LYCUS_HOME/logs/.
