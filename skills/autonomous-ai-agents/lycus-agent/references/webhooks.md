...
Create dynamic webhook subscriptions so external services (GitHub, GitLab, Stripe, CI/CD, IoT sensors, monitoring tools) can trigger Lycus agent runs by POSTing events to a URL.
...
Add to `${AUTOLYCUS_HOME:-~/.autolycus}/.env`:
```bash
...
All management is via the `lycus webhook` CLI command:

### Create a subscription
```bash
lycus webhook subscribe <name> \
...
```bash
lycus webhook remove <name>
```

### Test a subscription
```bash
lycus webhook test <name>
lycus webhook test <name> --payload '{"key": "value"}'
```
...
### GitHub: PR reviews
```bash
lycus webhook subscribe github-prs \
  --events "pull_request" \
...
```bash
lycus webhook subscribe ci-builds \
  --events "pipeline" \
...
```bash
lycus webhook subscribe alerts \
...
- Subscriptions persist to `~/.autolycus/webhook_subscriptions.json`

## How It Works

1. `lycus webhook subscribe` writes to `~/.autolycus/webhook_subscriptions.json`
