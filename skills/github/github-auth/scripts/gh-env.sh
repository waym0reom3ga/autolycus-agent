#!/usr/bin/env bash
# GitHub environment detection helper for Lycus Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _lycus_env="${AUTOLYCUS_HOME:-$HOME/.autolycus}/.env"; [ -f "$_lycus_env" ] && grep -q "^GITHUB_TOKEN=" "$_lycus_env" 2>/dev/null; then
