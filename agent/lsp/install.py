...
appropriate.  All installs go to a Lycus-owned bin staging dir,
``<AUTOLYCUS_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``lycus lsp
  status``.
...
# ``<AUTOLYCUS_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
def lycus_lsp_bin_dir() -> Path:
    """Return the Lycus-owned bin staging dir for LSP servers."""
    home = os.environ.get("AUTOLYCUS_HOME")
    if home is None:
        home = os.path.join(os.path.expanduser("~"), ".autolycus")
    p = Path(home) / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(lycus_lsp_bin_dir() / name):
...
def _install_npm(
    ...
    if npm is None:
        ...
        return None
    staging = lycus_lsp_bin_dir().parent  # <AUTOLYCUS_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a lycus-owned target dir.
    ...
    console script.
    """
    pip_target = lycus_lsp_bin_dir().parent / "python-packages"
    ...
    for script_dir in script_dirs:
        for bin_path in _native_binary_candidates(script_dir / bin_name):
            if bin_path.exists():
                link = lycus_lsp_bin_dir() / bin_path.name
                if not link.exists():
                    try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``lycus lsp status`` CLI to give users a quick
