...
Owns one Codex thread per Lycus session. Drives `turn/start`, consumes
...
# Lycus' tools.terminal.security_mode → Codex's permissions profile id.
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Lycus/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Lycus session, lifetime owned by AIAgent.
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            ...
            client_version=_get_lycus_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Lycus' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Lycus' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own lycus-tools callback we
            # auto-accept — the user already approved Lycus' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Lycus approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Lycus returns 'once', 'session', 'always', or 'deny'.
...
def _get_lycus_version() -> str:
    """Best-effort Lycus version string for codex's userAgent line."""
    try:
