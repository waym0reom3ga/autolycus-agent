...
A pre-run snapshot of ``~/.autolycus/skills/`` (excluding ``.curator_backups/``
itself) is taken before any mutating curator pass. Snapshots are tar.gz
files under ``~/.autolycus/skills/.curator_backups/<utc-iso>/`` with a
...
``~/.autolycus/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
def _backups_dir() -> Path:
    return get_lycus_home() / "skills" / ".curator_backups"
...
def _cron_jobs_file() -> Path:
    """Source path for the live cron jobs store (``~/.autolycus/cron/jobs.json``)."""
    return get_lycus_home() / "cron" / "jobs.json"
...
def snapshot_skills(reason: str = "manual") -> Optional[Path]:
    """Create a tar.gz snapshot of ``~/.autolycus/skills/`` and prune old ones.
    ...
    if not skills.exists():
        logger.debug("No ~/.autolycus/skills/ directory — nothing to back up")
        return None
...
def _restore_cron_skill_links(snapshot_dir: Path) -> Dict[str, Any]:
    ...
    try:
        from cron.temporal_bridge import bridge_load_jobs as load_jobs, bridge_save_jobs as save_jobs, bridge_jobs_lock as _jobs_lock
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    """Restore ``~/.autolycus/skills/`` from a snapshot.
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.autolycus/skills/``.
    ...
    if target is None:
        return (
            ...
            + " (use `lycus curator rollback --list` to see available snapshots)",
            None,
        )
