...
* Python plugins and shell hooks compose naturally: both flow through
  :func:`lycus_cli.plugins.invoke_hook` and its aggregators.  Python
...
* First-use consent is gated by the allowlist under
  ``~/.autolycus/shell-hooks-allowlist.json``.  Non-TTY callers must pass
  ``accept_hooks=True`` (resolved from ``--accept-hooks``,
  ``HERMES_ACCEPT_HOOKS``, or ``hooks_auto_accept: true`` in config)
  for registration to succeed without a prompt.
* Registration is idempotent — safe to invoke from both the CLI entry
  point (``lycus_cli/main.py``) and the gateway entry point
  (``gateway/run.py``).
...
**stdout** (JSON, optional — anything else is ignored)::
    ...
    {"action":   "block", "message": "Forbidden command"}   # Lycus-canonical
...
def register_from_config(
    ...
    *,
    ...
    ``cfg`` is the full parsed config dict (``lycus_cli.config.load_config``
...
def iter_configured_hooks(cfg: Optional[Dict[str, Any]]) -> List[ShellHookSpec]:
    ...
    registering anything.  Used by ``lycus hooks list`` and ``doctor``."""
    if not isinstance(cfg, dict):
        return []
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    """Translate stdout JSON into a Lycus wire-shape dict.

    For ``pre_tool_call`` the Claude-Code-style ``{"decision": "block",
    "reason": "..."}`` payload is translated into the canonical Lycus
    ``{"action": "block", "message": "..."}`` shape expected by
    :func:`lycus_cli.plugins.get_pre_tool_call_block_message`.  This is
...
def allowlist_path() -> Path:
    ...
    return get_lycus_home() / ALLOWLIST_FILENAME
...
def _prompt_and_record(
    ...
    print(
        f"\n⚠ Lycus is about to register a shell hook that will run a\n"
...
# ---------------------------------------------------------------------------
# Introspection (used by `lycus hooks` CLI)
# ---------------------------------------------------------------------------
...
def run_once(
    ...
    Used by ``lycus hooks test`` and ``lycus hooks doctor``.

    ``kwargs`` is the same dict that :func:`lycus_cli.plugins.invoke_hook`
    ...
    produce — otherwise scripts tested via ``lycus hooks test`` could
    diverge silently from production behaviour.

    Returns the :func:`_spawn` diagnostic dict plus a ``parsed`` field
    holding the canonical Lycus-wire-shape response."""
