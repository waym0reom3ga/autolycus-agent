...
upstream prefix cache warm.  See ``lycus-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Lycus never re-renders parts of this string mid-
    ...
    # Pointer to the lycus-agent skill + docs for user questions about Lycus itself.
    stable_parts.append(LYCUS_AGENT_HELP_GUIDANCE)
    ...
    # Coding posture (base Lycus, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Lycus profile the agent is running
    # under so it doesn't conflate ~/.autolycus/skills/ (default profile) with
    # ~/.autolycus/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Lycus profile: default. Other profiles (if any) live "
            "under ~/.autolycus/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Lycus profile: {active_profile}. This session reads "
            f"and writes ~/.autolycus/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.autolycus/skills/, ~/.autolycus/plugins/, "
            f"~/.autolycus/cron/, ~/.autolycus/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Lycus never rebuilds or reinjects parts of it
