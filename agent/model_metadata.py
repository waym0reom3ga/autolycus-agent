...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. Lycus also honours HERMES_CA_BUNDLE (its own convention)
...
def _get_model_metadata_cache_path() -> Path:
    ...
    return get_lycus_home() / "cache" / "openrouter_model_metadata.json"
...
# Minimum context length required to run Lycus Agent.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent Lycus from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    # (often 8K) makes it unusable for Lycus' system prompt, but mapping
...
def _get_context_cache_path() -> Path:
    ...
    from lycus_constants import get_lycus_home
    return get_lycus_home() / "context_length_cache.yaml"
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Lycus grow conversations past the runtime limit and Ollama
...
def get_model_context_length(
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from lycus_cli.models import get_copilot_model_context
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Lycus sends to providers:
