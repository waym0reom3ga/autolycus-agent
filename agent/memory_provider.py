...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - lycus_home (str): The active AUTOLYCUS_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.autolycus``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "lycus").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'lycus memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], lycus_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'lycus memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``lycus_home`` is the active AUTOLYCUS_HOME directory path.
