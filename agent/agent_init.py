...
from lycus_cli.timeouts import get_provider_request_timeout
...
def _build_codex_gpt55_autoraise_notice(autoraise: Dict[str, float]) -> str:
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: lycus config set compression.codex_gpt55_autoraise false"
    )
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of MASK.md, AGENTS.md, and .cursorrules
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.autolycus/MASK.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from lycus_cli.config import load_config as _load_pc_cfg
    ...
    # both live under ~/.autolycus/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from lycus_logging import setup_logging, setup_verbose_logging
    setup_logging(lycus_home=_ra()._lycus_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that lycus_logging.setup_logging() does not
        ...
        # handler level inside lycus_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from lycus_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from lycus_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `lycus model` to "
                        "select a provider, or run `lycus setup` for first-time "
                        "configuration."
                    )
    ...
    # Session logs go into ~/.autolycus/sessions/ alongside gateway sessions
    lycus_home = get_lycus_home()
    agent.logs_dir = lycus_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.autolycus/sessions/session_{sid}.json)
    ...
    try:
        from lycus_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from lycus_cli.config import load_config as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "lycus"
                    except Exception:
                        pass
    ...
    if not isinstance(_compression_cfg, dict):
        _compression_cfg = {}
    compression_threshold = float(_compression_cfg.get("threshold", 0.60))
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from lycus_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            try:
                from lycus_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Lycus Agent.  Choose a model with at least "
