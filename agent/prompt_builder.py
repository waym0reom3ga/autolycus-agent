...
from lycus_constants import get_lycus_home, get_skills_dir, is_wsl
...
_HERMES_MD_NAMES = (".autolycus.md", "HERMES.md")


def _find_lycus_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.autolycus.md`` or ``HERMES.md``.
...
_LYCUS_AGENT_NAME_FILE = ".autolycus_agent_name"  # stored in ~/.autolycus/
...
def _ensure_agent_name() -> str:
    ...
    to ~/.autolycus/.autolycus_agent_name.  Subsequent calls return the stored
    name.  This ensures every install gets a unique identity.
    """
    try:
        lycus_home = get_lycus_home()
        name_file = lycus_home / _AGENT_NAME_FILE
...
def _ensure_lycus_agent_name() -> str:
    ...
    try:
        lycus_home = get_lycus_home()
        name_file = lycus_home / _LYCUS_AGENT_NAME_FILE
...
LYCUS_AGENT_HELP_GUIDANCE = (
    ...
    "information. Load the `lycus-agent` skill with skill_view(name='lycus-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.autolycus/kanban.db`. Your task id is in "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `lycus kanban <verb>` for board operations. Use "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Lycus "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Lycus WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Lycus itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Lycus runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Lycus itself is running. The host OS, home, and cwd "
                f"of the Lycus process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Lycus "
    ...
    # Lycus desktop GUI — any agent running under the desktop app should know
    # it. HERMES_DESKTOP marks the backend powering the chat; HERMES_DESKTOP_TERMINAL
    # marks a lycus launched in the embedded terminal pane. Both set by main.cjs.
    _truthy = ("1", "true", "yes")
    ...
    if _in_desktop or _in_desktop_term:
        _desktop_hint = "Runtime surface: you're running inside the Lycus desktop GUI app."
        if _in_desktop_term:
            _desktop_hint += (
...
def _skills_prompt_snapshot_path() -> Path:
    return get_lycus_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.autolycus/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Lycus Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `lycus-agent` skill "
            "first. It has the actual commands (e.g. `lycus config set …`, `lycus tools`, "
            "`lycus setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: lycus setup, lycus setup tools, lycus setup terminal, lycus status.",
        ]
    )
    return "\n".join(lines)
...
def _load_lycus_md(cwd_path: Path) -> str:
    """.autolycus.md / HERMES.md — walk to git root."""
    lycus_md_path = _find_lycus_md(cwd_path)
    if not lycus_md_path:
        return ""
    try:
        content = lycus_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = lycus_md_path.name
        try:
            rel = str(lycus_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".autolycus.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", lycus_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .autolycus.md / HERMES.md  (walk to git root)
