...
Storage
-------
Bundles live in ``~/.autolycus/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``AUTOLYCUS_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``lycus bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under AUTOLYCUS_HOME.

    Honors ``HERMES_BUNDLES_DIR`` for tests; falls back to
    ``<AUTOLYCUS_HOME>/skill-bundles``.
    """
    override = os.environ.get("HERMES_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_lycus_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `lycus bundles` CLI subcommand.
# ---------------------------------------------------------------------------
