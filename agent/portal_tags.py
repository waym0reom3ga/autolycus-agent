...
Every Lycus request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Lycus Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=lycus-agent",
        "client=lycus-client-v<__version__>",
    ]

The version is sourced live from ``lycus_cli.__version__`` so it auto-aligns
...
``lycus_cli.__version__`` is the canonical source of truth.
"""
...
def _lycus_version() -> str:
    """Return the current Lycus release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``lycus_cli`` cannot be imported (should
...
def lycus_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=lycus-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=lycus-client-v{_lycus_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=lycus-agent", lycus_client_tag()]
