...
as ``kind: backend``) or ``~/.autolycus/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``lycus tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$AUTOLYCUS_HOME/cache/videos/``, creating parents as needed."""
    from lycus_constants import get_lycus_home

    path = get_lycus_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$AUTOLYCUS_HOME/cache/videos/``.
