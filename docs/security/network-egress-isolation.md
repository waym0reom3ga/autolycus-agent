...
When running Lycus inside Docker, the default `network_mode: host` gives the
...
The Lycus [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=lycus,lycus-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://lycus-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Lycus trust model and vulnerability reporting
