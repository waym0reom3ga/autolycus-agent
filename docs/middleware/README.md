...
Lycus middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Lycus'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Lycus calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Lycus retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Lycus guardrails,
...
- `telemetry_schema_version`: currently `lycus.observer.v1`
- `middleware_schema_version`: currently `lycus.middleware.v1`
...
Lycus stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Lycus runs
them as a nested chain in registration order. Middleware failures are fail-open:
Lycus logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Lycus applies middleware in this order:
...
For each tool call, Lycus applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Lycus pre-execution path against the effective arguments:
...
```bash
lycus plugins enable <plugin-name>
```

For isolated local testing, use one `AUTOLYCUS_HOME` for plugin enablement and the
agent run:

```bash
export AUTOLYCUS_HOME=/tmp/lycus-middleware-test
mkdir -p "$AUTOLYCUS_HOME"
lycus plugins enable <plugin-name>
lycus chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run lycus plugins enable <plugin-name>
uv run lycus chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["lycus_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/lycus-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Lycus expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Lycus treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Lycus preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Lycus does not convert downstream
