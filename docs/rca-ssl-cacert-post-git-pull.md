# RCA: SSL CA cert bundle corruption after `lycus update`
...
When the venv is partially refreshed, or when one of those env vars points at a file that no longer exists, provider client construction can fail before Lycus has enough context to produce a useful message.
...
`lycus_cli doctor` exposes the same check under `SSL / CA Certificates`, so users can diagnose the problem without starting a model session.
...
For a normal corrupted Lycus venv, reinstall the affected client dependencies:

```bash
python -m pip install --force-reinstall certifi openai httpx
```

For a custom/corporate CA setup, fix the env var so it points at a real PEM bundle, or unset it if Lycus should use the bundled `certifi` store.
