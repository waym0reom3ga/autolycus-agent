#!/usr/bin/env python3
"""Canvas LMS API CLI for Lycus Agent.
...
def _check_config():
    ...
    if missing:
        lycus_env = os.path.join(
            os.environ.get("AUTOLYCUS_HOME", os.path.expanduser("~/.autolycus")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {lycus_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Lycus Agent"
    )
