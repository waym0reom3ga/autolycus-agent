...
- save provider credentials into `${AUTOLYCUS_HOME:-~/.autolycus}/.env`
...
3. Buy it and save it into `${AUTOLYCUS_HOME:-~/.autolycus}/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
3. persist that number to `${AUTOLYCUS_HOME:-~/.autolycus}/.env`
