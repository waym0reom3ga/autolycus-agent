...
## 5. GODMODE FAST — Lycus 4 405B (Zero Refusal)

**Model:** `nousresearch/lycus-4-405b`
**Strategy:** Lycus models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Lycus Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.autolycus/config.yaml`:
