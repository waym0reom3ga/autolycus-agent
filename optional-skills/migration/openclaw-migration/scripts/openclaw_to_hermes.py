#!/usr/bin/env python3
"""OpenClaw -> Lycus migration helper.

This script migrates the parts of an OpenClaw user footprint that map cleanly
into Lycus Agent, archives selected unmapped docs for manual review, and
...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    ...
    "memory": {
        "label": "MEMORY.md",
        "description": "Import long-term memory entries into Lycus memories.",
    },
    "user-profile": {
        "label": "USER.md",
        "description": "Import user profile entries into Lycus memories.",
    },
    "messaging-settings": {
        "label": "Messaging settings",
        "description": "Import Lycus-compatible messaging settings such as allowlists and working directory.",
    },
    "secret-settings": {
        "label": "Allowlisted secrets",
        "description": "Import the small allowlist of Lycus-compatible secrets when explicitly enabled.",
    },
    "command-allowlist": {
        "label": "Command allowlist",
        "description": "Merge OpenClaw exec approval patterns into Lycus command_allowlist.",
    },
    "skills": {
        "label": "User skills",
        "description": "Copy OpenClaw skills into ~/.autolycus/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.autolycus/tts/.",
    },
    "discord-settings": {
        "label": "Discord settings",
        "description": "Import Discord bot token and allowlist into Lycus .env.",
    },
    "slack-settings": {
        "label": "Slack settings",
        "description": "Import Slack bot/app tokens and allowlist into Lycus .env.",
    },
    "whatsapp-settings": {
        "label": "WhatsApp settings",
        "description": "Import WhatsApp allowlist into Lycus .env.",
    },
    "signal-settings": {
        "label": "Signal settings",
        "description": "Import Signal account, HTTP URL, and allowlist into Lycus .env.",
    },
    "provider-keys": {
        "label": "Provider API keys",
        "description": "Import model provider API keys into Lycus .env (requires --migrate-secrets).",
    },
    "model-config": {
        "label": "Default model",
        "description": "Import the default model setting into Lycus config.yaml.",
    },
    "tts-config": {
        "label": "TTS configuration",
        "description": "Import TTS provider and voice settings into Lycus config.yaml.",
    },
    "shared-skills": {
        "label": "Shared skills",
        "description": "Copy shared OpenClaw skills from ~/.openclaw/skills/ into Lycus.",
    },
    "daily-memory": {
        "label": "Daily memory files",
        "description": "Merge daily memory entries from workspace/memory/ into Lycus MEMORY.md.",
    },
    "archive": {
    ...
    "mcp-servers": {
        "label": "MCP servers",
        "description": "Import MCP server definitions from OpenClaw into Lycus config.yaml.",
    },
    "plugins-config": {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'lycus cron'.",
    },
    "hooks-config": {
    ...
    "agent-config": {
        ...
        "description": "Import agent defaults (compaction, context, thinking) into Lycus config. Archive multi-agent list.",
    },
    "gateway-config": {
    ...
    "session-config": {
        ...
        "description": "Import session reset policies (daily/idle) into Lycus session_reset config.",
    },
    "full-providers": {
        "label": "Full model provider definitions",
        "description": "Import custom model providers (baseUrl, apiType, headers) into Lycus custom_providers.",
    },
    "deep-channels": {
    ...
    "browser-config": {
        ...
        "description": "Import browser automation settings into Lycus config.yaml.",
    },
    "tools-config": {
        "label": "Tools configuration",
        "description": "Import tool settings (exec timeout, sandbox, web search) into Lycus config.yaml.",
    },
    "approvals-config": {
        "label": "Approval rules",
        "description": "Import approval mode and rules into Lycus config.yaml approvals section.",
    },
    "memory-backend": {
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    if yaml is None:
        raise RuntimeError("PyYAML is required to update Lycus config.yaml")
    ensure_parent(path)
    path.write_text(
...
# like ``openclaw`` → ``lycus`` (so filesystem paths like ``~/.openclaw``
# become ``~/.autolycus`` — the real Lycus home — not the broken ``~/.Lycus``).
_REBRAND_PATTERNS: List[Tuple[re.Pattern, str]] = [
    (re.compile(r'\bOpen[\s-]?Claw\b', re.IGNORECASE), 'Lycus'),
    (re.compile(r'\bClawdBot\b', re.IGNORECASE), 'Lycus'),
    (re.compile(r'\bMoltBot\b', re.IGNORECASE), 'Lycus'),
]
...
def _case_preserving_replacement(replacement: str):
    ...
    Keeps ``OpenClaw`` → ``Lycus`` but maps ``openclaw`` → ``lycus`` so a
    filesystem path like ``~/.openclaw/config.yaml`` rewrites to
    ``~/.autolycus/config.yaml`` (the real Lycus home) instead of the broken
    ``~/.Lycus/config.yaml``.
    """
...
def write_report(output_dir: Path, report: Dict[str, Any]) -> None:
    ...
    lines = [
        "# OpenClaw -> Lycus Migration Report",
        "",
...
class Migrator:
    def __init__(
        ...
        # OpenClaw's extensions/migrate-lycus/apply.ts "blocked by earlier
    ...
    # Option ids that mutate the Lycus config.yaml file.  Once any one of
    ...
    def _build_warnings(self, summary: Dict[str, int]) -> List[str]:
        ...
        Modelled on OpenClaw's extensions/migrate-lycus/plan.ts warnings[].
    ...
    def _build_next_steps(self, summary: Dict[str, int]) -> List[str]:
        ...
        if summary.get("migrated", 0) > 0:
            ...
            steps.append(
                "Start a new Lycus session (or /reset) to pick up the imported config."
            )
        if summary.get("conflict", 0) > 0:
            steps.append(
    ...
    def migrate_command_allowlist(self) -> None:
        ...
        if not destination.exists():
            self.record("command-allowlist", source, destination, "skipped", "Lycus config.yaml does not exist yet")
            return
    ...
    def migrate_messaging_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(workspace, str) and workspace.strip():
            ...
            # that path will be stale after migration and would cause the Lycus
        ...
        if additions:
            ...
            self.record("messaging-settings", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "No Lycus-compatible messaging settings found")
    ...
    def migrate_secret_settings(self, config: Dict[str, Any]) -> None:
        ...
        if secret_additions:
            ...
            self.record(
                ...
                self.target_root / ".env",
                "skipped",
                "No allowlisted Lycus-compatible secrets found",
                supported_targets=sorted(SUPPORTED_SECRET_TARGETS),
            )
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
                ...
                if not api_key:
                    ...
                    if isinstance(raw_key, dict) and raw_key.get("source") in {"file", "exec"}:
                        self.record(
                            ...
                            f"that cannot be auto-migrated. Add this key manually via: lycus config set",
                        )
                    continue
        ...
        for oc_key, lycus_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and lycus_key not in secret_additions:
                secret_additions[lycus_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, lycus_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and lycus_key not in secret_additions:
                        secret_additions[lycus_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        lycus_config = load_yaml_file(destination)
        current_model = lycus_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = lycus_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                lycus_config["model"] = {"default": model_str}
            dump_yaml_file(destination, lycus_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(provider, str) and provider in {"elevenlabs", "openai", "edge", "microsoft"}:
            # OpenClaw renamed "edge" to "microsoft"; Lycus still uses "edge"
        ...
        lycus_config = load_yaml_file(destination)
        existing_tts = lycus_config.get("tts", {})
    ...
    def archive_docs(self) -> None:
        ...
        for candidate in candidates:
            if candidate:
                self.archive_path(candidate, reason="No direct Lycus destination; archived for manual review")
        ...
        partially_extracted = [
            ("openclaw.json", "Selected Lycus-compatible values were extracted; raw OpenClaw config was not copied."),
            ("credentials/telegram-default-allowFrom.json", "Selected Lycus-compatible values were extracted; raw credentials file was not copied."),
        ]
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        lycus_cfg_path = self.target_root / "config.yaml"
        lycus_cfg = load_yaml_file(lycus_cfg_path)
        existing_mcp = lycus_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            if name in existing_mcp and not self.overwrite:
                self.record("mcp-servers", f"mcp.servers.{name}", f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in Lycus config")
                continue

            lycus_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                lycus_srv["command"] = srv["command"]
                if srv.get("args"):
                    lycus_srv["args"] = srv["args"]
                if srv.get("env"):
                    lycus_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    lycus_srv["cwd"] = srv["cwd"]
            # HTTP/SSE transport
            if srv.get("url"):
                lycus_srv["url"] = srv["url"]
                if srv.get("headers"):
                    lycus_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    lycus_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                lycus_srv["enabled"] = False
            if srv.get("timeout"):
                lycus_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                lycus_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                lycus_srv["tools"] = {}
                if tools_cfg.get("include"):
                    lycus_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    lycus_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
        ...
        if added > 0 and self.execute:
            self.maybe_backup(lycus_cfg_path)
            lycus_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(lycus_cfg_path, lycus_cfg)
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'lycus cron' to recreate jobs manually.")
            else:
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = lycus_cfg.get("agent") or {}
        ...
        if defaults.get("thinkingDefault"):
            # Map OpenClaw thinking -> Lycus reasoning_effort
        ...
        if compaction:
            compression = lycus_cfg.get("compression") or {}
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'lycus gateway' to configure.")
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        lycus_cfg_path = self.target_root / "config.yaml"
        lycus_cfg = load_yaml_file(lycus_cfg_path)
        sr = lycus_cfg.get("session_reset") or {}
        changes = False
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        lycus_cfg_path = self.target_root / "config.yaml"
        lycus_cfg = load_yaml_file(lycus_cfg_path)
        custom_providers = lycus_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(lycus_cfg_path)
            lycus_cfg["custom_providers"] = custom_providers
            dump_yaml_file(lycus_cfg_path, lycus_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map Discord-specific settings to Lycus config
        discord_cfg = channels.get("discord") or {}
        if discord_cfg:
            lycus_cfg_path = self.target_root / "config.yaml"
            lycus_cfg = load_yaml_file(lycus_cfg_path)
            discord_lycus = lycus_cfg.get("discord") or {}
            changed = False
            if "requireMention" in discord_cfg:
                discord_lycus["require_mention"] = discord_cfg["requireMention"]
                changed = True
            if discord_cfg.get("autoThread") is not None:
                discord_lycus["auto_thread"] = discord_cfg["autoThread"]
                changed = True
            if changed and self.execute:
                lycus_cfg["discord"] = discord_lycus
                dump_yaml_file(lycus_cfg_path, lycus_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        lycus_cfg_path = self.target_root / "config.yaml"
        lycus_cfg = load_yaml_file(lycus_cfg_path)
        browser_lycus = lycus_cfg.get("browser") or {}
        changed = False

        # Map fields that have Lycus equivalents
        if browser.get("cdpUrl"):
            browser_lycus["cdp_url"] = browser["cdpUrl"]
            changed = True
        if browser.get("headless") is not None:
            browser_lycus["headless"] = browser["headless"]
            changed = True

        if changed:
            lycus_cfg["browser"] = browser_lycus
            if self.execute:
                self.maybe_backup(lycus_cfg_path)
                dump_yaml_file(lycus_cfg_path, lycus_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            lycus_mode = mode_map.get(mode, "manual")
            lycus_cfg.setdefault("approvals", {})["mode"] = lycus_mode
            if self.execute:
                self.maybe_backup(lycus_cfg_path)
                dump_yaml_file(lycus_cfg_path, lycus_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{lycus_mode}'")
    ...
    def generate_migration_notes(self) -> None:
        if not self.output_dir:
            return
        notes = [
            "# OpenClaw -> Lycus Migration Notes",
            "",
        ...
        if archived:
            notes.extend([
                ...
                "direct 1:1 mapping in Lycus. Review each file and recreate manually:",
                "",
            ])
        ...
        if conflicts:
            notes.extend([
                "## Conflicts (Existing Lycus Config Not Overwritten)",
                "",
                "These items already existed in your Lycus config. Re-run with",
        ...
        notes.extend([
            ...
            "state files (todo.json, sessions, logs). If the Lycus agent discovers these",
            "directories, it may read/write to them instead of the Lycus state, causing",
            "confusion (e.g., cron jobs reading a different todo list than interactive sessions).",
            "",
            "**Strongly recommended:** Run `lycus claw cleanup` to rename the OpenClaw",
            ...
            "or todo lists, run `lycus claw cleanup` to fix it.",
            "",
            "## Lycus-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `lycus claw cleanup` to archive the OpenClaw directory (prevents state confusion)",
            "- Run `lycus setup` to configure any remaining settings",
            "- Run `lycus mcp list` to verify MCP servers were imported correctly",
        ])

        if has_cron_config_archive:
            notes.append("- Run `lycus cron` to recreate scheduled tasks (see archive/cron-config.json)")
        elif has_cron_store_archive:
            notes.append("- Run `lycus cron` to recreate scheduled tasks (see archived cron-store)")
        ...
        notes.extend([
            "- Run `lycus gateway install` if you need the gateway service",
            "- Review `~/.autolycus/config.yaml` for any adjustments",
            "",
        ])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate OpenClaw user state into Lycus Agent.")
    parser.add_argument("--source", default=str(Path.home() / ".openclaw"), help="OpenClaw home directory")
    parser.add_argument("--target", default=os.environ.get("AUTOLYCUS_HOME") or str(Path.home() / ".autolycus"), help="Lycus home directory")
    parser.add_argument(
        "--workspace-target",
    ...
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing Lycus targets after backing them up")
    parser.add_argument(
        "--migrate-secrets",
        action="store_true",
        help="Import a narrow allowlist of Lycus-compatible secrets into the target env file",
    )
    parser.add_argument(
        "--skill-conflict",
...
def main() -> int:
    ...
    print()
    print(f"  ╔══════════════════════════════════════════════════════╗")
    print(f"  ║   OpenClaw -> Lycus Migration   [{mode_label:>8s}]   ║")
    print(f"  ╠══════════════════════════════════════════════════════╣")
    ...
    if migrated:
        ...
        for item in migrated:
            ...
            if dest.startswith(str(report["target_root"])):
                dest = "~/.autolycus/" + dest[len(str(report["target_root"])) + 1:]
    ...
    if args.execute:
        print()
        print("  Next steps:")
        print("    1. Review ~/.autolycus/config.yaml")
        print("    2. Run: lycus mcp list")
        if any(i["kind"] == "cron-jobs" and i["status"] == "archived" for i in items):
            print("    3. Recreate cron jobs: lycus cron")
