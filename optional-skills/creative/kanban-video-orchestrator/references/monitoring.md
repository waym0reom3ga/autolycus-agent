...
lycus kanban watch --tenant <project-slug>

# Snapshot of the board
lycus kanban list --tenant <project-slug>
lycus kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
lycus kanban stats --tenant <project-slug>

# Visual dashboard (browser)
lycus dashboard

# Inspect a specific task (includes comments + events)
lycus kanban show <task-id>

# Follow a single task's event stream
lycus kanban tail <task-id>
```

Verify available subcommands with `lycus kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `lycus kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `lycus kanban block <id>` then `lycus kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `lycus kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `lycus kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `lycus profile list` (profile exists?), `lycus status` (gateway/dispatcher up?), and verify tenant. |
...
lycus kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
lycus kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(lycus kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
lycus kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
lycus kanban block <task-id>
lycus kanban archive <task-id>

# Diagnose what happened
lycus kanban show <task-id>      # task body, comments, recent events
lycus kanban tail <task-id>      # follow the live event stream
lycus kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `lycus kanban create` for the director
...
while true; do
    clear
    lycus kanban list --tenant <slug>
    echo "---"
    lycus kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `lycus kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `lycus profile list` and start
