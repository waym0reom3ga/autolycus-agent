...
the actual `setup.sh` that creates the project workspace, configures Lycus
...
2. **Create profiles** — `lycus profile create <name> --clone`
...
```bash
lycus profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.autolycus/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.autolycus/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Lycus
...
```bash
lycus kanban create "Direct production of <video title>" \
    --assignee director \
...
the Lycus `.env` (`${AUTOLYCUS_HOME:-$HOME/.autolycus}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _lycus_env="${AUTOLYCUS_HOME:-$HOME/.autolycus}/.env"
    if grep -q "^${var}=" "$_lycus_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_lycus_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_lycus_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY lycus ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY lycus OPENROUTER_API_KEY || exit 1
# ...
```
