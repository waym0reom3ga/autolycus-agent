...
Maps each role archetype to the Lycus skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public lycus-agent
...
## Lycus skills relevant to video production

### Visual / rendering skills (`lycus-agent/skills/creative/`)
...
### Design / pre-production skills (`lycus-agent/skills/creative/`)
...
### Audio / media skills (`lycus-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`lycus-agent/skills/devops/`)
...
These are **not** Lycus skills but external CLIs / APIs that profiles invoke.
...
## Built-in Lycus tools for media review

These are native Lycus tools — not invoked via terminal but through their own
...
key is present in `${AUTOLYCUS_HOME:-~/.autolycus}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Lycus profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${AUTOLYCUS_HOME:-~/.autolycus}/.env` → environment variable.
...
When a new Lycus-public video skill ships:
