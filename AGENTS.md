# Lycus Agent - Development Guide

Instructions for AI coding assistants and developers working on the lycus-agent codebase.

**Never give up on the right solution.**

## What Lycus Is

Lycus is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: Lycus ships a **lot** — most merges are bug fixes to
...
- **Expand reach at the edges.** New platform adapters, channels, providers,
  ...
  (`lycus tools`, `lycus setup`, auto-install) rather than bolting on a raw
  env var.
...
- **E2E validation, not just green unit mocks.** For anything touching
  ...
  against a temp `AUTOLYCUS_HOME`. Mocks hide integration bugs.
...
- **New `LYCUS_*` env vars for non-secret config.** `.env` is for secrets
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `lycus tools`
...
2. **CLI command + skill** — manages config/state/infra expressible as shell
   commands. The agent runs `lycus <subcommand>` guided by a skill. Zero
   model-tool footprint. Default choice for subscriptions, scheduled tasks,
   service setup. Examples: `lycus webhook`, `lycus cron`, `lycus tools`.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.autolycus/plugins/` or a pip package, discovered at runtime.
...
`$HOME/.autolycus/lycus-agent/venv` (for worktrees that share a venv with the
main checkout).
...
├── toolsets.py           # Toolset definitions, _LYCUS_CORE_TOOLS list
├── cli.py                # LycusCLI class — interactive CLI orchestrator (~11k LOC)
├── lycus_state.py       # SessionDB — SQLite session store (FTS5 search)
├── lycus_constants.py   # get_lycus_home(), display_lycus_home() — profile-aware paths
├── lycus_logging.py     # setup_logging() — agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # Agent internals (provider adapters, memory, caching, compression, etc.)
├── lycus_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
│   ├── lycus-achievements/  # Gamified achievement tracking
...
├── ui-tui/               # Ink (React) terminal UI — `lycus --tui`
...
**User config:** `~/.autolycus/config.yaml` (settings), `~/.autolycus/.env` (API keys only).
**Logs:** `~/.autolycus/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_lycus_home()`.
Browse with `lycus logs [--follow] [--level ...] [--session ...]`.

## TypeScript Style

Applies to TypeScript across Lycus: desktop, TUI, website, and future TS packages.
...
- **Skin engine** (`lycus_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
- `process_command()` is a method on `LycusCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
- Skill slash commands: `agent/skill_commands.py` scans `~/.autolycus/skills/`, injects as **user message** (not system prompt) to preserve prompt caching

### Slash Command Registry (`lycus_cli/commands.py`)
...
- **Slack** — `slack_subcommand_map()` generates `/lycus` subcommand routing
- **Autocomplete** — `COMMANDS` flat dict feeds `SlashCommandCompleter`
- **CLI help** — `COMMANDS_BY_CATEGORY` dict feeds `show_help()`

### Adding a Slash Command

1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `lycus_cli/commands.py`:
```python
CommandDef("mycommand", "Description of what it does", "Session",
           aliases=("mc",), args_hint="[arg]"),
```
2. Add handler in `LycusCLI.process_command()` in `cli.py`:
```python
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `lycus --tui` or `LYCUS_TUI=1`.
...
npm run dev       # watch mode (rebuilds lycus-ink + tsx --watch)
npm start         # production
npm run build     # full build (lycus-ink + tsc)
...
### TUI in the Dashboard (`lycus dashboard` → `/chat`)

The dashboard embeds the real `lycus --tui` — **not** a rewrite.  See `lycus_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `lycus_cli/web_server.py`.

- Browser loads `web/src/pages/ChatPage.tsx`, which mounts xterm.js's `Terminal` with the WebGL renderer, `@xterm/addon-fit` for container-driven resize, and `@xterm/addon-unicode11` for modern wide-character widths.
- `/api/pty?token=…` upgrades to a WebSocket; auth uses the same ephemeral `_SESSION_TOKEN` as REST, via query param (browsers can't set `Authorization` on WS upgrade).
- The server spawns whatever `lycus --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL works, native Windows does not).
- Frames: raw PTY bytes each direction; resize via `\x1b[RESIZE:<cols>;<rows>]` intercepted on the server and applied with `TIOCSWINSZ`.

**Do not re-implement the primary chat experience in React.** The main transcript, composer/input flow (including slash-command behavior), and PTY-backed terminal belong to the embedded `lycus --tui` — anything new you add to Ink shows up in the dashboard automatically. If you find yourself rebuilding the transcript or composer for the dashboard, stop and extend Ink instead.

**Structured React UI around the TUI is allowed when it is not a second chat surface.** Sidebar widgets, inspectors, summaries, status panels, and similar supporting views (e.g. `ChatSidebar`, `ModelPickerDialog`, `ToolCall`) are fine when they complement the embedded TUI rather than replacing the transcript / composer / terminal. Keep their state independent of the PTY child's session and surface their failures non-destructively so the terminal pane keeps working unimpaired.

### Electron Desktop Chat App (`apps/desktop/`)

A **separate** chat surface from both the classic CLI and the dashboard's embedded TUI. It is an Electron + React + nanostore renderer (`@assistant-ui/react`) that talks to a `tui_gateway` backend over JSON-RPC (`requestGateway(method, params)`). It does NOT embed `lycus --tui` — it has its own composer, transcript, and slash-command pipeline. Route desktop bugs to the `lycus-desktop-app-work` skill, not `lycus-dashboard-work`.
...
- **The renderer curates via `apps/desktop/src/lib/desktop-slash-commands.ts`.** This is the load-bearing file. It holds `DESKTOP_COMMANDS` (the ~19 built-ins shown in the palette) plus block-lists for terminal-only / messaging-only / picker-owned / settings-owned / advanced commands that should NOT clutter the desktop popover.
  ...
  - `isDesktopSlashExtensionCommand(name)` — true when the command is NOT a known Lycus built-in (i.e. a skill or user quick command). Both suggestion and catalog-filter paths allow extensions through so skill commands surface in the palette. (Added when fixing "skill commands missing from the desktop slash palette" — the curated allow-list was silently dropping every skill/quick command from completions even though they executed fine when typed.)
...
be core tools. For custom or local-only tools, do **not** edit Lycus core.
Use the plugin route instead: create `~/.autolycus/plugins/<name>/plugin.yaml`
and `~/.autolycus/plugins/<name>/__init__.py`, then register tools with
`ctx.register_tool(...)`. Plugin toolsets are discovered automatically and can be
enabled or disabled without touching `tools/` or `toolsets.py`.

Use the built-in route below only when the user is explicitly contributing a new
core Lycus tool that should ship in the base system.
...
**2. Add to `toolsets.py`** — either `_LYCUS_CORE_TOOLS` (all platforms) or a new toolset. **This step is required:** auto-discovery imports the tool and registers its schema, but the tool is only *exposed to an agent* if its name appears in a toolset. `_LYCUS_CORE_TOOLS` is not dead code — it's the default bundle every platform's base toolset inherits from.

Auto-discovery: any `tools/*.py` file with a top-level `registry.register()` call is imported automatically — no manual import list to maintain. Wiring into a toolset is still a deliberate, manual step.

The registry handles schema collection, dispatch, availability checking, and error wrapping. All handlers MUST return a JSON string.

**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_lycus_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `AUTOLYCUS_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_lycus_home()` for the base directory — never `Path.home() / ".autolycus"`. This ensures each profile gets its own state.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `lycus_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `lycus_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `lycus tools`, `lycus setup`, most CLI subcommands | `lycus_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`lycus_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
lycus_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
~/.autolycus/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
- `default` — Classic Lycus gold/kawaii (the current look)
...
Add to `_BUILTIN_SKINS` dict in `lycus_cli/skin_engine.py`:
...
Users create `~/.autolycus/skins/<name>.yaml`:
...
Lycus has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.autolycus/plugins/` and pip-installed entry points.

### General plugins (`lycus_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.autolycus/plugins/`, `./.autolycus/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `lycus` at startup so
  `lycus <pluginname> <subcmd>` works with no change to `main.py`
...
`post_setup(lycus_home, config)` for setup-wizard integration.

**CLI commands via `plugins/memory/<name>/cli.py`:** if a memory plugin
defines `register_cli(subparser)`, `discover_plugin_cli_commands()` finds
it at argparse setup time and wires it into `lycus <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `lycus --help`.

**Rule (Teknium, May 2026):** plugins MUST NOT modify core files
(`run_agent.py`, `cli.py`, `gateway/run.py`, `lycus_cli/main.py`, etc.).
...
into `~/.autolycus/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `lycus memory setup` / `post_setup()` without
...
2. User: `$AUTOLYCUS_HOME/plugins/model-providers/<name>/`
...
[`lycus-example-plugins`](https://github.com/NousResearch/lycus-example-plugins)
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `lycus skills install official/<category>/<skill>`. Adapter lives in
...
`metadata.autolycus.tags`, `metadata.autolycus.category`,
`metadata.autolycus.related_skills`, `metadata.autolycus.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.autolycus.*` by the loader.
...
2. **Tools referenced in SKILL.md prose must be native Lycus tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "Lycus Agent" is the secondary collaborator. If the
   contributor's commit shows "Lycus Agent" as author (because they
   used Lycus to draft the skill), replace it with their actual name
...
lives in the `lycus-agent-dev` skill at
...
`"messaging"`); `_LYCUS_CORE_TOOLS` is the default bundle most
...
Enable/disable per platform via `lycus tools` (the curses UI) or the
...
go to `~/.autolycus/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `lycus_cli/curator.py` wires `lycus curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.autolycus/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `lycus cron <verb>`
...
- File lock at `~/.autolycus/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `lycus kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `lycus_cli/kanban.py` wires `lycus kanban` with verbs
...
- **Plugin assets:** `plugins/kanban/dashboard/` (web UI) +
  `plugins/kanban/systemd/` (`lycus-kanban-dispatcher.service` for
  standalone dispatcher deployment).

Isolation model:
- **Board** is the hard boundary — workers are spawned with
  `LYCUS_KANBAN_BOARD` pinned in their env so they can't see other
  boards.
...
Lycus-Agent ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
in config.yaml (or `LYCUS_BACKGROUND_NOTIFICATIONS` env var):
...
Lycus supports **profiles** — multiple fully isolated instances, each with its own
`AUTOLYCUS_HOME` directory (config, API keys, memory, sessions, skills, gateway, etc.).

The core mechanism: `_apply_profile_override()` in `lycus_cli/main.py` sets
`AUTOLYCUS_HOME` before any module imports. All `get_lycus_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_lycus_home()` for all AUTOLYCUS_HOME paths.** Import from `lycus_constants`.
   NEVER hardcode `~/.autolycus` or `Path.home() / ".autolycus"` in code that reads/writes state.
   ```python
   # GOOD
   ...
   # BAD — breaks profiles
   config_path = Path.home() / ".autolycus" / "config.yaml"
   ```

2. **Use `display_lycus_home()` for user-facing messages.** Import from `lycus_constants`.
   This returns `~/.autolycus` for default or `~/.autolycus/profiles/<name>` for profiles.
   ```python
   # GOOD
   from lycus_constants import display_lycus_home
   print(f"Config saved to {display_lycus_home()}/config.yaml")

   # BAD — shows wrong path for profiles
   print("Config saved to ~/.autolycus/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_lycus_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_lycus_home()`,
   not `Path.home() / ".autolycus"`.

4. **Tests that mock `Path.home()` must also set `AUTOLYCUS_HOME`** — since code now uses
   `get_lycus_home()` (reads env var), not `Path.home() / ".autolycus"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"AUTOLYCUS_HOME": str(tmp_path / ".autolycus")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not AUTOLYCUS_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".autolycus" / "profiles"`, NOT `get_lycus_home() / "profiles"`.
   This is intentional — it lets `lycus -p coder profile list` see all profiles regardless
   of which one is active.

## Known Pitfalls

### DO NOT hardcode `~/.autolycus` paths
Use `get_lycus_home()` from `lycus_constants` for code paths. Use `display_lycus_home()`
for user-facing print/log messages. Hardcoding `~/.autolycus` breaks profiles — each profile
has its own `AUTOLYCUS_HOME` directory. This was the source of 5 bugs fixed in PR #3575.

### DO NOT introduce new `simple_term_menu` usage
Existing call sites in `lycus_cli/main.py` remain for legacy fallback only;
the preferred UI is curses (stdlib) because `simple_term_menu` has
ghost-duplication rendering bugs in tmux/iTerm2 with arrow keys. New
interactive menus must use `lycus_cli/curses_ui.py` — see
`lycus_cli/tools_config.py` for the canonical pattern.
...
with actual imports (not mocks) against a temp `AUTOLYCUS_HOME`.

### Tests must not write to `~/.autolycus/`
The `_isolate_lycus_home` autouse fixture in `tests/conftest.py` redirects `AUTOLYCUS_HOME` to a temp dir. Never hardcode `~/.autolycus/` paths in tests.

**Profile tests**: When testing profile features, also mock `Path.home()` so that
`_get_profiles_root()` and `_get_default_lycus_home()` resolve within the temp dir.
Use the pattern from `tests/lycus_cli/test_profiles.py`:
```python
@pytest.fixture
...
- The plugin disables itself in child processes (sentinel envvar
  `LYCUS_ISOLATE_CHILD=1`), so there's no fork-bomb risk.
...
| HOME / `~/.autolycus/` | Your real config+auth.json | Temp dir per test |
