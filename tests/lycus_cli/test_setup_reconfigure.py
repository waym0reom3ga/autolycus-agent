...
- Bare `lycus setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `lycus setup --quick` runs the narrower "fill in missing items" flow.
- `lycus setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("lycus_cli.setup.ensure_lycus_home", {}),
        ("lycus_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("lycus_cli.config.is_managed", {"return_value": False}),
        ("lycus_cli.setup.load_config", {"return_value": {}}),
        ("lycus_cli.setup.save_config", {}),
        ("lycus_cli.setup.get_env_value", {"return_value": None}),
        ("lycus_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("lycus_cli.setup._print_setup_summary", {}),
        ("lycus_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("lycus_cli.setup.load_config", {"return_value": {}}),
        ("lycus_cli.setup.save_config", {}),
        ("lycus_cli.auth.get_active_provider", {"return_value": None}),
        ("lycus_cli.setup.get_env_value", {"return_value": None}),
        ("lycus_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `lycus setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="lycus_cli.setup.prompt_choice",
                quick="lycus_cli.setup._run_quick_setup",
                model="lycus_cli.setup.setup_model_provider",
                terminal="lycus_cli.setup.setup_terminal_backend",
                agent="lycus_cli.setup.setup_agent_settings",
                gateway="lycus_cli.setup.setup_gateway",
                tools="lycus_cli.setup.setup_tools",
            )
    ...
    def test_reconfigure_flag_is_backwards_compat_noop(self, existing_install):
        """`lycus setup --reconfigure` behaves the same as bare `lycus setup`."""
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("lycus_cli.setup.prompt_choice", {"return_value": 0}),
                first="lycus_cli.setup._run_first_time_quick_setup",
            )
            from lycus_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "lycus_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["lycus", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_quick_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["lycus", "setup", "--quick"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_bare_setup_has_both_flags_false(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["lycus", "setup"])
        try:
            main()
        except SystemExit:
            pass
