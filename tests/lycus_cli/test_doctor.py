...
import lycus_cli.doctor as doctor
import lycus_cli.gateway as gateway_cli
from lycus_cli import doctor as doctor_mod
from lycus_cli.doctor import _has_provider_env_config
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `lycus doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "AUTOLYCUS_HOME", lycus_home)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_lycus_home(self, tmp_path, provider=""):
        """Create a minimal AUTOLYCUS_HOME with config.yaml."""
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_lycus_home(tmp_path, provider)
...
def test_run_doctor_accepts_lycus_provider_ids_that_catalog_aliases(
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from lycus_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("lycus_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "lycus_cli.doctor", raising=False)
        from lycus_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
        assert _has_healthy_oauth_fallback_for_apikey_provider("xai") is False
...
class TestDoctorStaleMaxIterationsDrift:
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        (lycus_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "AUTOLYCUS_HOME", lycus_home)
        monkeypatch.setattr(doctor_mod, "get_lycus_home", lambda: lycus_home)
        ...
        return buf.getvalue(), lycus_home

    def test_detects_drift_warn_only(self, monkeypatch, tmp_path):
        out, lycus_home = self._run_config_section(
        ...
        assert "HERMES_MAX_ITERATIONS=90" in (lycus_home / ".env").read_text(encoding="utf-8")
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        env_after = (lycus_home / ".env").read_text(encoding="utf-8")
...
def test_npm_audit_fix_hint_avoids_crashing_workspace_flag(monkeypatch, tmp_path):
    """`lycus doctor` must not hand users `npm audit fix --workspace <name>`:
    ...
    # left thinking a crashing command means a broken Lycus install.
