"""Tests for lycus_cli.profile_distribution — git-based profile installs.
...
# ---------------------------------------------------------------------------
# Isolated profile env (matches tests/lycus_cli/test_profiles.py)
# ---------------------------------------------------------------------------
...
class TestManifestParsing:
    ...
    def test_full_manifest(self, tmp_path):
        ...
        assert m.distribution_owned == ["MASK.md", "skills"]
    ...
    def test_owned_paths_explicit(self):
        m = DistributionManifest(name="x", distribution_owned=["MASK.md", "skills"])
        assert m.owned_paths() == ["MASK.md", "skills"]
...
class TestVersionRequires:
    ...
    def test_check_matrix(self, spec, cur, ok):
        if ok:
            check_lycus_requires(spec, cur)
        else:
            with pytest.raises(DistributionError, match="requires Lycus"):
                check_lycus_requires(spec, cur)
...
class TestEnvTemplate:
    ...
    def test_empty_env_requires_is_header_only(self):
        m = DistributionManifest(name="x")
        out = _env_template_from_manifest(m)
        assert "Lycus distribution" in out
...
class TestInstall:
    ...
    def test_install_from_directory(self, profile_env):
        ...
        assert (plan.target_dir / "MASK.md").read_text() == "I am Source.\n"
    ...
    def test_install_enforces_lycus_requires(self, profile_env, monkeypatch):
        # Pin current Lycus version to something well below the requirement
        import lycus_cli
        monkeypatch.setattr(lycus_cli, "__version__", "0.1.0", raising=False)

        mf = DistributionManifest(
            name="future",
            version="1.0.0",
            lycus_requires=">=99.0.0",
        )
...
class TestUpdate:
    ...
    def test_update_preserves_user_data(self, profile_env):
        ...
        (staged / "MASK.md").write_text("I am Source v2.\n")

        # 4. Update
        update_distribution("telem", force_config=False)

        # 5. Dist-owned changed
        assert (plan.target_dir / "MASK.md").read_text() == "I am Source v2.\n"
...
class TestSecurity:
    ...
    def test_install_rejects_symlinked_distribution_files(self, profile_env, tmp_path):
        ...
        from lycus_cli.profiles import get_profile_dir
...
class TestInstalledAtStamp:
    ...
    def test_update_refreshes_installed_at(self, profile_env, monkeypatch):
        ...
        monkeypatch.setattr(
            "lycus_cli.profile_distribution.datetime", _FakeDT, raising=True
        )

        from lycus_cli.profile_distribution import update_distribution
...
class TestProfileInfoDistribution:
    ...
    def test_plain_profile_has_no_distribution_fields(self, profile_env):
        from lycus_cli.profiles import create_profile, list_profiles
        create_profile(name="plain", no_alias=True)
        rows = {p.name: p for p in list_profiles()}
    ...
    def test_malformed_manifest_does_not_break_list(self, profile_env):
        from lycus_cli.profiles import create_profile, list_profiles, get_profile_dir
