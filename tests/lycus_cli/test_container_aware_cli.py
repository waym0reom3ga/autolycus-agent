...
def container_env(tmp_path, monkeypatch):
    """Set up a fake AUTOLYCUS_HOME with .container-mode file."""
    lycus_home = tmp_path / ".autolycus"
    lycus_home.mkdir()
    monkeypatch.setenv("AUTOLYCUS_HOME", str(lycus_home))
    monkeypatch.delenv("HERMES_DEV", raising=False)

    container_mode = lycus_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=lycus-agent\n"
        "exec_user=lycus\n"
        "lycus_bin=/data/current-package/bin/lycus\n"
    )
    return lycus_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("lycus_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "lycus-agent"
    assert info["exec_user"] == "lycus"
    assert info["lycus_bin"] == "/data/current-package/bin/lycus"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("lycus_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_lycus_dev(container_env, monkeypatch):
...
def test_get_container_exec_info_not_skipped_when_lycus_dev_zero(container_env, monkeypatch):
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        lycus_home = Path(tmpdir) / ".autolycus"
        lycus_home.mkdir()
        (lycus_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("lycus_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_lycus_home": lambda: lycus_home}), \
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=lycus-custom\n"
        "exec_user=myuser\n"
        "lycus_bin=/opt/lycus/bin/lycus\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "lycus-custom"
    assert info["exec_user"] == "myuser"
    assert info["lycus_bin"] == "/opt/lycus/bin/lycus"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from lycus_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "lycus"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "lycus-agent" in cmd
    assert "/data/current-package/bin/lycus" in cmd
    assert "chat" in cmd
