"""Tests for `lycus memory setup [provider]` routing.
...
``lycus memory setup honcho``) without the interactive picker — which matters
because the per-provider ``lycus <provider>`` subcommand is only registered
...
from lycus_cli import memory_setup
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "lycus memory setup" in out
