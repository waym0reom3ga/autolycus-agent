"""Tests for the stale-dashboard handling run at the end of ``lycus update``.

``lycus update`` detects ``lycus dashboard`` processes left over from the
...
def _refresh_bindings_against_live_module():
    """Rebind module-level names to the *current* ``lycus_cli.main``.

    Other tests in the suite (notably ``test_env_loader.py`` and
    ``test_skills_subparser.py``) reload or delete ``lycus_cli.main`` from
    ``sys.modules``.  When that happens on the same xdist worker before we
    run, our top-of-file ``from lycus_cli.main import ...`` bindings end
    up pointing at the *old* module object.  ``patch(\"lycus_cli.main.X\")``
    ...
    live = sys.modules.get("lycus_cli.main")
    if live is None:
        live = importlib.import_module("lycus_cli.main")
...
class TestFindStaleDashboardPids:
    ...
    def test_matches_running_dashboard(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=_ps_line(12345, "python3 -m lycus_cli.main dashboard --port 9119") + "\n",
                stderr="",
            )
    ...
    def test_multiple_matches(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(12345, "python3 -m lycus_cli.main dashboard --port 9119"),
                    _ps_line(12346, "lycus dashboard --port 9120 --no-open"),
                    _ps_line(12347, "python /home/x/lycus_cli/main.py dashboard"),
                ]) + "\n",
                stderr="",
            )
    ...
    def test_self_pid_excluded(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(os.getpid(), "python3 -m lycus_cli.main dashboard"),
                    _ps_line(12345, "lycus dashboard --port 9119"),
                ]) + "\n",
                stderr="",
            )
    ...
    def test_unrelated_process_containing_word_dashboard_not_matched(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                ...
                stdout="\n".join([
                    _ps_line(12345, "python3 -m lycus_cli.main dashboard --port 9119"),
                    _ps_line(22222, "python3 -m lycus_cli.main chat -q 'rewrite my dashboard'"),
    ...
    def test_grep_lines_ignored(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(99999, "grep lycus dashboard"),
                    _ps_line(12345, "lycus dashboard --port 9119"),
                ]) + "\n",
                stderr="",
            )
    ...
    def test_exclude_pids_filters_specified_pids(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(11111, "lycus dashboard --port 9119"),
                    _ps_line(22222, "lycus dashboard --port 9120"),
                    _ps_line(33333, "lycus dashboard --port 9121"),
                ]) + "\n",
                stderr="",
            )
    ...
    def test_exclude_pids_none_is_noop(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=_ps_line(12345, "lycus dashboard --port 9119") + "\n",
                stderr="",
            )
...
class TestKillStaleDashboardPosix:
    ...
    def test_no_stale_processes_is_a_noop(self, capsys):
        with patch("lycus_cli.main._find_stale_dashboard_pids", return_value=[]):
            _kill_stale_dashboard_processes()
    ...
    def test_sigterm_graceful_exit(self, capsys):
        ...
        with patch("lycus_cli.main._find_stale_dashboard_pids",
    ...
    def test_permission_error_is_reported_not_raised(self, capsys):
        ...
        must not abort lycus update — it's reported as a failure and we
        move on."""
        def fake_kill(pid, sig):
...
class TestWindowsWmicEncoding:
    ...
    `lycus update` on non-UTF-8 system locales (e.g. cp936 on zh-CN).
    """
    ...
    def test_wmic_invoked_with_utf8_ignore_errors(self, monkeypatch):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=(
                    "CommandLine=python -m lycus_cli.main dashboard\n"
                    "ProcessId=12345\n"
                ),
                stderr="",
            )
    ...
    def test_wmic_returns_none_stdout_does_not_crash(self, monkeypatch):
        ...
        ``None.split('\\n')`` and aborting `lycus update` (#17049)."""
        monkeypatch.setattr(sys, "platform", "win32")
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
