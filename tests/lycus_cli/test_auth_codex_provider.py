"""Tests for Codex auth — tokens stored in Lycus auth store (~/.autolycus/auth.json)."""
...
def _setup_lycus_auth(lycus_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Lycus auth store."""
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_lycus_auth(lycus_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("AUTOLYCUS_HOME", str(lycus_home))
    ...
    monkeypatch.setattr("lycus_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    lycus_home = tmp_path / "lycus"
    _setup_lycus_auth(lycus_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("AUTOLYCUS_HOME", str(lycus_home))
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((lycus_home / "auth.json").read_text())
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``lycus auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``lycus auth add openai-codex``.
    ...
    (lycus_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # Independent account from `lycus auth add openai-codex` —
...
def test_save_codex_tokens_does_not_overwrite_independent_manual_entries(tmp_path, monkeypatch):
    ...
    Regression for #39236: ``lycus auth add openai-codex`` for accounts B and C
    ...
    (lycus_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # ``lycus auth add openai-codex`` (account B and account C).
...
def test_save_codex_tokens_still_refreshes_legacy_manual_alias(tmp_path, monkeypatch):
    ...
    ``lycus auth add openai-codex`` as a workaround when there was no
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Lycus auth store, not ~/.codex/."""
    lycus_home = tmp_path / "lycus"
    codex_home = tmp_path / "codex-cli"
    lycus_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "lycus-at", "refresh_token": "lycus-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Lycus store
    assert not (codex_home / "auth.json").exists()

    # Lycus auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "lycus-at"


def test_resolve_returns_lycus_auth_store_source(tmp_path, monkeypatch):
    lycus_home = tmp_path / "lycus"
    _setup_lycus_auth(lycus_home)
    monkeypatch.setenv("AUTOLYCUS_HOME", str(lycus_home))
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run lycus auth".
    """
    from lycus_cli.auth import (
    ...
    assert "lycus auth" not in rendered
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from lycus_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from lycus_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr("lycus_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("lycus_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
