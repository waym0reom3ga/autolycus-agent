"""Tests for the Kanban DB layer (lycus_cli.kanban_db)."""
...
def kanban_home(tmp_path, monkeypatch):
    """Isolated AUTOLYCUS_HOME with an empty kanban DB."""
...
def test_stale_claim_reclaimed(kanban_home, monkeypatch):
    import signal
    import lycus_cli.kanban_db as _kb
...
def test_dispatch_skips_nonspawnable_into_separate_bucket(kanban_home, monkeypatch):
    ...
    from lycus_cli import profiles
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real Lycus profile — preserves the
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="lycus-task", assignee="daily")
...
def test_scratch_workspace_created_under_lycus_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
...
def test_cleanup_workspace_removes_managed_scratch_dir(kanban_home):
    ...
    with kb.connect() as conn:
        ...
        assert ws.is_dir()
        kb.complete_task(conn, t, result="ok")
    assert not ws.exists(), "Lycus-managed scratch dir should be cleaned up"
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """Lycus' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` Lycus' DB/metadata/logs on task completion.
    """
...
# spawned with `lycus -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's AUTOLYCUS_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's AUTOLYCUS_HOME."""

    def _set_home(self, monkeypatch, tmp_path, lycus_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("AUTOLYCUS_HOME", str(lycus_home))
        monkeypatch.delenv("HERMES_KANBAN_HOME", raising=False)

    def test_default_install_anchors_at_home_dot_lycus(
        self, tmp_path, monkeypatch
    ):
        # Standard install: AUTOLYCUS_HOME == ~/.autolycus, no profile active.
        default_home = tmp_path / ".autolycus"
        default_home.mkdir()
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.autolycus/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.autolycus/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.autolycus/kanban.db.
        default_home = tmp_path / ".autolycus"
        default_home.mkdir()
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # AUTOLYCUS_HOME and confirm equality. This is the property the
        ...
        # Worker's perspective (profile activated by `lycus -p coder`).
    ...
    def test_docker_custom_lycus_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
        # Docker / custom deployment: AUTOLYCUS_HOME points outside ~/.autolycus.
        # `get_default_lycus_root()` returns env_home directly when it
        # is not a `<root>/profiles/<name>` shape and not under
        # `Path.home() / ".autolycus"`.
        custom_root = tmp_path / "opt" / "lycus"
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: AUTOLYCUS_HOME=/opt/lycus/profiles/coder;
        # `get_default_lycus_root()` walks up to /opt/lycus because
    ...
    def test_explicit_override_via_lycus_kanban_home(
        self, tmp_path, monkeypatch
    ):
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        default_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("AUTOLYCUS_HOME", str(default_home))
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # AUTOLYCUS_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile AUTOLYCUS_HOME and reads.
        monkeypatch.setenv("AUTOLYCUS_HOME", str(profile_home))
        with kb.connect() as conn:
    ...
    def test_lycus_kanban_db_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # HERMES_KANBAN_DB pins the file path directly and beats both
        # HERMES_KANBAN_HOME and the `get_default_lycus_root()` path.
    ...
    def test_lycus_kanban_workspaces_root_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # `-p <profile>` flag rewrites AUTOLYCUS_HOME.
        default_home = tmp_path / ".autolycus"
        default_home.mkdir()
        self._set_home(monkeypatch, tmp_path, default_home)
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see lycus_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    with _patch("lycus_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="lycus_state"):
            conn = kb.connect()
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'lycus kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_lycus_argv()
#
# Workers spawned by the dispatcher must use a `lycus` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["lycus", ...]`
...
def test_resolve_lycus_argv_prefers_path_shim(monkeypatch):
    """When `lycus` is on PATH, use the shim — preserves familiar ps output."""
    import shutil
    import lycus_cli.kanban_db as kb

    monkeypatch.delenv("HERMES_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: "/usr/local/bin/lycus")
    argv = kb._resolve_lycus_argv()
    assert argv == ["/usr/local/bin/lycus"]


def test_resolve_lycus_argv_absolutizes_relative_exe_shim(monkeypatch, tmp_path):
    """A relative executable override must not remain workspace-cwd-dependent."""
    import lycus_cli.kanban_db as kb

    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("HERMES_BIN", ".\\lycus.exe")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_lycus_argv() == [os.path.abspath(".\\lycus.exe")]


def test_resolve_lycus_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    bin_dir.mkdir()
    (bin_dir / "lycus.CMD").write_text("@echo off\n", encoding="utf-8")
    monkeypatch.delenv("HERMES_BIN", raising=False)
    monkeypatch.setenv("PATH", str(bin_dir))
    monkeypatch.setenv("PATHEXT", ".CMD")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_lycus_argv() == [sys.executable, "-m", "lycus_cli.main"]


def test_resolve_lycus_argv_honors_lycus_bin_path_override(monkeypatch, tmp_path):
    """An explicit path-like HERMES_BIN lets service managers pin the executable."""
    import shutil
    import lycus_cli.kanban_db as kb

    shim = tmp_path / "bin" / "lycus"
    shim.parent.mkdir()
    shim.write_text("#!/bin/sh\n", encoding="utf-8")
    monkeypatch.setenv("HERMES_BIN", str(shim))
    monkeypatch.setattr(shutil, "which", lambda name: None)

    assert kb._resolve_lycus_argv() == [str(shim)]


def test_resolve_lycus_argv_lycus_bin_bare_name_uses_path(monkeypatch, tmp_path):
    """Bare HERMES_BIN values keep PATH semantics instead of cwd shadowing."""
    import stat
    import lycus_cli.kanban_db as kb

    cwd_lycus = tmp_path / "lycus"
    cwd_lycus.write_text("wrong\n", encoding="utf-8")
    cwd_lycus.chmod(cwd_lycus.stat().st_mode | stat.S_IXUSR)
    path_lycus = tmp_path / "bin" / "lycus"
    path_lycus.parent.mkdir()
    path_lycus.write_text("right\n", encoding="utf-8")
    path_lycus.chmod(path_lycus.stat().st_mode | stat.S_IXUSR)
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("PATH", str(path_lycus.parent))
    monkeypatch.setenv("HERMES_BIN", "lycus")

    assert kb._resolve_lycus_argv() == [str(path_lycus)]


def test_resolve_lycus_argv_lycus_bin_bare_name_ignores_cwd(monkeypatch, tmp_path):
    """Bare HERMES_BIN does not accept current-directory shadow executables."""
    import sys
    import lycus_cli.kanban_db as kb

    (tmp_path / "lycus.exe").write_text("wrong\n", encoding="utf-8")
    monkeypatch.chdir(tmp_path)
...
def test_resolve_lycus_argv_lycus_bin_bare_cmd_uses_module_fallback(monkeypatch, tmp_path):
...
def test_resolve_lycus_argv_lycus_bin_unresolved_bare_name_falls_back(monkeypatch):
...
def test_resolve_lycus_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m lycus_cli.main`.

    Pins the correct module name (NOT `lycus` — there is no top-level
    `lycus` package). Regression for #23198: the original PR shipped
    `python -m lycus` which fails with `No module named lycus` on every
    invocation.
    """
    import shutil
    import sys
    ...
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kb._resolve_lycus_argv()
    assert argv == [sys.executable, "-m", "lycus_cli.main"]


def test_resolve_lycus_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `lycus_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m lycus_cli.main --version`
    ...
    assert "Lycus Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="lycus_cli.kanban_db"):
        with kb.connect() as conn:
...
def test_write_txn_healthy_commit_no_exception(tmp_path):
    ...
    from lycus_cli.kanban_db import connect, write_txn
    db = tmp_path / "test.db"
...
def test_write_txn_raises_on_truncated_file(tmp_path):
    ...
    with pytest.raises(sqlite3.DatabaseError, match="torn-extend|page count mismatch"):
        with unittest.mock.patch("lycus_cli.kanban_db.os.path.getsize", side_effect=fake_getsize):
            with write_txn(conn) as c:
                c.execute(
                    "INSERT INTO tasks (id, title, assignee, status, priority, created_at) "
...
def test_write_txn_post_commit_check_fires_every_call(tmp_path):
    ...
    from lycus_cli.kanban_db import connect, write_txn
    import lycus_cli.kanban_db as kanban_db_module
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
    call_count = 0
...
def test_connect_sets_wal_autocheckpoint_100(tmp_path):
    ...
    from lycus_cli.kanban_db import connect
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    import struct
    from lycus_cli.kanban_db import connect, _check_file_length_invariant
...
def test_reap_worker_zombies_returns_count():
    ...
    with patch("lycus_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("lycus_cli.kanban_db._record_worker_exit"):
...
def test_reap_worker_zombies_noop_on_windows(monkeypatch):
    ...
    monkeypatch.setattr("lycus_cli.kanban_db.os.name", "nt")
    with patch("lycus_cli.kanban_db.os.waitpid") as mock_waitpid:
...
def test_reap_worker_zombies_noop_no_children():
    ...
    with patch("lycus_cli.kanban_db.os.waitpid", side_effect=ChildProcessError):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_reap_worker_zombies_records_exit_status():
    ...
    with patch("lycus_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch(
            "lycus_cli.kanban_db._record_worker_exit",
...
def test_reap_worker_zombies_handles_waitpid_os_error():
    ...
    with patch("lycus_cli.kanban_db.os.waitpid", side_effect=OSError("test error")):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_zombie_reaper_survives_all_boards_failing():
    ...
    for tick in range(5):
        ...
        with patch(
            "lycus_cli.kanban_db.os.waitpid", side_effect=make_fake_waitpid(pids)
        ):
            with patch("lycus_cli.kanban_db._record_worker_exit"):
                pids = kb.reap_worker_zombies()
...
def test_dispatch_once_still_reaps_via_extracted_fn(kanban_home):
    ...
    with patch("lycus_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("lycus_cli.kanban_db._record_worker_exit"):
            with patch("lycus_cli.kanban_db.os.name", "posix"):
                pids = kb.reap_worker_zombies()
