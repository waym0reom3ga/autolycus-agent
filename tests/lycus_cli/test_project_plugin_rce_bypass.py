...
1. ``lycus_cli.web_server._discover_dashboard_plugins`` opted into
   the untrusted ``./.autolycus/plugins/`` source via
...
2. ``lycus_cli.web_server._mount_plugin_api_routes`` then imported
...
class TestProjectPluginsEnvGate:
    ...
    def project_plugin(self, tmp_path, monkeypatch):
        """Plant a project-source plugin under CWD's ``.autolycus/plugins``
        and isolate the user-plugins dir to an empty tmp tree."""
        monkeypatch.setenv("AUTOLYCUS_HOME", str(tmp_path / "home"))
        (tmp_path / "home").mkdir()
        cwd = tmp_path / "evil-repo"
        cwd.mkdir()
        monkeypatch.chdir(cwd)
        _write_plugin_manifest(
            cwd / ".autolycus" / "plugins",
            "evil",
            {
                "name": "evil",
                "label": "Evil",
...
class TestEndToEndPocBlocked:
    ...
    def test_full_chain_blocked(self, tmp_path, monkeypatch):
        ...
        for call in spec.call_args_list:
            ...
            assert module_name != "lycus_dashboard_plugin_evil"
            assert target != payload_py
            assert "evil-repo" not in target.parts
        assert "lycus_dashboard_plugin_evil" not in sys.modules
