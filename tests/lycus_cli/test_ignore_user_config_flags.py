"""Tests for --ignore-user-config and --ignore-rules flags on `lycus chat`.
...
files. In Lycus the equivalent isolation is:
...
argparse → cmd_chat → cli.main() → LycusCLI → AIAgent call chain.
"""
...
class TestIgnoreUserConfigEnvGate:
    ...
    When the env var is set, user config at ``<lycus_home>/config.yaml`` is
    ...
    def _reload_cli(self, monkeypatch, tmp_path):
        """Point cli._lycus_home at tmp_path and return a fresh load_cli_config."""
        import cli
        monkeypatch.setattr(cli, "_lycus_home", tmp_path)
...
class TestIgnoreRulesEnvGate:
    """The constructor / env var must propagate to ``LycusCLI.ignore_rules``
    so ``AIAgent`` is built with ``skip_context_files=True`` and
    ``skip_memory=True``.
    """

    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting HERMES_IGNORE_RULES=1 flips LycusCLI.ignore_rules True."""
        monkeypatch.setenv("HERMES_IGNORE_RULES", "1")

        # Import LycusCLI lazily — cli.py has heavy module-init side effects
        # that we don't want to run at test collection time.
        import cli
        importlib.reload(cli)

        # Build only enough of LycusCLI to reach the ignore_rules assignment.
        ...
        obj = object.__new__(cli.LycusCLI)
        # Replicate the exact logic from cli.py LycusCLI.__init__:
...
class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``lycus_cli/main.py`` must set
...
class TestArgparseFlagsRegistered:
    ...
    def test_main_py_registers_both_flags(self):
        """E2E: the real lycus parser accepts both flags."""
