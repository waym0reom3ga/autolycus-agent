"""Tests for the Phase 4 s6 dispatch helper in lycus_cli.gateway.

`_dispatch_via_service_manager_if_s6` decides whether a
`lycus gateway start/stop/restart` invocation should be routed to
...
def test_dispatch_returns_false_on_host(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from lycus_cli import gateway as gw
    monkeypatch.setattr(
        "lycus_cli.service_manager.detect_service_manager", lambda: "systemd",
    )
    # Should not even attempt to construct a manager.
    monkeypatch.setattr(
        "lycus_cli.service_manager.get_service_manager",
...
def test_dispatch_returns_true_and_calls_start_on_s6(
    ...
    monkeypatch.setattr(
        "lycus_cli.service_manager.get_service_manager", lambda: rec,
    )
...
def test_dispatch_defaults_profile_to_default(
    ...
    monkeypatch.setattr(
        "lycus_cli.gateway._profile_suffix", lambda: "",
    )
...
def test_dispatch_renders_gateway_not_registered_friendly(
    ...
    """`lycus -p typo gateway start` should print a clear message and
    exit 1 — not dump a traceback at the user."""
    from lycus_cli import gateway as gw
    from lycus_cli.service_manager import GatewayNotRegisteredError
    ...
    monkeypatch.setattr(
        "lycus_cli.service_manager.get_service_manager", lambda: _RaisesMissing(),
    )
    ...
    assert "lycus profile create typo" in out
...
def test_dispatch_renders_s6_command_error_friendly(
    ...
    monkeypatch.setattr(
        "lycus_cli.service_manager.get_service_manager", lambda: _RaisesS6Error(),
    )
...
def _stub_s6(monkeypatch: pytest.MonkeyPatch, *, on_s6: bool) -> _CallRecorder:
    ...
    rec = _CallRecorder()
    monkeypatch.setattr(
        "lycus_cli.service_manager.detect_service_manager",
...
def test_redirect_fires_inside_s6_container(
    ...
    monkeypatch.setattr("lycus_cli.gateway._profile_suffix", lambda: "")
    ...
    monkeypatch.setattr("lycus_cli.gateway.os.execvp", fake_execvp)
    # If the fallback ran, the normal sleep path was wrongly skipped.
    monkeypatch.setattr(
        "lycus_cli.gateway._block_until_terminated",
...
def test_redirect_falls_back_when_sleep_missing(
    ...
    monkeypatch.setattr("lycus_cli.gateway.os.execvp", missing_sleep)
...
def test_block_until_terminated_installs_sigterm_handler_and_blocks(
    ...
    monkeypatch.setattr(
        "lycus_cli.gateway.signal.signal",
    ...
    monkeypatch.setattr("lycus_cli.gateway.signal.pause", fake_pause)
...
def test_redirect_short_circuits_supervised_child(
    ...
    running execs `lycus gateway run --replace`, the
