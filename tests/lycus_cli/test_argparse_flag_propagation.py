...
subparser, placing the flag BEFORE the subcommand (e.g. 'lycus --yolo chat')
...
def _build_parser():
    """Build the lycus argument parser from the real code.
...
class TestChatVerboseArg:
    ...
    def test_cmd_chat_forwards_none_when_verbose_is_absent(self, monkeypatch):
        ...
        fake_banner = types.ModuleType("lycus_cli.banner")
        ...
        monkeypatch.setitem(sys.modules, "lycus_cli.banner", fake_banner)
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `lycus gateway run --accept-hooks` failed
    ...
    def test_accepted_at_every_position(self, argv):
        """Invoking `lycus <argv>` must exit 0 (help) rather than
