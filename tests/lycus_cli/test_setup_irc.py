"""Tests for IRC gateway configuration via `lycus setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Lycus install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Lycus install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#lycus")
            monkeypatch.setenv("IRC_NICKNAME", "lycus-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `lycus setup gateway` experience with IRC present."""
