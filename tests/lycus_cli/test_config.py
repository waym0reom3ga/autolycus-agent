"""Tests for lycus_cli configuration management."""
...
class TestGetLycusHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            home = get_lycus_home()
            assert home == Path.home() / ".autolycus"

    def test_env_override(self):
        with patch.dict(os.environ, {"AUTOLYCUS_HOME": "/custom/path"}):
            home = get_lycus_home()
            assert home == Path("/custom/path")


class TestEnsureLycusHome:
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``lycus logs`` surfaces it)
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        from lycus_cli import config as cfg_mod
        ...
        with patch.dict(os.environ, {"AUTOLYCUS_HOME": str(tmp_path)}):
            ...
            import logging
            with caplog.at_level(logging.WARNING, logger="lycus_cli.config"):
                config = load_config()
            ...
            # stands out at lycus startup before logging is configured)
            captured = capsys.readouterr()
            assert "lycus config:" in captured.err
    ...
    def test_rewarns_after_file_edit(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"AUTOLYCUS_HOME": str(tmp_path)}):
            ...
            assert "lycus config:" in after_edit, "edited file should re-warn"
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"AUTOLYCUS_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"AUTOLYCUS_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"AUTOLYCUS_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"AUTOLYCUS_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"AUTOLYCUS_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, {"AUTOLYCUS_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from lycus_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        ...
        Offering it as an editable env var (dashboard, `lycus setup`) lets a
...
class TestMemoryProviderEnvVarsRegistry:
    ...
    and `lycus setup` surface it (previously only Honcho was listed, leaving
...
class TestEnvWriteDenylist:
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Lycus
    process on next startup via the dotenv → ``os.environ`` chain in
    ``lycus_cli/env_loader.py``.
    ...
    def _lycus_home(self, tmp_path, monkeypatch):
    ...
    def test_lycus_integration_keys_still_writable(self, allowed_key):
