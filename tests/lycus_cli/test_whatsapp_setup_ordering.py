...
Before the fix, ``lycus whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``lycus gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    lycus = home / ".autolycus"
    lycus.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("AUTOLYCUS_HOME", str(lycus))
...
def _env_value(lycus_home: Path, key: str) -> str | None:
    env_file = lycus_home / ".env"
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    monkeypatch.setattr("lycus_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``lycus whatsapp`` with an existing paired session and
