...
class TestUnifiedDashboardRouting:
    def test_profile_launch_attaches_to_running_dashboard(self, main_mod, monkeypatch):
        monkeypatch.setattr(
            "lycus_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        # AUTOLYCUS_HOME.  For a standard install (AUTOLYCUS_HOME unset) that root is
        # the platform-native default (~/.autolycus), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from lycus_constants import get_default_lycus_root
        assert env.get("AUTOLYCUS_HOME") == str(get_default_lycus_root())

    def test_reexec_pins_docker_machine_root(self, main_mod, monkeypatch):
        """In the Docker layout (AUTOLYCUS_HOME=/opt/data, profiles under
        /opt/data/profiles/<name>) the reroute must pin the child to the
        machine root /opt/data — NOT drop AUTOLYCUS_HOME.

        Dropping it makes the child fall back to $HOME/.autolycus
        (= /opt/data/.autolycus), an empty auto-seeded home, so the dashboard
        ...
        """
        monkeypatch.setenv("AUTOLYCUS_HOME", "/opt/data/profiles/oracle")
        monkeypatch.setattr(
            "lycus_cli.profiles.get_active_profile_name", lambda: "oracle"
        )
        monkeypatch.setattr(main_mod, "_dashboard_listening", lambda host, port: False)
        execs = []
    ...
    def test_default_profile_launch_skips_routing(self, main_mod, monkeypatch):
        monkeypatch.setattr(
            "lycus_cli.profiles.get_active_profile_name", lambda: "default"
        )
        listening_calls = []
        monkeypatch.setattr(
            main_mod, "_dashboard_listening",
    ...
    def test_dashboard_starts_mcp_discovery_for_ws_backend(self, main_mod, monkeypatch):
        ...
        calls = []
        monkeypatch.setattr(
            "lycus_cli.mcp_startup.start_background_mcp_discovery",
            lambda **kwargs: calls.append(kwargs),
        )
        monkeypatch.setitem(
            sys.modules,
            "lycus_cli.web_server",
