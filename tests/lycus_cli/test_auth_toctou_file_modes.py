"""Regression tests for TOCTOU-safe credential file writers in ``lycus_cli.auth``.
...
# ---------------------------------------------------------------------------
# _save_auth_store  (~/.autolycus/auth.json — every native OAuth provider)
# ---------------------------------------------------------------------------
...
def test_shared_nous_store_writes_0o600_with_0o700_parent(tmp_path, monkeypatch):
    ...
    # home" reference — which currently tracks AUTOLYCUS_HOME via
    # get_default_lycus_root() — can't collide with our override and
