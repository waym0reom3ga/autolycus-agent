...
The terminal `lycus model` flow already exposes `custom_providers`, but the
shared slash-command pipeline (`/model` in CLI/gateway/Telegram) historically
only looked at `providers:`.
"""

import lycus_cli.providers as providers_mod
...
def test_switch_model_accepts_explicit_bare_custom_current_endpoint(monkeypatch):
    ...
    monkeypatch.setattr("lycus_cli.models.validate_requested_model", lambda *a, **k: _MOCK_VALIDATION)
    monkeypatch.setattr("lycus_cli.model_switch.get_model_info", lambda *a, **k: None)
    monkeypatch.setattr("lycus_cli.model_switch.get_model_capabilities", lambda *a, **k: None)
...
def test_list_enumerates_dict_format_models_alongside_default(monkeypatch):
    ...
    Regression: Lycus's own writer stores configured models as a dict
...
def test_lmstudio_picker_probes_active_config_base_url(monkeypatch):
    ...
    monkeypatch.setattr("lycus_cli.models.fetch_lmstudio_models", _fake_fetch)
