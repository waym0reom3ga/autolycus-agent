"""Regression tests for #34107 — Docker UID/GID handling in ensure_lycus_home.

When Lycus runs in Docker with ``HERMES_UID=1000`` / ``HERMES_GID=911``,
the entrypoint chowns the top-level ``AUTOLYCUS_HOME`` once at startup. But
...
The fix is a ``_chown_to_lycus_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_lycus_uid_gid
# ---------------------------------------------------------------------------


class TestResolveLycusUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        ...
        from lycus_cli.config import _resolve_lycus_uid_gid
...
class TestChownToLycusUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_lycus_uid(d)  # must not raise
