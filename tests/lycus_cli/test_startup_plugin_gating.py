...
``lycus_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``lycus --help``, ``lycus version``,
``lycus logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``lycus --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from lycus_cli import main as _main
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["lycus", "-w", "--tui"], None),
        (["lycus", "version"], "version"),
        (["lycus", "--tui", "chat"], "chat"),
        (["lycus", "-w", "logs"], "logs"),
        (["lycus", "chat", "hello world"], "chat"),
        (["lycus", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["lycus", "-m", "gpt5", "chat"], "chat"),
        (["lycus", "--model", "gpt5", "chat", "hi"], "chat"),
        (["lycus", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["lycus", "-z", "hello world"], None),
        (["lycus", "-z", "hello", "chat"], "chat"),
        (["lycus", "--model=gpt5", "chat"], "chat"),     # inline form
        (["lycus", "--", "chat"], "chat"),               # -- terminator
        (["lycus", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["lycus", "some-plugin-cmd"], "some-plugin-cmd"),
        (["lycus", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["lycus"],                          # bare → chat
        ["lycus", "--help"],                # top-level help
        ["lycus", "-h"],
        ["lycus", "version"],               # known built-in
        ["lycus", "logs"],
        ["lycus", "gateway", "run"],
        ["lycus", "--tui"],
        ["lycus", "-w", "--tui"],
        ["lycus", "chat", "hi"],
        ["lycus", "help"],                  # accepted built-in-ish
        ["lycus", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["lycus", "meet", "join"],          # potential google_meet plugin
        ["lycus", "honcho", "status"],      # potential memory plugin
        ["lycus", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``lycus help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"lycus_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
