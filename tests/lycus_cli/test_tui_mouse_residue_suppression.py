...
The Python launcher (`lycus --tui …`) has a ~100–300ms cold-start window
where stdin is still in cooked + echo mode. If a previous Lycus session
...
from lycus_cli.main import _suppress_mouse_residue_early
...
class TestEarlyMouseDisable:
    def test_writes_disable_sequence_when_tui_flag_in_argv(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["lycus", "--tui", "-c", "abc"])
    ...
    def test_writes_disable_sequence_when_lycus_tui_env_set(self, monkeypatch):
    ...
    def test_skips_when_stdout_is_not_a_tty(self, monkeypatch):
        # `lycus --tui … >log` or CI capture: pipe is fd 1, not a TTY. The
