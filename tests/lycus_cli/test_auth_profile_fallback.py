...
When ``AUTOLYCUS_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.autolycus/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.autolycus            (has its own auth.json fixture)
    * Profile     -> tmp_path/.autolycus/profiles/coder   (active, AUTOLYCUS_HOME points here)
    ...
    global_root = tmp_path / ".autolycus"
    global_root.mkdir()
...
# ``_load_provider_state`` itself, those helpers raise ``"Lycus is not
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from lycus_cli.auth import _load_auth_store, _load_provider_state
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    lycus_home = tmp_path / "classic"
    lycus_home.mkdir()
    monkeypatch.setenv("AUTOLYCUS_HOME", str(lycus_home))

    _write(lycus_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (AUTOLYCUS_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.autolycus as the "real home" — which is NOT equal
    # to the AUTOLYCUS_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    lycus_home = tmp_path / "classic"
    ...
    _write(lycus_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from lycus_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: AUTOLYCUS_HOME is set to a custom path that is NOT under
    # ~/.autolycus/profiles/ — get_default_lycus_root() returns AUTOLYCUS_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from lycus_cli.auth import read_credential_pool, write_credential_pool
