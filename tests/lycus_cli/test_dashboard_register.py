"""Tests for ``lycus dashboard register``.
...
import lycus_cli.dashboard_register as dr
...
class TestFastFails:
    def test_not_logged_in_exits_1_with_setup_hint(self, capsys):
        ...
        with patch.object(dr, "cmd_dashboard_register", dr.cmd_dashboard_register):
            with patch(
                "lycus_cli.auth.resolve_nous_access_token", side_effect=err
            ), patch("lycus_cli.config.is_managed", return_value=False):
                with pytest.raises(SystemExit) as exc:
                    dr.cmd_dashboard_register(_ns())
        assert exc.value.code == 1
        out = capsys.readouterr().out
        assert "not logged into Nous Portal" in out
        assert "lycus setup" in out
...
class TestHappyPath:
    def _run(self, *, args, account_token="tok_abc", portal="https://portal.nousresearch.com",
        ...
        with patch(
            "lycus_cli.auth.resolve_nous_access_token", return_value=account_token
        ), patch("lycus_cli.config.is_managed", return_value=False), patch.object(
            dr, "_resolve_portal_base_url", return_value=portal
        ), patch(
            "lycus_cli.config.get_env_value", side_effect=fake_get_env
        ), patch(
            "lycus_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
    ...
    def test_custom_redirect_uri_is_forwarded(self, capsys):
        captured: dict = {}
        self._run(
            args=_ns(redirect_uri="https://lycus.example.com/auth/callback"),
            captured=captured,
        )
        assert (
            captured["body"]["custom_redirect_uri"]
            == "https://lycus.example.com/auth/callback"
        )
...
class TestCustomPortalPersistence:
    ...
    def _run(self, *, args, portal, existing_portal):
        ...
        with patch(
            "lycus_cli.auth.resolve_nous_access_token", return_value="tok"
        ), patch("lycus_cli.config.is_managed", return_value=False), patch.dict(
            dr.os.environ, {}, clear=False
        ), patch.object(
            dr, "_resolve_portal_base_url", return_value=portal
        ), patch(
            "lycus_cli.config.get_env_value", side_effect=fake_get_env_value
        ), patch(
            "lycus_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
...
class TestPublicUrlPersistence:
    ...
    https://lycus.example.com/auth/callback). At serve time the dashboard auth
    ...
    def test_origin_derived_from_full_callback_path(self, capsys):
        ...
        assert saved["HERMES_DASHBOARD_PUBLIC_URL"] == "https://lycus.example.com"
    ...
    def test_origin_preserves_port(self, capsys):
        saved = self._run(
            args=_ns(redirect_uri="https://lycus.example.com:8443/auth/callback"),
            existing_public=None,
        )
        assert saved["HERMES_DASHBOARD_PUBLIC_URL"] == "https://lycus.example.com:8443"
    ...
    def test_public_url_equal_to_existing_is_noop(self, capsys):
        ...
        saved = self._run(
            args=_ns(redirect_uri="https://lycus.example.com/auth/callback"),
            existing_public="https://lycus.example.com",
        )
    ...
    def test_public_url_persisted_alongside_portal_url(self, capsys):
        ...
        response = {
            ...
            "name": "dreamy_tesla",
            "kind": "SELF_HOSTED",
            "custom_redirect_uri": "https://lycus.example.com/auth/callback",
            "created_at": "2026-06-04T12:00:00.000Z",
        }
        saved: dict = {}
        ...
        with patch(
            ...
            "lycus_cli.config.get_env_value", return_value=None
        ), patch(
            "lycus_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
            dr.urllib.request, "urlopen", return_value=_fake_http_ok(response)
            ...
            dr.cmd_dashboard_register(
                _ns(
                    ...
                    redirect_uri="https://lycus.example.com/auth/callback",
                )
            )
        assert saved["HERMES_DASHBOARD_PORTAL_URL"] == "https://preview.example.com"
        assert saved["HERMES_DASHBOARD_PUBLIC_URL"] == "https://lycus.example.com"
