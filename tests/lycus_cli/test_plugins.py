"""Tests for the Lycus plugin system (lycus_cli.plugins)."""
...
from lycus_cli.middleware import (
    VALID_MIDDLEWARE,
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<lycus_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<lycus_home>/plugins/``; we derive
    ``<lycus_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <AUTOLYCUS_HOME>/config.yaml.
        # Config is always read from AUTOLYCUS_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        lycus_home_str = os.environ.get("AUTOLYCUS_HOME")
        if lycus_home_str:
            lycus_home = Path(lycus_home_str)
        else:
            lycus_home = base.parent
        lycus_home.mkdir(parents=True, exist_ok=True)
        cfg_path = lycus_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.autolycus/plugins/ are discovered."""
        plugins_dir = tmp_path / "lycus_test" / "plugins"
    ...
    def test_execution_middleware_does_not_retry_downstream_failure(self, monkeypatch):
        ...
        monkeypatch.setattr("lycus_cli.plugins.get_plugin_manager", lambda: manager)
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        plugins_dir = project_dir / ".autolycus" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_missing_init(self, tmp_path, monkeypatch):
        ...
        # missing-init error.
        lycus_home = tmp_path / "lycus_test"
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under lycus_plugins.<name>."""
        plugins_dir = tmp_path / "lycus_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("AUTOLYCUS_HOME", str(tmp_path / "lycus_test"))

        # Clean up any prior namespace module
        sys.modules.pop("lycus_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "lycus_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="lycus_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import lycus_cli.plugins as plugins_mod
...
class TestPluginManagerList:
    ...
    def test_shared_hook_name_credited_to_every_plugin(self, tmp_path, monkeypatch):
        ...
        `lycus plugins list`. Attribution now counts what each plugin's own
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("lycus_cli.plugins._plugin_manager", mgr):
    ...
    def test_get_plugin_context_engine_discovers_plugins_lazily(self, tmp_path, monkeypatch):
        ...
        lycus_home = tmp_path / "lycus_test"
        plugins_dir = lycus_home / "plugins"
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "lycus_test" / "plugins"
        # Set AUTOLYCUS_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("lycus_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("lycus_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("lycus_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("lycus_cli.plugins.PluginContext.dispatch_tool.__module__", "lycus_cli.plugins"):
...
class TestPluginDebugLogging:
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        from lycus_cli import plugins as plugins_mod
