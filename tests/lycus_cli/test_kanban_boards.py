"""Tests for the multi-board kanban layer (``lycus kanban boards …``).
...
* CLI surface: ``lycus kanban boards list/create/switch/rm``.
...
def fresh_home(tmp_path, monkeypatch):
    """Isolated AUTOLYCUS_HOME with no prior kanban state.

    The autouse hermetic conftest already nukes credentials + TZ; this
    fixture layers a per-test AUTOLYCUS_HOME plus a path-init cache reset
    ...
    # Also reset lycus_constants cache so get_default_lycus_root() re-reads.
    try:
        import lycus_constants
        lycus_constants._cached_default_lycus_root = None  # type: ignore[attr-defined]
    except Exception:
        pass
...
class TestSlugValidation:
    @pytest.mark.parametrize("good", [
        "default", "atm10-server", "lycus-agent", "proj_1", "a",
...
def _cli(args: list[str], env_extra: dict | None = None) -> subprocess.CompletedProcess:
    """Run ``lycus kanban …`` with PYTHONPATH pinned to the worktree."""
    env = dict(os.environ)
    env["PYTHONPATH"] = str(_WORKTREE)
    if env_extra:
        env.update(env_extra)
    return subprocess.run(
        [sys.executable, "-m", "lycus_cli.main", "kanban"] + args,
        env=env,
        capture_output=True,
        text=True,
        cwd=str(_WORKTREE),
...
class TestCLI:
    def test_boards_list_default_only(self, tmp_path):
        env = {"AUTOLYCUS_HOME": str(tmp_path)}
