...
def _install_fake_gateway_run(monkeypatch, start_gateway):
    ...
    # /lycus-gateway.service"`` and runs ``systemctl --user daemon-reload``
    # — both target the *real* user environment because the conftest only
    # sandboxes ``AUTOLYCUS_HOME``, not ``HOME``. Tests that drive
    ...
    # unit (baking in the test's pytest-tmp ``AUTOLYCUS_HOME`` value, which
...
def test_run_gateway_refuses_root_in_official_docker(monkeypatch, tmp_path, capsys):
    project_root = tmp_path / "opt" / "lycus"
    ...
    assert exc_info.value.code == 1
    out = capsys.readouterr().out
    assert "Refusing to run the Lycus gateway as root" in out
    assert "/opt/lycus/docker/entrypoint.sh" in out
...
def test_run_gateway_refuses_when_service_supervising(monkeypatch, capsys):
    ...
    assert "lycus gateway restart" in out
    assert "--force" in out
...
def test_run_gateway_refuses_existing_process_before_importing_gateway_run(monkeypatch, capsys):
    ...
    assert "lycus gateway run --replace" in out
...
def test_s6_runtime_snapshot_reports_supervised_service(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("lycus_cli.service_manager.detect_service_manager", lambda: "s6")
    monkeypatch.setattr("lycus_cli.service_manager.get_service_manager", lambda: FakeS6Manager())
...
def test_running_under_gateway_supervisor_markers(monkeypatch):
    ...
    monkeypatch.setenv("XPC_SERVICE_NAME", "org.nousresearch.autolycus.gateway")
...
def test_gateway_restart_on_windows_without_service_uses_detached_backend(monkeypatch):
    ...
    A Telegram-hosted agent may run `lycus gateway restart` via the terminal
...
def test_systemd_install_checks_linger_status(monkeypatch, tmp_path, capsys):
    unit_path = tmp_path / "systemd" / "user" / "lycus-gateway.service"

    monkeypatch.setattr(gateway, "get_systemd_unit_path", lambda system=False: unit_path)
    # Synthetic unit with a non-temp home: the real generator bakes the
    # hermetic test AUTOLYCUS_HOME (a tmp dir), which the temp-home write
...
def test_systemd_install_can_skip_enable_on_startup(monkeypatch, tmp_path, capsys):
    ...
    # hermetic test AUTOLYCUS_HOME) stays out of the way.
    monkeypatch.setattr(
        gateway,
        "generate_systemd_unit",
        lambda system=False, run_as_user=None: (
...
def test_systemd_install_system_scope_skips_linger_and_uses_systemctl(monkeypatch, tmp_path, capsys):
    unit_path = tmp_path / "etc" / "systemd" / "system" / "lycus-gateway.service"
...
def test_conflicting_systemd_units_warning(monkeypatch, tmp_path, capsys):
    user_unit = tmp_path / "user" / "lycus-gateway.service"
    system_unit = tmp_path / "system" / "lycus-gateway.service"
    ...
    assert "lycus gateway uninstall" in out
...
def test_install_linux_gateway_from_setup_system_choice_without_root_prints_followup(monkeypatch, capsys):
    ...
    assert "sudo lycus gateway install --system --run-as-user alice" in out
    assert "sudo lycus gateway start --system" in out
...
def test_scan_gateway_pids_detects_windows_lycus_exe_case_variants(monkeypatch):
    monkeypatch.setattr(gateway, "is_windows", lambda: True)
    ...
    def fake_run(cmd, **kwargs):
        if cmd[:4] == ["wmic.exe", "process", "get", "ProcessId,CommandLine"]:
            return SimpleNamespace(
                returncode=0,
                stdout=(
                    "CommandLine=C:\\Program Files\\Lycus\\Lycus.EXE gateway run --replace\n"
                    "ProcessId=2468\n\n"
                ),
                stderr="",
            )
...
def test_module_has_logger():
    ...
    assert gateway.logger.name == "lycus_cli.gateway"
