"""Tests for the top-level `./lycus` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./lycus` should use `lycus_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "lycus"
    called = []

    fake_main_module = types.ModuleType("lycus_cli.main")

    def fake_main():
        called.append("lycus_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "lycus_cli.main", fake_main_module)
