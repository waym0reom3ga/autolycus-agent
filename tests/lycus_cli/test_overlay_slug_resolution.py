...
_PROVIDER_MODELS and config.yaml use Lycus IDs ("copilot").  The slug
...
def test_copilot_uses_lycus_slug():
...
def test_kimi_for_coding_overlay_uses_lycus_slug():
...
def test_kilo_overlay_uses_lycus_slug():
...
def test_mapped_provider_credential_pool_visibility(monkeypatch):
    ...
    monkeypatch.setattr(
        "lycus_cli.auth._load_auth_store",
