...
Bug 1 — OpenAI picker dumped the raw ``/v1/models`` catalog
    ...
    moderation, gpt-3.5, …). The ``lycus model`` CLI shows only the curated
...
Bug 2 — OpenRouter appeared authenticated whenever OPENAI_API_KEY was set
    OpenRouter's LycusOverlay carried ``extra_env_vars=("OPENAI_API_KEY",)``.
...
from lycus_cli import models as M
