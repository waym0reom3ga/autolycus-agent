...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "lycus gateway stop",
        "lycus gateway start",
        "lycus  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "HERMES GATEWAY RESTART",           # uppercase
    ])
    def test_lycus_gateway_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "launchctl kickstart gui/501/ai.autolycus.gateway",
        "launchctl unload ~/Library/LaunchAgents/ai.autolycus.gateway.plist",
        "launchctl stop ai.autolycus.gateway",
        "systemctl restart lycus-gateway",
        "systemctl stop lycus-gateway.service",
        "systemctl start lycus-gateway",
    ])
    def test_service_manager_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "kill lycus gateway process",
        "pkill -f lycus.*gateway",
    ])
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_lycus_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade lycus then run lycus gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.autolycus.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
...
class TestGatewaySelfTargetingGuard:
    """Verify lycus gateway stop/restart refuse when _HERMES_GATEWAY=1."""
