...
Lycus keeps its OWN copy of the Codex OAuth token (per profile + top-level),
separate from the Codex CLI's ``~/.codex/auth.json``. OAuth refresh_tokens are
single-use, so when the Codex CLI (or another Lycus process) rotates the shared
...
import lycus_cli.auth as auth
from lycus_cli.auth import AuthError, _refresh_codex_auth_tokens, resolve_codex_runtime_credentials
...
def test_self_heals_on_stale_refresh_token(monkeypatch):
    ...
    # the recovered token was persisted to the Lycus auth store
...
def test_self_heals_missing_singleton_access_token_from_codex_cli(tmp_path, monkeypatch):
    """Exact cron failure path: Lycus auth has refresh_token but missing access_token."""
    ...
    assert resolved["source"] == "lycus-auth-store"
    stored = json.loads((lycus_home / "auth.json").read_text())
