...
for interactive ``lycus chat -q`` invocations, but kanban workers spawned by
...
def _synthetic_worker_script() -> str:
    ...
    shape without needing the full lycus_cli boot path (config, providers,
...
def _is_alive_like_dispatcher(pid: int) -> bool:
    """Mirrors lycus_cli/kanban_db.py:_pid_alive on Linux.
...
def test_sigterm_without_kanban_task_env_uses_keyboard_interrupt_path():
    ...
    interactive ``lycus chat -q`` (no env var), behavior is unchanged. The
