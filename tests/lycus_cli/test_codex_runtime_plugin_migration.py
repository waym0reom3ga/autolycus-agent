...
from lycus_cli.codex_runtime_plugin_migration import (
    MIGRATION_MARKER,
    MIGRATION_END_MARKER,
    _build_lycus_tools_mcp_entry,
    _format_toml_value,
...
class TestMigrate:
    ...
    def test_no_servers_still_writes_permissions_default(self, tmp_path):
        ...
        report = migrate({}, codex_home=tmp_path, discover_plugins=False, expose_lycus_tools=False)
        assert report.written
        text = (tmp_path / "config.toml").read_text()
    ...
    def test_plugin_discovery_writes_plugin_blocks(self, tmp_path, monkeypatch):
        ...
        from lycus_cli import codex_runtime_plugin_migration as crpm
    ...
    def test_plugin_discovery_skips_unavailable_plugins(self):
        ...
        from lycus_cli.codex_runtime_plugin_migration import _query_codex_plugins
    ...
    def test_plugin_discovery_failure_non_fatal(self, tmp_path, monkeypatch):
        ...
        report = migrate({"mcp_servers": {"x": {"command": "y"}}},
                         codex_home=tmp_path, discover_plugins=True, expose_lycus_tools=False)
        assert report.written
    ...
    def test_discover_plugins_false_skips_query(self, tmp_path, monkeypatch):
        ...
        migrate({"mcp_servers": {"x": {"command": "y"}}},
                codex_home=tmp_path, discover_plugins=False, expose_lycus_tools=False)
    ...
    def test_dry_run_skips_plugin_query(self, tmp_path, monkeypatch):
        ...
        migrate({"mcp_servers": {"x": {"command": "y"}}},
                codex_home=tmp_path, dry_run=True, discover_plugins=True, expose_lycus_tools=False)
        assert called["yes"] is False
    ...
    def test_expose_lycus_tools_writes_callback_mcp_entry(self, tmp_path):
        """When expose_lycus_tools=True (production default), an
        [mcp_servers.autolycus-tools] entry is written so codex calls back
        into Lycus for browser/web/delegate_task/vision/memory tools.

        This is the fix for 'all other tools that codex doesn't provide
        should be useable by lycus' — quirk #7."""
        report = migrate({}, codex_home=tmp_path,
                         discover_plugins=False,
                         default_permission_profile=None,
                         expose_lycus_tools=True)
        text = (tmp_path / "config.toml").read_text()
        assert "[mcp_servers.autolycus-tools]" in text
        assert "lycus_tools_mcp_server" in text
        ...
        assert "lycus-tools" in report.migrated

    def test_expose_lycus_tools_disabled_skips_entry(self, tmp_path):
        """expose_lycus_tools=False suppresses the callback registration."""
        migrate({}, codex_home=tmp_path,
                discover_plugins=False,
                default_permission_profile=None,
                expose_lycus_tools=False)
        text = (tmp_path / "config.toml").read_text()
        assert "[mcp_servers.autolycus-tools]" not in text
        assert "lycus_tools_mcp_server" not in text

    def test_dry_run_doesnt_write(self, tmp_path):
        report = migrate({"mcp_servers": {"x": {"command": "y"}}},
                         codex_home=tmp_path, dry_run=True, expose_lycus_tools=False)
    ...
    def test_full_migration_round_trip(self, tmp_path):
        ...
        report = migrate(lycus_cfg, codex_home=tmp_path, expose_lycus_tools=False)
        assert report.written
        text = (tmp_path / "config.toml").read_text()
    ...
    def test_idempotent_re_run_replaces_managed_block(self, tmp_path):
        # First migration
        migrate({"mcp_servers": {"a": {"command": "x"}}}, codex_home=tmp_path, expose_lycus_tools=False)
        first_text = (tmp_path / "config.toml").read_text()
        assert "[mcp_servers.a]" in first_text
        # Second migration with different servers
        migrate({"mcp_servers": {"b": {"command": "y"}}}, codex_home=tmp_path, expose_lycus_tools=False)
    ...
    def test_managed_root_keys_stay_top_level_when_config_ends_in_table(self, tmp_path):
        """TOML has no explicit 'leave current table' syntax. If Lycus appends
        ...
        migrate({}, codex_home=tmp_path, discover_plugins=False, expose_lycus_tools=False)
        new_text = target.read_text()
    ...
    def test_preserves_user_mcp_server_outside_managed_block(self, tmp_path):
        ...
        to ~/.codex/config.toml outside Lycus' managed block, re-running
        ...
        migrate({"mcp_servers": {"lycus-mcp": {"command": "npx"}}},
        ...
        assert "[mcp_servers.autolycus-mcp]" in final
    ...
    def test_skipped_keys_reported(self, tmp_path):
        ...
        }, codex_home=tmp_path, expose_lycus_tools=False)
    ...
    def test_invalid_mcp_servers_value(self, tmp_path):
        report = migrate({"mcp_servers": "notadict"}, codex_home=tmp_path, expose_lycus_tools=False)
...
class TestStripUnmanagedPluginTables:
    ...
    ``lycus codex-runtime migrate`` would re-emit them inside the managed
    ...
    def test_migrate_dedups_codex_owned_plugin_tables(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "lycus_cli.codex_runtime_plugin_migration._query_codex_plugins",
            fake_query,
        )
        migrate({}, codex_home=tmp_path, discover_plugins=True, expose_lycus_tools=False)
        new_text = target.read_text()
...
# ---- Bug C: AUTOLYCUS_HOME tempdir leak into ~/.codex/config.toml ----


class TestLycusHomeLeakGuard:
    """Regression tests for issue #26250 Bug C.

    Previously ``_build_lycus_tools_mcp_entry()`` read ``AUTOLYCUS_HOME``
    ...
    once codex spawned the lycus-tools MCP subprocess.
    """

    def test_tempdir_detector_recognizes_pytest_paths(self):
        assert _looks_like_test_tempdir(
            "/private/var/folders/abc/pytest-of-kshitij/pytest-137/popen-gw2/test_X/lycus_test"
        )
        assert _looks_like_test_tempdir(
            "/tmp/pytest-of-user/pytest-12/test_X/lycus"
        )
        assert _looks_like_test_tempdir(
            "/private/var/folders/zz/T/pytest-of-bob/pytest-1"
        )

    def test_tempdir_detector_accepts_real_lycus_home(self):
        assert not _looks_like_test_tempdir("/Users/alice/.autolycus")
        assert not _looks_like_test_tempdir("/home/bob/.autolycus")
        assert not _looks_like_test_tempdir("/opt/lycus")
        assert not _looks_like_test_tempdir("")

    def test_pytest_tempdir_not_burned_into_mcp_env(self, monkeypatch):
        """The headline regression: even when AUTOLYCUS_HOME points at a pytest
        tempdir, _build_lycus_tools_mcp_entry() must NOT propagate it."""
        monkeypatch.setenv(
            "AUTOLYCUS_HOME",
            "/private/var/folders/xx/pytest-of-user/pytest-99/test_x/lycus_test",
        )
        entry = _build_lycus_tools_mcp_entry()
        env = entry.get("env", {})
        assert "AUTOLYCUS_HOME" not in env, (
            f"pytest-tempdir AUTOLYCUS_HOME leaked into codex MCP entry: "
            f"{env.get('AUTOLYCUS_HOME')!r}"
        )

    def test_real_lycus_home_propagates(self, monkeypatch, tmp_path):
        """A legitimate AUTOLYCUS_HOME (not a tempdir path) DOES propagate so the
        ...
        real_path = "/Users/alice/.autolycus"
        monkeypatch.setenv("AUTOLYCUS_HOME", real_path)
        entry = _build_lycus_tools_mcp_entry()
        env = entry.get("env", {})
        assert env.get("AUTOLYCUS_HOME") == real_path

    def test_unset_lycus_home_omits_env_key(self, monkeypatch):
        """When AUTOLYCUS_HOME is unset in the environment, the MCP entry MUST
        NOT bake in a resolved-default path. The codex subprocess should
        inherit whatever AUTOLYCUS_HOME its launcher (systemd, gateway, shell)
        ...
        env = entry.get("env", {})
        assert "AUTOLYCUS_HOME" not in env, (
            f"AUTOLYCUS_HOME should not be set when env var is unset, got: "
            f"{env.get('AUTOLYCUS_HOME')!r}"
        )
