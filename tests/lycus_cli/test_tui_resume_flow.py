...
def test_main_top_level_tui_accepts_toolsets(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(sys, "argv", ["lycus", "--tui", "--toolsets", "web,terminal"])
    monkeypatch.setitem(
        sys.modules,
        "lycus_cli.plugins",
...
def test_termux_fast_tui_launch_uses_light_parser(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(
        sys, "argv", ["lycus", "--tui", "--toolsets", "web,terminal"]
    )
    monkeypatch.setattr(
        main_mod,
        "cmd_chat",
...
def test_termux_fast_tui_launch_skips_help(monkeypatch, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.setattr(sys, "argv", ["lycus", "--tui", "--help"])
...
def test_fast_tui_launch_is_termux_only(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(sys, "argv", ["lycus", "--tui"])
...
def test_termux_fast_cli_launch_chat_uses_light_parser(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(
        sys, "argv", ["lycus", "chat", "-q", "hello", "--toolsets", "web,terminal"]
    )
    monkeypatch.setattr(
...
def test_termux_fast_cli_launch_bare_defers_agent_startup(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(sys, "argv", ["lycus"])
    monkeypatch.setattr(
        main_mod, "_prepare_agent_startup", lambda args: prepared.append(args.command)
    )
    monkeypatch.setattr(
...
def test_termux_fast_cli_launch_oneshot_uses_light_parser(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["lycus", "-z", "hello", "--model", "gpt-test", "--provider", "openai"],
    )
    monkeypatch.setattr(
        main_mod, "_prepare_agent_startup", lambda args: prepared.append(args.command)
    )
...
def test_termux_fast_cli_launch_version_skips_update_check(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("HERMES_TUI", raising=False)
    monkeypatch.setattr(sys, "argv", ["lycus", "version"])
    monkeypatch.setattr(
...
def test_termux_fast_cli_launch_skips_help(monkeypatch, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("HERMES_TUI", raising=False)
    monkeypatch.setattr(sys, "argv", ["lycus", "chat", "--help"])
...
def test_termux_bundled_skills_stamp_controls_sync(monkeypatch, tmp_path, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.setattr(main_mod, "get_lycus_home", lambda: tmp_path)
...
def test_main_top_level_oneshot_accepts_toolsets(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(
        sys, "argv", ["lycus", "-z", "hello", "--toolsets", "web,terminal"]
    )
    monkeypatch.setitem(
        sys.modules,
        "lycus_cli.plugins",
...
def test_oneshot_fails_closed_on_empty_final_response(monkeypatch, capsys):
    _stub_plugin_discovery(monkeypatch)
    import lycus_cli.oneshot as oneshot_mod
...
def test_oneshot_filters_invalid_toolsets_before_redirect(monkeypatch, capsys):
    _stub_plugin_discovery(monkeypatch)
    from lycus_cli.oneshot import _validate_explicit_toolsets
...
def test_oneshot_rejects_disabled_mcp_toolset(monkeypatch, capsys):
    ...
    assert valid is None
    assert error == "lycus -z: --toolsets did not contain any valid toolsets.\n"
    err = capsys.readouterr().err
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """lycus -z bypasses LycusCLI, but recall still needs SessionDB."""
    from lycus_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "lycus_state", mod("lycus_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "lycus_cli.config",
        mod("lycus_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "lycus_cli.models",
        mod("lycus_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
        "lycus_cli.runtime_provider",
        mod(
            "lycus_cli.runtime_provider",
    ...
    monkeypatch.setitem(
        sys.modules,
        "lycus_cli.tools_config",
        mod("lycus_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="nous/lycus-test", provider="nous", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["HERMES_MODEL"] == "nous/lycus-test"
    assert env["HERMES_INFERENCE_MODEL"] == "nous/lycus-test"
    assert env["HERMES_TUI_PROVIDER"] == "nous"
    assert env["HERMES_INFERENCE_PROVIDER"] == "nous"
    assert env["HERMES_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["HERMES_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("lycus-tui-active-session-")
...
def test_launch_tui_applies_terminal_backend_config(
    monkeypatch, main_mod, _isolate_lycus_home
):
    captured = {}
    ...
    assert captured["env"]["TERMINAL_DOCKER_IMAGE"] == "example/lycus-tools:latest"
...
def test_launch_tui_exit_code_42_relaunches_update(monkeypatch, main_mod):
    ...
    with patch("lycus_cli.relaunch.relaunch") as mock_relaunch:
        with pytest.raises(SystemExit) as exc:
            main_mod._launch_tui()
...
def test_make_tui_argv_dev_prebuilds_lycus_ink(monkeypatch, main_mod, tmp_path):
...
def test_print_tui_exit_summary_includes_resume_and_token_totals(monkeypatch, capsys):
    ...
    monkeypatch.setitem(
        sys.modules, "lycus_state", types.SimpleNamespace(SessionDB=lambda: _FakeDB())
    )
    ...
    assert "lycus --tui --resume 20260409_000001_abc123" in out
    assert 'lycus --tui -c "demo title"' in out
...
def test_print_tui_exit_summary_prefers_actual_active_session_file(
    ...
    assert "lycus --tui --resume actual_session" in out
