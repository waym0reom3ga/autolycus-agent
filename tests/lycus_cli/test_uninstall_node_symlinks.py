"""Tests for lycus_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $AUTOLYCUS_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Lycus-managed node dir.
"""
...
def _make_lycus_node(lycus_home: Path) -> Path:
    """Create a fake $AUTOLYCUS_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = lycus_home / "node" / "bin"
...
def test_removes_symlinks_pointing_into_lycus_node(fake_home):
    lycus_home = fake_home / ".autolycus"
    node_bin = _make_lycus_node(lycus_home)
    local_bin = fake_home / ".local" / "bin"

    for name in ("node", "npm", "npx"):
        (local_bin / name).symlink_to(node_bin / name)

    removed = uninstall.remove_node_symlinks(lycus_home)
...
def test_handles_missing_local_bin(fake_home):
    ...
    assert uninstall.remove_node_symlinks(lycus_home) == []


def test_removes_dangling_symlink_into_lycus_node(fake_home):
    """A link into the Lycus node dir is removed even if the target file is
...
def test_only_some_links_present(fake_home):
    """Removes the Lycus links that exist; ignores the ones that don't."""
    lycus_home = fake_home / ".autolycus"
    node_bin = _make_lycus_node(lycus_home)
    local_bin = fake_home / ".local" / "bin"

    # Only npm and npx are Lycus-managed; node is a real user binary.
