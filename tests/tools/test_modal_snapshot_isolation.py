...
def _restore_tool_modules():
    original_lycus_home = os.environ.get("AUTOLYCUS_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_lycus_home is None:
            os.environ.pop("AUTOLYCUS_HOME", None)
        else:
            os.environ["AUTOLYCUS_HOME"] = original_lycus_home
        _reset_modules(("tools", "lycus_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    lycus_cli = types.ModuleType("lycus_cli")
    lycus_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["lycus_cli"] = lycus_cli
    lycus_home = tmp_path / "lycus-home"
    os.environ["AUTOLYCUS_HOME"] = str(lycus_home)
    sys.modules["lycus_cli.config"] = types.SimpleNamespace(
        get_lycus_home=lambda: lycus_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="lycus-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": lycus_home / "modal_snapshots.json",
