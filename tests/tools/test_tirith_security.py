...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._lycus_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(lycus_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(lycus_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_lycus_bin(self):
        """After _INSTALL_FAILED, manual install in AUTOLYCUS_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        lycus_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(lycus_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(lycus_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._lycus_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == lycus_bin
            assert _tirith_mod._resolved_path == lycus_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# AUTOLYCUS_HOME isolation
# ---------------------------------------------------------------------------

class TestLycusHomeIsolation:
    def test_lycus_bin_dir_respects_lycus_home(self):
        """_lycus_bin_dir must use AUTOLYCUS_HOME, not hardcoded ~/.autolycus."""
        from tools.tirith_security import _lycus_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"AUTOLYCUS_HOME": tmpdir}):
            result = _lycus_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_lycus_home(self):
        """_failure_marker_path must use AUTOLYCUS_HOME, not hardcoded ~/.autolycus."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"AUTOLYCUS_HOME": "/custom/lycus"}):
            result = _failure_marker_path()
        assert result == "/custom/lycus/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets AUTOLYCUS_HOME; verify it's active."""
        lycus_home = os.getenv("AUTOLYCUS_HOME")
        assert lycus_home is not None, "AUTOLYCUS_HOME should be set by conftest"
        assert "lycus_test" in lycus_home, "Should point to test temp dir"

    def test_get_lycus_home_fallback(self):
        """Without AUTOLYCUS_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_lycus_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove AUTOLYCUS_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".autolycus")
            result = _get_lycus_home()
