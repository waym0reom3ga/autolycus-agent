...
class TestOrphanedPipeReconciliation:
    ...
    `lycus update` in Feishu spawned a background subprocess that restarted
...
class TestSpawnEnvSanitization:
    ...
    def test_spawn_via_env_uses_backend_temp_dir_for_artifacts(self, registry):
        ...
        assert "/data/data/com.termux/files/usr/tmp/lycus_bg_" in bg_command
        assert ".exit" in bg_command
        assert "rc=$?;" in bg_command
        assert " > /tmp/lycus_bg_" not in bg_command
        assert "cat /tmp/lycus_bg_" not in bg_command
    ...
    def test_env_poller_quotes_temp_paths_with_spaces(self, registry):
        ...
        with patch("tools.process_registry.time.sleep", return_value=None), \
            ...
            registry._env_poller_loop(
                session,
                env,
                "/path with spaces/lycus_bg.log",
                "/path with spaces/lycus_bg.pid",
                "/path with spaces/lycus_bg.exit",
            )

        assert env.commands[0][0] == "cat '/path with spaces/lycus_bg.log' 2>/dev/null"
        assert env.commands[1][0] == "kill -0 \"$(cat '/path with spaces/lycus_bg.pid' 2>/dev/null)\" 2>/dev/null; echo $?"
        assert env.commands[2][0] == "cat '/path with spaces/lycus_bg.exit' 2>/dev/null"
