...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        lycus_home.mkdir()
        (lycus_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"AUTOLYCUS_HOME": str(lycus_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(lycus_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.autolycus/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        lycus_home = tmp_path / ".autolycus"
        lycus_home.mkdir()
        (lycus_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        lycus_home = tmp_path / ".autolycus"
        lycus_home.mkdir()
        (lycus_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        lycus_home = tmp_path / ".autolycus"
        lycus_home.mkdir()
        (lycus_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.autolycus/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        lycus_home = tmp_path / ".autolycus"
        (lycus_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"AUTOLYCUS_HOME": str(lycus_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.autolycus")

        assert mounts[0]["container_path"] == "/home/user/.autolycus/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.autolycus/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.autolycus/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape AUTOLYCUS_HOME."""
        monkeypatch.setenv("AUTOLYCUS_HOME", str(tmp_path / ".autolycus"))
        (tmp_path / ".autolycus").mkdir()

        # Create a sensitive file one level above lycus_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside lycus_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside AUTOLYCUS_HOME must still be registered."""
        lycus_home = tmp_path / ".autolycus"
        lycus_home.mkdir()
        monkeypatch.setenv("AUTOLYCUS_HOME", str(lycus_home))
        (lycus_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_lycus_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of AUTOLYCUS_HOME must be allowed."""
        lycus_home = tmp_path / ".autolycus"
        lycus_home.mkdir()
        subdir = lycus_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside AUTOLYCUS_HOME pointing outside must be rejected."""
        lycus_home = tmp_path / ".autolycus"
        lycus_home.mkdir()
        monkeypatch.setenv("AUTOLYCUS_HOME", str(lycus_home))

        # Create a sensitive file outside lycus_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside lycus_home pointing outside
        symlink = lycus_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes AUTOLYCUS_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, lycus_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape AUTOLYCUS_HOME."""
        lycus_home = tmp_path / ".autolycus"
        lycus_home.mkdir()
        monkeypatch.setenv("AUTOLYCUS_HOME", str(lycus_home))
        ...
        sensitive.write_text("{}")
        self._write_config(lycus_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(lycus_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside AUTOLYCUS_HOME via config must still mount."""
        lycus_home = tmp_path / ".autolycus"
        lycus_home.mkdir()
        monkeypatch.setenv("AUTOLYCUS_HOME", str(lycus_home))

        (lycus_home / "oauth.json").write_text("{}")
        self._write_config(lycus_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        lycus_home = tmp_path / ".autolycus"
        lycus_home.mkdir()
        (lycus_home / "cache" / "documents").mkdir(parents=True)
        (lycus_home / "cache" / "audio").mkdir(parents=True)
        (lycus_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("AUTOLYCUS_HOME", str(lycus_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.autolycus/cache/documents" in paths
        assert "/root/.autolycus/cache/audio" in paths
        assert "/root/.autolycus/cache/videos" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.autolycus/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        lycus_home = tmp_path / ".autolycus"
        lycus_home.mkdir()
        # Use legacy dir name — get_lycus_dir prefers old if it exists
        (lycus_home / "document_cache").mkdir()
        (lycus_home / "image_cache").mkdir()
        monkeypatch.setenv("AUTOLYCUS_HOME", str(lycus_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(lycus_home / "document_cache") in host_paths
        assert str(lycus_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.autolycus/cache/documents" in container_paths
        assert "/root/.autolycus/cache/images" in container_paths
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        lycus_home = tmp_path / ".autolycus"
        img_dir = lycus_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.autolycus/cache/images/generated.png"
        )
    ...
    def test_custom_container_base_for_remote_home(self, tmp_path, monkeypatch):
        ...
        assert (
            map_cache_path_to_container(host_path, container_base="/home/agent/.autolycus")
            == "/home/agent/.autolycus/cache/images/remote.png"
        )

    def test_returns_none_when_outside_cache_dirs(self, tmp_path, monkeypatch):
        lycus_home = tmp_path / ".autolycus"
        (lycus_home / "cache" / "images").mkdir(parents=True)
        monkeypatch.setenv("AUTOLYCUS_HOME", str(lycus_home))
    ...
    def test_returns_none_when_no_cache_dirs_exist(self, tmp_path, monkeypatch):
        ...
        assert map_cache_path_to_container(str(lycus_home / "cache" / "images" / "x.png")) is None
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        lycus_home = tmp_path / ".autolycus"
        doc_dir = lycus_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        lycus_home = tmp_path / ".autolycus"
        ss_dir = lycus_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.autolycus/cache/screenshots/session_abc/screen1.png"
