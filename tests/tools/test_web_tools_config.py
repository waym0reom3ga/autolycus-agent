...
class TestFirecrawlClientConfig:
    ...
    def test_nous_auth_token_respects_lycus_home_override(self, tmp_path):
        """Auth lookup should read from AUTOLYCUS_HOME/auth.json, not ~/.autolycus/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".autolycus").mkdir(parents=True)
...
class TestBackendSelection:
    ...
    ``lycus tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
...
class TestCheckWebApiKey:
    ...
    def test_tool_gateway_availability_skips_refresh_for_expired_cached_token(
        ...
        monkeypatch.setattr(
            "lycus_cli.auth.resolve_nous_access_token",
            _record_refresh,
        )
