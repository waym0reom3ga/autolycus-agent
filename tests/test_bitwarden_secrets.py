...
binary download are exercised manually by `lycus secrets bitwarden
...
def lycus_home(tmp_path, monkeypatch):
    """Point Lycus at an isolated home directory."""
    home = tmp_path / ".autolycus"
    home.mkdir()
    monkeypatch.setenv("AUTOLYCUS_HOME", str(home))
    # Some modules cache get_lycus_home; clear if needed.
    import lycus_constants
    if hasattr(lycus_constants, "_AUTOLYCUS_HOME_CACHE"):
        lycus_constants._AUTOLYCUS_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(lycus_home, monkeypatch):
...
def test_install_bws_happy_path(lycus_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(lycus_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(lycus_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from lycus_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under lycus_home/cache/."""
    home = tmp_path / ".autolycus"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
