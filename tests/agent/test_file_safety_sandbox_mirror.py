...
class TestClassifySandboxMirrorTarget:
    ...
    def test_other_backends_and_inner_files_match(self, tmp_path, backend, inner):
        ...
        target = (
            tmp_path
            / "sandboxes" / backend / "task-42" / "home" / ".autolycus"
            / Path(inner)
        )
    ...
    def test_sandboxes_segment_without_home_lycus_returns_none(self, tmp_path):
        """A ``sandboxes/`` directory unrelated to Lycus-state mirroring (e.g.
    ...
    def test_sandboxes_segment_with_home_but_no_lycus_returns_none(self, tmp_path):
        """``sandboxes/<backend>/<task>/home/anything-not-lycus`` is not a mirror."""
        from agent.file_safety import classify_sandbox_mirror_target
    ...
    def test_truncated_sandbox_path_returns_none(self, tmp_path):
        """``…/sandboxes/<backend>/<task>`` without ``home/.autolycus/<thing>`` is not a mirror."""
        from agent.file_safety import classify_sandbox_mirror_target
...
class TestSandboxMirrorIsOrthogonalToCrossProfile:
    ...
    def test_same_profile_mirror_still_flagged(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(fs, "_lycus_root_path", lambda: tmp_path)
        monkeypatch.setattr(fs, "_lycus_home_path", lambda: tmp_path / "profiles" / "group1")
