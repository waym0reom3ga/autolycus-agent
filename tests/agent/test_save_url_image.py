...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate AUTOLYCUS_HOME under tmp_path."""
    monkeypatch.setenv("AUTOLYCUS_HOME", str(tmp_path / ".autolycus"))
    (tmp_path / ".autolycus").mkdir()

    # Force the constants/image cache helpers to re-read AUTOLYCUS_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("lycus_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_lycus_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under AUTOLYCUS_HOME — gateway cleanup
