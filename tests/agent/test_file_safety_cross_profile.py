"""Tests for the cross-Lycus-profile write guard in agent/file_safety.

The guard fires when a tool tries to write into another Lycus profile's
...
Reference: May 2026 incident — a lycus-security profile session
accidentally edited skills under both ~/.autolycus/profiles/lycus-security/skills/
AND ~/.autolycus/skills/ (the default profile's skills), realizing only
...
# ---------------------------------------------------------------------------
# Helpers — set up a fake Lycus root with two profiles, monkeypatch the
# resolver helpers so the classifier sees the test layout.
# ---------------------------------------------------------------------------


@pytest.fixture
def fake_lycus(tmp_path, monkeypatch):
    """Build a fake Lycus layout:
    ...
    sec_home = root / "profiles" / "lycus-security"
    ...
    monkeypatch.setattr(lycus_constants, "get_default_lycus_root", lambda: root)
...
def _set_active_home(monkeypatch, lycus_home: Path):
    """Point file_safety._lycus_home_path at a specific profile dir."""
    import agent.file_safety as fs
    monkeypatch.setattr(fs, "_lycus_home_path", lambda: lycus_home)
...
class TestResolveActiveProfileName:
    def test_default_when_home_is_root(self, fake_lycus, monkeypatch):
        _set_active_home(monkeypatch, fake_lycus["default_home"])
        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "default"

    def test_named_profile(self, fake_lycus, monkeypatch):
        _set_active_home(monkeypatch, fake_lycus["security_home"])
        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "lycus-security"

    def test_falls_back_to_default_on_resolution_failure(self, fake_lycus, monkeypatch):
        """If AUTOLYCUS_HOME resolution raises, return 'default' rather than crashing the tool."""
        import agent.file_safety as fs

        def _boom():
            raise RuntimeError("simulated")

        monkeypatch.setattr(fs, "_lycus_home_path", _boom)
...
class TestClassifyCrossProfileTarget:
    def test_same_profile_write_returns_none(self, fake_lycus, monkeypatch):
        _set_active_home(monkeypatch, fake_lycus["security_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_lycus["security_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is None

    def test_security_writing_default_skill(self, fake_lycus, monkeypatch):
        """The exact incident from May 2026."""
        _set_active_home(monkeypatch, fake_lycus["security_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_lycus["default_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is not None
        assert result["active_profile"] == "lycus-security"
        assert result["target_profile"] == "default"
        assert result["area"] == "skills"

    def test_default_writing_security_skill(self, fake_lycus, monkeypatch):
        ...
        assert result["target_profile"] == "lycus-security"

    def test_named_to_named_cross_profile(self, fake_lycus, monkeypatch):
        _set_active_home(monkeypatch, fake_lycus["security_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_lycus["coder_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is not None
        assert result["target_profile"] == "coder"

    @pytest.mark.parametrize("area", ["skills", "plugins", "cron", "memories"])
    def test_all_profile_scoped_areas_classified(self, fake_lycus, monkeypatch, area):
        _set_active_home(monkeypatch, fake_lycus["security_home"])
        from agent.file_safety import classify_cross_profile_target
        target = fake_lycus["default_home"] / area / "foo.txt"
        result = classify_cross_profile_target(str(target))
        assert result is not None
        assert result["area"] == area

    def test_non_lycus_path_returns_none(self, fake_lycus, monkeypatch, tmp_path):
        _set_active_home(monkeypatch, fake_lycus["security_home"])
        from agent.file_safety import classify_cross_profile_target
        # Path outside any Lycus root
        assert classify_cross_profile_target(str(tmp_path / "random.txt")) is None

    def test_lycus_config_not_classified_as_cross_profile(self, fake_lycus, monkeypatch):
        ...
        result = classify_cross_profile_target(
            str(fake_lycus["default_home"] / "config.yaml")
        )
        assert result is None
...
class TestGetCrossProfileWarning:
    def test_in_profile_returns_none(self, fake_lycus, monkeypatch):
        _set_active_home(monkeypatch, fake_lycus["security_home"])
    ...
    def test_cross_profile_warning_names_both_profiles(self, fake_lycus, monkeypatch):
        _set_active_home(monkeypatch, fake_lycus["security_home"])
        from agent.file_safety import get_cross_profile_warning
        ...
        assert "lycus-security" in warn
        # Must name the bypass kwarg.
        assert "cross_profile=True" in warn
        # Must reference the area.
        assert "skills" in warn

    def test_warning_is_defense_in_depth_not_boundary(self, fake_lycus, monkeypatch):
        _set_active_home(monkeypatch, fake_lycus["security_home"])
        from agent.file_safety import get_cross_profile_warning
        warn = get_cross_profile_warning(
            str(fake_lycus["default_home"] / "skills" / "foo" / "SKILL.md")
