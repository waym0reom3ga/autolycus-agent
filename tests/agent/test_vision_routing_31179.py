...
def isolated_home(monkeypatch):
    """Temp AUTOLYCUS_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="lycus_test_31179_")
    lycus_home = os.path.join(test_home, ".autolycus")
    os.makedirs(lycus_home)
...
def _fresh_modules():
    """Drop cached lycus modules so each test reloads against current env."""
