...
""",
        ...
        from lycus_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        lycus_home = tmp_path / ".autolycus"
        lycus_home.mkdir()
        (lycus_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("AUTOLYCUS_HOME", str(lycus_home))
        from lycus_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from lycus_cli.plugins import PluginContext, PluginManifest, PluginManager
