...
def backup_env(monkeypatch, tmp_path):
    """Isolate AUTOLYCUS_HOME + reload modules so every test starts clean."""
    ...
    # Reload so get_lycus_home picks up the env var fresh.
...
def test_real_run_takes_pre_snapshot(backup_env, monkeypatch):
    ...
    # Reload curator module against the freshly-env'd lycus_constants
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under AUTOLYCUS_HOME. Returns the path.
...
def test_snapshot_without_cron_jobs_file_still_succeeds(backup_env):
    ...
    cb = backup_env["cb"]
    _write_skill(backup_env["skills"], "alpha")
    # Deliberately do not create ~/.autolycus/cron/jobs.json
