...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_lycus_env(self):
        """Lycus' own .env inside AUTOLYCUS_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: lycus internal .env is in ~/.autolycus/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.autolycus/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.autolycus/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Lycus cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        lycus_home = tmp_path / ".autolycus"
        cache = lycus_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._lycus_home_path", return_value=lycus_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Lycus cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        lycus_home = tmp_path / ".autolycus"
        hub = lycus_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_lycus_home(self, tmp_path):
        """The env basename guard does not depend on AUTOLYCUS_HOME resolution."""
        lycus_home = tmp_path / ".autolycus"
        lycus_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        lycus_home = tmp_path / ".autolycus"
        cache = lycus_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
