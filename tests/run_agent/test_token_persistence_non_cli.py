...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    lycus_state = ModuleType("lycus_state")
    lycus_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "lycus_state", lycus_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Lycus"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Lycus"
