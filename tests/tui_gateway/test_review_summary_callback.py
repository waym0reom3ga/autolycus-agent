...
def server():
    with patch.dict(
        "sys.modules",
        {
            "lycus_constants": MagicMock(
                get_lycus_home=MagicMock(return_value="/tmp/lycus_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'lycus-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'lycus-release' patched"
    }
