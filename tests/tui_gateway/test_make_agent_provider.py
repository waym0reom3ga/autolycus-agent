...
def test_make_agent_tolerates_null_config_sections():
    """Bare `agent:` / `display:` keys in ~/.autolycus/config.yaml parse as
...
def test_apply_model_switch_does_not_leak_process_env():
    ...
    with (
        patch("lycus_cli.model_switch.parse_model_flags",
              return_value=("glm-5.1", None, False, False)),
        patch("lycus_cli.model_switch.switch_model", return_value=_FakeResult()),
