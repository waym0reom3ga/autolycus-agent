"""Tests for lycus_constants module."""
...
class TestGetDefaultLycusRoot:
    """Tests for get_default_lycus_root() — Docker/custom deployment awareness."""

    def test_no_lycus_home_returns_native(self, tmp_path, monkeypatch):
        """When AUTOLYCUS_HOME is not set, returns ~/.autolycus."""
        monkeypatch.delenv("AUTOLYCUS_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_lycus_root() == tmp_path / ".autolycus"

    def test_lycus_home_is_native(self, tmp_path, monkeypatch):
        """When AUTOLYCUS_HOME = ~/.autolycus, returns ~/.autolycus."""
        native = tmp_path / ".autolycus"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("AUTOLYCUS_HOME", str(native))
        assert get_default_lycus_root() == native

    def test_lycus_home_is_profile(self, tmp_path, monkeypatch):
        """When AUTOLYCUS_HOME is a profile under ~/.autolycus, returns ~/.autolycus."""
        native = tmp_path / ".autolycus"
    ...
    def test_lycus_home_is_docker(self, tmp_path, monkeypatch):
        """When AUTOLYCUS_HOME points outside ~/.autolycus (Docker), returns AUTOLYCUS_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("AUTOLYCUS_HOME", str(docker_home))
        assert get_default_lycus_root() == docker_home

    def test_lycus_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any AUTOLYCUS_HOME outside ~/.autolycus is treated as the root."""
        custom = tmp_path / "my-lycus-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("AUTOLYCUS_HOME", str(custom))
        assert get_default_lycus_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (AUTOLYCUS_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("AUTOLYCUS_HOME", str(profile))
        assert get_default_lycus_root() == docker_root

    def test_no_lycus_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\lycus, not ~/.autolycus."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("AUTOLYCUS_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(lycus_constants.sys, "platform", "win32")

        assert get_default_lycus_root() == local_appdata / "lycus"

    def test_no_lycus_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        """Windows fallback still uses AppData/Local/lycus without LOCALAPPDATA."""
        home = tmp_path / "Home"
        monkeypatch.delenv("AUTOLYCUS_HOME", raising=False)
        ...
        assert get_default_lycus_root() == home / "AppData" / "Local" / "lycus"


class TestGetLycusHome:
    """Tests for get_lycus_home() platform-aware fallback."""

    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When AUTOLYCUS_HOME is unset on Windows, use %LOCALAPPDATA%\\lycus."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("AUTOLYCUS_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(lycus_constants.sys, "platform", "win32")
        monkeypatch.setattr(lycus_constants, "_profile_fallback_warned", False)

        assert get_lycus_home() == local_appdata / "lycus"
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(lycus_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(lycus_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".autolycus"
