"""Tests for ACP Registry metadata shipped with Lycus."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "lycus-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Lycus Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/NousResearch/lycus-agent"
    assert data["website"].startswith("https://lycus-agent.nousresearch.com/")
    assert data["authors"] == ["Technetia Inc"]
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"lycus-agent[acp]=={data['version']}"
    assert uvx["args"] == ["lycus-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"lycus-agent[acp]=={_pyproject_version()}"
    )
