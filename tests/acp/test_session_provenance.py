...
def test_root_session_no_compression(db):
    ...
    assert prov["currentLycusSessionId"] == "root1"
    assert prov["rootLycusSessionId"] == "root1"
    assert prov["parentLycusSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_lycus_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentLycusSessionId"] == "old"
    assert prov["rootLycusSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousLycusSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootLycusSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootLycusSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_lycus_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousLycusSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"lycus"}
    assert "sessionProvenance" in meta["lycus"]
    assert meta["lycus"]["sessionProvenance"]["currentLycusSessionId"] == "root1"
