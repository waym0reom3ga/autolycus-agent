"""Tests for acp_adapter.server — LycusACPAgent ACP server."""
...
from acp_adapter.server import LycusACPAgent, HERMES_VERSION
...
def agent(mock_manager):
    """LycusACPAgent backed by a mock session manager."""
    return LycusACPAgent(session_manager=mock_manager)
...
class TestInitialize:
    ...
    async def test_initialize_returns_agent_info(self, agent):
        ...
        assert resp.agent_info.name == "lycus-agent"
        ...
        assert payloads == [
            {
                "args": ["--setup"],
                "description": (
                    "Open Lycus' interactive model/provider setup in a terminal. "
                    "Use this when Lycus has not been configured on this machine yet."
                ),
                "id": TERMINAL_SETUP_AUTH_METHOD_ID,
                "name": "Configure Lycus provider",
                "type": "terminal",
            }
        ]
...
class TestSessionOps:
    ...
    async def test_new_session_returns_model_state(self):
        ...
        acp_agent = LycusACPAgent(session_manager=manager)

        with patch(
            "lycus_cli.models.curated_models_for_provider",
    ...
    async def test_load_session_replays_persisted_history_to_client(self, agent):
        ...
        state.history = [
            ...
            {"role": "assistant", "content": "LycusACPAgent._ADVERTISED_COMMANDS controls them."},
            {
                "role": "assistant",
                "content": "",
                "tool_calls": [
        ...
        assert replay_calls[1].kwargs["update"].content.text.startswith("LycusACPAgent")
...
class TestSessionConfiguration:
    ...
    async def test_set_session_model_accepts_provider_prefixed_choice(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("lycus_cli.config.load_config", lambda: {
        ...
        monkeypatch.setattr(
            "lycus_cli.models.parse_model_input",
...
class TestPrompt:
    ...
    @pytest.mark.asyncio
    async def test_prompt_propagates_lycus_session_id_env(self, agent, monkeypatch):
    ...
    @pytest.mark.asyncio
    async def test_prompt_restores_prior_lycus_session_id(self, agent, monkeypatch):
...
class TestRegisterSessionMcpServers:
    ...
    async def test_registers_stdio_servers(self, agent, mock_manager):
        ...
        state.agent.enabled_toolsets = ["lycus-acp"]
    ...
    async def test_refreshes_agent_tool_surface(self, agent, mock_manager):
        ...
        mock_defs.assert_called_once_with(
            enabled_toolsets=["lycus-acp", "mcp-srv"],
            disabled_toolsets=None,
            quiet_mode=True,
        )
        assert state.agent.enabled_toolsets == ["lycus-acp", "mcp-srv"]
