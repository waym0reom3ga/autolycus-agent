...
The shim (docker/lycus-exec-shim.sh, installed at /opt/lycus/bin/lycus)
exists to prevent the auth.json ownership-mismatch bug where
`docker exec <c> lycus login` would write /opt/data/auth.json as
...
failed: Lycus is not logged into Nous Portal" on every message.

These tests verify:

1. ``docker exec <c> lycus …`` (defaulting to root) gets dropped to the
   lycus user before the real binary runs.
2. ``docker exec --user lycus <c> lycus …`` (already non-root) short-
   circuits and doesn't try to drop again.
3. Files written under $AUTOLYCUS_HOME from a ``docker exec`` session land
   as lycus:lycus — the actual user-visible invariant.
...
def test_shim_drops_root_to_lycus_uid(sleep_container: str) -> None:
    """docker exec defaults to root; the shim should drop to uid 10000.

    We invoke `lycus` with a Python-style `-c` shim equivalent — there's no
    pure-lycus "print my uid" command, so we use the venv's python directly
    ...
    fact that the venv's `lycus` is a console_scripts entry — under the
    hood it's a tiny Python wrapper. We can't easily inject "print my uid"
    into it without forking subcommands. Simplest approach: have `lycus`
    do anything that writes to disk, then check the file's owner.

    Use `lycus config set` which writes config.yaml under AUTOLYCUS_HOME.
    ...
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "lycus", "config", "set", "_test.shim_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: stdout={r.stdout!r} stderr={r.stderr!r}"

    # The written file must be owned by lycus, not root.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    assert r.stdout.strip() == "lycus:lycus", (
        f"config.yaml owned by {r.stdout.strip()!r}, expected lycus:lycus. "
        "The shim did not drop privileges before invoking lycus."
    )


def test_shim_short_circuits_for_non_root_exec(sleep_container: str) -> None:
    """docker exec --user lycus already runs as 10000; shim should be a no-op.
    ...
    r = subprocess.run(
        ["docker", "exec", "--user", "lycus", sleep_container,
         "lycus", "config", "set", "_test.shim_short_circuit", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, (
        f"docker exec --user lycus failed: {r.stderr!r} stdout={r.stdout!r}. "
        "If the shim mis-handled the non-root path, this would fail with EPERM."
    )

    # File still ends up lycus:lycus — orthogonally confirms uid.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "stat", "-c", "%U:%G", "/opt/data/config.yaml"],
        capture_output=True, text=True, timeout=10,
...
def test_shim_opt_out_keeps_root(sleep_container: str) -> None:
    ...
    r = subprocess.run(
        ["docker", "exec",
         ...
         sleep_container,
         "lycus", "config", "set", "_test.opt_out", "1"],
        capture_output=True, text=True, timeout=30,
    )
...
def test_shim_opt_out_strict_truthiness(
    ...
    r = subprocess.run(
        ["docker", "exec",
         ...
         sleep_container,
         "lycus", "config", "set", "_test.falsy", "1"],
        capture_output=True, text=True, timeout=30,
    )
    ...
    assert r.stdout.strip() == "lycus:lycus", (
        ...
        f"file owner is {r.stdout.strip()!r}, expected lycus:lycus"
    )
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    The shim sits at /opt/lycus/bin earliest on PATH; main-wrapper.sh
    invokes `s6-setuidgid lycus lycus <args>` which resolves `lycus`
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    Pre-shim: ``docker exec <c> lycus login`` (root) wrote
    ...
    "Lycus is not logged into Nous Portal" on every message.

    We can't do a real OAuth login in a unit test, but we can stand in
    for it by writing the same file shape via `lycus config set`-style
    writes — what matters is the *file ownership invariant* downstream
    of `_save_auth_store`. If the shim works, every file the
    `docker exec` path produces is lycus-readable.

    Specifically: pretend the operator ran `lycus login` (writes
    auth.json) and verify (a) the file exists and (b) it's readable by
    the lycus UID. We use `lycus auth list` since that touches the
    ...
    # `lycus auth list` is read-only but still exercises _load_auth_store
    # under the shim's UID. We invoke `lycus config set` first to
    # provoke a write into AUTOLYCUS_HOME so we have something concrete to
    # owner-check.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "lycus", "config", "set", "_test.e2e_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: {r.stderr}"

    # The supervised UID (10000) must be able to read everything under
    # AUTOLYCUS_HOME that docker exec just wrote.
    r = subprocess.run(
        ["docker", "exec", "--user", "lycus", sleep_container,
    ...
    assert not unreadable, (
        "Files written by `docker exec` are unreadable to the lycus user "
