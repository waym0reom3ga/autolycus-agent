...
Override the image with ``LYCUS_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("LYCUS_TEST_IMAGE", "lycus-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``LYCUS_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("LYCUS_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged lycus user
# ---------------------------------------------------------------------------
#
# Background: every Lycus runtime path inside the container drops to UID
# 10000 (the ``lycus`` user) via ``s6-setuidgid lycus``. ``docker exec``
...
# fails (PermissionError swallowed) for lycus, so a test that ran as root
...
# helpers below run every probe under ``-u lycus`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: lycus).
