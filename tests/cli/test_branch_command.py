...
def session_db(tmp_path):
    ...
    os.environ["AUTOLYCUS_HOME"] = str(tmp_path / ".autolycus")
    os.makedirs(tmp_path / ".autolycus", exist_ok=True)
    from lycus_state import SessionDB
    db = SessionDB(db_path=tmp_path / ".autolycus" / "test_sessions.db")
    yield db
    db.close()
...
def cli_instance(tmp_path, session_db):
    """Create a minimal LycusCLI-like object for testing _handle_branch_command."""
...
class TestBranchCommandCLI:
    ...
    def test_branch_creates_new_session(self, cli_instance, session_db):
        ...
        LycusCLI._handle_branch_command(cli_instance, "/branch")
    ...
    def test_branch_with_custom_name(self, cli_instance, session_db):
        ...
        LycusCLI._handle_branch_command(cli_instance, "/branch refactor approach")
    ...
    def test_branch_rotates_lycus_session_id_env_and_context(self, cli_instance, session_db):
