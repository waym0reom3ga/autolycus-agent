"""Tests for lycus_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Lycus entry point
(lycus, lycus-agent, lycus-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Lycus entry point imports lycus_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported lycus_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("lycus_bootstrap", None)
    import lycus_bootstrap  # noqa: WPS433
    return lycus_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Lycus CLI
...
class TestEntryPointsImportBootstrap:
    """Every Lycus entry point must import lycus_bootstrap as its
    ...
    # Entry points that invoke Lycus as a process.  Each one must
    # import lycus_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "lycus_cli/main.py",   # lycus CLI (console_script)
        "run_agent.py",          # lycus-agent (console_script)
        "acp_adapter/entry.py",  # lycus-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import lycus_bootstrap' and that
        ...
        half-finished ``lycus update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``lycus_bootstrap`` as a top-level module) leaves lycus
        ...
        """
        # Resolve relative to the lycus-agent repo root.  Tests live
        # at tests/test_lycus_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # lycus start even when lycus_bootstrap hasn't been
        ...
        assert first_import_name == "lycus_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'lycus_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import lycus_bootstrap' line to be the first import."
        )
