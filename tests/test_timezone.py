"""
Tests for timezone support (lycus_time module + integration points).
...
def _reset_lycus_time_cache():
    """Reset the lycus_time module cache (replacement for removed reset_cache)."""
    lycus_time._cached_tz = None
    lycus_time._cached_tz_name = None
    lycus_time._cache_resolved = False


# =========================================================================
# lycus_time.now() — core helper
# =========================================================================

class TestLycusTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_lycus_time_cache()
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="lycus_time"):
            result = lycus_time.now()
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=lycus_tz) which shifted
        absolute time when system-local tz != Lycus tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_lycus_tz(self):
        """Already-aware datetimes should be normalized to Lycus tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Lycus tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Lycus tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Lycus is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Lycus tz
        ...
        # Use a Lycus timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _lycus_now's wall time — this would
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Lycus timezone"
        )
