...
Run from the PR worktree:
    ...
    cd ~/.autolycus/lycus-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.autolycus/lycus-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.autolycus/lycus-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a lycus-agent checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a lycus-agent checkout"
)
...
# Isolated AUTOLYCUS_HOME for the config write.
