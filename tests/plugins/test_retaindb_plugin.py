...
def _isolate_env(tmp_path, monkeypatch):
    """Ensure AUTOLYCUS_HOME and RETAINDB vars are isolated."""
...
class TestRetainDBMemoryProvider:
    ...
    def test_initialize_profile_project(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", lycus_home=profile_home)
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("AUTOLYCUS_HOME", str(lycus_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", lycus_home=str(lycus_home))
        return p
