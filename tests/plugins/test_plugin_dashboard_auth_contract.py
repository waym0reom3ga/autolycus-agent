...
loopback (``X-Lycus-Session-Token`` header) and gated OAuth
(``lycus_session_at`` cookie / single-use ``?ticket=``).
...
def test_there_are_plugin_bundles_to_check() -> None:
    ...
    # kanban + lycus-achievements are bundled today; assert at least one is
