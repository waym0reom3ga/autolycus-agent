...
- Path resolution (absolute, relative to AUTOLYCUS_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp AUTOLYCUS_HOME."""
    lycus_home = tmp_path / ".autolycus"
    lycus_home.mkdir()
    (lycus_home / "cron").mkdir()
    (lycus_home / "cron" / "output").mkdir()
    (lycus_home / "scripts").mkdir()
    monkeypatch.setenv("AUTOLYCUS_HOME", str(lycus_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "HERMES_DIR", lycus_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", lycus_home / "cron")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.autolycus/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
