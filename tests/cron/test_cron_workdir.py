...
class TestRunJobTerminalCwd:
    ...
    def _install_stubs(monkeypatch, observed: dict):
        ...
        # Bypass the real provider resolver — it reads ~/.autolycus and credentials.
        from lycus_cli import runtime_provider as _rtp
        monkeypatch.setattr(
            _rtp,
        ...
        # run_job calls load_dotenv(~/.autolycus/.env, override=True), which will
    ...
    def test_no_workdir_leaves_terminal_cwd_untouched(self, monkeypatch):
        ...
        # Cron still forces MASK.md identity even when cwd context files stay off.
        assert observed["load_soul_identity"] is True
