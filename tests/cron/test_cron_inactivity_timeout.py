...
class TestSysPathOrdering:
    ...
    def test_lycus_time_importable(self):
        """lycus_time should be importable when cron.scheduler loads."""
        # This import would fail if sys.path.insert comes after the import
        from cron.scheduler import _lycus_now
        assert callable(_lycus_now)

    def test_lycus_constants_importable(self):
        """lycus_constants should be importable from cron context."""
        from lycus_constants import get_lycus_home
        assert callable(get_lycus_home)
