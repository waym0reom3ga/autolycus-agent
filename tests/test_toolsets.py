...
class TestToolsetConsistency:
    ...
    def test_lycus_platforms_share_core_tools(self):
        """All lycus-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on lycus-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["lycus-cli", "lycus-telegram", "lycus-discord", "lycus-whatsapp", "lycus-slack", "lycus-signal", "lycus-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_lycus_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("lycus-whatsapp")

    def test_lycus_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("lycus-api-server")
