...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - the child process resolves lycus_cli.kanban_db on its own
  ...
  - worker logs are captured to AUTOLYCUS_HOME/kanban/logs/<task>.log
...
def main():
    home = tempfile.mkdtemp(prefix="lycus_e2e_")
    os.environ["AUTOLYCUS_HOME"] = home
    os.environ["HOME"] = home
    sys.path.insert(0, WT)
    from lycus_cli import kanban_db as kb

    # Point the `lycus` CLI child processes will run at the worktree
    # lycus_cli.main. We do this by putting a shim on PATH.
    shim_dir = os.path.join(home, "bin")
    os.makedirs(shim_dir, exist_ok=True)
    shim_path = os.path.join(shim_dir, "lycus")
    with open(shim_path, "w") as f:
        f.write(f"""#!/bin/sh
exec {PY} -m lycus_cli.main "$@"
""")
    ...
    print("=" * 60)
    # Scenario A workers wrote to /tmp/lycus_e2e_*/worker_*.log
    import glob
