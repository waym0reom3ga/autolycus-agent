...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.autolycus."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /lycus, /btw, /stop,
        ...
        for expected in ("/lycus", "/btw", "/stop", "/model", "/help"):
...
class TestBangPrefixCommands:
    ...
    async def test_bang_with_bot_suffix_resolves(self, adapter):
        """``!stop@lycus`` matches the get_command() ``@suffix`` stripping."""
        await adapter._handle_slack_message(self._make_event("!stop@lycus"))

        msg_event = adapter.handle_message.call_args[0][0]
        assert msg_event.text.startswith("/stop@lycus")
        assert msg_event.message_type == MessageType.COMMAND
...
class TestSlashCommands:
    ...
    # instead of as /lycus subcommands. This is the Discord/Telegram parity
    # fix: the slash name itself becomes the command.
    # ------------------------------------------------------------------

    @pytest.mark.asyncio
    async def test_native_btw_slash(self, adapter):
        """/btw with args must dispatch to /background, not /lycus btw."""
        command = {
            "command": "/btw",
    ...
    @pytest.mark.asyncio
    async def test_legacy_lycus_prefix_still_works(self, adapter):
        """Backward compat: /lycus btw foo must still route to /btw foo.

        Old workspace manifests only declared /lycus as the single slash.
    ...
    @pytest.mark.asyncio
    async def test_legacy_lycus_freeform_question(self, adapter):
        """/lycus <free-form text> must stay as the raw text (non-command)."""
        command = {
            "command": "/lycus",
...
class TestSlashEphemeralAck:
    ...
    @pytest.mark.asyncio
    async def test_legacy_lycus_slash_stashes_context(self, adapter):
        """Legacy /lycus <subcommand> also stashes context."""
        command = {
            "command": "/lycus",
            "text": "help",
            "user_id": "U_H",
    ...
    @pytest.mark.asyncio
    async def test_freeform_lycus_question_does_not_stash_context(self, adapter):
        """Free-form /lycus <question> must NOT route agent reply ephemeral."""
        command = {
            "command": "/lycus",
