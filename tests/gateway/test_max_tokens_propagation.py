...
def isolated_home(tmp_path, monkeypatch):
    """Isolated AUTOLYCUS_HOME with a writable config.yaml and a clean module cache.

    These tests deliberately re-import ``lycus_cli`` / ``gateway`` so each
    ...
    _saved = {
        ...
        if k.startswith(("lycus_cli", "gateway"))
    }

    def write_cfg(body: str) -> None:
        (lycus_home / "config.yaml").write_text(textwrap.dedent(body))

    def fresh_gateway():
        for mod in list(sys.modules.keys()):
            if mod.startswith(("lycus_cli", "gateway")):
                del sys.modules[mod]
...
def test_lift_helper_accepts_alias_and_rejects_garbage(isolated_home):
    ...
    write_cfg("model:\n  provider: openrouter\n")
    for mod in list(sys.modules.keys()):
        if mod.startswith("lycus_cli"):
            del sys.modules[mod]
    rp = importlib.import_module("lycus_cli.runtime_provider")
