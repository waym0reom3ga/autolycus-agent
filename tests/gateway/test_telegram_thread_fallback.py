...
Supergroup forum topics route with ``message_thread_id``. Lycus-created
...
async def test_send_typing_attempts_api_call_for_dm_topic_reply_fallback():
    """Lycus-created DM topic lanes should still attempt scoped typing.
...
@pytest.mark.asyncio
async def test_send_uses_reply_fallback_for_lycus_dm_topics():
    """Lycus-created Telegram DM topics route with thread id plus reply anchor."""
    adapter = _make_adapter()
    call_log = []
...
async def test_send_uses_metadata_reply_fallback_for_streaming_dm_topics():
    """Metadata-only sends still stay in Lycus-created Telegram DM topics."""
    adapter = _make_adapter()
    call_log = []
