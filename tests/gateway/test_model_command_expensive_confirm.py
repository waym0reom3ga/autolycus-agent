...
def _fake_warning():
    return SimpleNamespace(
        message=(
            ...
            "openai/gpt-5.5-pro has known pricing above Lycus' safety threshold.\n"
...
def _setup_isolated_home(tmp_path, monkeypatch, *, warn):
    ...
    monkeypatch.setattr(gateway_run, "_lycus_home", lycus_home)
    monkeypatch.setattr("agent.models_dev.fetch_models_dev", lambda: {})
    monkeypatch.setattr(
        "lycus_cli.model_switch.switch_model",
        lambda **kw: _fake_switch_result(),
    )
    monkeypatch.setattr("lycus_constants.get_lycus_home", lambda: lycus_home)
    monkeypatch.setattr("lycus_cli.config.get_lycus_home", lambda: lycus_home)
    monkeypatch.setattr(
