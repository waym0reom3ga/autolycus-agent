...
Uses a file-based lock (~/.autolycus/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `lycus update` reloads
# the module) fail with ModuleNotFoundError for lycus_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from lycus_constants import get_lycus_home
from lycus_cli._subprocess_compat import windows_hide_flags
from lycus_cli.config import load_config, _expand_env_vars
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``lycus tools`` config for the ``cron`` platform.
    ...
    if per_job:
        return per_job
    try:
        from lycus_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
_lycus_home: Path | None = None


def _get_lycus_home() -> Path:
    """Resolve Lycus home dynamically while preserving test monkeypatch hooks."""
    return _lycus_home or get_lycus_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    lycus_home = _get_lycus_home()
    lock_dir = lycus_home / "cron"
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within AUTOLYCUS_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    Args:
        script_path: Path to the script.  Relative paths are resolved
            against AUTOLYCUS_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_lycus_home() / "scripts"
    ...
    # escape — scripts MUST reside within AUTOLYCUS_HOME/scripts/.
    try:
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    if job.get("no_agent"):
        ...
        now_iso = _lycus_now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_lycus_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    origin = _resolve_origin(job)
    _cron_session_id = f"cron_{job_id}_{_lycus_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    #     (notify_on_complete / watch_patterns) reads LYCUS_SESSION_PLATFORM
    #     and LYCUS_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    ...
    #     LYCUS_SESSION_PLATFORM == "telegram".
    ...
    #     LYCUS_SESSION_PLATFORM.
    #   - tools/send_message_tool.py: mirror source labelling and the
    #     send_message gate read LYCUS_SESSION_PLATFORM.
    ...
    try:
        ...
        try:
            load_dotenv(str(_get_lycus_home() / ".env"), override=True, encoding="utf-8")
        except UnicodeDecodeError:
            load_dotenv(str(_get_lycus_home() / ".env"), override=True, encoding="latin-1")
        ...
        try:
            import yaml
            _cfg_path = str(_get_lycus_home() / "config.yaml")
...
**Job ID:** {job_id}
**Run Time:** {_lycus_now().strftime('%Y-%m-%d %H:%M:%S')}
...
"""
        ...
        if _session_db:
            ...
            try:
                ...
                _cron_title = f"{_title_base} · {_lycus_now().strftime('%b %d %H:%M')}"
...
def tick(verbose: bool = True, adapters=None, loop=None, sync: bool = True) -> int:
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _lycus_now().strftime('%H:%M:%S'))
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _lycus_now().strftime('%H:%M:%S'), len(due_jobs))
