# OpenAI-Compatible API Server for Lycus Agent
...
SSE streaming. By exposing this endpoint, lycus-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  lycus-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.autolycus/.env`
2. Run `lycus gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with lycus-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns lycus-agent as a model) |
...
Authorization: Bearer lycus-api-key-here
...
But lycus-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "lycus-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured lycus-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of lycus-agent's core prompt
...
Once implemented, lycus-agent works as a drop-in backend for:
