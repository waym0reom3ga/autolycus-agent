#!/usr/bin/env python3
"""
Lycus Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the Lycus Agent, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills lycus-agent-dev,github-auth
    python cli.py --list-tools             # List available tools and exit
"""

# IMPORTANT: lycus_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See lycus_bootstrap.py for full rationale.
try:
    import lycus_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Graceful fallback when lycus_bootstrap isn't registered in the venv
    # yet — happens during partial ``lycus update`` where git-reset landed
...
from lycus_cli.fallback_config import get_fallback_chain
from lycus_cli.cli_agent_setup_mixin import CLIAgentSetupMixin
from lycus_cli.cli_commands_mixin import CLICommandsMixin
...
try:
    from lycus_cli.pt_input_extras import (
...
from lycus_cli.banner import _format_context_length, format_banner_version_label
...
from lycus_cli.browser_connect import (
...
load_lycus_dotenv(lycus_home=_lycus_home, project_env=_project_env)
...
def _load_prefill_messages(file_path: str) -> List[Dict[str, Any]]:
    ...
    Relative paths are resolved from ~/.autolycus/.
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.autolycus/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If HERMES_IGNORE_USER_CONFIG=1 is set (via ``lycus chat --ignore-user-config``),
    the user config at ``~/.autolycus/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({HERMES_HOME}/config.yaml)
    user_config_path = _lycus_home / 'config.yaml'
    ...
    defaults = {
        ...
        "compression": {
            ...
            "threshold": 0.60,    # Compress at 60% of model's context limit
        },
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain Lycus, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me Lycus - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        "display": {
            "compact": False,
            "resume_display": "full",
            # Recap tuning for /resume — see lycus_cli/config.py DEFAULT_CONFIG.
            "resume_exchanges": 10,
        ...
        try:
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and LycusCLI.__init__ checks "default" first.
    ...
    # Normalize config key: the new config system (lycus_cli/config.py) and all
    ...
    # Local backend: always os.getcwd(). Use `cd /dir && lycus` to control it.
...
# Initialize centralized logging early — agent.log + errors.log in ~/.autolycus/logs/.
# This ensures CLI sessions produce a log trail even before AIAgent is instantiated.
try:
    from lycus_logging import setup_logging
...
try:
    from lycus_cli.config import print_config_warnings, print_platform_warnings
...
try:
    from lycus_cli.skin_engine import init_skin_from_config
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``lycus --help`` paths inside the chat command flow).  See
...
def get_tool_definitions(*args, **kwargs):
    from lycus_cli.mcp_startup import wait_for_mcp_discovery
...
from lycus_cli.banner import build_welcome_banner
from lycus_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from lycus_cli.callbacks import prompt_for_secret
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run lycus -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"lycus-{short_id}"
    branch_name = f"lycus/{wt_name}"
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`lycus_cli.config.load_config` (the authoritative loader that
    ...
    if session_db is None:
        return
    try:
        from lycus_cli.config import load_config as _load_full_config
        from lycus_constants import get_lycus_home as _get_lycus_home
        _lycus_home_maint = _get_lycus_home()
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_lycus_home_maint / "sessions"
                )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`lycus_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``lycus/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("lycus-"):
            continue
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``lycus/lycus-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``lycus -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("lycus/lycus-") or b.startswith("pr-"))
    ]
...
#   6. Default: assume dark (matches the legacy Lycus assumption)
#
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from lycus_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
    if getattr(SkinConfig, "_lycus_light_mode_hook_installed", False):
        return
    ...
    SkinConfig._lycus_light_mode_hook_installed = True  # type: ignore[attr-defined]
...
class _SkinAwareAnsi:
    ...
    def __str__(self) -> str:
        if self._cached is None:
            try:
                from lycus_cli.skin_engine import get_active_skin
...
def _strip_markdown_syntax(text: str) -> str:
    ...
    # but in Lycus output it's common to display cron schedules verbatim.
    #
...
from lycus_constants import is_termux as _is_termux_environment
...
def _apply_bracketed_paste_timeout_patch() -> None:
    ...
    ``_lycus_bp_timeout_patched`` sentinel on the module.
    """
    try:
        ...
        if getattr(_vt100_mod, "_lycus_bp_timeout_patched", False):
            return
        ...
        def _patched_vt100_feed(self_parser, data: str) -> None:
            if self_parser._in_bracketed_paste:
                ...
                if end_mark in self_parser._paste_buffer:
                    ...
                    self_parser._lycus_bp_start = None
                    if remaining:
                        _patched_vt100_feed(self_parser, remaining)
                else:
                    bp_start = getattr(self_parser, "_lycus_bp_start", None)
                    now = time.monotonic()
                    if bp_start is None:
                        self_parser._lycus_bp_start = now
        ...
        _vt100_mod._lycus_bp_timeout_patched = True
...
# ASCII Art - Lycus Caduceus (compact, fits in left panel)
...
def _build_compact_banner() -> str:
    ...
    if os.environ.get("HERMES_FAST_STARTUP_BANNER") == "1":
        from lycus_cli import __release_date__ as _release_date
        from lycus_cli import __version__ as _version

        version_line = f"Lycus Agent v{_version} ({_release_date})"
    else:
        version_line = format_banner_version_label()

    w = min(shutil.get_terminal_size().columns - 2, 88)
    if w < 30:
        return f"\n[{title_color}]{tiny_line}[/] [dim {dim_color}]- Technetia Inc[/]\n"
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from lycus_cli.plugins import get_plugin_commands
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.autolycus/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.autolycus/config.yaml on first use)
...
class LycusCLI(CLIAgentSetupMixin, CLICommandsMixin):
    """
    Interactive CLI for the Lycus Agent.
    ...
    def __init__(
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from lycus_cli.runtime_provider import _auto_detect_local_model
        ...
        # it's shared across all Lycus processes for this HERMES_HOME.
        # Never blocks startup on failure.
        _run_state_db_auto_maintenance(self._session_db)

        # Opportunistic shadow-repo cleanup — deletes orphan/stale
        # checkpoint repos under ~/.autolycus/checkpoints/.  Opt-in via
        ...
        self._history_file = _lycus_home / ".autolycus_history"
    ...
    def _claim_active_session(self, surface: str = "cli", *, stderr: bool = False) -> bool:
        ...
        if self._active_session_lease is not None:
            return True
        try:
            from lycus_cli.active_sessions import try_acquire_active_session
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from lycus_cli.voice import format_voice_record_key_for_status
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            return f"🔱 {self.model if getattr(self, 'model', None) else 'Lycus'}"
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from lycus_cli.model_normalize import (
        ...
        if resolved_provider == "copilot":
            try:
                from lycus_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from lycus_cli.models import normalize_opencode_model_id, opencode_model_api_mode
        ...
        if self._model_is_default:
            ...
            try:
                from lycus_cli.codex_models import get_codex_model_ids
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from lycus_cli.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", "🔱 Lycus")
            ...
            w = self._scrollback_box_width()
            fill = w - 2 - LycusCLI._status_bar_display_width(label)
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``lycus doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``lycus doctor`` so the banner stays
        small.
        """
        try:
            from lycus_cli.security_advisories import (
                detect_compromised,
                startup_banner,
            )
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   Lycus needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        # Warn if the configured model is a Nous Lycus LLM (not agentic)
        from lycus_cli.model_switch import is_nous_lycus_non_agentic

        model_name = getattr(self, "model", "") or ""
        if is_nous_lycus_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Nous Research Lycus 3 & 4 models are NOT agentic and are not "
                "designed for use with Lycus Agent.[/]"
            )
            self._console_print(
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.autolycus/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from lycus_cli.clipboard import save_clipboard_image

        img_dir = get_lycus_home() / "images"
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'lycus setup' to configure[/]")
        except Exception:
    ...
    def show_help(self):
        ...
        from lycus_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from lycus_cli.skin_engine import get_active_help_header
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with Lycus!{_RST}")
    ...
    def _list_recent_sessions(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        if not self._session_db:
            return []
        try:
            from lycus_cli.session_listing import query_session_listing
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from lycus_cli.main import _relative_time
    ...
    def _discard_session_if_empty(self, session_id: Optional[str]) -> bool:
        ...
        ``/resume`` and ``lycus sessions list``. Delegates the
    ...
    def new_session(self, silent=False, title=None):
        ...
        if self._session_db and old_session_id:
            ...
            # /resume and `lycus sessions list` (gemini-cli#27770 port).
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.autolycus/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``lycus --resume <id>``
        ...
        saved_dir = get_lycus_home() / "sessions" / "saved"
        try:
            saved_dir.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            print(f"(x_x) Failed to create save directory {saved_dir}: {e}")
            return
        path = saved_dir / f"lycus_conversation_{timestamp}.json"
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: lycus --resume {self.session_id}")
        except Exception as e:
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from lycus_cli.curses_ui import curses_single_select
    ...
    def _confirm_expensive_model_switch(self, result) -> bool:
        ...
        choices = [
            ("once", "Switch anyway", "Use this model for the current Lycus session."),
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `lycus model` and gateway pickers).
            # Only fall back to the live provider catalog when the curated
            # list is empty (e.g. user-defined endpoints with no curated list).
            model_list = provider_data.get("models", [])
            if not model_list:
                try:
                    from lycus_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from lycus_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from lycus_cli.model_switch import switch_model, parse_model_flags
    ...
    def _handle_codex_runtime(self, cmd_original: str) -> None:
        ...
        Usage:
            ...
            /codex-runtime auto                  — Lycus default (chat_completions)
    ...
    def _should_handle_model_command_inline(self, text: str, has_images: bool = False) -> bool:
        ...
        if not text or has_images or not _looks_like_slash_command(text):
            return False
        try:
            from lycus_cli.commands import resolve_command
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_lycus_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_lycus_home()}/config.yaml")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in lycus_cli/commands.py instead of touching every dispatch site.
        from lycus_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            from lycus_cli.main import _print_version_info
            ...
            from lycus_cli.config import reload_env
            count = reload_env()
            ...
            try:
                # Discover from disk (bundled + user), matching `lycus plugins
                ...
                # catalog lives behind `lycus plugins list`.
                user_entries = [e for e in entries if e[3] != "bundled"]
                bundled_count = len(entries) - len(user_entries)

                if not user_entries:
                    print("No user plugins installed.")
                    print("  Install one: lycus plugins install owner/repo")
                    print(f"  Or drop a plugin directory into {display_lycus_home()}/plugins/")
                    if bundled_count:
                        print(f"  ({bundled_count} bundled plugins available — see: lycus plugins list)")
                else:
                    ...
                    if bundled_count:
                        print(f"  (+{bundled_count} bundled — see: lycus plugins list)")
                    print("  Enable/disable: lycus plugins enable/disable <name>")
            except Exception as e:
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from lycus_cli.commands import COMMANDS
    ...
    def _get_goal_manager(self):
        ...
        """
        try:
            from lycus_cli.goals import GoalManager
            from lycus_cli.config import load_config
        except Exception as exc:
    ...
    def _toggle_verbose(self):
        ...
        from lycus_cli.colors import Colors as _Colors
        labels = {
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            # Console quietness is enforced by lycus_logging not
    ...
    def _show_credits(self):
        ...
        if not view.logged_in:
            ...
            print("  Run `lycus portal` to log in, then /credits.")
            return
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from lycus_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.autolycus/skills/ and queue a note for the
        next user turn.
                ...
                try:
                    if (
                        ...
                        if not is_seen(CLI_CONFIG, TOOL_PROGRESS_FLAG):
                            ...
                            mark_seen(_lycus_home / "config.yaml", TOOL_PROGRESS_FLAG)
    ...
    def _voice_speak_response(self, text: str):
        ...
        try:
            ...
            os.makedirs(os.path.join(tempfile.gettempdir(), "lycus_voice"), exist_ok=True)
            mp3_path = os.path.join(
                tempfile.gettempdir(), "lycus_voice",
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _lycus_home / "interrupt_debug.log"
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            print(f"  lycus --resume {self.session_id}{profile_flag}")
            if session_title:
                print(f"  lycus -c \"{session_title}\"{profile_flag}")
            print()
            ...
            try:
                from lycus_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from lycus_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from lycus_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Lycus Agent! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to Lycus Agent! Type your message or /help for commands."
        ...
        try:
            from lycus_cli.model_switch import prewarm_picker_cache_async
        ...
        # after an OpenClaw→Lycus migration (especially migrations done by
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from lycus_cli.config import get_config_path as _get_cfg_path_resid
        ...
        def handle_ignored_terminal_sequence(event):
            ...
            install_ignored_terminal_sequences() in lycus_cli.pt_input_extras
                ...
                if self._agent_running and not (text and _looks_like_slash_command(text)):
                    ...
                    try:
                        ...
                        if not is_seen(CLI_CONFIG, BUSY_INPUT_FLAG):
                            ...
                            mark_seen(_lycus_home / "config.yaml", BUSY_INPUT_FLAG)
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting Lycus binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from lycus_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Lycus Agent")
        ...
        # Dynamic prompt: shows Lycus symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("Lycus needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'Lycus needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("Lycus needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = LycusCLI._compute_model_picker_viewport(
        ...
        try:
            ...
            if not getattr(_pt_renderer, "_lycus_osd_patched", False):
                ...
                _pt_renderer._lycus_osd_patched = True
        except Exception:
            pass
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the lycus process when
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: lycus setup"
            )
            _run_cleanup()
        ...
        try:
            ...
            if hasattr(self, '_session_db') and self._session_db and self.agent:
                ...
                # drop the empty row so /resume and `lycus sessions list`
...
def _run_kanban_goal_loop_q(cli: "LycusCLI", first_response: str) -> None:
    ...
    from lycus_cli.goals import run_kanban_goal_loop as _run_loop, DEFAULT_MAX_TURNS as _DEF_TURNS
...
def main(
    ...
    """
    Lycus Agent CLI - Interactive AI Assistant
    ...
    if gateway:
        ...
        print("Starting Lycus Gateway (messaging platforms)...")
    ...
    # Default to lycus-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
        ...
        # Coding posture (base Lycus): with no explicit --toolsets, collapse
    ...
    # mode registers its own inside LycusCLI.run(), but `-q` runs
    ...
    if query or image:
        ...
        try:
            ...
            # Kanban workers spawn with ``lycus chat -q "work kanban task <id>"``;
            ...
            if quiet:
                ...
                if cli._ensure_runtime_credentials():
                    ...
                    if cli._init_agent(
                        ...
                        # machine-readable (no styled "Lycus" box, no tool-gen
                        ...
                        if isinstance(result, dict) and result.get("failed"):
                            ...
                            if os.environ.get("HERMES_KANBAN_TASK") and result.get(
                                ...
                                try:
                                    from lycus_cli.kanban_db import (
                ...
                # Single-query mode (`lycus chat -q "…"`): skip the welcome
...
def main(
    ...
    # Give the worker a grace window to call _kill_process (SIGTERM to the
    # process group, then SIGKILL after 1 s), then raise KeyboardInterrupt
    # so main unwinds normally.  LYCUS_SIGTERM_GRACE overrides the 1.5 s
    # default for debugging.
    def _signal_handler_q(signum, frame):
        logger.debug("Received signal %s in single-query mode", signum)
        try:
            ...
            if _agent is not None:
                _agent.interrupt(f"received signal {signum}")
                try:
                    _grace = float(os.getenv("LYCUS_SIGTERM_GRACE", "1.5"))
                except (TypeError, ValueError):
                    _grace = 1.5
                if _grace > 0:
                    time.sleep(_grace)
